...
struct App
{
    ...
    void InitVulkan()
    {
        ...
        for (VkPhysicalDevice g : gpus)
        {
            ...
            for (const char* want : devExts)
            {
                bool found = false;
                for (auto& e : exts)
                    found = found || strcmp(e.extensionName, want) == 0;
                all = all && found;
            }
            if (!all)
                continue;
    ...
    void CreatePipeline()
    {
        ...
        for (uint32_t m = 0; m < M; ++m)
            memcpy(table + hitOffset + m * stride, &handles[(2 + m) * handleSize], handleSize);
    ...
    void Cleanup()
    {
        ...
        for (DeviceBuffer* b : {&tlasMemory, &instances, &scratch, &spheres, &cameraBuffer})
            DestroyBuffer(*b);
        for (VkSemaphore s : renderFinished)
            vkDestroySemaphore(device, s, nullptr);
...
int RunApp(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        if (!strcmp(argv[i], "--frames") && i + 1 < argc)
            ...
            app.samplesPerFrame = std::max(1, atoi(argv[++i]));
        else if (!strcmp(argv[i], "--depth") && i + 1 < argc)
            app.maxDepth = std::max(1, atoi(argv[++i]));
