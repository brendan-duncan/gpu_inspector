...
// that draws a spinning, textured cube into a framebuffer of its own and then that framebuffer's
// texture onto the screen, with what the OpenGL ES plugin's capture has to handle on a phone: an
// OpenGL ES 3.2 context of EGL's, a vertex array with an index buffer, a uniform block, an immutable
// mipmapped texture, an ETC2 compressed texture (read back from the upload, as glReadPixels cannot),
// debug groups and object labels, and an invalidated depth attachment.
#include <EGL/egl.h>
...
const char* kBlitVs = R"(#version 300 es
out vec2 vUv;
void main() {
...
const char* kBlitFs = R"(#version 300 es
precision mediump float;
...
GLuint Link(const char* vs, const char* fs, const char* label) {
...
struct App {
    ...
    void CreateResources() {
        resources = true;
        LOGI("%s, %s", (const char*)glGetString(GL_RENDERER), (const char*)glGetString(GL_VERSION));
...
void OnAppCommand(android_app* app, int32_t cmd) {
    ...
    switch (cmd) {
        case APP_CMD_INIT_WINDOW:
            if (self->context == EGL_NO_CONTEXT) self->CreateContext();
            self->DestroySurface();
            self->CreateSurface();
            break;
