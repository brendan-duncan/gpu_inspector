...
// (app/src/renderer/vulkan/spirv_ablate.ts), and the replay times the draw with each.
//
...
VkShaderStageFlagBits StageBit(const std::string& stage) {
    ...
    if (stage == "tess_control") return VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT;
    if (stage == "tess_eval") return VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT;
...
void Replayer::NoteStreamCommand(StreamState& stream, const std::string& method, const JValue& args, uint32_t index) {
    if (method == "vkCmdBindPipeline") {
        ...
        if (point == "VK_PIPELINE_BIND_POINT_GRAPHICS") stream.graphicsPipeline = IdOf(args.Get("pipeline"));
        else if (point == "VK_PIPELINE_BIND_POINT_COMPUTE") stream.computePipeline = IdOf(args.Get("pipeline"));
...
bool Replayer::PrepareAblation() {
    ...
    if (props.limits.timestampPeriod <= 0 || _queueFamily >= families.size() || !families[_queueFamily].timestampValidBits) return false;
    ...
    if (_fns.CreateQueryPool(_device, &info, nullptr, &_ablationPool) != VK_SUCCESS) _ablationPool = VK_NULL_HANDLE;
    return _ablationPool != VK_NULL_HANDLE;
}

void Replayer::ResetAblationQueries(VkCommandBuffer cb, const CommandGroup& group) {
    if (!_ablationPool) return;
    const uint32_t rounds = std::max(1u, _options.ablation.rounds) + 1;
    for (size_t t = 0; t < _options.ablation.targets.size(); ++t) {
        const auto& target = _options.ablation.targets[t];
        if (target.command <= group.first || target.command >= group.last) continue;
...
VkPipeline Replayer::AblationPipeline(uint64_t pipelineId, size_t target, int variant, bool compute) {
    ...
    if (auto it = _ablationPipelines.find(key); it != _ablationPipelines.end()) return it->second;
    ...
    if (variant >= 0) {
        ...
        m.pCode = words.data();
        if (words.empty() || _fns.CreateShaderModule(_device, &m, nullptr, &module) != VK_SUCCESS) return VK_NULL_HANDLE;
    }
    ...
    if (compute) {
        ...
        if (object && args && Str(object->Get("cmd")) == "vkCreateComputePipelines") {
            ...
            if (usable) {
                ...
                if (!module) own = ModuleFromBlob(*object, std::string("compute:") + (info.stage.pName ? info.stage.pName : "main"));
                ...
                if (own) _fns.DestroyShaderModule(_device, own, nullptr);
            }
            _arena.Reset();
        }
    } else {
        pipeline = CopyGraphicsPipeline(pipelineId, "ablation", [&](PipelineCopy& p) {
            ...
            if (s == p.stages.end()) return false;
            if (module) {
                s->module = module;
                s->pNext = nullptr;
            }
            ...
            if (p.hasDepthStencil) {
                if (!depthDynamic) p.depthStencil.depthWriteEnable = VK_FALSE;
                if (!stencilDynamic) p.depthStencil.front.writeMask = p.depthStencil.back.writeMask = 0;
            }
            return true;
        });
    }
    if (module) _fns.DestroyShaderModule(_device, module, nullptr);
...
void Replayer::IssueAblation(VkCommandBuffer cb, uint32_t index, const std::string& method, const JValue& args, uint32_t frame,
    ...
    if (found == _ablationTargets.end()) return;
    ...
    if (!_ablationPool) return fail("the replay's queue writes no timestamps");
    if (!StartsWith(method, "vkCmdDraw") && !compute) return fail(method + " is not a draw or a dispatch");
    if (!result.pipeline) return fail("no pipeline is bound at the command");
    if (!StageBit(target.stage)) return fail("the " + target.stage + " stage cannot be measured");
    if (!compute && _passViews > 1) return fail("draws in multiview passes are not measured yet (each timestamp takes one query per view)");
    ...
    if (!pipelines[0]) return fail("the pipeline could not be copied (the capture has no code for its " + target.stage + " stage, or the copy was refused)");
    ...
    if (!compute && dynamic.first) {
        if (_fns.CmdSetDepthWriteEnable) _fns.CmdSetDepthWriteEnable(cb, VK_FALSE);
        else if (_fns.CmdSetDepthWriteEnableEXT) _fns.CmdSetDepthWriteEnableEXT(cb, VK_FALSE);
    }
    if (!compute && dynamic.second) _fns.CmdSetStencilWriteMask(cb, VK_STENCIL_FACE_FRONT_AND_BACK, 0);
    ...
    for (const std::vector<uint32_t>* list : {&stream.depthWriteCommands, &stream.stencilWriteCommands}) {
        for (uint32_t i : *list) {
            ...
            if (ReplayFn restore = FindReplayCommand(Str(c.Get("method"))); restore && c.Get("args")) restore(_ctx, *c.Get("args"), cb);
            _arena.Reset();
        }
    }
    _ctx.problems.resize(problems);
        ...
        for (uint32_t round = 1; round < rounds; ++round) {   // the first round only warms up
            for (size_t v = 0; v < count; ++v) {
                ...
                if (_fns.GetQueryPoolResults(_device, _ablationPool, query, 2, sizeof(stamps), stamps, sizeof(uint64_t),
                    ...
                    continue;
                if (stamps[1] >= stamps[0]) samples[v].push_back((double)(stamps[1] - stamps[0]) * _timestampPeriod / 1e6 / std::max(1u, r.repeat));
            }
        }
...
void Replayer::DestroyAblation() {
    if (_ablationPool) _fns.DestroyQueryPool(_device, _ablationPool, nullptr);
