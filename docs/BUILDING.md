...
| `--geometry` | Draws the cube through a geometry shader that emits each triangle twice, the copy shifted right, for the mesh view's GS Out |
| `--tessellation` | Draws each of the cube's triangles as a patch the tessellator divides into six, for the mesh view's DS Out; combines with `--geometry`, `--multiview` and `--shader-object` |
