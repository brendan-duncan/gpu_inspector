...
while it captures, the way the Metal library does them, because a D3D12 capture cannot be replayed
(`src/d3d12/src/overdraw.h`).
...
- The other replay-based analyses are Vulkan-only, since `vkinsp_replay` replays Vulkan captures: no
  draw overlays, mesh output, per-draw timings (**Measure draws**), shader cost by ablation
  (**Measure shader**) or hardware counters (**Measure hardware counters**). The capture panel says
  so where they are offered.
