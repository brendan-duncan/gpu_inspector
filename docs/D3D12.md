...
The launch dialog does this for you. Choose **A web page in a browser (WebGPU, WebGL)** under *Run On*,
...
recorded from its first command after the capture was asked for. Its passes are timed and their
render targets read back like any other's; it carries no pipeline statistics.
...
| `--async-compute` | the wave dispatch goes to a compute queue of its own, in a list of its own, and runs beside the render pass; the direct queue waits for it only at the frame's end. Gives a capture passes on two queues; implies `--compute` |
| `--pool` | the frame is recorded into one of four lists in turn, each reset as soon as it has run, so the list a capture's frame runs was reset before the capture was asked for and is adopted |
...
| `--keep-depth` | the depth buffer is cleared only when it is new, so every frame tests against what the frames before left in it |
| `--late-descriptor` | the cubes' SRV range is `DESCRIPTORS_VOLATILE`, and its slot is rewritten with another texture after the list is closed and before it is submitted, so the GPU draws what a snapshot at the draw does not hold |
| `--bindless` | the cubes' root signature lets shaders index the heap directly, and their pixel shader (shader model 6.6) multiplies in a texture taken from a heap slot no root table covers |
| `--local-root` | with `--ray-tracing`, the tinted hit group has a local root signature -- a root constant, a root CBV and a table of one CBV, over buffers nothing else binds -- whose arguments its binding table record holds |
