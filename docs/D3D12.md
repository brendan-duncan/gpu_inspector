...
Microsoft Store or the Xbox app, which is what Minecraft for Windows (Bedrock) is. Running
`Minecraft.Windows.exe` from the install folder only asks Windows to activate the package: that
process exits at once, and the game that appears a few seconds later is a child of the
activation host, not of whatever asked for it. **Capture child processes** therefore never sees
it, and neither does RenderDoc's equivalent.

What reaches it is the **Direct3D 12 application to wait for** target, which goes by image name
and does not care whose child the process is: put `Minecraft.Windows.exe` in it, press **Wait**,
then start the game. No ACL work is needed on the capture library — Minecraft declares
`runFullTrust`, so it is a full-trust packaged application rather than a sandboxed one, and it
reads the library from wherever it is built. Nor are any code-integrity mitigations in the way
(`MicrosoftSignedOnly` and `BlockDynamicCode` are both off), so an unsigned build loads.
...
Minecraft is still not a reliable target, and what remains is not understood. On the machine this
was worked out on the game became unwilling to start at all after repeated launches — with
nothing of ours running, three launches in a row produced no process — and when it did start and
was caught, it sometimes exited during the injection with `STATUS_PARTIAL_COPY` (0x8000000D)
rather than completing it. Worth suspecting: the watch freezes the process the moment it appears
and holds it for something over a hundred milliseconds, and a packaged application is under the
app model's process lifetime management from its first instruction. Capturing it does work — a
frame of 435 commands over 23 passes was taken this way — but expect to retry.
