...
- The other replay-based analyses are Vulkan-only: no draw overlays, mesh output, per-draw timings
  (**Measure draws**), shader cost by ablation (**Measure shader**) or hardware counters (**Measure
  hardware counters**). The capture panel says so where they are offered. A D3D12 capture does
  replay (`dxinsp_replay`, [Capture replay](REPLAY.md#direct3d-12)), which is what **Export to
  C++** writes its project from, but the analyses are built on `vkinsp_replay`.
