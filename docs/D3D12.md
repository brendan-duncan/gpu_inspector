...
A render pass the application suspends across command lists takes no queries either way: nothing may
be added between a suspension and its resume. Most of a Unity frame's passes are of that kind, so
much of such a frame is still unmeasured; the capture says how many in its log and in the
`suspended-pass` finding.
