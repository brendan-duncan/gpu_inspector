...
Recorded earlier than that -- an engine that builds its lists several frames ahead, or a pooled list
adopted mid-recording -- and a list carries neither the commands nor the queries.
`DXINSP_RECORD_ALWAYS=1` records every list from the start, which is what those need.
