...
Microsoft Store or the Xbox app, which includes Minecraft for Windows): Windows starts those for
the caller, and the process that appears is a child of the activation host rather than of
whatever asked for it. Use the **Direct3D 12 application to wait for** target with its image
name — that goes by name and does not care whose child it is. Note that a packaged application
also runs in an AppContainer, which can only load a library whose file grants access to
`ALL APPLICATION PACKAGES`; without that the injection fails with `LoadLibraryW in the target
failed`.
