...
Getting the library into Chrome's GPU process, which the browser spawns itself, uses Chrome's own
`--gpu-launcher` hook so the GPU process is started through `dxinsp_launch.exe`:

```
chrome.exe --gpu-launcher="<path>\dxinsp_launch.exe --dll <path>\dxinsp_capture.dll --" ^
           --disable-gpu-sandbox --disable-gpu-watchdog --enable-unsafe-webgpu
```

The sandbox has to be off for the injected library to open its socket, and the watchdog off so it
does not kill the process while the first capture runs. Then **Connect** to the port. If the
compositor also presents on a hooked D3D12 device and steals the capture, set
