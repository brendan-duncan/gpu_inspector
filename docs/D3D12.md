...
One thing is unreliable, and it is ours rather than the game's. Having loaded the library, the
launcher confirms it by looking for it in the target's module list, and that list cannot be read
while a process is held still in the middle of its own start-up: both `CreateToolhelp32Snapshot`
and `EnumProcessModulesEx` fail with `ERROR_PARTIAL_COPY` (299). A hundred-library application
caught two milliseconds after it started is in exactly that state, so roughly half of Minecraft
launches are reported as `the target's module list could not be read` and give up, even though
the library had loaded. Try again when that happens. The fix is for the launcher to stop needing
the module list at all — a remote stub that loads the library and calls its initializer in one
go, rather than finding the module from outside to work out where the initializer is.
