...
| `export_cpp` | Export to C++: a Vulkan capture's frame written as a standalone CMake project (every object, the contents the frame read, every command) whose program runs the frame and compares its render targets with the capture's — for a driver bug report; replayed with `vkinsp_replay` to write it; not Metal or D3D12 |
