...
| `export_cpp` | Export to C++: a Vulkan or Direct3D 12 capture's frame written as a standalone CMake project (every object, the contents the frame read, every command) whose program runs the frame and compares its render targets with the capture's — for a driver bug report; replayed with `vkinsp_replay` (`dxinsp_replay` for D3D12) to write it; not Metal |
