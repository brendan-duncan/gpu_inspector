...
| `debug_shader` | Runs one vertex, pixel or compute invocation in a SPIR-V interpreter: outputs, every source line's values in order, the first NaN or infinity, and the GPU's result to compare with (a pixel needs the replay) |
