...
void WriteHeader(JsonWriter& w, uint64_t id, uint32_t mip, uint32_t layer, uint32_t width, uint32_t height, uint32_t depth,
    ...
    w.Key("format"); w.String(format ? format : "VK_FORMAT_UNDEFINED");
    w.Key("aspect"); w.String(depthAspect ? "depth" : "color");
...
DeviceReadback* ReadbackFor(ID3D12Device* device, uint64_t bufferSize) {
    ...
    if (!rb.event) rb.event = CreateEventW(nullptr, FALSE, FALSE, nullptr);
    if (!rb.event) return nullptr;
...
void OnQueueCreated(ID3D12Device* device, ID3D12CommandQueue* queue, D3D12_COMMAND_LIST_TYPE type) {
    ...
    if (!q.firstDirect && type == D3D12_COMMAND_LIST_TYPE_DIRECT) q.firstDirect = queue;
}
    ...
    for (auto& [device, q] : s.queues) {
        ...
        if (q.firstDirect == queue) {
            ...
            for (ID3D12CommandQueue* other : q.queues) {
                ScopedInternal internal;
                if (other->GetDesc().Type == D3D12_COMMAND_LIST_TYPE_DIRECT) { q.firstDirect = other; break; }
            }
        }
    }
}
...
ID3D12CommandQueue* ReadbackQueue(ID3D12Device* device) {
    ...
    std::lock_guard lock(s.mutex);
    DeviceQueues& q = s.queues[device];
    if (q.firstDirect) return q.firstDirect;
    if (!q.own) {
        D3D12_COMMAND_QUEUE_DESC desc{};
        desc.Type = D3D12_COMMAND_LIST_TYPE_DIRECT;
        ScopedInternal internal;
        if (FAILED(device->CreateCommandQueue(&desc, IID_PPV_ARGS(q.own.put())))) q.own.reset();
    }
    return q.own.get();
}
...
void ReadBackImage(uint64_t objectId, uint32_t mip, uint32_t layer) {
    ...
    if (!object || tracker.TypeOf(object) != "ID3D12Resource") return Fail(objectId, mip, layer, "image no longer exists");
    ID3D12Resource* resource = static_cast<ID3D12Resource*>(object);
    ResourceInfo info;
    if (!ResourceTracker::Get().Get(resource, info)) return Fail(objectId, mip, layer, "resource is not tracked");
    const D3D12_RESOURCE_DESC& desc = info.desc;
    if (desc.Dimension == D3D12_RESOURCE_DIMENSION_BUFFER) return Fail(objectId, mip, layer, "not a texture");
    ...
    if (mip >= mipLevels) return Fail(objectId, mip, layer, "mip level out of range");
    if (layer >= arraySize) return Fail(objectId, mip, layer, "array layer out of range");
    if (desc.SampleDesc.Count > 1) return Fail(objectId, mip, layer, "multisampled textures are not read back live");
    ...
    if (!fmt.protocolName) return Fail(objectId, mip, layer, "format cannot be decoded");
    ...
    if (!size) return Fail(objectId, mip, layer, "empty image");
    if (size > kMaxReadback) return Fail(objectId, mip, layer, "image is larger than 256 MB");

    ID3D12Device* device = DeviceOf(resource);
    if (!device) return Fail(objectId, mip, layer, "device unavailable");
    ID3D12CommandQueue* queue = ReadbackQueue(device);
    if (!queue) return Fail(objectId, mip, layer, "no direct queue to execute on");
    ...
    if (!total) return Fail(objectId, mip, layer, "the subresource has no copyable layout");
    DeviceReadback* rb = ReadbackFor(device, total);
    if (!rb) return Fail(objectId, mip, layer, "read-back objects could not be created");
    ...
    HRESULT hr;
    {
        ...
        if (FAILED(list->Close())) return Fail(objectId, mip, layer, "read-back command list could not be closed");
        ...
        if (SUCCEEDED(hr) && rb->fence->GetCompletedValue() < value) {
            ...
            if (SUCCEEDED(hr) && WaitForSingleObject(rb->event, kWaitMs) != WAIT_OBJECT_0) hr = E_FAIL;
        }
        if (FAILED(hr)) return Fail(objectId, mip, layer, "read-back timed out");

        void* mapped = nullptr;
        D3D12_RANGE range{0, (SIZE_T)total};
        if (FAILED(rb->buffer->Map(0, &range, &mapped)) || !mapped) return Fail(objectId, mip, layer, "readback buffer could not be mapped");
