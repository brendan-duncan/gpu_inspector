...
Capturing Direct3D 12, the way `vulkan/` captures Vulkan and `metal/` captures Metal. A library
...
to learn the D3D12 spellings of a few things (`app/src/renderer/d3d12/`).
...
snapshot's bindings carry too. `app/src/renderer/d3d12/reflection.ts` reads it into the same
...
-Qembed_debug`; `app/src/main/shader_tools.ts` runs it the way it runs `spirv-dis`. A DXIL
