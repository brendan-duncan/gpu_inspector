...
/** Where the refresh period a Frame Bound budget uses came from, by FrameStats' refreshSource. */
export const REFRESH_SOURCE_NOTE: Record<string, string> = {
...
export interface FrameBound {
  verdict: string;
  kind: FrameBoundKind;
  /** The display refresh period with vsync on, else the live frame interval. */
  budgetMs: number;
...
/**
 * The Frame Bound verdict: compares the GPU span of the captured passes and the CPU submit time
 * against the frame interval and names the likely bottleneck, like WebGPU Inspector's card. Null
 * when there is no budget to compare against.
 */
export function frameBound(t: { frameMs: number; refreshMs: number; submitMs: number; gpuSpanMs: number; frames: number }): FrameBound | null {
