...
int64_t JValue::Int() const
{
    if (type == JType::Bool)
        return boolean ? 1 : 0;
    if (type != JType::Number && type != JType::String)
        return 0;
    int64_t value = 0;
    auto result = std::from_chars(text, text + length, value);
    if (result.ec == std::errc() && result.ptr == text + length)
...
bool JsonDocument::ParseValue(JValue& out, int depth)
{
    ...
    if (c == '{')
    {
        ...
        if (n)
        {
            ...
            for (size_t i = 0; i < n; ++i)
                out.members[i] = _members[start + i];
        }
    ...
    {
        const char* start = _pos++;
        while (_pos < _end && (IsDigit(*_pos) || *_pos == '.' || *_pos == 'e' || *_pos == 'E' || *_pos == '+' || *_pos == '-'))
            ++_pos;
