...
double KhrValue(const VkPerformanceCounterResultKHR& r, VkPerformanceCounterStorageKHR storage, VkPerformanceCounterUnitKHR unit) {
    ...
    if (unit == VK_PERFORMANCE_COUNTER_UNIT_NANOSECONDS_KHR) return v;  // already nanoseconds
    return v;
}
...
bool Replayer::PrepareCounters() {
    ...
    if (const JValue* commands = _capture->Commands(); commands && commands->IsArray()) {
        for (uint32_t i = 0; i < commands->count; ++i) {
            ...
            if (IsAction(m)) ++draws;
            else if (IsBeginRenderPass(m) || IsBeginRendering(m)) ++passes;
        }
    }
    if (!draws) {
...
bool Replayer::PrepareKhrCounters(uint32_t draws) {
    ...
    for (auto& c : hw.khrCounters) c.sType = VK_STRUCTURE_TYPE_PERFORMANCE_COUNTER_KHR;
    for (auto& d : hw.khrDescs) d.sType = VK_STRUCTURE_TYPE_PERFORMANCE_COUNTER_DESCRIPTION_KHR;
    ...
    auto index = [&](const std::string& name) -> int {
        for (uint32_t i = 0; i < count; ++i)
            if (hw.khrCounters[i].scope == VK_PERFORMANCE_COUNTER_SCOPE_COMMAND_KHR && name == hw.khrDescs[i].name) return (int)i;
        return -1;
    };
    if (_options.counters.list) return true;
    ...
    if (wanted.empty()) {
        ...
        for (const std::string& name : wanted) {
            ...
            if (i < 0) { _report->counters.notes.push_back("counter \"" + name + "\" is not a command-scoped counter on this device"); continue; }
    ...
    if (hw.khrPasses == 0) hw.khrPasses = 1;
...
void Replayer::ListCounters() {
    if (!_hw) return;
    if (_hw->nvperf) _hw->session.ListMetrics(_report->counters.available);
    else if (_hw->khr) {
        for (uint32_t i = 0; i < _hw->khrCounters.size(); ++i) {
            if (_hw->khrCounters[i].scope != VK_PERFORMANCE_COUNTER_SCOPE_COMMAND_KHR) continue;
            HwCounterInfo info;
...
bool Replayer::BeginCounterRound() {
    ...
    if (hw.nvperf) return hw.session.BeginPass();
    if (hw.khr) {
...
void Replayer::BeginCounterPass(VkCommandBuffer cb, const PassState& pass) {
    HwCounterState& hw = *_hw;
    if (!hw.nvperf) return;   // KHR cannot wrap a pass
...
void Replayer::EndCounterPass(VkCommandBuffer cb) {
    if (_hw->nvperf) _hw->session.PopRange(cb);
}
...
void Replayer::CompleteCounters() {
    ...
    if (hw.nvperf) {
        ...
        for (const auto& r : hw.ranges) {
            if (r.pass) passByIndex[r.passIndex] = &r;
            else drawByCommand[r.command] = &r;
        }
        for (const auto& [leaf, values] : results) {
            ...
            if (leaf[0] == 'P') {
                ...
                if (it == passByIndex.end()) continue;
                range.pass = true;
                ...
                if (it == drawByCommand.end()) continue;
                range.command = it->second->command;
                range.frame = it->second->frame;
                range.commandBuffer = it->second->commandBuffer;
                range.passIndex = it->second->passIndex;
        ...
        if (hw.nextQuery && _fns.GetQueryPoolResults(_device, hw.khrPool, 0, hw.nextQuery, results.size() * sizeof(results[0]),
                                                     results.data(), n * sizeof(results[0]), VK_QUERY_RESULT_WAIT_BIT) == VK_SUCCESS) {
            for (const auto& r : hw.ranges) {
                if (r.pass) continue;
                HwCounterRange range;
...
void Replayer::DestroyCounters() {
    ...
    _submitNext = nullptr;
    if (_hw->khr) {
        if (_hw->lockHeld && _fns.ReleaseProfilingLockKHR) _fns.ReleaseProfilingLockKHR(_device);
        if (_hw->khrPool) _fns.DestroyQueryPool(_device, _hw->khrPool, nullptr);
    }
    _hw->session.End();
    delete _hw;
    _hw = nullptr;
