...
XfbPatch PatchForTransformFeedback(const uint32_t* words, size_t count, const std::string& entryPoint)
{
    ...
    for (size_t at = 5; at < count;)
    {
        ...
        {
            case OpCapability:
                if (w[1] == CapabilityTransformFeedback)
                    hasCapability = true;
            ...
            case OpDecorate:
                if (n >= 3 && w[2] == DecorationBuiltIn && n >= 4)
                    builtins[w[1]] = w[3];
                if (n >= 3 && w[2] == DecorationLocation && n >= 4)
                    locations[w[1]] = (int32_t)w[3];
                if (n >= 3 && w[2] == DecorationBlock)
                    blocks.insert(w[1]);
                if (n >= 3 && (w[2] == DecorationXfbBuffer || w[2] == DecorationOffset))
                    hasXfb.insert(w[1]);
                break;
            case OpMemberDecorate:
                if (n >= 5 && w[3] == DecorationBuiltIn)
                    memberBuiltins[{w[1], w[2]}] = w[4];
                if (n >= 4 && w[3] == DecorationOffset)
                    hasOffsets.insert(w[1]);
                break;
            case OpTypeBool:
            case OpTypeInt:
            case OpTypeFloat:
            ...
            case OpTypePointer:
            {
                ...
                if (op == OpTypeFloat)
                    t.width = w[2];
                if (op == OpTypeVector || op == OpTypeMatrix || op == OpTypeArray)
                {
                    t.element = w[2];
                    t.count = w[3];
                }
    ...
    std::function<Flat(uint32_t)> flatten = [&](uint32_t id) -> Flat {
        ...
        if (t.op == OpTypeFloat && t.width == 32)
            return Flat{true, 4, 1, "float"};
        if (t.op == OpTypeInt && t.width == 32)
            return Flat{true, 4, 1, t.isSigned ? "int" : "uint"};
    ...
    for (uint32_t id : entry->interface)
    {
        ...
        if (ptr == types.end() || ptr->second.op != OpTypePointer || ptr->second.storage != StorageClassOutput)
            continue;
        if (hasXfb.count(id))
            continue;
    ...
    if (!hasCapability)
        Emit(capability, OpCapability, {CapabilityTransformFeedback});
    ...
    if (!afterEntries)
        afterEntries = afterCapabilities;

    patch.words.reserve(count + capability.size() + modes.size() + decorations.size());
    patch.words.insert(patch.words.end(), words, words + 5);
    for (size_t at = 5; at <= count;)
    {
        if (at == afterCapabilities)
            patch.words.insert(patch.words.end(), capability.begin(), capability.end());
        if (at == afterEntries)
            patch.words.insert(patch.words.end(), modes.begin(), modes.end());
        if (at == afterAnnotations)
            patch.words.insert(patch.words.end(), decorations.begin(), decorations.end());
        if (at == count)
            break;
        const uint32_t n = words[at] >> 16;
