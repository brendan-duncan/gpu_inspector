...
const JValue* PNextEntry(const JValue* info, std::string_view sType)
{
    ...
    for (uint32_t i = 0; chain && chain->IsArray() && i < chain->count; ++i)
        if (Str(chain->items[i].Get("sType")) == sType)
...
void CreateInfosWithLibraries(const CaptureFile& capture, const JValue& object, std::vector<std::pair<const JValue*, bool>>& out, int depth = 0)
{
    ...
    for (uint32_t k = 0; list && list->IsArray() && k < list->count; ++k)
        if (const JValue* library = capture.Object(IdOf(&list->items[k])))
            CreateInfosWithLibraries(capture, *library, out, depth + 1);
}
...
const JValue* Replayer::PipelineState(uint64_t pipelineId, std::string_view member, std::string_view part) const
{
    ...
    for (const auto& [info, isLibrary] : infos)
    {
        ...
        if ((isLibrary || parts) && (!parts || Str(parts->Get("flags")).find(part) == std::string::npos))
            continue;
...
bool Replayer::MergeLibraries(const JValue& linked, const JValue& info, PipelineCopy& p, std::vector<VkShaderModule>& temporary, int depth)
{
    ...
    for (uint32_t k = 0; k < list->count; ++k)
    {
        ...
        if (!libraryInfo || Str(library->Get("cmd")) != "vkCreateGraphicsPipelines")
            return false;
        ...
        if (!a.pCreateInfos || index >= a.createInfoCount)
            return false;
        ...
        for (uint32_t s = 0; s < l.stageCount; ++s)
        {
            ...
            if (stage.stage == VK_SHADER_STAGE_FRAGMENT_BIT ? !fragment : !preRasterization)
                continue;
            if (std::any_of(p.stages.begin(), p.stages.end(), [&](const VkPipelineShaderStageCreateInfo& x) { return x.stage == stage.stage; }))
                continue;
            ...
            if (!module)
                module = ModuleFromBlob(*library, name);
            if (module)
            {
        ...
        if (vertexInput)
        {
            if (!info.pVertexInputState)
                info.pVertexInputState = l.pVertexInputState;
            if (!info.pInputAssemblyState)
                info.pInputAssemblyState = l.pInputAssemblyState;
        }
        if (preRasterization)
        {
            if (!info.pViewportState)
                info.pViewportState = l.pViewportState;
            if (!info.pRasterizationState)
                info.pRasterizationState = l.pRasterizationState;
            if (!info.pTessellationState)
                info.pTessellationState = l.pTessellationState;
        }
        if (fragment && !info.pDepthStencilState)
            info.pDepthStencilState = l.pDepthStencilState;
        if ((fragment || output) && !info.pMultisampleState)
            info.pMultisampleState = l.pMultisampleState;
        if (output && !info.pColorBlendState)
            info.pColorBlendState = l.pColorBlendState;
        if ((vertexInput || preRasterization || fragment || output) && l.pDynamicState && l.pDynamicState->pDynamicStates)
        {
            p.hasDynamic = true;
            for (uint32_t d = 0; d < l.pDynamicState->dynamicStateCount; ++d)
                p.AddDynamic(l.pDynamicState->pDynamicStates[d]);
        }
        if (!info.layout)
        ...
        for (auto* in = static_cast<const VkBaseInStructure*>(l.pNext); in; in = in->pNext)
        {
            if (in->sType != VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO)
                continue;
            bool present = false;
            for (auto* q = static_cast<const VkBaseInStructure*>(info.pNext); q; q = q->pNext)
                present = present || q->sType == in->sType;
            if (!present)
            {
...
VkPipeline Replayer::CopyGraphicsPipeline(uint64_t pipelineId, const std::string& purpose, const std::function<bool(PipelineCopy&)>& edit)
{
    ...
    if (p.info.pRasterizationState)
    {
        p.rasterization = *p.info.pRasterizationState;
        p.hasRasterization = true;
    }
    if (p.info.pMultisampleState)
    {
        p.multisample = *p.info.pMultisampleState;
        p.hasMultisample = true;
    }
    if (p.info.pDepthStencilState)
    {
        p.depthStencil = *p.info.pDepthStencilState;
    ...
    if (p.info.pColorBlendState)
    {
        ...
        p.hasBlend = true;
        if (p.blend.pAttachments)
            p.blendAttachments.assign(p.blend.pAttachments, p.blend.pAttachments + p.blend.attachmentCount);
    }
    ...
    for (VkShaderModule m : p.temporary)
        _fns.DestroyShaderModule(_device, m, nullptr);
    _arena.Reset();
    return pipeline;
}
