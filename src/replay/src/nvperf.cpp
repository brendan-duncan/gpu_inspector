...
bool Load(std::string& note)
{
    ...
    if (dirs.empty())
    {
        g_loadNote = std::string("NVIDIA's Nsight Perf SDK library (") + kLibraryName +
            ") was not found: put it beside vkinsp_replay, "
...
void InstanceExtensions(uint32_t apiVersion, std::vector<const char*>& out)
{
    ...
    if (nv::perf::VulkanAppendInstanceRequiredExtensions(names, apiVersion))
...
void DeviceExtensions(VkInstance instance, VkPhysicalDevice physical, PFN_vkGetInstanceProcAddr gipa, std::vector<const char*>& out)
{
    if (!g_loaded)
        return;
    std::vector<const char*> names;
    if (nv::perf::VulkanAppendDeviceRequiredExtensions(instance, physical, (void*)gipa, names))
        out.insert(out.end(), names.begin(), names.end());
    g_log.clear();
}
