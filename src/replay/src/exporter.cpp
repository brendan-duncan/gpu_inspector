...
const char* DestroyFunction(const std::string& type)
{
    static const std::map<std::string, const char*> kDestroy = {
        ...
        {"VkShaderEXT", "vkDestroyShaderEXT"},
...
std::string Template(const char* name)
{
    ...
    {
        if (std::strcmp(kExportTemplates[i].name, name) != 0)
            continue;
        std::string out;
        for (size_t p = 0; p < kExportTemplates[i].count; ++p)
            out += kExportTemplates[i].pieces[p];
        return out;
    }
    return std::string();
}
...
std::string Exporter::VariableName(const std::string& type, uint64_t id)
{
    ...
    if (!stem.empty())
        stem[0] = (char)std::tolower((unsigned char)stem[0]);
...
{
    if (s.writer.Lines() >= kPartLines)
        SplitNow(s);
}

void Exporter::SplitNow(Section& s)
{
    if (s.writer.text.empty())
        return;
...
void Exporter::Instance(uint32_t apiVersion, const std::vector<const char*>& extensions)
{
    ...
    for (const char* e : extensions)
        wanted += (wanted.empty() ? "" : ", ") + SourceWriter::String(e);
    std::string s;
...
void Exporter::Device(const std::string& capturedName, const std::string& replayName, const VkDeviceCreateInfo& info, uint32_t queueFamily)
{
    ...
    if (!capturedName.empty() && capturedName != replayName)
        w.Comment("Captured on " + capturedName + "; this source was exported from a replay on " + replayName + ".");
    ...
    const std::vector<StageModule>& modules)
{
    ...
    if (graphics)
    {
        ...
        expr = Emit(w, *graphics, w.indent);
    }
    else if (compute)
    {
        type = "VkComputePipelineCreateInfo";
        expr = Emit(w, *compute, w.indent);
    }
    else if (rayTracing)
    {
        type = "VkRayTracingPipelineCreateInfoKHR";
        expr = Emit(w, *rayTracing, w.indent);
    }
    w.Line("const " + type + " info = " + expr + ";");
    w.Use(function.c_str());
    if (rayTracing)
        w.Line("VK_CHECK(" + function + "(device, VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &info, nullptr, &" + name + "));");
    else
        w.Line("VK_CHECK(" + function + "(device, VK_NULL_HANDLE, 1, &info, nullptr, &" + name + "));");
...
void Exporter::FrameOutput(uint64_t id, VkImage image, VkImageLayout layout, VkFormat format, VkExtent2D extent, const std::string& comment)
{
    ...
    _outputSource = "// What the window shows: " + comment +
        ...
        "    output->image = " +
        w.Handle("VkImage", (uint64_t)image) + ";   // image " + std::to_string(id) +
        "\n"
        "    output->layout = " +
        w.Enum(kEnum_VkImageLayout, kEnumCount_VkImageLayout, layout, "VkImageLayout") +
        ";\n"
        "    output->format = " +
        w.Enum(kEnum_VkFormat, kEnumCount_VkFormat, format, "VkFormat") +
        ";\n"
        "    output->extent = {" +
...
void Exporter::InitialLayouts(uint64_t id, VkImage image, const std::vector<VkImageLayout>& targets, const std::vector<VkImageLayout>* current)
{
    ...
    if (std::all_of(targets.begin(), targets.end(), [](VkImageLayout l) { return l == VK_IMAGE_LAYOUT_UNDEFINED; }))
        return;
    const std::string name = w.Handle("VkImage", (uint64_t)image);
    if (current)
    {
        if (*current == targets)
            return;   // the frame leaves it where it found it
        std::string items;
...
void Exporter::Command(uint32_t index, const std::string& method, const JValue& args, DecodeContext& ctx)
{
    ...
    FrameStatement("[" + std::to_string(index) + "]", [&](SourceWriter& w) {
        ...
        if (leftOut)
            w.text = std::string((size_t)w.indent * 4, ' ') + "// [" + std::to_string(index) + "] " + method + ": " + Trimmed(w.text).substr(3) + "\n";
    });
    if (leftOut)
        ++_leftOut;
    else
...
void Exporter::Submit(VkQueue queue, const std::vector<VkCommandBuffer>& cbs)
{
    FrameStatement("without the application's semaphores and fence, and waited for", [&](SourceWriter& w) {
        ...
        for (size_t i = 0; i < cbs.size(); ++i)
            items += (i ? ", " : "") + w.Handle("VkCommandBuffer", (uint64_t)(uintptr_t)cbs[i]);
    ...
    if (_frameSection.writer.Lines() >= kPartLines / 2)
        SplitNow(_frameSection);
}
...
bool Exporter::WriteSection(Section& s, std::vector<std::string>& files, std::string& error)
{
    ...
    for (const std::string& n : s.writer.notes)
        _notes.push_back(n);
    const std::string header =
        "// Generated by GPU Inspector's Export to C++ (vkinsp_replay --export). See README.md.\n"
    ...
    for (size_t f = 0; f < fileTexts.size(); ++f)
    {
        ...
        if (!WriteText(name, fileTexts[f], error))
            return false;
...
{
    std::string s =
        "// The Vulkan functions this frame calls, loaded by name from the loader opened at run time\n"
...
std::string Exporter::FunctionsSource(const std::set<std::string>& functions) const
{
    ...
    for (const std::string& f : functions)
    {
        if (f == "vkGetInstanceProcAddr" || f == "vkGetDeviceProcAddr")
            continue;
        const int level = VulkanFunctionLevel(f);
        std::string& into = level == 0 ? global : level == 1 ? inst
                                                             : dev;
        const char* getter = level == 0 ? "vkGetInstanceProcAddr(nullptr, \"" : level == 1 ? "vkGetInstanceProcAddr(instance, \""
...
{
    std::string s =
        "// One variable per object of the capture, named by its type and its id in the capture (the id\n"
        "// GPU Inspector shows), and the functions the frame is made of.\n"
        "#pragma once\n\n#include \"vk_support.h\"\n\n";
    for (const auto& [type, name] : _handles)
        s += "extern " + type + " " + name + ";\n";
...
bool Exporter::Finish(const ReplayReport& report, ExportReport& out)
{
    ...
    for (Section* s : {&_objectsSection, &_contentsSection, &_frameSection, &_destroySection, &_restoreSection})
        if (!WriteSection(*s, sources, out.error))
            return false;
    if (_outputSource.empty())
        _outputSource =
            "// The frame has no output the export could name, so there is nothing for a window to show.\n"
            "bool FrameOutput(FrameOutputInfo* output) {\n    (void)output;\n    return false;\n}\n";

    std::set<std::string> functions(_used);
    for (const char* f : kSupportFunctions)
        functions.insert(f);
    ...
    const std::string deviceSource =
        "// The instance and the device the frame runs on, as the replay created them.\n"
        "#include \"frame_handles.h\"\n#include \"vk_support.h\"\n\n" +
        _instanceSource + "\n" + _deviceSource + "\n" + _outputSource;
    ...
    out.files = sources;
    for (const char* name : {"vk_support.h", "vk_functions.h", "frame_handles.h", "CMakeLists.txt", "README.md", "frame_data.bin"})
    ...
    for (size_t i = 0; i < kExportTemplatesCount; ++i)
    {
        ...
        if (!WriteText(name, Template(name.c_str()), out.error))
            return false;
        if (name.rfind("vulkan_headers/", 0) == 0)
            out.files.push_back(name);
    }
    return true;
}
