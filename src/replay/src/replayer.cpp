...
{
    if (handle)
        _created.push_back({type, handle});
}
...
bool Replayer::CreateInstance()
{
    ...
    if (_options.counters.enabled || _options.allFeatures)
    {
        if (nvperf::Load(_nvperfNote))
        {
            ...
            if (_fns.EnumerateInstanceVersion)
                _fns.EnumerateInstanceVersion(&version);
    ...
    if (_options.validation)
    {
        ...
        if (found)
            ...
            Problem("the validation layer is not installed (Vulkan SDK)");
    }
    ...
    if (_exporter)
    {
        ...
        for (const char* e : extensions)
            if (!std::strcmp(e, VK_EXT_DEBUG_UTILS_EXTENSION_NAME) || !std::strcmp(e, VK_KHR_SURFACE_EXTENSION_NAME))
...
bool Replayer::CreateDevice()
{
    ...
    if (const JValue* objects = _capture->Objects(); objects && objects->IsArray())
    {
        for (uint32_t i = 0; i < objects->count && !deviceObject; ++i)
            if (Str(objects->items[i].Get("type")) == "VkDevice")
                deviceObject = &objects->items[i];
    }
    const JValue* deviceArgs = deviceObject ? deviceObject->Get("args") : nullptr;
    std::string capturedName;
    if (deviceArgs)
    {
        if (const JValue* props = deviceArgs->Get("properties"))
            capturedName = Str(props->Get("deviceName"));
    }
    ...
    for (VkPhysicalDevice p : physicals)
    {
        ...
        if (!capturedName.empty() && capturedName == props.deviceName)
        {
            chosen = p;
            chosenProps = props;
            break;
    ...
    if (queuesValid)
    {
        for (uint32_t i = 0; i < captured.pCreateInfo->queueCreateInfoCount; ++i)
        {
            ...
            if (q.queueFamilyIndex >= families.size() || q.queueCount > families[q.queueFamilyIndex].queueCount)
                queuesValid = false;
        }
    }
    if (queuesValid)
    ...
    if (captured.pCreateInfo)
    {
        for (uint32_t i = 0; i < captured.pCreateInfo->enabledExtensionCount; ++i)
        {
            ...
            if (name && hasExtension(name))
                ...
                Problem(std::string("device extension not available here: ") + name);
        }
    }
    ...
    if (xfbExtension && _fns.GetPhysicalDeviceFeatures2)
    {
        ...
        if (supported.transformFeedback)
        {
            ...
            for (auto* s = (VkBaseOutStructure*)const_cast<void*>(info.pNext); s; s = s->pNext)
                if (s->sType == VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_TRANSFORM_FEEDBACK_FEATURES_EXT)
                    existing = (VkPhysicalDeviceTransformFeedbackFeaturesEXT*)s;
            if (existing)
            {
    ...
    if (wantCounters)
    {
        if (nvperf::Load(_nvperfNote))
        {
            ...
            for (const char* name : perf)
            {
                ...
                if (std::none_of(extensions.begin(), extensions.end(), [&](const char* e) { return !std::strcmp(e, name); }))
                    extensions.push_back(name);
            }
            _nvperfReady = all;   // an NVIDIA device with every extension the SDK asked for
            if (!perf.empty() && !all)
                _nvperfNote = "this device is missing an extension the Nsight Perf SDK needs";
        }
        ...
        if (!_nvperfReady && hasExtension(VK_KHR_PERFORMANCE_QUERY_EXTENSION_NAME) && _fns.GetPhysicalDeviceFeatures2)
        {
            ...
            if (supported.performanceCounterQueryPools)
            {
                ...
                for (auto* s = (VkBaseOutStructure*)const_cast<void*>(info.pNext); s; s = s->pNext)
                    if (s->sType == VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PERFORMANCE_QUERY_FEATURES_KHR)
                        existing = (VkPhysicalDevicePerformanceQueryFeaturesKHR*)s;
                if (existing)
                {
    ...
    if (deviceObject)
    {
        ...
        if (const JValue* id = deviceObject->Get("id"))
            _handles[id->Uint()] = (uint64_t)(uintptr_t)_device;
    }
    ...
    if (const JValue* objects = _capture->Objects(); objects && objects->IsArray())
    {
        for (uint32_t i = 0; i < objects->count; ++i)
        {
            ...
            if (Str(o.Get("type")) != "VkDevice" || &o == deviceObject)
                continue;
            if (const JValue* id = o.Get("id"))
                _handles[id->Uint()] = (uint64_t)(uintptr_t)_device;
            if (const JValue* args = o.Get("args"))
                _handles[IdOf(args->Get("physicalDevice"))] = (uint64_t)(uintptr_t)_physical;
        }
    }
    return true;
}
...
bool Replayer::CreateStaging(VkDeviceSize size, Staging& staging, VkBufferUsageFlags usage)
{
    ...
    info.usage = usage;
    if (_fns.CreateBuffer(_device, &info, nullptr, &staging.buffer) != VK_SUCCESS)
        return false;
...
bool Replayer::RunOneTime(const std::function<void(VkCommandBuffer)>& record)
{
    ...
    if (_fns.AllocateCommandBuffers(_device, &alloc, &cb) != VK_SUCCESS)
        return false;
...
void Replayer::UploadToBuffer(VkBuffer buffer, VkDeviceSize offset, const uint8_t* data, size_t size)
{
    if (_batchingUploads)
    {
        ...
        if (!_pendingUploads.empty() && _uploadBytes.size() + size > kBatchBytes)
            FlushUploads();
...
void Replayer::FlushUploads()
{
    ...
    if (!CreateStaging(bytes.size(), staging))
    {
        ...
        for (const PendingUpload& u : uploads)
            UploadToBuffer(u.buffer, u.offset, bytes.data() + u.at, u.size);
        ...
        for (uint32_t l = 0; l < image.layers;)
        {
            ...
            if (to != VK_IMAGE_LAYOUT_UNDEFINED && to != from)
            {
                ...
                for (uint32_t k = l; k < end; ++k)
                    image.layouts[(size_t)m * image.layers + k] = to;
            }
            l = end;
        }
    }
...
uint64_t Replayer::CreateImage(uint64_t id, const VkImageCreateInfo& captured)
{
    ...
    info.pNext = StripPNext(info.pNext, {VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_IMAGE_CREATE_INFO, VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_IMAGE_CREATE_INFO_NV, VK_STRUCTURE_TYPE_IMAGE_DRM_FORMAT_MODIFIER_LIST_CREATE_INFO_EXT, VK_STRUCTURE_TYPE_IMAGE_DRM_FORMAT_MODIFIER_EXPLICIT_CREATE_INFO_EXT});
    ...
    _images[id] = rec;
    if (_exporter)
        _exporter->CreateImage(id, image, info, _exportComment);
...
VkShaderModule Replayer::ModuleFromBlob(const JValue& object, const std::string& blobName, const uint8_t** code, size_t* codeSize)
{
    ...
    {
        if (r.pipeline != id || r.words.empty() || blobName.rfind(r.stage + ":", 0) != 0)
            continue;
        data = reinterpret_cast<const uint8_t*>(r.words.data());
        size = r.words.size() * 4;
        break;
    }
    if (!data && (!_capture->Blob(object, blobName, data, size) || size < 4))
    ...
    if (_fns.CreateShaderModule(_device, &info, nullptr, &module) != VK_SUCCESS)
        return VK_NULL_HANDLE;
    return module;
}
...
uint64_t Replayer::CreatePipeline(const JValue& object, std::string_view cmd, uint32_t index, const JValue& args, size_t unresolvedBefore)
{
    ...
    if (cmd == "vkCreateGraphicsPipelines")
    {
        ...
        if (!a.pCreateInfos || index >= a.createInfoCount || _ctx.unresolved != unresolvedBefore)
            return 0;
    ...
    else if (cmd == "vkCreateComputePipelines")
    {
        ...
        if (_exporter && pipeline)
            _exporter->CreatePipeline(object.Get("id")->Uint(), pipeline, std::string(cmd), nullptr, &info, nullptr, exportedModules);
    }
    else if (cmd == "vkCreateRayTracingPipelinesKHR")
    {
        ...
        {
            Problem("pipeline " + std::to_string(object.Get("id")->Uint()) + ": this device has no ray tracing pipelines");
        }
        else
        {
            r = _fns.CreateRayTracingPipelinesKHR(_device, VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &info, nullptr, &pipeline);
            if (_exporter && pipeline)
                _exporter->CreatePipeline(object.Get("id")->Uint(), pipeline, std::string(cmd), nullptr, nullptr, &info, exportedModules);
        }
    }
    else
    {
    ...
    for (VkShaderModule m : temporary)
        _fns.DestroyShaderModule(_device, m, nullptr);
...
uint64_t Replayer::CreateShaderObject(const JValue& object, uint32_t index, const JValue& args, size_t unresolvedBefore)
{
    ...
    if (_exporter)
        _exporter->CreateShaderObject(object.Get("id")->Uint(), shader, info);
...
void Replayer::CreateObject(const JValue& o)
{
    ...
    if (_handles.count(id))
        return;  // the device and its physical device
    ...
    else if (type == "VkQueue")
    {
        ...
        if (_exporter)
            _exporter->Queue(id, (VkQueue)(uintptr_t)handle, family, qi);
    }
        ...
        type == "VkAccelerationStructureNV" || type == "VkDeferredOperationKHR")
    {
        ...
        if (_exporter)
            _exporter->Skipped(type, id, "left out on purpose: the frame does not need it to run again");
        return;
    }
    ...
    else if (type == "VkImage" && cmd == "vkCreateImage")
    {
        ...
        DecodeArgs(_ctx, *args, a);
        if (a.pCreateInfo)
            handle = CreateImage(id, *a.pCreateInfo);
    }
    else if (type == "VkBuffer" && cmd == "vkCreateBuffer")
    {
        ...
        DecodeArgs(_ctx, *args, a);
        if (a.pCreateInfo)
            handle = CreateBuffer(id, *a.pCreateInfo);
    }
    else if (type == "VkImageView")
    {
        ...
        if (a.pCreateInfo && a.pCreateInfo->image && resolved() && _fns.CreateImageView(d, a.pCreateInfo, nullptr, &view) == VK_SUCCESS)
        {
            ...
            if (_exporter)
                _exporter->Create(type, id, handle, "vkCreateImageView", *a.pCreateInfo);
            ViewRecord rec;
    ...
    else if (type == "VkBufferView")
    {
        ...
        if (a.pCreateInfo && resolved() && _fns.CreateBufferView(d, a.pCreateInfo, nullptr, &view) == VK_SUCCESS)
            handle = (uint64_t)view;
        if (handle && _exporter)
            _exporter->Create(type, id, handle, "vkCreateBufferView", *a.pCreateInfo);
    }
    else if (type == "VkSampler")
    {
        ...
        if (a.pCreateInfo && resolved() && _fns.CreateSampler(d, a.pCreateInfo, nullptr, &s) == VK_SUCCESS)
            handle = (uint64_t)s;
        if (handle && _exporter)
            _exporter->Create(type, id, handle, "vkCreateSampler", *a.pCreateInfo);
    }
    else if (type == "VkDescriptorSetLayout")
    {
        ...
        if (a.pCreateInfo && resolved() && _fns.CreateDescriptorSetLayout(d, a.pCreateInfo, nullptr, &l) == VK_SUCCESS)
            handle = (uint64_t)l;
        if (handle && _exporter)
            _exporter->Create(type, id, handle, "vkCreateDescriptorSetLayout", *a.pCreateInfo);
    }
    else if (type == "VkPipelineLayout")
    {
        ...
        if (a.pCreateInfo && resolved() && _fns.CreatePipelineLayout(d, a.pCreateInfo, nullptr, &l) == VK_SUCCESS)
            handle = (uint64_t)l;
        if (handle && _exporter)
            _exporter->Create(type, id, handle, "vkCreatePipelineLayout", *a.pCreateInfo);
    }
    else if (type == "VkDescriptorPool")
    {
        ...
        if (a.pCreateInfo && resolved() && _fns.CreateDescriptorPool(d, a.pCreateInfo, nullptr, &p) == VK_SUCCESS)
            handle = (uint64_t)p;
        if (handle && _exporter)
            _exporter->Create(type, id, handle, "vkCreateDescriptorPool", *a.pCreateInfo);
    }
    else if (type == "VkCommandPool")
    {
        ...
        if (a.pCreateInfo)
        {
            ...
            if (_fns.CreateCommandPool(d, &info, nullptr, &p) == VK_SUCCESS)
                handle = (uint64_t)p;
            if (handle && _exporter)
                _exporter->Create(type, id, handle, "vkCreateCommandPool", info, "its command buffers can be reset one at a time");
        }
    }
    else if (type == "VkCommandBuffer")
    {
        ...
        if (a.pAllocateInfo && a.pAllocateInfo->commandPool)
        {
            ...
            VkCommandBuffer cb = VK_NULL_HANDLE;
            if (_fns.AllocateCommandBuffers(d, &info, &cb) == VK_SUCCESS)
                handle = (uint64_t)(uintptr_t)cb;
            if (handle && _exporter)
                _exporter->AllocateCommandBuffer(id, cb, info);
        }
    }
    else if (type == "VkDescriptorSet")
    {
        ...
        if (a.pAllocateInfo && index < a.pAllocateInfo->descriptorSetCount && resolved())
        {
            ...
            if (r == VK_SUCCESS)
                handle = (uint64_t)set;
            if (handle && _exporter)
                _exporter->AllocateDescriptorSet(id, set, info);
            if (r != VK_SUCCESS)
                Problem("descriptor set " + std::to_string(id) + ": vkAllocateDescriptorSets failed (" + std::to_string(r) + ")");
        }
    }
    else if (type == "VkFence")
    {
        ...
        if (a.pCreateInfo && resolved() && _fns.CreateFence(d, a.pCreateInfo, nullptr, &f) == VK_SUCCESS)
            handle = (uint64_t)f;
        if (handle && _exporter)
            _exporter->Create(type, id, handle, "vkCreateFence", *a.pCreateInfo);
    }
    else if (type == "VkSemaphore")
    {
        ...
        if (a.pCreateInfo && resolved() && _fns.CreateSemaphore(d, a.pCreateInfo, nullptr, &s) == VK_SUCCESS)
            handle = (uint64_t)s;
        if (handle && _exporter)
            _exporter->Create(type, id, handle, "vkCreateSemaphore", *a.pCreateInfo);
    }
    else if (type == "VkEvent")
    {
        ...
        if (a.pCreateInfo && resolved() && _fns.CreateEvent(d, a.pCreateInfo, nullptr, &e) == VK_SUCCESS)
            handle = (uint64_t)e;
        if (handle && _exporter)
            _exporter->Create(type, id, handle, "vkCreateEvent", *a.pCreateInfo);
    }
    else if (type == "VkQueryPool")
    {
        ...
        if (a.pCreateInfo && resolved() && _fns.CreateQueryPool(d, a.pCreateInfo, nullptr, &q) == VK_SUCCESS)
            handle = (uint64_t)q;
        if (handle && _exporter)
            _exporter->Create(type, id, handle, "vkCreateQueryPool", *a.pCreateInfo);
    }
    else if (type == "VkRenderPass" && cmd == "vkCreateRenderPass")
    {
        ...
        if (a.pCreateInfo)
        {
            ...
            if (_fns.CreateRenderPass(d, &info, nullptr, &rp) == VK_SUCCESS)
            {
                ...
                if (_exporter)
                    _exporter->Create(type, id, handle, "vkCreateRenderPass", info, "every attachment stored, so each pass's result can be read");
            }
        }
    }
    else if (type == "VkRenderPass" && (cmd == "vkCreateRenderPass2" || cmd == "vkCreateRenderPass2KHR"))
    {
        ...
        if (a.pCreateInfo)
        {
            ...
            if (_fns.CreateRenderPass2(d, &info, nullptr, &rp) == VK_SUCCESS)
            {
                ...
                _renderPasses[id] = rec;
                if (_exporter)
                    _exporter->Create(type, id, handle, "vkCreateRenderPass2", info, "every attachment stored, so each pass's result can be read");
            }
        }
    }
        ...
        if (a.pCreateInfo && resolved() && _fns.CreateFramebuffer(d, a.pCreateInfo, nullptr, &fb) == VK_SUCCESS)
        {
            ...
            if (const JValue* list = args->Get("pCreateInfo")->Get("pAttachments"); list && list->IsArray())
                for (uint32_t i = 0; i < list->count; ++i)
                    views.push_back(IdOf(&list->items[i]));
    ...
    else if (type == "VkShaderModule")
    {
        ...
        if (!m && !hasCode && args)
        {
            ...
            if (words && words->IsArray())
            {
                ...
                if (m && _exporter)
                    _exporter->Create(type, id, (uint64_t)m, "vkCreateShaderModule", *a.pCreateInfo);
            }
        }
        handle = (uint64_t)m;
        if (!m)
    ...
    {
        if (args)
            handle = CreatePipeline(o, cmd, index, *args, unresolvedBefore);
    }
    else if (type == "VkShaderEXT")
    {
        if (args)
            handle = CreateShaderObject(o, index, *args, unresolvedBefore);
    }
    else
    {
        known = false;
    ...
    if (!known)
    {
        ...
        _report->objectsSkipped++;
        if (_exporter)
            _exporter->Skipped(type, id, "left out: the replay does not make objects of this type yet");
        return;
    }
    if (!handle)
    {
        ...
        _report->objectsSkipped++;
        if (_exporter)
            _exporter->Skipped(type, id, quiet ? "left out: the capture holds no code for it, and pipelines carry their own" : resolved() ? "left out: the replay could not create it"
                                                                                                                                          : "left out: it names objects the replay does not have");
        return;
    }
    _handles[id] = handle;
    if (type != "VkInstance" && type != "VkPhysicalDevice" && type != "VkQueue")
        Track(type, handle);
...
void Replayer::DestroyAll()
{
    if (_device)
    {
        ...
        ReleaseTransients();
        if (_countModule)
            _fns.DestroyShaderModule(_device, _countModule, nullptr);
        if (_backFaceModule)
            _fns.DestroyShaderModule(_device, _backFaceModule, nullptr);
        _countModule = VK_NULL_HANDLE;
        _backFaceModule = VK_NULL_HANDLE;
        if (_primitiveIdModule)
            _fns.DestroyShaderModule(_device, _primitiveIdModule, nullptr);
        ...
        for (auto it = _created.rbegin(); it != _created.rend(); ++it)
        {
            ...
            if (t == "VkImage")
                _fns.DestroyImage(_device, (VkImage)h, nullptr);
            else if (t == "VkBuffer")
                _fns.DestroyBuffer(_device, (VkBuffer)h, nullptr);
            else if (t == "VkImageView")
                _fns.DestroyImageView(_device, (VkImageView)h, nullptr);
            else if (t == "VkBufferView")
                _fns.DestroyBufferView(_device, (VkBufferView)h, nullptr);
            else if (t == "VkSampler")
                _fns.DestroySampler(_device, (VkSampler)h, nullptr);
            else if (t == "VkDescriptorSetLayout")
                _fns.DestroyDescriptorSetLayout(_device, (VkDescriptorSetLayout)h, nullptr);
            else if (t == "VkPipelineLayout")
                _fns.DestroyPipelineLayout(_device, (VkPipelineLayout)h, nullptr);
            else if (t == "VkDescriptorPool")
                _fns.DestroyDescriptorPool(_device, (VkDescriptorPool)h, nullptr);
            else if (t == "VkCommandPool")
                _fns.DestroyCommandPool(_device, (VkCommandPool)h, nullptr);
            else if (t == "VkFence")
                _fns.DestroyFence(_device, (VkFence)h, nullptr);
            else if (t == "VkSemaphore")
                _fns.DestroySemaphore(_device, (VkSemaphore)h, nullptr);
            else if (t == "VkEvent")
                _fns.DestroyEvent(_device, (VkEvent)h, nullptr);
            else if (t == "VkQueryPool")
                _fns.DestroyQueryPool(_device, (VkQueryPool)h, nullptr);
            else if (t == "VkRenderPass")
                _fns.DestroyRenderPass(_device, (VkRenderPass)h, nullptr);
            else if (t == "VkFramebuffer")
                _fns.DestroyFramebuffer(_device, (VkFramebuffer)h, nullptr);
            else if (t == "VkShaderModule")
                _fns.DestroyShaderModule(_device, (VkShaderModule)h, nullptr);
            else if (t == "VkPipeline")
                _fns.DestroyPipeline(_device, (VkPipeline)h, nullptr);
            else if (t == "VkShaderEXT" && _fns.DestroyShaderEXT)
                _fns.DestroyShaderEXT(_device, (VkShaderEXT)h, nullptr);
        ...
        _bindingTableSize = 0;
        for (VkDeviceMemory m : _memories)
            _fns.FreeMemory(_device, m, nullptr);
        if (_utilityPool)
            _fns.DestroyCommandPool(_device, _utilityPool, nullptr);
        _fns.DestroyDevice(_device, nullptr);
    }
    if (_messenger)
        _fns.DestroyDebugUtilsMessengerEXT(_instance, _messenger, nullptr);
    if (_instance)
        _fns.DestroyInstance(_instance, nullptr);
    _created.clear();
    _memories.clear();
...
void Replayer::ComputeInitialLayouts()
{
    ...
    auto note = [&](uint64_t id, uint32_t baseMip, uint32_t mips, uint32_t baseLayer, uint32_t layers, VkImageLayout layout, bool leaves = false) {
        ...
        if (it == _images.end() || layout == VK_IMAGE_LAYOUT_UNDEFINED)
            return;
        const ImageRecord& image = it->second;
        if (baseMip >= image.mips || baseLayer >= image.layers)
            return;
        ...
        for (uint32_t m = baseMip; m < baseMip + mips; ++m)
            for (uint32_t l = baseLayer; l < baseLayer + layers; ++l)
            {
                ...
                if (!leaves && layouts[k] == VK_IMAGE_LAYOUT_UNDEFINED)
                    layouts[k] = layout;
    ...
    auto noteLayers = [&](uint64_t id, const JValue* layers, VkImageLayout layout) {
        if (layers)
            note(id, u32(layers->Get("mipLevel"), 0), 1, u32(layers->Get("baseArrayLayer"), 0), u32(layers->Get("layerCount"), 1), layout);
    };
    ...
    for (uint32_t i = 0; i < commands->count; ++i)
    {
        ...
        if (IsBeginRenderPass(m))
        {
            ...
            if (rit != _renderPasses.end() && fit != _framebufferViews.end())
            {
                ...
                for (size_t a = 0; a < fit->second.size() && a < rit->second.initialLayouts.size(); ++a)
                {
                    ...
                    if (a < rit->second.finalLayouts.size())
                        noteView(fit->second[a], rit->second.finalLayouts[a], true);
                }
            }
        }
        else if (IsBeginRendering(m))
        {
            ...
            if (info && !resuming)
            {
                if (const JValue* colors = info->Get("pColorAttachments"); colors && colors->IsArray())
                    for (uint32_t k = 0; k < colors->count; ++k)
                        attachment(&colors->items[k]);
        ...
        else if (m == "vkCmdClearColorImage" || m == "vkCmdClearDepthStencilImage")
        {
            if (const JValue* ranges = args->Get("pRanges"); ranges && ranges->IsArray())
                for (uint32_t k = 0; k < ranges->count; ++k)
                    noteRange(IdOf(args->Get("image")), &ranges->items[k], layoutOf(args->Get("imageLayout")));
        }
        else if (StartsWith(m, "vkCmdCopy") || StartsWith(m, "vkCmdBlitImage") || StartsWith(m, "vkCmdResolveImage"))
        {
            noteTransfer(args);
            for (const char* name : kTransferInfos)
                noteTransfer(args->Get(name));
        }
    }
    _arena.Reset();
}
...
void Replayer::UploadImageContents()
{
    ...
    for (const char* kind : {"sampled", "initial"})
        for (uint32_t i = 0; i < textures->count; ++i)
            if (const JValue* info = textures->items[i].Get("info"); info && Str(info->Get("kind")) == kind)
                ordered.push_back(&textures->items[i]);
    for (const JValue* texture : ordered)
    {
        ...
        const uint8_t* data = nullptr;
        size_t size = 0;
        if (!_capture->Payload(t.Get("payload"), data, size))
            continue;
        ...
        if (regions.empty())
            continue;
        Staging staging;
        if (!CreateStaging(offset, staging))
            continue;
        ...
        DestroyStaging(staging);
        if (_exporter)
            _exporter->UploadImage(info->Get("id")->Uint(), image.image, initial, regions, data, (size_t)offset);
        if (initial)
            _report->initialImagesUploaded++;
        else
...
{
    if (_exporter)
        _exporter->BeginInitialLayouts();
    RunOneTime([&](VkCommandBuffer cb) {
        for (auto& [id, image] : _images)
        {
            auto it = _initialLayouts.find(id);
            if (it == _initialLayouts.end())
                continue;
            std::vector<VkImageLayout> targets = it->second;
            if (!_hasSwapchainExtension)
                for (VkImageLayout& t : targets)
                    if (t == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR)
                        t = VK_IMAGE_LAYOUT_UNDEFINED;
            if (_exporter)
                _exporter->InitialLayouts(id, image.image, targets);
            TransitionSubresources(cb, image, targets);
        }
    });
    if (_exporter)
        _exporter->EndInitialLayouts();
}
...
void Replayer::ApplyBufferData(const CommandGroup& group)
{
    ...
    auto apply = [&](uint64_t dataId) {
        if (!dataId || applied.count(dataId))
            return;
        ...
        if (const JValue* d = c.Get("descriptors"))
        {
            ...
            for (uint32_t s = 0; sets && s < sets->count; ++s)
            {
                ...
                for (uint32_t b = 0; bindings && b < bindings->count; ++b)
                {
                    ...
                    {
                        const JValue& desc = list->items[k];
                        if (desc.Get("buffer") && desc.Get("data"))
                            apply(desc.Get("data")->Uint());
                    }
                }
            }
        }
...
void Replayer::ApplyDescriptorSnapshot(const JValue* descriptors)
{
    ...
    for (uint32_t s = 0; s < sets->count; ++s)
    {
        ...
        if (!setId || !handle)
            continue;  // push descriptors are recorded by their own command
        DescriptorWrites w;
        BuildDescriptorWrites(set, handle, w);
        auto& last = _descriptorContents[setId];
        if (last == w.key)
            continue;  // rewriting a bound set would invalidate the command buffers that bound it
        last = w.key;
        if (!w.writes.empty())
            _fns.UpdateDescriptorSets(_device, (uint32_t)w.writes.size(), w.writes.data(), 0, nullptr);
        if (_exporter)
            _exporter->UpdateDescriptorSets(setId, w.writes);
    }
    _arena.Reset();
}
...
{
    if (!bufferId || !_fns.GetBufferDeviceAddress)
        return 0;
...
{
    if (!size || !_fns.GetBufferDeviceAddress)
        return false;
    ...
    if (!_scratch || at + size > _scratchSize)
    {
        ...
        if (_scratch)
            _retiredScratch.emplace_back(_scratch, _scratchMemory);
        _scratch = VK_NULL_HANDLE;
        _scratchMemory = VK_NULL_HANDLE;
        _scratchSize = 0;
        ...
        info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
        if (_fns.CreateBuffer(_device, &info, nullptr, &_scratch) != VK_SUCCESS)
            return false;
        VkMemoryRequirements requirements{};
        _fns.GetBufferMemoryRequirements(_device, _scratch, &requirements);
        if (!AllocateBound(requirements, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, _scratchMemory, false, true))
            return false;
        if (_fns.BindBufferMemory(_device, _scratch, _scratchMemory, 0) != VK_SUCCESS)
            return false;
        _scratchSize = want;
        _scratchUsed = 0;
        at = 0;
...
{
    if (!capturedAddress || !_fns.GetAccelerationStructureDeviceAddressKHR)
        return 0;
    ...
    std::vector<uint8_t>* patchedOut, const uint8_t** capturedOut, size_t* sizeOut)
{
    ...
    const uint8_t* data = nullptr;
    size_t size = 0;
    if (!_capture->Payload(it->second->Get("payload"), data, size) || size < kStride)
        return false;
    ...
    bool exportOneTime)
{
    ...
    auto entryFor = [&](uint32_t info, uint32_t geometry, std::string_view field) -> const JValue* {
        ...
        for (uint32_t k = 0; k < resolved->count; ++k)
        {
            ...
            if ((ij ? ij->Uint() : 0) != info || (gj ? gj->Uint() : 0) != geometry)
                continue;
            return &e;
        }
        return nullptr;
    ...
    for (uint32_t i = 0; i < a.infoCount; ++i)
    {
        ...
        for (uint32_t g = 0; g < info.geometryCount; ++g)
        {
            const VkAccelerationStructureGeometryKHR* source = info.pGeometries ? &info.pGeometries[g]
                : info.ppGeometries                                             ? info.ppGeometries[g]
                                                                                : nullptr;
            if (!source)
                continue;
            ...
            const bool missing =
                (geometry.geometryType == VK_GEOMETRY_TYPE_TRIANGLES_KHR && !geometry.geometry.triangles.vertexData.deviceAddress) || (geometry.geometryType == VK_GEOMETRY_TYPE_AABBS_KHR && !geometry.geometry.aabbs.data.deviceAddress) || (geometry.geometryType == VK_GEOMETRY_TYPE_INSTANCES_KHR && !geometry.geometry.instances.data.deviceAddress);
            if (missing)
            {
...
std::string EarlierBuildArgs(uint64_t structureId, const JValue& build)
{
    ...
    for (uint32_t g = 0; g < count; ++g)
    {
        ...
        {
            data =
                "{\"triangles\":{\"sType\":\"VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_TRIANGLES_DATA_KHR\",\"pNext\":null,"
                "\"vertexFormat\":" +
                QuotedMember(geometry, "vertexFormat") +
                ",\"vertexData\":{\"deviceAddress\":0},"
                "\"vertexStride\":" +
                NumberMember(geometry, "vertexStride") + ",\"maxVertex\":" + NumberMember(geometry, "maxVertex") + ",\"indexType\":" + QuotedMember(geometry, "indexType") +
        ...
        {
            data =
                "{\"aabbs\":{\"sType\":\"VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_AABBS_DATA_KHR\",\"pNext\":null,"
                "\"data\":{\"deviceAddress\":0},\"stride\":" +
                NumberMember(geometry, "stride") + "}}";
        }
        else if (type == "VK_GEOMETRY_TYPE_INSTANCES_KHR")
        {
            const JValue* pointers = geometry.Get("arrayOfPointers");
            data =
                "{\"instances\":{\"sType\":\"VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_INSTANCES_DATA_KHR\",\"pNext\":null,"
                "\"arrayOfPointers\":" +
                std::string(pointers && pointers->IsBool() && pointers->boolean ? "true" : "false") + ",\"data\":{\"deviceAddress\":0}}}";
        }
        else
        {
            return "";
        ...
        list += g ? "," : "";
        list += "{\"sType\":\"VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_KHR\",\"pNext\":null,\"geometryType\":\"" + std::string(type) + "\",\"geometry\":" + data + ",\"flags\":" + QuotedMember(geometry, "flags") + "}";
        ranges += g ? "," : "";
        ranges += "{\"primitiveCount\":" + NumberMember(geometry, "primitiveCount") + ",\"primitiveOffset\":0,\"firstVertex\":0,\"transformOffset\":0}";
    }
    if (!count)
        return "";
    return "{\"infoCount\":1,\"pInfos\":[{\"sType\":\"VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_GEOMETRY_INFO_KHR\",\"pNext\":null,"
           "\"type\":" +
        QuotedMember(build, "type") + ",\"flags\":" + QuotedMember(build, "flags") +
        ",\"mode\":\"VK_BUILD_ACCELERATION_STRUCTURE_MODE_BUILD_KHR\",\"srcAccelerationStructure\":null,"
        "\"dstAccelerationStructure\":{\"__id\":" +
        std::to_string(structureId) +
        ",\"__class\":\"VkAccelerationStructureKHR\"},"
        "\"geometryCount\":" +
        std::to_string(count) + ",\"pGeometries\":[" + list +
        "],\"ppGeometries\":null,"
        "\"scratchData\":{\"deviceAddress\":0}}],\"ppBuildRangeInfos\":[[" +
        ranges + "]]}";
}
...
void Replayer::BuildEarlierStructures()
{
    const JValue* objects = _capture->Objects();
    const JValue* commands = _capture->Commands();
    if (!objects || !objects->IsArray() || !_fns.CmdBuildAccelerationStructuresKHR)
        return;
    ...
    for (uint32_t i = 0; commands && i < commands->count; ++i)
    {
        ...
        if (!method || method->Str().find("BuildAccelerationStructures") == std::string_view::npos)
            continue;
        const JValue* args = c.Get("args");
    ...
    for (uint32_t i = 0; i < objects->count; ++i)
    {
        ...
        if (!type || type->Str() != "VkAccelerationStructureKHR")
            continue;
        const uint64_t id = o.Get("id") ? o.Get("id")->Uint() : 0;
        if (!id || builtInFrame.count(id) || !Handle(id))
            continue;
        const JValue* updates = o.Get("updates");
        ...
        if (!build || !inputs || !inputs->IsArray())
            continue;
        ...
        for (uint32_t k = 0; k < inputs->count; ++k)
        {
            ...
            if (!info || info->Get("error") || !info->Get("buffer") || info->Get("buffer")->Uint() != (input.Get("buffer") ? input.Get("buffer")->Uint() : 0))
                continue;
            if (!UploadCapturedBuffer(capture))
                continue;
            const JValue* field = input.Get("field");
            buildData += taken++ ? "," : "";
            buildData += "{\"info\":0,\"geometry\":" + NumberMember(input, "geometry") + ",\"field\":\"" + std::string(field ? field->Str() : std::string_view()) + "\",\"buffer\":" + NumberMember(input, "buffer") + ",\"offset\":" + NumberMember(input, "offset") + ",\"capture\":" + std::to_string(capture) + "}";
        }
        if (!taken)
            continue;
...
bool Replayer::TraceRays(const JValue& command, const JValue& args, VkCommandBuffer cb, uint32_t exportIndex)
{
    ...
    properties.pNext = &rt;
    if (_fns.GetPhysicalDeviceProperties2)
        _fns.GetPhysicalDeviceProperties2(_physical, &properties);
    ...
    for (uint32_t i = 0; i < regions->count; ++i)
    {
        ...
        const JValue* region = args.Get(name == "raygen" ? "pRaygenShaderBindingTable"
                ...
                : name == "hit"                          ? "pHitShaderBindingTable"
        ...
        for (size_t at = 0; at + handleSize <= p.bytes.size(); at += (size_t)stride)
        {
            ...
            {
                if (!memcmp(&p.bytes[at], capturedHandles + (size_t)g * handleSize, handleSize))
                    group = g;
            }
            if (group == UINT32_MAX)
            {
...
{
    if (size <= _bindingTableSize && _bindingTable)
        return true;
    if (_bindingTable)
    {
        _fns.DestroyBuffer(_device, _bindingTable, nullptr);
    ...
    VkBufferCreateInfo info{VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};
    info.size = size;
    info.usage = VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
    info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    if (_fns.CreateBuffer(_device, &info, nullptr, &_bindingTable) != VK_SUCCESS)
        return false;
    VkMemoryRequirements requirements{};
    ...
    if (!AllocateBound(requirements, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            _bindingTableMemory, false, true))
        return false;
    if (_fns.BindBufferMemory(_device, _bindingTable, _bindingTableMemory, 0) != VK_SUCCESS)
        return false;
    void* mapped = nullptr;
    if (_fns.MapMemory(_device, _bindingTableMemory, 0, VK_WHOLE_SIZE, 0, &mapped) != VK_SUCCESS)
        return false;
...
void Replayer::IssueCommand(ReplayFn fn, const JValue& command, const JValue& args, VkCommandBuffer cb, uint32_t index)
{
    ...
    if (w.writes.empty())
        return;
    if (_fns.CmdPushDescriptorSetKHR)
        _fns.CmdPushDescriptorSetKHR(cb, bindPoint, layout, set, (uint32_t)w.writes.size(), w.writes.data());
    else if (_fns.CmdPushDescriptorSet)
        _fns.CmdPushDescriptorSet(cb, bindPoint, layout, set, (uint32_t)w.writes.size(), w.writes.data());
    else
        _ctx.Problem("left out: push descriptors are not available on this device");
...
{
    {
        ...
        for (uint32_t b = 0; bindings && b < bindings->count; ++b)
        {
            ...
            {
                while (k < list->count && list->items[k].IsNull())
                    ++k;
                if (k >= list->count)
                    break;
                uint32_t start = k;
                ...
                for (; k < list->count && !list->items[k].IsNull(); ++k)
                {
                    ...
                    else
                    {
                        ...
                        if (!desc.Get("immutable") || !desc.Get("immutable")->boolean)
                            ii.sampler = (VkSampler)Handle(IdOf(desc.Get("sampler")));
                        images->push_back(ii);
                ...
                if (!views->empty())
                    w.pTexelBufferView = views->data();
                if (!structures->empty())
                {
                    // A structure the replay did not make leaves the binding out rather than writing a null.
                    if (std::find(structures->begin(), structures->end(), VK_NULL_HANDLE) != structures->end())
                        continue;
...
void Replayer::InjectStorageReadbacks(VkCommandBuffer cb, const CommandGroup& group, std::vector<PendingReadback>& readbacks)
{
    ...
    for (uint32_t i = 0; i < textures->count; ++i)
    {
        ...
        if (!info || Str(info->Get("kind")) != "sampled")
            continue;
        if (info->Get("error"))
            continue;
        const JValue* owner = info->Get("commandBuffer");
        if (!owner || owner->Uint() != group.commandBuffer)
            continue;
        auto it = _images.find(info->Get("id")->Uint());
        if (it == _images.end() || !it->second.storage)
            continue;
        const ImageRecord& image = it->second;
        if (image.samples != VK_SAMPLE_COUNT_1_BIT)
            continue;   // a multisampled storage image is not a thing
...
void Replayer::InjectReadbacks(VkCommandBuffer cb, const PassState& pass, std::vector<PendingReadback>& readbacks, const char* skipReason)
{
    ...
    for (uint32_t i = 0; i < textures->count; ++i)
    {
        ...
        if (!info || (info->Get("kind") && Str(info->Get("kind")) != "attachment"))
            continue;
        ...
        if (cmp.attachment >= views.size())
        {
            skip("the replayed pass has no such attachment");
            continue;
        }
        auto vit = _views.find(views[cmp.attachment]);
        if (vit == _views.end())
        {
            skip("the attachment's view was not replayed");
            continue;
        }
        auto iit = _images.find(vit->second.image);
        if (iit == _images.end())
        {
            skip("the attachment's image was not replayed");
            continue;
        }
        ...
        const uint32_t layers = std::max<uint32_t>(1, (uint32_t)info->Get("layers")->Uint());
        if (image.samples != VK_SAMPLE_COUNT_1_BIT && layers > 1)
        {
            skip("multisampled layered targets are not compared yet");
            continue;
        }
        PendingReadback pending;
        pending.target = target;
        pending.texture = &t;
        if (!CreateStaging(size, pending.staging))
        {
            skip("no staging memory");
            continue;
        }
        VkBufferImageCopy copy{};
    ...
    VkImageLayout layout, std::string& why)
{
    ...
    if (!rp)
    {
        ...
        if (_fns.CreateRenderPass2(_device, &info, nullptr, &rp) == VK_SUCCESS)
    ...
    if (resolved.image)
        _fns.CreateImageView(_device, &view, nullptr, &source);
    if (!rp || !resolved.view || !source)
    {
        if (source)
            _fns.DestroyImageView(_device, source, nullptr);
...
void Replayer::CompareReadbacks(std::vector<PendingReadback>& readbacks)
{
    for (PendingReadback& p : readbacks)
    {
        ...
        else
        {
            ...
            if (capturedSize != p.staging.size)
                cmp.note = "sizes differ: captured " + std::to_string(capturedSize) + ", replayed " + std::to_string(p.staging.size);
    ...
    for (uint32_t s = 0; list && s < list->count; ++s)
    {
        ...
        for (uint32_t i = (uint32_t)executeIndex + 1; i < commands->count; ++i)
        {
            ...
            if (m == "vkBeginCommandBuffer")
            {
                ...
                if (_exporter && a.pBeginInfo)
                    _exporter->BeginCommandBuffer(id, cb, *a.pBeginInfo, i, i, true);
                begun = true;
            }
            else if (m == "vkEndCommandBuffer")
            {
            ...
            else if (ReplayFn fn = FindReplayCommand(m); fn && args && begun)
            {
                ...
                if (_options.history.enabled)
                    NoteHistoryBindings(c.Get("descriptors"));
                NoteStreamCommand(stream, m, *args, i);
                if (_options.ablation.enabled && IsAction(m))
                ...
                if (drawSlot >= 0)
                    EndDrawQuery(cb, drawSlot);
                if (StartsWith(m, "vkCmdBeginQuery"))
                    ++_appQueryDepth;
                else if (StartsWith(m, "vkCmdEndQuery") && _appQueryDepth)
                    --_appQueryDepth;
    ...
    std::vector<PendingHistory>& histories)
{
    ...
    if (const JValue* args = beginCommand.Get("args"))
        DecodeArgs(_ctx, *args, begin);
    VkCommandBufferBeginInfo beginInfo{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};
    if (begin.pBeginInfo)
        beginInfo = *begin.pBeginInfo;
    _fns.ResetCommandBuffer(cb, 0);
    _fns.BeginCommandBuffer(cb, &beginInfo);
    if (_exporter)
        _exporter->BeginCommandBuffer(group.commandBuffer, cb, beginInfo, group.first, group.last, false);
    // The submission's first command buffer resets the pools its actions write into.
    if (_options.drawStats && _drawQueryCapacity && _drawSlot == 0)
        ResetDrawQueries(cb);
    if (_options.ablation.enabled)
        ResetAblationQueries(cb, group);
    _arena.Reset();
    ...
    for (uint32_t i = group.first + 1; i < group.last; ++i)
    {
        ...
        {
            if (_exporter)
                _exporter->LeftOut(i, m, "with its pass, whose begin names objects the replay does not have");
            if (IsEndPass(m))
            {
                if (_options.compareTargets)
        ...
        if (IsBeginRenderPass(m))
        {
            ...
            if (const JValue* clears = beginInfo2 ? beginInfo2->Get("pClearValues") : nullptr; clears && clears->IsArray())
                for (uint32_t k = 0; k < clears->count; ++k)
                    pass.clearValues.push_back(ClearValueOf(clears->items[k]));
            ...
            {
                if (_options.overdraw || overlay)
                    PrepareOverdraw(cb, pass);
                if (_options.history.enabled)
                    PrepareHistory(cb, pass, histories);
            }
        }
        else if (IsBeginRendering(m))
        {
            ...
            if (_ctx.unresolved != unresolvedBefore)
            {
                ...
                if (_exporter)
                    _exporter->LeftOut(i, m, "it names objects the replay does not have");
                skippingPass = true;
            }
            else if (a.pRenderingInfo)
            {
                ...
                if (info.pStencilAttachment)
                {
                    ...
                    if (!info.pDepthAttachment || stencil.imageView != depth.imageView)
                        pass.dynamicStencil = note(stencil, ri ? ri->Get("pStencilAttachment") : nullptr);
                    else
                        pass.dynamicStencil = pass.dynamicDepth;
                }
                const bool overlay = PassHoldsAny(i, _options.overlay.commands);
                ...
                if (_options.counters.enabled && _hw)
                    BeginCounterPass(cb, pass);
                _fns.CmdBeginRendering(cb, &info);
                if (_exporter)
                    _exporter->CmdBeginRendering(i, info);
                _report->commandsRecorded++;
            }
            _arena.Reset();
            continue;
        ...
        if (_options.history.enabled)
            NoteHistoryBindings(c.Get("descriptors"));
        if (m == "vkCmdExecuteCommands")
            RecordSecondaries(i, c, frame, group.commandBuffer, pass.active ? pass.index : UINT32_MAX);
        ...
        if (!fn)
        {
            ...
            if (_exporter)
                _exporter->LeftOut(i, m, "the replay does not record it");
            continue;
        }
        NoteStreamCommand(stream, m, *args, i);
        ...
        if (_options.counters.enabled && _hw && IsBeginRenderPass(m) && pass.active)
            BeginCounterPass(cb, pass);
            ...
            m == "vkCmdResolveImage2" || m == "vkCmdResolveImage2KHR")
        {
            for (const char* name : {"pCopyImageInfo", "pBlitImageInfo", "pResolveImageInfo"})
                if (const JValue* info = args->Get(name))
                    NoteImageWrite(IdOf(info->Get("dstImage")), false);
        }
        if (direct.writes)
            HistoryDirectPixel(cb, group, direct, histories, i, m, frame);
        if (countDraw)
            EndCounterDraw(cb, counterRange);
        if (drawSlot >= 0)
            EndDrawQuery(cb, drawSlot);
        ...
        if (IsEndPass(m) && pass.active)
        {
            ...
            if (_options.overlay.enabled && pass.extent.width)
                RecordOverlay(cb, group, pass, i);
            if (_options.mesh.enabled && pass.extent.width)
                RecordMesh(cb, group, pass, i);
            if (_options.overdraw)
                RecordOverdraw(cb, group, pass, i, overdraws);
            if (_options.history.enabled)
                RecordHistory(cb, group, pass, i, histories);
            pass.active = false;
            _passViews = 1;
        }
    }
    ...
    if (_options.compareTargets)
        InjectStorageReadbacks(cb, group, readbacks);
    _fns.EndCommandBuffer(cb);
    if (_exporter)
        _exporter->EndCommandBuffer(cb);
}
...
void Replayer::ReplayCommands()
{
    ...
    for (uint32_t i = 0; i < commands->count; ++i)
    {
        const JValue& c = commands->items[i];
        const std::string m = Str(c.Get("method"));
        if (m != "vkQueueSubmit" && m != "vkQueueSubmit2" && m != "vkQueueSubmit2KHR")
            continue;
        const JValue* args = c.Get("args");
        if (!args)
            continue;
        ...
        for (uint32_t s = 0; submits && s < submits->count; ++s)
        {
            ...
            if (m == "vkQueueSubmit")
            {
                if (const JValue* list = submit.Get("pCommandBuffers"))
                    for (uint32_t k = 0; k < list->count; ++k)
                        ids.push_back(IdOf(&list->items[k]));
            }
            else if (const JValue* list = submit.Get("pCommandBufferInfos"))
            {
                for (uint32_t k = 0; k < list->count; ++k)
                    ids.push_back(IdOf(list->items[k].Get("commandBuffer")));
            }
            for (uint64_t id : ids)
            {
                ...
                if (VkCommandBuffer cb = (VkCommandBuffer)(uintptr_t)Handle(id))
                    cbs.push_back(cb);
            }
        }
        if (cbs.empty())
    ...
    for (const CommandGroup& g : _groups)
        if (!g.used)
            Problem("command buffer " + std::to_string(g.commandBuffer) + " was recorded but its submission is not in the capture");
}
...
{
    if (!Setup(capture, options, report))
        return false;
...
void Replayer::RunFrame(const ReplayOptions& requested, ReplayReport& report)
{
    ...
    {
        if (std::any_of(report.ablations.begin(), report.ablations.end(), [&](const AblationResult& a) { return a.command == target.command; }))
            continue;
        AblationResult missing;
        missing.command = target.command;
        missing.stage = target.stage;
        for (const auto& v : target.variants)
            missing.variants.push_back({v.name});
    ...
    {
        if (std::any_of(report.overlays.begin(), report.overlays.end(), [&](const OverlayResult& o) { return o.command == command; }))
            continue;
        OverlayResult missing;
        missing.command = command;
        const JValue* commands = capture.Commands();
        if (commands && command < commands->count)
            missing.method = Str(commands->items[command].Get("method"));
    ...
    {
        if (std::any_of(report.meshes.begin(), report.meshes.end(), [&](const MeshResult& m) { return m.command == command; }))
            continue;
        MeshResult missing;
        missing.command = command;
        const JValue* commands = capture.Commands();
    ...
    {
        ExportFrameEnd();
        if (!_exporter->Finish(report, report.exported))
            Problem("export to C++: " + report.exported.error);
        _exporter.reset();
    }
}
...
void Replayer::NoteImageWrite(uint64_t image, bool colorTarget)
{
    if (!image)
        return;
    if (_swapchainImages.count(image))
        _lastSwapchainWrite = image;
    if (colorTarget)
        _lastColorTarget = image;
}
...
void Replayer::ExportFrameEnd()
{
    ...
    auto leftIn = [&](uint64_t id, const ImageRecord& image) {
        ...
        for (size_t k = 0; it != _finalLayouts.end() && k < left.size() && k < it->second.size(); ++k)
            if (it->second[k] != VK_IMAGE_LAYOUT_UNDEFINED)
                left[k] = it->second[k];
        return left;
    };
    ...
    for (const Created& c : _created)
        if (c.type == "VkCommandPool")
            _exporter->RestorePool((VkCommandPool)c.handle);
    _exporter->BeginInitialLayouts(true);
    for (auto& [id, image] : _images)
    {
        auto initial = _initialLayouts.find(id);
        if (initial == _initialLayouts.end())
            continue;
        ...
        for (size_t k = 0; k < targets.size() && k < left.size(); ++k)
            if (targets[k] == VK_IMAGE_LAYOUT_UNDEFINED)
                targets[k] = left[k];
...
void Replayer::ResetFrameState()
{
    ...
    for (const Created& c : _created)
        if (c.type == "VkCommandPool")
            _fns.ResetCommandPool(_device, (VkCommandPool)c.handle, 0);
