...
{
    return aspect == "depth" ? VK_IMAGE_ASPECT_DEPTH_BIT : aspect == "stencil" ? VK_IMAGE_ASPECT_STENCIL_BIT
...
inline VkClearValue ClearValueOf(const JValue& v)
{
    ...
    if (const JValue* color = v.Get("color"))
    {
        if (const JValue* bits = color->Get("uint32"); bits && bits->IsArray())
            for (uint32_t k = 0; k < 4 && k < bits->count; ++k)
                clear.color.uint32[k] = (uint32_t)bits->items[k].Uint();
    }
    return clear;
}
