...
bool Replayer::PrepareDrawStats()
{
    ...
    uint32_t views = 1;
    for (const auto& [id, rp] : _renderPasses)
        views = std::max(views, rp.views);
    if (const JValue* commands = _capture->Commands(); commands && commands->IsArray())
    {
        for (uint32_t i = 0; i < commands->count; ++i)
        {
            if (!IsBeginRendering(Str(commands->items[i].Get("method"))))
                continue;
            const JValue* args = commands->items[i].Get("args");
            const JValue* info = args ? args->Get("pRenderingInfo") : nullptr;
            if (const JValue* mask = info ? info->Get("viewMask") : nullptr)
                views = std::max(views, ViewCount((uint32_t)mask->Uint()));
        }
    }
    if (actions > kMaxMeasuredActions)
    {
        _report->drawStatsNote = "the frame has " + std::to_string(actions) + " draws and dispatches; the first " + std::to_string(kMaxMeasuredActions) + " are measured";
    ...
    const bool timestamps = props.limits.timestampPeriod > 0 && _queueFamily < families.size() && families[_queueFamily].timestampValidBits > 0;
    ...
    if (timestamps)
    {
        ...
        if (_fns.CreateQueryPool(_device, &info, nullptr, &_drawTimestamps) != VK_SUCCESS)
    ...
    if (_drawCountersAvailable)
    {
        ...
        if (_fns.CreateQueryPool(_device, &info, nullptr, &_drawStatistics) != VK_SUCCESS)
    ...
    if (_drawSamplesAvailable)
    {
        ...
        info.queryCount = actions * views;
        if (_fns.CreateQueryPool(_device, &info, nullptr, &_drawOcclusion) != VK_SUCCESS)
    ...
    if (!_drawTimestamps)
        _report->drawStatsNote = "the replay's queue writes no timestamps, so the draws carry counters only";
    else if (!_drawStatistics)
        _report->drawStatsNote = "this device has no pipeline statistics queries, so the draws carry timings only";
    return true;
}
...
{
    if (_drawTimestamps)
        _fns.CmdResetQueryPool(cb, _drawTimestamps, 0, _drawQueryCapacity * 2);
    if (_drawStatistics)
        _fns.CmdResetQueryPool(cb, _drawStatistics, 0, _drawQueryCapacity);
    if (_drawOcclusion)
        _fns.CmdResetQueryPool(cb, _drawOcclusion, 0, _drawQueryCapacity);
}
...
int Replayer::BeginDrawQuery(VkCommandBuffer cb, uint32_t command, uint32_t frame, uint64_t commandBuffer, uint32_t passIndex)
{
    ...
    if (_drawSlot + views > _drawQueryCapacity)
        return -1;
    ...
    if (d.timed)
        _fns.CmdWriteTimestamp(cb, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, _drawTimestamps, slot * 2);
    if (d.counted)
        _fns.CmdBeginQuery(cb, _drawStatistics, slot, 0);
    if (d.sampled)
        _fns.CmdBeginQuery(cb, _drawOcclusion, slot, VK_QUERY_CONTROL_PRECISE_BIT);
    return (int)_pendingDraws.size() - 1;
}

void Replayer::EndDrawQuery(VkCommandBuffer cb, int pending)
{
    if (pending < 0 || (size_t)pending >= _pendingDraws.size())
        return;
    const DrawResult& d = _pendingDraws[(size_t)pending];
    const auto [slot, views] = _pendingDrawSlots[(size_t)pending];
    if (d.sampled)
        _fns.CmdEndQuery(cb, _drawOcclusion, slot);
    if (d.counted)
        _fns.CmdEndQuery(cb, _drawStatistics, slot);
    // The draw's timestamps: its views' begin times, then their end times.
    if (d.timed)
        _fns.CmdWriteTimestamp(cb, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, _drawTimestamps, slot * 2 + views);
}
...
void Replayer::CompleteDrawStats(bool submitted)
{
    ...
    if (_drawOcclusion)
    {
        ...
        if (_fns.GetQueryPoolResults(_device, _drawOcclusion, 0, _drawSlot, samples.size() * sizeof(uint64_t), samples.data(),
                2 * sizeof(uint64_t), VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT | VK_QUERY_RESULT_WITH_AVAILABILITY_BIT) != VK_SUCCESS)
        {
            samples.clear();
        }
    }
...
{
    if (_drawTimestamps)
        _fns.DestroyQueryPool(_device, _drawTimestamps, nullptr);
    if (_drawStatistics)
        _fns.DestroyQueryPool(_device, _drawStatistics, nullptr);
    if (_drawOcclusion)
        _fns.DestroyQueryPool(_device, _drawOcclusion, nullptr);
    _drawTimestamps = VK_NULL_HANDLE;
    _drawStatistics = VK_NULL_HANDLE;
    _drawOcclusion = VK_NULL_HANDLE;
