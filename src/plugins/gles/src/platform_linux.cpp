...
void ResolveEglProcs()
{
    ...
    if (resolved || !g_egl.eglGetProcAddress)
        return;
    resolved = true;
    ...
    {
        if (!slots[i])
            slots[i] = (void*)g_egl.eglGetProcAddress(kCommandNames[i]);
    }
}
...
{
    Dl_info info{};
    if (!dladdr((void*)&SelfHandle, &info) || !info.dli_fname)
        return nullptr;
            ...
            if (dlinfo(map, RTLD_DI_SERINFOSIZE, paths) == 0 && dlinfo(map, RTLD_DI_SERINFO, paths) == 0)
            {
                for (unsigned i = 0; i < paths->dls_cnt; ++i)
                {
                    ...
                    if (access(candidate.c_str(), F_OK) == 0)
                        return RealDlopen()(candidate.c_str(), flags);
                }
            }
        }
    }
...
void ResolveReal()
{
    ...
    {
        if (!*kHooks[i].real && (*kHooks[i].real = RealSymbol(kHooks[i].name)))
            ++found;
    }
    void** slots = reinterpret_cast<void**>(&g_gl);
    for (size_t i = 0; i < kCommandCount; ++i)
    {
        if (!slots[i])
            slots[i] = RealSymbol(kCommandNames[i]);
    }
    for (size_t i = 0; i < kEglHookCount; ++i)
    {
        if (!*kEglHooks[i].real)
            *kEglHooks[i].real = RealSymbol(kEglHooks[i].name);
    }
    for (size_t i = 0; i < kEglImportCount; ++i)
    {
        if (!*kEglImports[i].slot)
            *kEglImports[i].slot = RealSymbol(kEglImports[i].name);
    }
...
extern "C" __attribute__((visibility("default"))) void* dlopen(const char* file, int flags)
{
    ...
    if (!handle || !file || !IsGlesLibrary(file))
        return handle;
    Dl_info from{};
    if (dladdr(caller, &from) && from.dli_fname)
    {
        const char* base = BaseName(from.dli_fname);
        if (StartsWith(base, "libEGL") || StartsWith(base, "libGL") || strstr(base, "glesinsp"))
            return handle;
    }
