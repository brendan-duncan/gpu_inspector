...
public:
    explicit GlWork(Context* c) : _c(c)
    {
        ...
        if (e != GL_NO_ERROR && !_c->savedError)
            _c->savedError = e;
    }
    ~GlWork()
    {
...
struct Bindings
{
    ...
    explicit Bindings(Context* c) : es3(c->es3())
    {
        ...
        if (es3)
        {
            ...
            if (rowLength)
                g_gl.glPixelStorei(GL_PACK_ROW_LENGTH, 0);
            if (skipRows)
                g_gl.glPixelStorei(GL_PACK_SKIP_ROWS, 0);
            if (skipPixels)
                g_gl.glPixelStorei(GL_PACK_SKIP_PIXELS, 0);
        }
        if (packAlign != 1)
            g_gl.glPixelStorei(GL_PACK_ALIGNMENT, 1);
    }
    ...
    ~Bindings()
    {
        if (es3)
        {
            ...
            if (packBuffer)
                g_gl.glBindBuffer(GL_PIXEL_PACK_BUFFER, (GLuint)packBuffer);
            if (rowLength)
                g_gl.glPixelStorei(GL_PACK_ROW_LENGTH, rowLength);
            if (skipRows)
                g_gl.glPixelStorei(GL_PACK_SKIP_ROWS, skipRows);
            if (skipPixels)
                g_gl.glPixelStorei(GL_PACK_SKIP_PIXELS, skipPixels);
        ...
        if (packAlign != 1)
            g_gl.glPixelStorei(GL_PACK_ALIGNMENT, packAlign);
...
bool ReadPixels(const ReadFormat& rf, int width, int height, bool flip, std::vector<uint8_t>& out)
{
    ...
    if (g_gl.glGetError && g_gl.glGetError() != GL_NO_ERROR)
        return false;
    if (flip)
    {
    ...
    int width, int height, bool flip, TextureCapture& out)
{
    ...
    if (f.compressed)
    {
        out.error = "compressed textures are not read back from the GPU";
        return false;
    }
    if (f.depth || f.stencil)
    {
        out.error = "OpenGL ES cannot read depth or stencil back (glReadPixels reads color only)";
        return false;
    }
    if (!rf.format)
    {
        out.error = std::string("this format cannot be read back in OpenGL ES (") + f.vk + ")";
        return false;
    }
    if (width <= 0 || height <= 0)
    {
        out.error = "the attachment has no size";
        return false;
    }
    if (!c->es3() && rf.format != GL_RGBA)
    {
        out.error = "OpenGL ES 2 reads back 8-bit color only";
        return false;
    }
    ...
    {
        if (!g_gl.glFramebufferTextureLayer)
        {
            out.error = "no glFramebufferTextureLayer";
            return false;
        }
    ...
    else if (o.samples > 1 && c->es3())
    {
        ...
        if (!ok)
            out.error = "glReadPixels failed after resolving the samples";
    }
    else
    {
        ok = ReadPixels(rf, width, height, flip, out.data);
        if (!ok)
            out.error = "glReadPixels failed";
    }
    // Leave nothing attached, so the library's framebuffer keeps no texture alive.
    b.BindRead(fb);
    if (kind == ObjType::Renderbuffer)
        g_gl.glFramebufferRenderbuffer(target, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, 0);
    else
        g_gl.glFramebufferTexture2D(target, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 0, 0);
    if (ok)
    {
        out.format = rf.vk;
        out.width = width;
...
{
    return type == GL_UNSIGNED_INT ? 4 : type == GL_UNSIGNED_SHORT ? 2
                                                                   : 1;
}
...
{
    if (offset < 0 || (o.size && offset >= (int64_t)o.size))
    {
        error = "the range is outside the buffer";
        return false;
    }
    if (o.size && offset + (int64_t)size > (int64_t)o.size)
        size = (size_t)(o.size - offset);
    if (!size)
    {
    ...
    {
        if (o.shadow.size() < (size_t)offset + size)
        {
            error = "OpenGL ES 2 cannot read a buffer back, and its contents were not all seen";
            return false;
        }
...
{
    if (!c->es3() || !g_gl.glCopyBufferSubData || !g_gl.glMapBufferRange)
        return false;
    g_gl.glBindBuffer(GL_COPY_READ_BUFFER, o.name);
    ...
    if (mapped)
    {
        b.error = "the application has the buffer mapped";
        return true;
    }
    if (offset < 0 || offset >= real)
    {
        b.error = "the range is outside the buffer";
        return true;
    }
    size = std::min(size, (size_t)(real - offset));
    if (!size)
    {
        b.error = "the range is empty";
        return true;
    }
...
{
    if (!g_options.captureBuffers || size == 0)
        return 0;
    ...
    {
        if (!client)
            b.error = "no buffer bound and no client pointer";
        else
            b.data.assign((const uint8_t*)client, (const uint8_t*)client + size);
    }
...
void WriteAttachments(Context* c, GLuint fb, gpuinsp::sdk::JsonWriter& w)
{
    ...
    if (Object* o = ObjectOf(c, ObjType::Framebuffer, fb))
    {
        for (const auto& [slot, a] : o->attachments)
        {
            ...
            {
                w.Key("format");
                w.String(FormatOf(t->internalFormat).vk);
                w.Key("width");
                w.Int(std::max(1, t->width >> a.level));
                w.Key("height");
                w.Int(std::max(1, t->height >> a.level));
            }
            w.EndObject();
        }
    }
...
{
    if (!g_options.captureTextures || c->passReadBack)
        return;
    c->passReadBack = true;
    GlWork work(c);
    Bindings b(c);
    if (c->passFramebuffer == 0)
    {
        ...
        if (c->es3())
            g_gl.glReadBuffer((GLenum)readBuffer);
    ...
    {
        if (slot < GL_COLOR_ATTACHMENT0 || slot > GL_COLOR_ATTACHMENT0 + 15)
            continue;
...
std::string PassArgs(Context* c)
{
    ...
    w.BeginObject();
    w.Key("framebuffer");
    w.Ref(c->passFramebuffer ? RefOf(ObjType::Framebuffer, c->passFramebuffer) : 0, "GLFramebuffer");
    ...
    else
    {
        const Drawable surface = CurrentDrawable(c);
        w.Key("surface");
        w.Ref(surface.id, "GLSurface");
        w.Key("width");
        w.Int(surface.width);
        w.Key("height");
        w.Int(surface.height);
    }
    w.Key("passIndex");
    w.Uint(c->passIndex);
    ...
    if (c->passCleared)
    {
        w.Key("cleared");
        w.BeginArray();
        if (c->passCleared & GL_COLOR_BUFFER_BIT)
            w.String("color");
        if (c->passCleared & GL_DEPTH_BUFFER_BIT)
            w.String("depth");
        if (c->passCleared & GL_STENCIL_BUFFER_BIT)
            w.String("stencil");
        w.EndArray();
    }
    ...
    if (!c->passInvalidated.empty())
    {
        w.Key("invalidated");
        w.BeginArray();
        for (GLenum a : c->passInvalidated)
            w.Enum(EnumName_InvalidateFramebufferAttachment(a), a);
        w.EndArray();
    }
    w.Key("synthetic");
    w.Boolean(true);
...
int TimingMode(Context* c)
{
    ...
    GLint bits = 0;
    if (t.queryiv)
        t.queryiv(GL_TIMESTAMP_EXT, GL_QUERY_COUNTER_BITS_EXT, &bits);
    if (bits > 0 && t.counter)
        c->timestamps = 1;
    else if (t.begin && t.end)
        c->timestamps = 2;
    LogAlways("pass timings: %s", c->timestamps == 1 ? "timestamps" : c->timestamps == 2 ? "elapsed-time queries (no timestamp counter)"
                                                                                         : "none");
    return c->timestamps;
}
...
GLuint BeginPassTiming(Context* c)
{
    ...
    GLint active = 0;
    if (mode == 2 && g_timer.queryiv)
        g_timer.queryiv(GL_TIME_ELAPSED_EXT, GL_CURRENT_QUERY_EXT, &active);
    if (active)
        return 0;
    GLuint q = 0;
    g_timer.gen(1, &q);
    if (mode == 1)
        g_timer.counter(q, GL_TIMESTAMP_EXT);
    else
        g_timer.begin(GL_TIME_ELAPSED_EXT, q);
    return q;
}
...
void EndPass(Context* c)
{
    ...
    c->passOpen = false;
    if (c->passFramebuffer == 0)
        c->surfacePassEnded = true;
}

void BeginPassIfNeeded(Context* c)
{
    if (c->passOpen && c->passFramebuffer == c->drawFramebuffer)
        return;
    EndPass(c);
    c->passOpen = true;
...
const UniformType kUniformTypes[] = {
    {GL_FLOAT, "float", 1, 'f', 0, 0},
    {GL_FLOAT_VEC2, "vec2", 2, 'f', 0, 0},
    {GL_FLOAT_VEC3, "vec3", 3, 'f', 0, 0},
    {GL_FLOAT_VEC4, "vec4", 4, 'f', 0, 0},
    {GL_INT, "int", 1, 'i', 0, 0},
    {GL_INT_VEC2, "ivec2", 2, 'i', 0, 0},
    {GL_INT_VEC3, "ivec3", 3, 'i', 0, 0},
    {GL_INT_VEC4, "ivec4", 4, 'i', 0, 0},
    {GL_UNSIGNED_INT, "uint", 1, 'u', 0, 0},
    {GL_UNSIGNED_INT_VEC2, "uvec2", 2, 'u', 0, 0},
    {GL_UNSIGNED_INT_VEC3, "uvec3", 3, 'u', 0, 0},
    {GL_UNSIGNED_INT_VEC4, "uvec4", 4, 'u', 0, 0},
    {GL_BOOL, "bool", 1, 'b', 0, 0},
    {GL_BOOL_VEC2, "bvec2", 2, 'b', 0, 0},
    {GL_BOOL_VEC3, "bvec3", 3, 'b', 0, 0},
    {GL_BOOL_VEC4, "bvec4", 4, 'b', 0, 0},
    {GL_FLOAT_MAT2, "mat2", 4, 'f', 0, 0},
    {GL_FLOAT_MAT3, "mat3", 9, 'f', 0, 0},
    {GL_FLOAT_MAT4, "mat4", 16, 'f', 0, 0},
    {GL_FLOAT_MAT2x3, "mat2x3", 6, 'f', 0, 0},
    {GL_FLOAT_MAT2x4, "mat2x4", 8, 'f', 0, 0},
    {GL_FLOAT_MAT3x2, "mat3x2", 6, 'f', 0, 0},
    {GL_FLOAT_MAT3x4, "mat3x4", 12, 'f', 0, 0},
    {GL_FLOAT_MAT4x2, "mat4x2", 8, 'f', 0, 0},
    {GL_FLOAT_MAT4x3, "mat4x3", 12, 'f', 0, 0},
...
{
    for (const UniformType& t : kUniformTypes)
        if (t.type == type)
            return &t;
    return nullptr;
}
...
bool CopyTextureLevel(Context* c, const Object& tex, int layers, size_t index)
{
    ...
    if (!copy || !ReadFormatOf(FormatOf(tex.internalFormat)).format || !g_gl.glTexStorage3D)
        return false;
...
uint32_t CaptureTexture(Context* c, const Bindings& b, Object& tex)
{
    ...
    const int layers = cube ? 6 : (tex.target == GL_TEXTURE_2D_ARRAY || tex.target == GL_TEXTURE_3D) ? std::max(1, tex.depth)
                                                                                                     : 1;
    t.layers = layers;
    ...
    else if (f.compressed)
    {
        ...
        {
            for (const auto& layer : tex.compressedLevel0)
                t.data.insert(t.data.end(), layer.begin(), layer.end());
        }
        else
        {
...
bool IndexRange(const std::vector<uint8_t>& data, GLenum type, size_t count, uint32_t& lo, uint32_t& hi)
{
    ...
    for (size_t i = 0; i < n; ++i)
    {
        ...
        if ((size == 1 && v == 0xFF) || (size == 2 && v == 0xFFFF) || (size == 4 && v == 0xFFFFFFFFu))
            continue;
        lo = std::min(lo, v);
        hi = std::max(hi, v);
    }
...
void WriteVertexInput(Context* c, Object* prog, const DrawParams& p, gpuinsp::sdk::JsonWriter& w)
{
    ...
    {
        w.Key("elementBuffer");
        w.Ref(elementBuffer ? RefOf(ObjType::Buffer, (GLuint)elementBuffer) : 0, "GLBuffer");
        w.Key("indexType");
        WriteEnumValue(w, p.indexType, EnumName_DrawElementsType);
    ...
    if (prog)
    {
        ...
        {
            if (a.location < 0 || a.name.rfind("gl_", 0) == 0)
                continue;
            ...
            w.Key("buffer");
            w.Ref(buffer ? RefOf(ObjType::Buffer, (GLuint)buffer) : 0, "GLBuffer");
            w.Key("size");
            w.Int(size);
            w.Key("componentType");
            WriteEnumValue(w, (GLenum)type, EnumName_VertexAttribPointerType);
            w.Key("normalized");
            w.Boolean(normalized != 0);
            w.Key("integer");
            w.Boolean(integer != 0);
            w.Key("stride");
            w.Uint(step);
            w.Key("offset");
            w.Int(buffer ? (int64_t)(intptr_t)pointer : 0);
            w.Key("divisor");
            w.Int(divisor);
...
void WriteProgramResources(Context* c, const Bindings& b, Object* prog, gpuinsp::sdk::JsonWriter& w)
{
    ...
    for (const ProgramVariable& u : prog->uniforms)
    {
        ...
        for (GLint e = 0; e < std::min<GLint>(u.size, 16); ++e)
        {
            ...
            w.BeginObject();
            w.Key("uniform");
            w.String(u.size > 1 ? u.name + "[" + std::to_string(e) + "]" : u.name);
            w.Key("type");
            w.String(t->glsl);
            w.Key("unit");
            w.Int(unit);
            w.Key("target");
            WriteEnumValue(w, t->target, EnumName_TextureTarget);
            w.Key("texture");
            w.Ref(name ? RefOf(ObjType::Texture, (GLuint)name) : 0, "GLTexture");
            w.Key("sampler");
            w.Ref(sampler ? RefOf(ObjType::Sampler, (GLuint)sampler) : 0, "GLSampler");
            if (Object* tex = name ? ObjectOf(c, ObjType::Texture, (GLuint)name) : nullptr)
            {
                g_gl.glActiveTexture((GLenum)activeTexture);
                w.Key("capture");
                w.Uint(CaptureTexture(c, b, *tex));
            }
            w.EndObject();
        }
    }
    ...
    for (const ProgramBlock& block : prog->blocks)
    {
        ...
        if (g_gl.glGetIntegeri_v)
            g_gl.glGetIntegeri_v(GL_UNIFORM_BUFFER_BINDING, (GLuint)binding, &buffer);
        ...
        if (buffer)
        {
            ...
            w.Key("data");
            w.Uint(CaptureBuffer(c, (GLuint)buffer, start, bytes));
        }
        w.EndObject();
    }
    w.EndArray();
    ...
    for (const ProgramVariable& u : prog->uniforms)
    {
        if (u.location < 0 || u.blockIndex >= 0)
            continue;
        const UniformType* t = UniformTypeOf(u.type);
        if (!t)
            continue;
...
void WriteFixedFunction(Context* c, gpuinsp::sdk::JsonWriter& w)
{
    ...
    WriteInts(w, v, 4);
    w.Key("scissorTest");
    w.Boolean(IsOn(GL_SCISSOR_TEST));
    g_gl.glGetIntegerv(GL_SCISSOR_BOX, v);
    w.Key("scissor");
    WriteInts(w, v, 4);
    w.Key("cullFace");
    w.Boolean(IsOn(GL_CULL_FACE));
    w.Key("cullMode");
    WriteEnumValue(w, (GLenum)GetInt(GL_CULL_FACE_MODE, GL_BACK), EnumName_TriangleFace);
    w.Key("frontFace");
    WriteEnumValue(w, (GLenum)GetInt(GL_FRONT_FACE, GL_CCW), EnumName_FrontFaceDirection);
    w.Key("polygonOffsetFill");
    w.Boolean(IsOn(GL_POLYGON_OFFSET_FILL));
    ...
    if (c->es3())
    {
        w.Key("rasterizerDiscard");
        w.Boolean(IsOn(GL_RASTERIZER_DISCARD));
        w.Key("primitiveRestart");
        w.Boolean(IsOn(GL_PRIMITIVE_RESTART_FIXED_INDEX));
    }
    w.Key("dither");
    w.Boolean(IsOn(GL_DITHER));
    w.EndObject();

    w.Key("depth");
    w.BeginObject();
    w.Key("test");
    w.Boolean(IsOn(GL_DEPTH_TEST));
    w.Key("func");
    WriteEnumValue(w, (GLenum)GetInt(GL_DEPTH_FUNC, GL_LESS), EnumName_DepthFunction);
    ...
    w.BeginObject();
    w.Key("test");
    w.Boolean(IsOn(GL_STENCIL_TEST));
    const struct
    {
        const char* key;
        GLenum func, ref, valueMask, fail, zfail, zpass, writeMask;
    } faces[] = {
    ...
    for (const auto& face : faces)
    {
        ...
        w.BeginObject();
        w.Key("func");
        WriteEnumValue(w, (GLenum)GetInt(face.func), EnumName_StencilFunction);
        w.Key("ref");
        w.Int(GetInt(face.ref));
        w.Key("valueMask");
        w.Uint((GLuint)GetInt(face.valueMask));
        w.Key("fail");
        WriteEnumValue(w, (GLenum)GetInt(face.fail), EnumName_StencilOp);
        w.Key("depthFail");
        WriteEnumValue(w, (GLenum)GetInt(face.zfail), EnumName_StencilOp);
        w.Key("pass");
        WriteEnumValue(w, (GLenum)GetInt(face.zpass), EnumName_StencilOp);
        w.Key("writeMask");
        w.Uint((GLuint)GetInt(face.writeMask));
        w.EndObject();
    }
    w.EndObject();
    ...
    w.BeginObject();
    w.Key("enabled");
    w.Boolean(IsOn(GL_BLEND));
    w.Key("equationRgb");
    WriteEnumValue(w, (GLenum)GetInt(GL_BLEND_EQUATION_RGB, GL_FUNC_ADD), EnumName_BlendEquationModeEXT);
    w.Key("equationAlpha");
    WriteEnumValue(w, (GLenum)GetInt(GL_BLEND_EQUATION_ALPHA, GL_FUNC_ADD), EnumName_BlendEquationModeEXT);
    w.Key("srcRgb");
    WriteEnumValue(w, (GLenum)GetInt(GL_BLEND_SRC_RGB, GL_ONE), EnumName_BlendingFactor);
    w.Key("dstRgb");
    WriteEnumValue(w, (GLenum)GetInt(GL_BLEND_DST_RGB, GL_ZERO), EnumName_BlendingFactor);
    w.Key("srcAlpha");
    WriteEnumValue(w, (GLenum)GetInt(GL_BLEND_SRC_ALPHA, GL_ONE), EnumName_BlendingFactor);
    w.Key("dstAlpha");
    WriteEnumValue(w, (GLenum)GetInt(GL_BLEND_DST_ALPHA, GL_ZERO), EnumName_BlendingFactor);
...
std::string Snapshot(Context* c, const DrawParams* draw)
{
    ...
    w.Key("program");
    w.Ref(prog ? prog->id : 0, "GLProgram");
    if (draw)
    {
        w.Key("framebuffer");
        w.Ref(c->drawFramebuffer ? RefOf(ObjType::Framebuffer, c->drawFramebuffer) : 0, "GLFramebuffer");
        w.Key("passIndex");
        w.Uint(c->passIndex);
        w.Key("mode");
        WriteEnumValue(w, draw->mode, EnumName_PrimitiveType);
        // What the draw draws, indirect draws' counts read from their buffer: the mesh view's input.
        w.Key("draw");
        w.BeginObject();
        w.Key("indexed");
        w.Boolean(draw->indexed);
        w.Key("count");
        w.Int(draw->count);
        w.Key("first");
        w.Int(draw->first);
        w.Key("instances");
        w.Int(draw->instances);
        w.Key("baseVertex");
        w.Int(draw->baseVertex);
        w.Key("indirect");
        w.Boolean(draw->indirect);
        w.EndObject();
...
{
    if (g_pendingBuffers.empty() && g_pendingTextures.empty())
        return;
    if (!c)
    {
        for (auto& p : g_pendingBuffers)
            g_buffers[p.id - 1].error = "no context was current when the capture finished";
        for (auto& p : g_pendingTextures)
            g_textures[p.index].error = "no context was current when the capture finished";
...
std::string PassTimes(Context* c)
{
    ...
    if (c && !g_passQueries.empty())
    {
        ...
        {
            if (q.context != c->id || !q.begin || (!q.elapsed && !q.end))
                continue;
    ...
    for (const auto& t : times)
        origin = std::min(origin, t.second.first);
    w.Key("count");
    w.Uint(times.size());
    w.Key("passes");
    w.BeginArray();
    for (const auto& [q, t] : times)
    {
        ...
        w.String("render");
        w.Key("startMs");
        w.Double((double)(t.first - origin) / 1e6);
        w.Key("durationMs");
        w.Double((double)(t.second - t.first) / 1e6);
        w.EndObject();
    }
    w.EndArray();
    w.EndObject();
...
void SendCapture(Context* c)
{
    ...
    const uint32_t frames = g_frame;
    {
        ...
        w.Uint(g_startSwap);
        w.Key("frames");
        w.Uint(std::max<uint32_t>(1, frames));
        w.Key("count");
        w.Uint(g_commands.size());
        constexpr size_t kBatch = 1000;
        w.Key("batches");
        w.Uint((g_commands.size() + kBatch - 1) / kBatch);
        w.Key("api");
        w.String("gles");
        w.EndObject();
        ...
        for (size_t start = 0, batch = 0; start < g_commands.size(); start += kBatch, ++batch)
        {
            ...
            out.BeginObject();
            out.Key("action");
            out.String("CaptureFrameCommands");
            out.Key("frame");
            out.Uint(g_startSwap);
            out.Key("index");
            out.Uint(batch);
            out.Key("commands");
            out.BeginArray();
            for (size_t i = start; i < std::min(g_commands.size(), start + kBatch); ++i)
            {
                ...
                out.String(r.method);
                out.Key("object");
                out.Ref(r.context, "GLContext");
                out.Key("args");
                out.Raw(r.args.empty() ? std::string("{}") : r.args);
                if (!r.state.empty())
                {
                    out.Key("state");
                    out.Raw(r.state);
    ...
    }
    {
        ...
        for (const TextureCapture& t : g_textures)
        {
            ...
            w.Uint(t.attachment);
            w.Key("format");
            w.String(t.format.empty() ? "VK_FORMAT_UNDEFINED" : t.format);
            w.Key("aspect");
            w.String("color");
            w.Key("width");
            w.Int(t.width);
...
void FrameStats()
{
    ...
    g_lastSwap = now;
    if (!g_lastReport.time_since_epoch().count())
        g_lastReport = now;
    if (now - g_lastReport < std::chrono::milliseconds(100) || !g_intervals)
        return;
...
void RequestCapture(const JsonValue& msg)
{
    ...
    if (const JsonValue* at = msg.Get("atFrame"); at && at->kind == JsonValue::Number)
        o.atFrame = (int64_t)at->num;
    ...
    if (msg.Get("maxBufferSize"))
        o.maxBufferSize = (size_t)msg.GetNumber("maxBufferSize");
    if (msg.Get("maxBufferTotal"))
        o.maxBufferTotal = (size_t)msg.GetNumber("maxBufferTotal");
    if (msg.Get("maxImageTotal"))
        o.maxImageTotal = (size_t)msg.GetNumber("maxImageTotal");
    g_requested = o;
    g_armed = true;
...
void BeforeFramebufferWrite(Context* c, GLbitfield clearMask)
{
    ...
    if (clearMask && !c->passDrawn && !(g_gl.glIsEnabled && g_gl.glIsEnabled(GL_SCISSOR_TEST)))
        c->passCleared |= clearMask;
    else if (!clearMask)
        c->passDrawn = true;
}
...
void BeforeInvalidate(Context* c, GLenum target, GLsizei count, const GLenum* attachments)
{
    ...
    {
        if (std::find(c->passInvalidated.begin(), c->passInvalidated.end(), attachments[i]) == c->passInvalidated.end())
            c->passInvalidated.push_back(attachments[i]);
    }
}
...
void BeforeAttachmentChange(Context* c, GLenum target)
{
    ...
    if (c->passOpen && fb == c->passFramebuffer && fb != 0)
        EndPass(c);
}
...
void BeforeSwap(Context* c, EGLDisplay display, EGLSurface surface, const char* method)
{
    ...
    w.Key(c->wgl ? "hdc" : c->glx ? "drawable"
                                  : "surface");
    w.Ref(SurfaceId(surface), "GLSurface");
    w.EndObject();
