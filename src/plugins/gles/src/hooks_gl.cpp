...
GLuint BoundTexture(GLenum target) {
    switch (target) {
        ...
        default:
            if (target >= GL_TEXTURE_CUBE_MAP_POSITIVE_X && target <= GL_TEXTURE_CUBE_MAP_NEGATIVE_Z) return (GLuint)GetInt(GL_TEXTURE_BINDING_CUBE_MAP);
            if (target == GL_TEXTURE_CUBE_MAP) return (GLuint)GetInt(GL_TEXTURE_BINDING_CUBE_MAP);
            return 0;
    }
}
...
void DescribeTexture(Object& t) {
    ...
    if (t.depth > 1 || t.target == GL_TEXTURE_3D || t.target == GL_TEXTURE_2D_ARRAY) Describe(t, "depth", JsonInt(t.depth));
    Describe(t, "levels", JsonInt(t.levels));
    if (t.samples > 1) Describe(t, "samples", JsonInt(t.samples));
}
...
void TextureShape(Context* c, GLenum target, GLint level, GLenum internalFormat, GLsizei w, GLsizei h, GLsizei d, GLint levels, GLsizei samples) {
    ...
    if (!t) return;
    ++t->generation;
    if (!t->target || t->target == target || IsCubeFace(target)) t->target = IsCubeFace(target) ? GL_TEXTURE_CUBE_MAP : target;
    ...
    if (!t->compressed) t->compressedLevel0.clear();
    DescribeTexture(*t);
}
...
void KeepCompressed(Context* c, Object& t, GLenum target, GLsizei layers, GLsizei imageSize, const void* data) {
    ...
    if (t.compressedLevel0.size() != count) t.compressedLevel0.assign(count, {});
    ...
    if (face) {
        ...
        for (size_t i = 0; i < count; ++i) t.compressedLevel0[i].assign(bytes + i * perLayer, bytes + (i + 1) * perLayer);
    }
...
void RenderbufferShape(Context* c, GLenum internalFormat, GLsizei w, GLsizei h, GLsizei samples) {
    ...
    if (samples > 1) Describe(*r, "samples", JsonInt(samples));
}
...
void DescribeAttachments(Object& fb) {
    ...
    for (const auto& [slot, a] : fb.attachments) {
        ...
        if (a.face) { w.Key("face"); w.Enum(EnumName(a.face), a.face); }
        w.EndObject();
    }
    w.EndArray();
...
void BufferShape(Context* c, GLenum target, GLsizeiptr size, const void* data, GLenum usage, bool storage) {
    ...
    if (!c->es3()) {
        ...
        if (data) b->shadow.assign((const uint8_t*)data, (const uint8_t*)data + size);
        else b->shadow.assign((size_t)size, 0);
    }
...
void Generated(ObjType kind, GLsizei n, const GLuint* names, const char* cmd) {
    Context* c = Current();
    if (!c || !names) return;
    for (GLsizei i = 0; i < n; ++i) RegisterName(c, kind, names[i], cmd);
}

void Deleted(ObjType kind, GLsizei n, const GLuint* names) {
    Context* c = Current();
    if (!c || !names) return;
    for (GLsizei i = 0; i < n; ++i) ForgetName(c, kind, names[i]);
}
...
void Reflect(Context* c, Object& p) {
    ...
    if (p.linked) {
        ...
        for (GLint i = 0; i < count; ++i) {
            ...
            if (v.size > 1 && v.name.size() > 3 && v.name.compare(v.name.size() - 3, 3, "[0]") == 0) v.name.resize(v.name.size() - 3);
        ...
        if (c->es3()) {
            ...
            for (GLint i = 0; i < count; ++i) {
                ...
                for (const ProgramVariable& u : p.uniforms) if (u.blockIndex == i) b.members.push_back(u);
    ...
    for (const ProgramVariable& a : p.attributes) {
        ...
        w.Key("type"); w.Raw(type(a.type));
        if (a.size > 1) { w.Key("size"); w.Int(a.size); }
        w.EndObject();
    }
    w.EndArray();
    ...
    for (const ProgramBlock& b : p.blocks) {
        w.BeginObject();
        w.Key("name"); w.String(b.name);
        w.Key("index"); w.Uint(b.index);
        w.Key("dataSize"); w.Int(b.dataSize);
        w.Key("members");
        w.BeginArray();
        for (const ProgramVariable& m : b.members) {
            ...
            w.Key("name"); w.String(dot == std::string::npos ? m.name : m.name.substr(dot + 1));
            w.Key("type"); w.Raw(type(m.type));
            w.Key("offset"); w.Int(m.offset);
            if (m.size > 1) { w.Key("count"); w.Int(m.size); }
            if (m.arrayStride) { w.Key("arrayStride"); w.Int(m.arrayStride); }
            if (m.matrixStride) { w.Key("matrixStride"); w.Int(m.matrixStride); }
            if (m.rowMajor) { w.Key("rowMajor"); w.Boolean(true); }
            w.EndObject();
        }
        w.EndArray();
        w.EndObject();
...
void DescribeShaders(Context* c, Object& p) {
    JsonWriter w;
    w.BeginArray();
    for (GLuint s : p.shaders) w.Ref(RefOf(ObjType::Shader, s), "GLShader");
    w.EndArray();
...
ObjType LabelType(GLenum identifier) {
    switch (identifier) {
        case GL_BUFFER: case GL_BUFFER_OBJECT_EXT: return ObjType::Buffer;
        case GL_SHADER: case GL_SHADER_OBJECT_EXT: return ObjType::Shader;
        case GL_PROGRAM: case GL_PROGRAM_OBJECT_EXT: return ObjType::Program;
        case GL_VERTEX_ARRAY: case GL_VERTEX_ARRAY_OBJECT_EXT: return ObjType::VertexArray;
        case GL_QUERY: case GL_QUERY_OBJECT_EXT: return ObjType::Query;
        case GL_PROGRAM_PIPELINE: case GL_PROGRAM_PIPELINE_OBJECT_EXT: return ObjType::ProgramPipeline;
...
void Label(GLenum identifier, GLuint name, GLsizei length, const GLchar* label) {
    ...
    if (kind == ObjType::Count) return;
    Object* o = ObjectOf(Current(), kind, name);
    if (!o) return;
    SetLabel(*o, !label ? std::string() : length < 0 ? std::string(label) : std::string(label, (size_t)length));
}
...
void DrawIndirect(GLenum mode, GLenum type, const void* indirect) {
    ...
    if (type) {
        p.indices = (const void*)(uintptr_t)(command[2] * (type == GL_UNSIGNED_INT ? 4 : type == GL_UNSIGNED_SHORT ? 2 : 1));
...
void Pre_glMultiDrawArraysEXT(GLenum mode, const GLint* first, const GLsizei* count, GLsizei n) {
    if (n > 0 && first && count) Draw(mode, first[0], count[0], 1);
}
void Pre_glMultiDrawElementsEXT(GLenum mode, const GLsizei* count, GLenum type, const void* const* indices, GLsizei n) {
    if (n > 0 && count && indices) DrawIndexed(mode, count[0], type, indices[0], 1, 0);
}
...
GLbitfield ClearBit(GLenum buffer) {
    return buffer == GL_COLOR ? GL_COLOR_BUFFER_BIT : buffer == GL_DEPTH ? GL_DEPTH_BUFFER_BIT : buffer == GL_STENCIL ? GL_STENCIL_BUFFER_BIT
...
void Post_glBindFramebuffer(GLenum target, GLuint framebuffer) {
    ...
    if (target == GL_FRAMEBUFFER || target == GL_DRAW_FRAMEBUFFER) c->drawFramebuffer = framebuffer;
    if (target == GL_FRAMEBUFFER || target == GL_READ_FRAMEBUFFER) c->readFramebuffer = framebuffer;
}
...
void Post_glBufferData(GLenum target, GLsizeiptr size, const void* data, GLenum usage) {
    if (Context* c = Current()) BufferShape(c, target, size, data, usage, false);
}
void Post_glBufferStorageEXT(GLenum target, GLsizeiptr size, const void* data, GLbitfield flags) {
    if (Context* c = Current()) BufferShape(c, target, size, data, flags, true);
}
void Post_glBufferSubData(GLenum target, GLintptr offset, GLsizeiptr size, const void* data) {
    ...
    if (!b) return;
    ++b->generation;
    if (!c->es3() && data && b->shadow.size() >= (size_t)(offset + size)) memcpy(b->shadow.data() + offset, data, (size_t)size);
}
void Post_glCopyBufferSubData(GLenum, GLenum writeTarget, GLintptr, GLintptr, GLsizeiptr) {
    if (Context* c = Current()) {
        if (Object* b = BufferAt(c, writeTarget)) ++b->generation;
    }
}
void Post_glMapBufferRange(GLenum target, GLintptr, GLsizeiptr, GLbitfield, void*) {
    if (Context* c = Current()) {
        if (Object* b = BufferAt(c, target)) ++b->generation;
    }
}
void Post_glMapBufferRangeEXT(GLenum target, GLintptr, GLsizeiptr, GLbitfield, void*) {
    if (Context* c = Current()) {
        if (Object* b = BufferAt(c, target)) {
            ++b->generation;
            if (!c->es3()) b->shadow.clear();   // written through a mapping the library cannot see
        }
    }
}
...
void Post_glTexImage2D(GLenum target, GLint level, GLint internalformat, GLsizei w, GLsizei h, GLint, GLenum format, GLenum type, const void*) {
    if (Context* c = Current()) TextureShape(c, target, level, SizedFormat((GLenum)internalformat, format, type), w, h, 1, 0, 0);
}
void Post_glTexImage3D(GLenum target, GLint level, GLint internalformat, GLsizei w, GLsizei h, GLsizei d, GLint, GLenum format, GLenum type, const void*) {
    if (Context* c = Current()) TextureShape(c, target, level, SizedFormat((GLenum)internalformat, format, type), w, h, d, 0, 0);
}
void Post_glTexImage3DOES(GLenum target, GLint level, GLenum internalformat, GLsizei w, GLsizei h, GLsizei d, GLint, GLenum format, GLenum type, const void*) {
    if (Context* c = Current()) TextureShape(c, target, level, SizedFormat(internalformat, format, type), w, h, d, 0, 0);
}
void Post_glCopyTexImage2D(GLenum target, GLint level, GLenum internalformat, GLint, GLint, GLsizei w, GLsizei h, GLint) {
    if (Context* c = Current()) TextureShape(c, target, level, SizedFormat(internalformat, internalformat, GL_UNSIGNED_BYTE), w, h, 1, 0, 0);
}
void Post_glTexStorage2D(GLenum target, GLsizei levels, GLenum internalformat, GLsizei w, GLsizei h) {
    if (Context* c = Current()) TextureShape(c, target, 0, internalformat, w, h, 1, levels, 0);
}
...
void Post_glTexStorage3D(GLenum target, GLsizei levels, GLenum internalformat, GLsizei w, GLsizei h, GLsizei d) {
    if (Context* c = Current()) TextureShape(c, target, 0, internalformat, w, h, d, levels, 0);
}
...
void Post_glTexStorage2DMultisample(GLenum target, GLsizei samples, GLenum internalformat, GLsizei w, GLsizei h, GLboolean) {
    if (Context* c = Current()) TextureShape(c, target, 0, internalformat, w, h, 1, 1, samples);
}
void Post_glTexStorage3DMultisample(GLenum target, GLsizei samples, GLenum internalformat, GLsizei w, GLsizei h, GLsizei d, GLboolean) {
    if (Context* c = Current()) TextureShape(c, target, 0, internalformat, w, h, d, 1, samples);
}
void Post_glCompressedTexImage2D(GLenum target, GLint level, GLenum internalformat, GLsizei w, GLsizei h, GLint, GLsizei imageSize, const void* data) {
    ...
    if (level == 0) {
        if (Object* t = TextureAt(c, target)) KeepCompressed(c, *t, target, 1, imageSize, data);
    }
}
void Post_glCompressedTexImage3D(GLenum target, GLint level, GLenum internalformat, GLsizei w, GLsizei h, GLsizei d, GLint, GLsizei imageSize, const void* data) {
    ...
    if (level == 0) {
        if (Object* t = TextureAt(c, target)) KeepCompressed(c, *t, target, d, imageSize, data);
    }
}
...
void Post_glCompressedTexSubImage2D(GLenum target, GLint level, GLint x, GLint y, GLsizei w, GLsizei h, GLenum, GLsizei imageSize, const void* data) {
    ...
    if (x == 0 && y == 0 && w == t->width && h == t->height) KeepCompressed(c, *t, target, 1, imageSize, data);
}
...
void Post_glRenderbufferStorage(GLenum, GLenum internalformat, GLsizei w, GLsizei h) {
    if (Context* c = Current()) RenderbufferShape(c, internalformat, w, h, 0);
}
void Post_glRenderbufferStorageMultisample(GLenum, GLsizei samples, GLenum internalformat, GLsizei w, GLsizei h) {
    if (Context* c = Current()) RenderbufferShape(c, internalformat, w, h, samples);
}
...
void Post_glCreateProgram(GLuint result) {
    if (Context* c = Current()) RegisterName(c, ObjType::Program, result, "glCreateProgram");
}
...
void Post_glAttachShader(GLuint program, GLuint shader) {
    ...
    if (!p) return;
    if (std::find(p->shaders.begin(), p->shaders.end(), shader) == p->shaders.end()) p->shaders.push_back(shader);
    DescribeShaders(c, *p);
}
...
void Post_glLinkProgram(GLuint program) {
    ...
    for (GLuint s : p->shaders) {
        ...
        w.BeginObject();
        w.Key("shader"); w.Ref(shader ? shader->id : 0, "GLShader");
        w.Key("type"); w.Raw(shader ? JsonEnum(shader->shaderType, EnumName_ShaderType) : "null");
        w.Key("source"); w.String(shader ? shader->source : std::string());
        w.EndObject();
    }
    w.EndArray();
...
GLenum Override_glGetError() {
    if (const GLenum saved = TakeSavedError()) return saved;
