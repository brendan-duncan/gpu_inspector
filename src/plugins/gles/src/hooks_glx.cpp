...
T Real(T& slot, const char* name) {
    if (!slot) slot = (T)RealSymbol(name);
    return slot;
}
...
GLXFuncPtr RealGetProcAddress(const char* name) {
    ...
    if (!get) get = Real(g_glx.glXGetProcAddress, "glXGetProcAddress");
...
void EnsureSurface(Display* display, GLXDrawable drawable) {
    if (!drawable || SurfaceId((void*)drawable)) return;
    int w = 0, h = 0;
...
void ResolveProcs() {
    ...
    for (size_t i = 0; i < kCommandCount; ++i) {
        if (!slots[i]) slots[i] = (void*)RealGetProcAddress(kCommandNames[i]);
    }
}
...
GLESINSP_EXPORT GLXContext glXCreateContextAttribsARB(Display* display, GLXFBConfig config, GLXContext share, Bool direct, const int* attribs) {
    if (!g_glx.glXCreateContextAttribsARB) g_glx.glXCreateContextAttribsARB = (PFN_glXCreateContextAttribsARB)RealGetProcAddress("glXCreateContextAttribsARB");
    if (!g_glx.glXCreateContextAttribsARB) return nullptr;
...
GLESINSP_EXPORT Bool glXMakeCurrent(Display* display, GLXDrawable drawable, GLXContext context) {
    if (!Real(g_glx.glXMakeCurrent, "glXMakeCurrent")) return 0;
...
GLESINSP_EXPORT Bool glXMakeContextCurrent(Display* display, GLXDrawable draw, GLXDrawable read, GLXContext context) {
    if (!Real(g_glx.glXMakeContextCurrent, "glXMakeContextCurrent")) return 0;
...
GLESINSP_EXPORT void glXDestroyContext(Display* display, GLXContext context) {
    if (!Real(g_glx.glXDestroyContext, "glXDestroyContext")) return;
...
GLESINSP_EXPORT void glXSwapBuffers(Display* display, GLXDrawable drawable) {
    if (!Real(g_glx.glXSwapBuffers, "glXSwapBuffers")) return;
    Context* c = CurrentKnown();
    if (c && !c->glx) c = nullptr;
    if (c) BeforeSwap(c, nullptr, (EGLSurface)drawable, "glXSwapBuffers");
...
GLXFuncPtr GetProcAddress(const unsigned char* name, PFN_glXGetProcAddress real) {
    ...
    if (strcmp(n, "glXCreateContextAttribsARB") == 0) {
        if (!g_glx.glXCreateContextAttribsARB) g_glx.glXCreateContextAttribsARB = (PFN_glXCreateContextAttribsARB)p;
        return (GLXFuncPtr)&glXCreateContextAttribsARB;
    }
    if (strcmp(n, "glXMakeCurrent") == 0) return (GLXFuncPtr)&glXMakeCurrent;
    if (strcmp(n, "glXMakeContextCurrent") == 0) return (GLXFuncPtr)&glXMakeContextCurrent;
    if (strcmp(n, "glXDestroyContext") == 0) return (GLXFuncPtr)&glXDestroyContext;
    if (strcmp(n, "glXSwapBuffers") == 0) return (GLXFuncPtr)&glXSwapBuffers;
    // A GL entry point: our hook stands in for the driver's, which is what the hook calls.
    for (size_t i = 0; i < kHookCount; ++i) {
        if (strcmp(kHooks[i].name, n) != 0) continue;
        if (!*kHooks[i].real) *kHooks[i].real = (void*)p;
