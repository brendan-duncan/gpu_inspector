...
bool LogEnabled()
{
    ...
    if (enabled < 0)
        enabled = Config::Get().Flag("GLESINSP_LOG") ? 1 : 0;
    return enabled == 1;
}
...
#if defined(_WIN32)
    if (c->wgl)
        return WglDrawable(c);
#elif defined(__linux__) && !defined(__ANDROID__)
    if (c->glx)
        return GlxDrawable(c);
#endif
...
#if defined(_WIN32)
    if (c->wgl)
        return WglLookupProc(name);
#elif defined(__linux__) && !defined(__ANDROID__)
    if (c->glx)
        return GlxLookupProc(name);
#endif
...
Context* Current()
{
    ...
    if (it == s.contexts.end())
    {
        ...
        EGLint version = 0;
        if (g_egl.eglQueryContext && g_egl.eglQueryContext(c->display, handle, EGL_CONTEXT_CLIENT_VERSION, &version))
            c->major = version;
...
uint64_t RegisterName(Context* c, ObjType kind, GLuint name, const char* cmd)
{
    ...
    auto it = names->find(name);
    if (it != names->end())
        return it->second;   // a name made twice is the same object
...
uint64_t EnsureName(Context* c, ObjType kind, GLuint name, const char* cmd)
{
    if (!name || !c)
        return 0;
    if (Object* o = ObjectOf(c, kind, name))
        return o->id;
...
void Forget(uint64_t id)
{
    LibraryState& s = State();
    std::lock_guard lock(s.mutex);
    if (!s.objects.erase(id))
        return;
    if (Server::Get().Connected())
        Server::Get().SendJson("{\"action\":\"DeleteObjects\",\"ids\":[" + std::to_string(id) + "]}");
}
...
{
    if (Server::Get().Connected())
        Server::Get().SendJson(AddObjectJson(o));
}
...
void WriteRefs(JsonWriter& w, ObjType type, const char* className, const GLuint* names, size_t n)
{
    ...
    for (size_t i = 0; i < n; ++i)
        w.Ref(RefOf(type, names[i]), className);
    w.EndArray();
}
...
void WriteEnums(JsonWriter& w, const char* (*nameOf)(GLenum), const GLenum* values, size_t n)
{
    ...
    w.BeginArray();
    for (size_t i = 0; i < n; ++i)
        w.Enum(nameOf(values[i]), (int64_t)values[i]);
    w.EndArray();
}
...
void WriteString(JsonWriter& w, const GLchar* s, GLsizei length)
{
    ...
    if (length < 0)
        ...
        w.String(std::string_view(s, (size_t)length));
}
...
std::string JoinStrings(GLsizei count, const GLchar* const* strings, const GLint* lengths)
{
    ...
    for (GLsizei i = 0; i < count; ++i)
    {
        ...
        if (lengths && lengths[i] >= 0)
            out.append(strings[i], (size_t)lengths[i]);
        else
