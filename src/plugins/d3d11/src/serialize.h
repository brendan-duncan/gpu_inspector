...
template <typename T>
inline void WriteRefs(JsonWriter& w, T* const* objects, UINT count)
{
    WriteRefs(w, (IUnknown* const*)objects, count);
}
...
public:
    ...
    Args& s(const char* key, const char* value)
    {
        w_.Key(key);
        if (value)
            w_.String(value);
    ...
    Args& s(const char* key, const std::string& value)
    {
        w_.Key(key);
        w_.String(value);
        return *this;
    }
    Args& ws(const char* key, const wchar_t* value)
    {
        w_.Key(key);
        if (value)
            w_.String(Narrow(value));
        else
            w_.Null();
        return *this;
    }
    /** An enum by its generated name, the number when the table has none. */
    Args& e(const char* key, const char* name, int64_t value)
    {
        w_.Key(key);
        w_.Enum(name, value);
        return *this;
    }
    Args& ref(const char* key, const void* object, const char* fallbackClass = "ID3D11DeviceChild")
    {
        w_.Key(key);
        WriteRef(w_, object, fallbackClass);
        return *this;
    }
    Args& refs(const char* key, IUnknown* const* objects, UINT count)
    {
        w_.Key(key);
        WriteRefs(w_, objects, count);
        return *this;
    }
    template <typename T>
    Args& refs(const char* key, T* const* objects, UINT count)
    {
        return refs(key, (IUnknown* const*)objects, count);
    }
    Args& uints(const char* key, const UINT* values, UINT count)
    {
        w_.Key(key);
        WriteUints(w_, values, count);
        return *this;
    }
    Args& floats(const char* key, const FLOAT* values, UINT count)
    {
        w_.Key(key);
        WriteFloats(w_, values, count);
        return *this;
    }
    Args& ptr(const char* key, const void* p)
    {
        w_.Key(key);
        w_.Pointer(p);
        return *this;
