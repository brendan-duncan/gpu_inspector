...
HRESULT STDMETHODCALLTYPE RecordingContext::SetPrivateData(REFGUID guid, UINT DataSize, const void* pData)
{
    ...
    if (SUCCEEDED(hr) && pData && DataSize)
    {
        if (guid == WKPDID_D3DDebugObjectName)
            OnObjectNamed(real_, std::string((const char*)pData, ((const char*)pData)[DataSize - 1] == 0 ? DataSize - 1 : DataSize));
        else if (guid == WKPDID_D3DDebugObjectNameW)
            OnObjectNamed(real_, Narrow((const wchar_t*)pData, DataSize / sizeof(wchar_t)));
    }
    return hr;
}
...
{
    for (UINT i = 0; i < count && start + i < N; ++i)
        slots[start + i] = views ? views[i] : nullptr;
}
...
#define STAGE_HOOKS(Prefix, StageIndex, ShaderType)                                                                                                                                                                                                         \
    ...
    {                                                                                                                                                                                                                                                       \
        ...
        {                                                                                                                                                                                                                                                   \
            Args a;                                                                                                                                                                                                                                         \
            a.u("StartSlot", StartSlot).u("NumBuffers", NumBuffers).refs("ppConstantBuffers", ppConstantBuffers, ppConstantBuffers ? NumBuffers : 0).uints("pFirstConstant", pFirstConstant, NumBuffers).uints("pNumConstants", pNumConstants, NumBuffers); \
...
void STDMETHODCALLTYPE RecordingContext::CSSetUnorderedAccessViews(UINT StartSlot, UINT NumUAVs, ID3D11UnorderedAccessView* const* ppUnorderedAccessViews, const UINT* pUAVInitialCounts)
{
    ...
    if (!c)
        return;
    for (UINT i = 0; i < NumUAVs && StartSlot + i < c->state.csUavs.size(); ++i)
        c->state.csUavs[StartSlot + i] = ppUnorderedAccessViews ? ppUnorderedAccessViews[i] : nullptr;
    if (Recording())
    {
        Args a;
...
void STDMETHODCALLTYPE RecordingContext::IASetVertexBuffers(UINT StartSlot, UINT NumBuffers, ID3D11Buffer* const* ppVertexBuffers, const UINT* pStrides, const UINT* pOffsets)
{
    ...
    {
        Args a;
        a.u("StartSlot", StartSlot).u("NumBuffers", NumBuffers).refs("ppVertexBuffers", ppVertexBuffers, ppVertexBuffers ? NumBuffers : 0).uints("pStrides", pStrides, NumBuffers).uints("pOffsets", pOffsets, NumBuffers);
...
#define DRAW_PROLOGUE(call)       \
    Context* c = ctx_;            \
    std::string state;            \
    if (c)                        \
        state = BeforeDraw(c, p); \
    call;                         \
    if (!c)                       \
        return;                   \
    AfterDraw(c);                 \
...
void STDMETHODCALLTYPE RecordingContext::DrawIndexedInstanced(UINT IndexCountPerInstance, UINT InstanceCount, UINT StartIndexLocation, INT BaseVertexLocation, UINT StartInstanceLocation)
{
    ...
    Args a;
    a.u("IndexCountPerInstance", IndexCountPerInstance).u("InstanceCount", InstanceCount).u("StartIndexLocation", StartIndexLocation).i("BaseVertexLocation", BaseVertexLocation).u("StartInstanceLocation", StartInstanceLocation);
...
void STDMETHODCALLTYPE RecordingContext::Dispatch(UINT ThreadGroupCountX, UINT ThreadGroupCountY, UINT ThreadGroupCountZ)
{
    ...
    std::string state;
    if (c)
        state = BeforeDispatch(c, false, nullptr, 0);
...
void STDMETHODCALLTYPE RecordingContext::DispatchIndirect(ID3D11Buffer* pBufferForArgs, UINT AlignedByteOffsetForArgs)
{
    ...
    std::string state;
    if (c)
        state = BeforeDispatch(c, true, pBufferForArgs, AlignedByteOffsetForArgs);
...
void SetRenderTargets(Context* c, UINT NumViews, ID3D11RenderTargetView* const* ppRenderTargetViews, ID3D11DepthStencilView* pDepthStencilView)
{
    ...
    for (UINT i = 0; i < s.renderTargets.size(); ++i)
        s.renderTargets[i] = i < n && ppRenderTargetViews ? ppRenderTargetViews[i] : nullptr;
    s.renderTargetCount = ppRenderTargetViews ? n : 0;
    // Trailing nulls do not count as targets.
    while (s.renderTargetCount && !s.renderTargets[s.renderTargetCount - 1])
        --s.renderTargetCount;
...
void STDMETHODCALLTYPE RecordingContext::OMSetRenderTargetsAndUnorderedAccessViews(UINT NumRTVs, ID3D11RenderTargetView* const* ppRenderTargetViews, ID3D11DepthStencilView* pDepthStencilView, UINT UAVStartSlot, UINT NumUAVs, ID3D11UnorderedAccessView* const* ppUnorderedAccessViews, const UINT* pUAVInitialCounts)
{
    ...
    if (c)
    {
        if (NumRTVs != D3D11_KEEP_RENDER_TARGETS_AND_DEPTH_STENCIL)
            SetRenderTargets(c, NumRTVs, ppRenderTargetViews, pDepthStencilView);
        if (NumUAVs != D3D11_KEEP_UNORDERED_ACCESS_VIEWS)
        {
            for (UINT i = 0; i < NumUAVs && UAVStartSlot + i < c->state.psUavs.size(); ++i)
                c->state.psUavs[UAVStartSlot + i] = ppUnorderedAccessViews ? ppUnorderedAccessViews[i] : nullptr;
        }
        AfterSetRenderTargets(c);
    }
    if (c && Recording())
    {
        ...
        if (keepTargets)
            a.s("NumRTVs", "D3D11_KEEP_RENDER_TARGETS_AND_DEPTH_STENCIL");
        else
            a.u("NumRTVs", NumRTVs);
        ...
        if (keepUavs)
            a.s("NumUAVs", "D3D11_KEEP_UNORDERED_ACCESS_VIEWS");
        else
            a.u("NumUAVs", NumUAVs);
...
void STDMETHODCALLTYPE RecordingContext::OMSetBlendState(ID3D11BlendState* pBlendState, const FLOAT BlendFactor[4], UINT SampleMask)
{
    ...
    for (int i = 0; i < 4; ++i)
        c->state.blendFactor[i] = BlendFactor ? BlendFactor[i] : 1.0f;
...
void STDMETHODCALLTYPE RecordingContext::RSSetViewports(UINT NumViewports, const D3D11_VIEWPORT* pViewports)
{
    ...
    for (UINT i = 0; i < n && pViewports; ++i)
        c->state.viewports[i] = pViewports[i];
...
void STDMETHODCALLTYPE RecordingContext::RSSetScissorRects(UINT NumRects, const D3D11_RECT* pRects)
{
    ...
    for (UINT i = 0; i < n && pRects; ++i)
        c->state.scissors[i] = pRects[i];
...
void STDMETHODCALLTYPE RecordingContext::SOSetTargets(UINT NumBuffers, ID3D11Buffer* const* ppSOTargets, const UINT* pOffsets)
{
    ...
    if (!c)
        return;
    for (UINT i = 0; i < c->state.streamOutput.size(); ++i)
        c->state.streamOutput[i] = i < NumBuffers && ppSOTargets ? ppSOTargets[i] : nullptr;
    if (Recording())
    {
        Args a;
...
void STDMETHODCALLTYPE RecordingContext::ClearRenderTargetView(ID3D11RenderTargetView* pRenderTargetView, const FLOAT ColorRGBA[4])
{
    Context* c = ctx_;
    if (c)
        BeforeClear(c, pRenderTargetView, false, false);
...
void STDMETHODCALLTYPE RecordingContext::ClearDepthStencilView(ID3D11DepthStencilView* pDepthStencilView, UINT ClearFlags, FLOAT Depth, UINT8 Stencil)
{
    Context* c = ctx_;
    if (c)
        BeforeClear(c, pDepthStencilView, (ClearFlags & D3D11_CLEAR_DEPTH) != 0, (ClearFlags & D3D11_CLEAR_STENCIL) != 0);
...
void STDMETHODCALLTYPE RecordingContext::ClearUnorderedAccessViewUint(ID3D11UnorderedAccessView* pUnorderedAccessView, const UINT Values[4])
{
    Context* c = ctx_;
    if (c)
        BeforeClear(c, pUnorderedAccessView, false, false);
...
void STDMETHODCALLTYPE RecordingContext::ClearView(ID3D11View* pView, const FLOAT Color[4], const D3D11_RECT* pRect, UINT NumRects)
{
    ...
    if (c && NumRects == 0)
        BeforeClear(c, pView, true, false);
    else if (c && pView)
        if (Object* v = Find(pView))
...
void STDMETHODCALLTYPE RecordingContext::DiscardResource(ID3D11Resource* pResource)
{
    Context* c = ctx_;
    if (c)
        BeforeDiscard(c, nullptr, pResource);
...
void STDMETHODCALLTYPE RecordingContext::DiscardView(ID3D11View* pResourceView)
{
    Context* c = ctx_;
    if (c)
        BeforeDiscard(c, pResourceView, nullptr);
...
HRESULT STDMETHODCALLTYPE RecordingContext::Map(ID3D11Resource* pResource, UINT Subresource, D3D11_MAP MapType, UINT MapFlags, D3D11_MAPPED_SUBRESOURCE* pMappedResource)
{
    ...
    if (!c)
        return hr;
    if (SUCCEEDED(hr) && MapType != D3D11_MAP_READ)
        OnResourceWritten(pResource);
    if (Recording())
    {
        Args a;
        ...
        if (pMappedResource && SUCCEEDED(hr))
            Write(a.key("pMappedResource"), *pMappedResource);
        else
            a.null("pMappedResource");
        a.i("result", hr);
        Record(c, "Map", a.str());
    }
    return hr;
...
uint64_t UpdateBytes(ID3D11Resource* dst, const D3D11_BOX* box, UINT rowPitch, UINT depthPitch)
{
    ...
    if (!o)
        return 0;
    if (o->kind == ObjKind::Buffer)
        return box ? (box->right > box->left ? box->right - box->left : 0) : o->size;
    ...
    if (depth > 1 && depthPitch)
        return (uint64_t)depthPitch * depth;
...
void STDMETHODCALLTYPE RecordingContext::CopySubresourceRegion(ID3D11Resource* pDstResource, UINT DstSubresource, UINT DstX, UINT DstY, UINT DstZ, ID3D11Resource* pSrcResource, UINT SrcSubresource, const D3D11_BOX* pSrcBox)
{
    ...
    if (!Recording())
        return;
    Args a;
    a.ref("pDstResource", pDstResource).u("DstSubresource", DstSubresource).u("DstX", DstX).u("DstY", DstY).u("DstZ", DstZ).ref("pSrcResource", pSrcResource).u("SrcSubresource", SrcSubresource);
...
void STDMETHODCALLTYPE RecordingContext::ResolveSubresource(ID3D11Resource* pDstResource, UINT DstSubresource, ID3D11Resource* pSrcResource, UINT SrcSubresource, DXGI_FORMAT Format)
{
    ...
    if (!Recording())
        return;
    Args a;
    a.ref("pDstResource", pDstResource).u("DstSubresource", DstSubresource).ref("pSrcResource", pSrcResource).u("SrcSubresource", SrcSubresource).e("Format", ToString_DXGI_FORMAT(Format), Format);
...
void STDMETHODCALLTYPE RecordingContext::GenerateMips(ID3D11ShaderResourceView* pShaderResourceView)
{
    ...
    if (!c)
        return;
    if (Object* v = Find(pShaderResourceView))
        if (Object* r = FindById(v->resourceId))
            ++r->generation;
    if (!Recording())
        return;
...
void STDMETHODCALLTYPE RecordingContext::ExecuteCommandList(ID3D11CommandList* pCommandList, BOOL RestoreContextState)
{
    Context* c = ctx_;
    if (c)
        OnExecuteCommandList(c, pCommandList, RestoreContextState);
    real_->ExecuteCommandList(pCommandList, RestoreContextState);
    if (c && !RestoreContextState)
        ClearShadow(c);
}
...
HRESULT STDMETHODCALLTYPE RecordingContext::FinishCommandList(BOOL RestoreDeferredContextState, ID3D11CommandList** ppCommandList)
{
    ...
    if (list)
    {
        if (!VtableHooked(list))
            HookDeviceChild(list, "ID3D11CommandList", kDeviceChildSlots, {});
...
enum AnnotationSlot : uint32_t
{
    kAnnotationBeginEvent = 3,
    kAnnotationEndEvent = 4,
    kAnnotationSetMarker = 5,
    kAnnotationCount = 7
};
...
Context* AnnotationContext(ID3DUserDefinedAnnotation* This)
{
    ...
    ID3D11DeviceContext* ctx = nullptr;
    {
        ...
        if (FAILED(This->QueryInterface(IID_PPV_ARGS(&ctx))) || !ctx)
            return nullptr;
        ctx->Release();
    }
...
void DestroyContextsOf(ID3D11Device* device)
{
    std::vector<ID3D11DeviceContext*> gone;
    {
        ...
        for (auto& [ptr, c] : s.contexts)
            if (c->device == device)
                gone.push_back(c->ptr);
    }
    for (ID3D11DeviceContext* real : gone)
        DestroyContext(real);
}
...
ID3D11DeviceContext* WrapContext(ID3D11DeviceContext* context, ID3D11Device* device, const char* cmd)
{
    ...
    if (Context* mine = ContextOfProxy(context))
        return mine->proxy;
    // One the application already has a proxy for (GetImmediateContext again): the same proxy.
    if (Context* known = FindContext(context); known && known->proxy)
        return known->proxy;
    ...
    if (!c.proxy)
    {
        ID3D11DeviceContext4* real = nullptr;
        {
            ...
            if (FAILED(context->QueryInterface(IID_PPV_ARGS(&real))) || !real)
                real = (ID3D11DeviceContext4*)context;
            else
                real->Release();
        }
