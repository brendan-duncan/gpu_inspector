...
// Every format with a memory layout the inspector's decoders read, plus the layouts of the ones
// they do not, so a read-back still knows its size. Typeless formats take the layout of their
// family; TypedFormat() picks the spelling the data is decoded as. The same table as the D3D12
// library's (src/d3d12/src/formats.cpp): DXGI formats are the same in both APIs.
