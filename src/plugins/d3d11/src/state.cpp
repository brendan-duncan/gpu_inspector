...
bool LogEnabled()
{
    ...
    if (enabled < 0)
        enabled = Config::Get().Flag("D3D11INSP_LOG") ? 1 : 0;
    return enabled == 1;
}
...
static std::string AddObjectJson(const Object& o)
{
    ...
    w.Uint(0);
    w.Key("handle");
    w.String(Hex((uint64_t)(uintptr_t)o.ptr));
    w.Key("label");
    if (o.label.empty())
        w.Null();
    else
...
Context& ContextOf(ID3D11DeviceContext* ctx)
{
    ...
    c->id = IdOf(ctx);
    {
        ...
        if (c->device)
            c->device->Release();   // the device outlives its contexts; a weak pointer is enough
