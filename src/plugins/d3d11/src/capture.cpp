...
ID3D11Buffer* MakeStagingBuffer(ID3D11Device* device, size_t size)
{
    ...
    if (FAILED(device->CreateBuffer(&d, nullptr, &b)))
        return nullptr;
    return b;
}
...
{
    if (!g_options.captureBuffers || !buffer || size == 0)
        return 0;
...
DXGI_FORMAT DecodeFormat(const Object& tex, DXGI_FORMAT viewFormat)
{
    ...
    if (viewFormat != DXGI_FORMAT_UNKNOWN)
    {
        ...
        if (FormatOf(typed).protocolName)
            return typed;
    }
...
bool CopyTexture(Context* c, Object& tex, UINT mip, UINT firstSlice, UINT slices, DXGI_FORMAT viewFormat, PendingTexture& p, std::string& error)
{
    ...
    if (firstSlice >= arraySize)
    {
        error = "the view's slices are outside the texture";
        return false;
    }
    ...
    if (!f.protocolName)
    {
        error = std::string("the inspector cannot decode ") + FormatName(tex.format);
        return false;
    }
    ...
    if (tex.samples > 1)
    {
        ...
        {
            error = "a multisampled depth target cannot be resolved for reading";
            return false;
        }
        if (f.compressed)
        {
            error = "a multisampled compressed texture cannot be resolved";
            return false;
        }
        ...
        if (FAILED(device->CreateTexture2D(&rd, nullptr, &r)))
        {
            error = "the resolve texture could not be created";
            return false;
        }
        resolved.reset(r);
    ...
    if (tex.kind == ObjKind::Texture1D)
    {
        ...
        if (FAILED(device->CreateTexture1D(&sd, nullptr, &t)))
        {
            error = "the staging texture could not be created";
            return false;
        }
        staging = t;
    }
    else if (volume)
    {
        ...
        if (FAILED(device->CreateTexture3D(&sd, nullptr, &t)))
        {
            error = "the staging texture could not be created";
            return false;
        }
    ...
    else
    {
        ...
        if (FAILED(device->CreateTexture2D(&sd, nullptr, &t)))
        {
            error = "the staging texture could not be created";
            return false;
        }
...
bool SameTargets(const Context* c)
{
    ...
    if (p.targetCount != s.renderTargetCount || p.depth != s.depthStencil)
        return false;
    for (UINT i = 0; i < p.targetCount; ++i)
        if (p.targets[i] != s.renderTargets[i])
            return false;
    return true;
}
...
void WriteAttachment(JsonWriter& w, ID3D11View* view, uint32_t attachment, bool depth)
{
    ...
    w.Uint(attachment);
    w.Key("aspect");
    w.String(depth ? "depth" : "color");
    w.Key("view");
    WriteRef(w, view);
    w.Key("resource");
    if (r)
        w.Ref(r->id, r->type.c_str());
    else
        w.Null();
    if (v)
    {
        ...
        w.Key("slices");
        w.Uint(v->sliceCount);
        if (v->readOnlyDepth || v->readOnlyStencil)
        {
            w.Key("readOnly");
            w.Boolean(true);
        }
    ...
    if (r)
    {
        ...
        const FormatInfo f = FormatOf(typed);
        w.Key("format");
        w.String(f.protocolName ? f.protocolName : "VK_FORMAT_UNDEFINED");
        w.Key("dxgiFormat");
        w.String(FormatName(typed));
        w.Key("width");
        w.Uint(std::max(1u, r->width >> (v ? v->mip : 0)));
        w.Key("height");
        w.Uint(std::max(1u, r->height >> (v ? v->mip : 0)));
        if (r->samples > 1)
        {
            w.Key("samples");
            w.Uint(r->samples);
        ...
        {
            w.Key("swapChain");
            w.Ref(r->swapChain, "IDXGISwapChain");
        }
    }
    w.EndObject();
}
...
std::string PassArgs(Context* c)
{
    ...
    {
        if (p.targets[i])
            WriteAttachment(w, p.targets[i], i, false);
    }
    if (p.depth)
        WriteAttachment(w, p.depth, p.targetCount, true);
    w.EndArray();
...
void ReadTarget(Context* c, ID3D11View* view, uint32_t attachment, bool depth)
{
    ...
    else
    {
        ...
        if (depth && f.stencil)
            p.stencilIndex = g_textures.size() + 1;
        g_pendingTextures.push_back(p);
    }
    const bool stencil = depth && f.stencil && t.error.empty();
    if (c->deferred && c->recorder)
        c->recorder->passTextures.push_back({g_textures.size(), c->pass.index});
    g_textures.push_back(std::move(t));
    if (stencil)
    {
...
{
    if (!g_options.captureTextures || c->pass.readBack || c->pass.compute)
        return;
    ...
    {
        if (p.targets[i])
            ReadTarget(c, p.targets[i], i, false);
    }
    if (p.depth)
        ReadTarget(c, p.depth, p.targetCount, true);
}
...
{
    if (!g_options.profilePasses || c->deferred)
        return nullptr;
    if (!c->disjointQuery)
    {
...
{
    const PipelineState& s = c->state;
    if (c->pass.open && !c->pass.compute && SameTargets(c))
        return true;
    EndPass(c);
    if (s.renderTargetCount == 0 && !s.depthStencil)
        return false;
    OpenPass& p = c->pass;
    p = OpenPass();
    p.open = true;
...
void WriteView(Context* c, JsonWriter& w, ID3D11View* view, bool contents)
{
    ...
    if (r->kind == ObjKind::Buffer)
    {
        ...
        {
            w.Key("data");
            w.Uint(CaptureBuffer(c, (ID3D11Buffer*)r->ptr, (int64_t)v->viewOffset, (size_t)v->viewSize));
        }
    }
    else if (contents)
    {
        w.Key("capture");
        w.Uint(CaptureTexture(c, *r, v->viewFormat));
    }
}
...
void WriteVertexInput(Context* c, JsonWriter& w, const DrawParams& p)
{
    const PipelineState& s = c->state;
    w.Key("topology");
    w.Enum(ToString_D3D_PRIMITIVE_TOPOLOGY(s.topology), s.topology);
    w.Key("inputLayout");
    WriteRef(w, s.inputLayout);
    ...
    if (layout)
    {
        ...
        for (const InputElement& e : layout->elements)
        {
            ...
            w.Uint(location++);
            w.Key("name");
            w.String(e.semanticName + (e.semanticIndex ? std::to_string(e.semanticIndex) : std::string()));
            w.Key("slot");
            w.Uint(e.slot);
            w.Key("offset");
            w.Uint(e.offset);
    ...
    for (UINT slot = 0; slot < s.vertexBuffers.size(); ++slot)
    {
        ...
        if (used && o && g_options.captureBuffers)
        {
            ...
            w.Key("data");
            w.Uint(CaptureBuffer(c, vb.buffer, vb.offset, size));
        }
        w.EndObject();
    }
    w.EndArray();
    if (s.indexBuffer)
    {
        ...
        w.BeginObject();
        w.Key("buffer");
        WriteRef(w, s.indexBuffer);
        w.Key("format");
        w.Enum(ToString_DXGI_FORMAT(s.indexFormat), s.indexFormat);
        w.Key("offset");
        w.Uint(s.indexOffset);
        if (p.indexed && g_options.captureBuffers)
        {
            ...
            if (p.indirect && o)
                size = o->size > (size_t)offset ? o->size - (size_t)offset : 0;
            w.Key("data");
            w.Uint(CaptureBuffer(c, s.indexBuffer, offset, size));
        }
        w.EndObject();
    }
}
...
void WriteFixedFunction(Context* c, JsonWriter& w)
{
    ...
    WriteRef(w, s.rasterizerState);
    w.Key("viewports");
    WriteViewports(w, s.viewports.data(), s.viewportCount);
    w.Key("scissors");
    WriteRects(w, s.scissors.data(), s.scissorCount);
    w.Key("blend");
    w.BeginObject();
    w.Key("state");
    WriteRef(w, s.blendState);
    w.Key("factor");
    WriteFloats(w, s.blendFactor, 4);
    w.Key("sampleMask");
    w.Uint(s.sampleMask);
    w.EndObject();
    ...
    w.BeginObject();
    w.Key("state");
    WriteRef(w, s.depthStencilState);
    w.Key("stencilRef");
    w.Uint(s.stencilRef);
    w.EndObject();
    if (s.predicate)
    {
        w.Key("predicate");
        WriteRef(w, s.predicate);
        w.Key("predicateValue");
        w.Boolean(s.predicateValue != 0);
    }
    bool so = false;
    for (ID3D11Buffer* b : s.streamOutput)
        so = so || b != nullptr;
    if (so)
    {
        w.Key("streamOutput");
...
std::string Snapshot(Context* c, const DrawParams* draw, bool indirect, ID3D11Buffer* argsBuffer, UINT argsOffset)
{
    ...
    w.BeginObject();
    w.Key("passIndex");
    w.Uint(c->pass.open ? c->pass.index : 0);
    if (draw)
    {
        w.Key("draw");
        w.BeginObject();
        ...
        w.Key("instances");
        w.Uint(draw->instances);
        w.Key("firstInstance");
        w.Uint(draw->startInstance);
        w.Key("indirect");
    ...
    if (indirect && argsBuffer)
    {
        ...
        w.BeginObject();
        w.Key("buffer");
        WriteRef(w, argsBuffer);
        w.Key("offset");
        w.Uint(argsOffset);
        w.Key("data");
        w.Uint(CaptureBuffer(c, argsBuffer, argsOffset, draw ? 20 : 12));
        w.EndObject();
    }
    w.EndObject();
    return w.str();
    ...
    for (UINT z = 0; z < p.depth; ++z)
    {
        ...
        if (!p.depthStencil)
        {
            for (uint32_t y = 0; y < rows; ++y)
                t.data.insert(t.data.end(), slice + (size_t)y * m.RowPitch, slice + (size_t)y * m.RowPitch + rowBytes);
            continue;
        }
        ...
        for (uint32_t y = 0; y < rows; ++y)
        {
            ...
            for (uint32_t x = 0; x < p.width; ++x)
            {
                if (d32s8)
                {
                    ...
                    if (stencil)
                        stencil->data.push_back(row[x * 8 + 4]);
                }
                else if (f.bytes == 4)
                {
                    t.data.insert(t.data.end(), row + x * 4, row + x * 4 + 4);
                    if (stencil)
                        stencil->data.push_back(row[x * 4 + 3]);
                }
                else
                {
...
std::string PassTimes()
{
    ...
    std::vector<Time> times;
    {
        ...
        for (Context* c : g_timedContexts)
        {
            ...
            else
            {
                ...
                {
                    if (q.context != c || !q.begin || !q.end)
                        continue;
                    UINT64 begin = 0, end = 0;
                    if (!QueryResult(c->ptr, q.begin, begin) || !QueryResult(c->ptr, q.end, end) || end < begin)
                        continue;
    ...
    for (const Time& t : times)
        origin = std::min(origin, t.startMs);
    w.Key("count");
    w.Uint(times.size());
    w.Key("passes");
    w.BeginArray();
    for (const Time& t : times)
    {
        ...
        w.Uint(t.q->frame);
        w.Key("commandBuffer");
        w.Uint(t.q->contextId);
        w.Key("passIndex");
        w.Uint(t.q->passIndex);
        w.Key("kind");
        w.String(t.q->compute ? "compute" : "render");
        w.Key("startMs");
        w.Double(t.startMs - origin);
        w.Key("durationMs");
...
void WriteCommand(JsonWriter& out, const Recorded& r, size_t index, bool child)
{
    ...
    {
        out.Key("object");
        out.Ref(r.stream, "ID3D11DeviceContext");
    }
    out.Key("args");
    ...
    if (r.children)
    {
        ...
        out.BeginArray();
        out.BeginObject();
        out.Key("commandBuffer");
        out.Uint(r.childStream);
        out.Key("commands");
        out.BeginArray();
        for (size_t i = 0; i < r.children->size(); ++i)
            WriteCommand(out, (*r.children)[i], i, true);
        out.EndArray();
        out.EndObject();
        out.EndArray();
    }
...
void SendCapture()
{
    ...
    const uint32_t frames = g_frame;
    {
        ...
        for (size_t start = 0, batch = 0; start < g_commands.size(); start += kBatch, ++batch)
        {
            ...
            out.Key("frame");
            out.Uint(g_startPresent);
            out.Key("index");
            out.Uint(batch);
            out.Key("commands");
            out.BeginArray();
            for (size_t i = start; i < std::min(g_commands.size(), start + kBatch); ++i)
                WriteCommand(out, g_commands[i], i, false);
            out.EndArray();
            out.EndObject();
...
void Record(Context* c, const char* method, std::string args, std::string state)
{
    ...
    if (!Recording())
        return;
    if (kComputePassEnd.count(method))
        EndComputePass(c);
...
void AfterDraw(Context* c)
{
    ...
    for (ID3D11UnorderedAccessView* u : c->state.psUavs)
        if (u)
            if (Object* v = Find(u))
                if (Object* r = FindById(v->resourceId))
                    ++r->generation;
    for (ID3D11Buffer* b : c->state.streamOutput)
        if (b)
            if (Object* o = Find(b))
                ++o->generation;
    for (UINT i = 0; i < c->state.renderTargetCount; ++i)
        if (c->state.renderTargets[i])
            if (Object* v = Find(c->state.renderTargets[i]))
                if (Object* r = FindById(v->resourceId))
                    ++r->generation;
    if (c->state.depthStencil)
        if (Object* v = Find(c->state.depthStencil))
            if (Object* r = FindById(v->resourceId))
                ++r->generation;
}
...
void AfterDispatch(Context* c)
{
    if (!c)
        return;
    std::lock_guard lock(g_mutex);
    for (ID3D11UnorderedAccessView* u : c->state.csUavs)
        if (u)
            if (Object* v = Find(u))
                if (Object* r = FindById(v->resourceId))
                    ++r->generation;
...
void BeforeDiscard(Context* c, ID3D11View* view, ID3D11Resource* resource)
{
    ...
    if (!Recording())
        return;
    std::lock_guard lock(g_mutex);
    if (!Recording() || !c->pass.open || c->pass.compute)
        return;
    const OpenPass& p = c->pass;
    ...
    uint32_t bits = 0;
    for (UINT i = 0; i < p.targetCount; ++i)
        if (sameResource(p.targets[i]))
            bits |= 1u << i;
    if (sameResource(p.depth))
        bits |= (1u << 8) | (1u << 9);
    if (!bits)
        return;
