...
void WriteRef(JsonWriter& w, const void* object, const char* fallbackClass)
{
    ...
    if (Object* o = Find(object))
        w.Ref(o->id, o->type.c_str());
    else
        w.UntrackedRef((uint64_t)(uintptr_t)object, fallbackClass);
}
...
void Write(JsonWriter& w, const DXGI_SWAP_CHAIN_DESC& v)
{
    ...
    S(SampleDesc);
    KEY(BufferUsage);
    WriteUsage(w, v.BufferUsage);
    U(BufferCount);
    KEY(OutputWindow);
    w.Pointer(v.OutputWindow);
    B(Windowed);
...
void Write(JsonWriter& w, const DXGI_PRESENT_PARAMETERS& v)
{
    ...
    U(DirtyRectsCount);
    KEY(pDirtyRects);
    WriteRects(w, v.pDirtyRects, v.DirtyRectsCount);
    KEY(pScrollRect);
    if (v.pScrollRect)
    ...
    if (v.pScrollOffset)
    {
        w.BeginObject();
        w.Key("x");
        w.Int(v.pScrollOffset->x);
        w.Key("y");
        w.Int(v.pScrollOffset->y);
        w.EndObject();
    }
    else
        w.Null();
...
static void WriteTexture2D(JsonWriter& w, const D3D11_TEXTURE2D_DESC& v, const D3D11_TEXTURE_LAYOUT* layout)
{
    ...
    {
        KEY(TextureLayout);
        w.Enum(ToString_D3D11_TEXTURE_LAYOUT(*layout), *layout);
    }
    w.EndObject();
}
...
void WriteInitialData(JsonWriter& w, const D3D11_SUBRESOURCE_DATA* data, UINT count)
{
    ...
    for (UINT i = 0; i < count; ++i)
    {
        w.BeginObject();
        w.Key("pSysMem");
        w.Pointer(data[i].pSysMem);
        w.Key("SysMemPitch");
        w.Uint(data[i].SysMemPitch);
        w.Key("SysMemSlicePitch");
        w.Uint(data[i].SysMemSlicePitch);
        w.EndObject();
    }
    w.EndArray();
}
...
void Write(JsonWriter& w, const D3D11_SHADER_RESOURCE_VIEW_DESC* p)
{
    ...
    switch (v.ViewDimension)
    {
        ...
        case D3D11_SRV_DIMENSION_BUFFEREX:
            ...
            w.BeginObject();
            w.Key("FirstElement");
            w.Uint(v.BufferEx.FirstElement);
            w.Key("NumElements");
            w.Uint(v.BufferEx.NumElements);
            w.Key("Flags");
            Flags_D3D11_BUFFEREX_SRV_FLAG(w, v.BufferEx.Flags);
            w.EndObject();
            break;
...
void Write(JsonWriter& w, const D3D11_UNORDERED_ACCESS_VIEW_DESC* p)
{
    ...
    switch (v.ViewDimension)
    {
        case D3D11_UAV_DIMENSION_BUFFER:
            ...
            w.Uint(v.Buffer.NumElements);
            w.Key("Flags");
            Flags_D3D11_BUFFER_UAV_FLAG(w, v.Buffer.Flags);
            w.EndObject();
            break;
...
void Write(JsonWriter& w, const D3D11_INPUT_ELEMENT_DESC* elements, UINT count)
{
    ...
    for (UINT i = 0; i < count; ++i)
    {
        ...
        if (v.AlignedByteOffset == D3D11_APPEND_ALIGNED_ELEMENT)
            w.String("D3D11_APPEND_ALIGNED_ELEMENT");
        else
            w.Uint(v.AlignedByteOffset);
...
static void WriteTargetBlend(JsonWriter& w, const D3D11_RENDER_TARGET_BLEND_DESC& v)
{
    ...
    Flags_D3D11_COLOR_WRITE_ENABLE(w, v.RenderTargetWriteMask);
    w.EndObject();
}
...
void Write(JsonWriter& w, const D3D11_BLEND_DESC& v)
{
    ...
    w.BeginArray();
    for (UINT i = 0; i < 8; ++i)
        WriteTargetBlend(w, v.RenderTarget[i]);
    w.EndArray();
    w.EndObject();
}
