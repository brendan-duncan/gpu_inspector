...
static bool EnablesMultiview(const VkDeviceCreateInfo& info)
{
    if (auto* mv = ChainFind(info.pNext, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MULTIVIEW_FEATURES))
        if (((const VkPhysicalDeviceMultiviewFeatures*)mv)->multiview)
            return true;
    if (auto* v11 = ChainFind(info.pNext, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_1_FEATURES))
        if (((const VkPhysicalDeviceVulkan11Features*)v11)->multiview)
            return true;
    return false;
}
...
{
    if (!inst || !inst->dispatch.GetPhysicalDeviceFeatures)
        return;
    if (ConfigFlag("VKINSP_NO_PIPELINE_STATISTICS"))
        return;   // leave the device exactly as asked
    ...
    if (EnablesMultiview(info))
        Log("pass counters: multiview is enabled; passes that render several views go uncounted");
    ...
    if (found)
        appFeatures = &((const VkPhysicalDeviceFeatures2*)found)->features;
    else if (info.pEnabledFeatures)
        appFeatures = info.pEnabledFeatures;
    ...
    if (!setup.inheritedQueries)
        Log("pass counters: the device does not support inheritedQueries; passes that execute secondary command buffers go uncounted");
    if (!wantStats && !wantOcclusion && !wantInherited)
        return;   // the application already enables what is wanted
    ...
    if (found)
    {
        ...
        if (wantOcclusion)
            setup.features2.features.occlusionQueryPrecise = VK_TRUE;
        if (wantInherited)
            setup.features2.features.inheritedQueries = VK_TRUE;
    ...
    else
    {
        ...
        if (wantOcclusion)
            setup.features.occlusionQueryPrecise = VK_TRUE;
        if (wantInherited)
            setup.features.inheritedQueries = VK_TRUE;
