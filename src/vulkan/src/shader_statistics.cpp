...
{
    if (!ConfigFlag("VKINSP_SHADER_STATISTICS"))
        return;
    ...
    if (!already)
        setup.extensionNames.push_back(VK_KHR_PIPELINE_EXECUTABLE_PROPERTIES_EXTENSION_NAME);
...
{
    if (!ShaderStatisticsEnabled(dev) || !infos || !count)
        return infos;
...
{
    if (!ShaderStatisticsEnabled(dev) || !pipeline)
        return;
    ...
    if (d.GetPipelineExecutablePropertiesKHR(dev->device, &pi, &executables, nullptr) != VK_SUCCESS || !executables)
        return;
    ...
    if (d.GetPipelineExecutablePropertiesKHR(dev->device, &pi, &executables, props.data()) < VK_SUCCESS)
        return;
    ...
    for (uint32_t e = 0; e < executables; ++e)
    {
        ...
        if (d.GetPipelineExecutableStatisticsKHR(dev->device, &ei, &count, stats.data()) < VK_SUCCESS)
            continue;

        any = true;
        w.BeginObject();
        w.Key("name");
        w.String(props[e].name);
        w.Key("description");
        w.String(props[e].description);
        w.Key("stages");
        StageNames(w, props[e].stages);
        w.Key("subgroupSize");
        w.Uint(props[e].subgroupSize);
        w.Key("statistics");
        w.BeginArray();
        for (uint32_t i = 0; i < count; ++i)
        {
            w.BeginObject();
            w.Key("name");
            w.String(stats[i].name);
            w.Key("description");
            w.String(stats[i].description);
            w.Key("value");
            StatisticValue(w, stats[i]);
            w.EndObject();
        }
        w.EndArray();
        w.EndObject();
    ...
    w.EndObject();
    if (!any)
        return;   // the driver offered executables but no statistics for any of them
