...
{
    if (!dev || !dev->inheritedQueries || !info)
        return info;
    {
        std::shared_lock lock(dev->secondariesMutex);
        if (!dev->secondaries.count(cb))
            return info;
    }
    ...
    if (dev->pipelineStatistics)
        t_inheritance.pipelineStatistics |= kPipelineStatistics;
...
{
    DeviceData* dev = GetDeviceData(device);
    if (!dev || !dev->inheritedQueries || !pAllocateInfo || !pCommandBuffers)
        return;
    ...
    for (uint32_t i = 0; i < pAllocateInfo->commandBufferCount; ++i)
    {
        if (!pCommandBuffers[i])
            continue;
        if (secondary)
            dev->secondaries.insert(pCommandBuffers[i]);
        else
            dev->secondaries.erase(pCommandBuffers[i]);
    }
}
...
{
    DeviceData* dev = GetDeviceData(device);
    if (!dev || !(dev->pipelineStatistics || dev->occlusionPrecise) || !pCreateInfo || !pQueryPool || !*pQueryPool)
        return;
    ...
    const VkShaderEXT* pShaders)
{
    ...
    JsonWriter replaced(&Tracker::Get());
    replaced.BeginArray();
    for (VkShaderEXT original : originals)
        replaced.Handle(HT_VkShaderEXT, "VkShaderEXT", (uint64_t)(uintptr_t)original);
    replaced.EndArray();
    rec->SetExtraOnLast(",\"replaced\":" + replaced.str());
}
...
static void BeforePass(VkCommandBuffer commandBuffer, const PassShape& shape)
{
    ...
    if (CommandRecorder* rec = dev->RecorderFor(commandBuffer))
        CaptureManager::Get().OnBeforePass(dev, rec, shape);
}
...
static const VkRenderPassBeginInfo* StoreAllBegin(VkCommandBuffer cb, const VkRenderPassBeginInfo* info)
{
    ...
    if (!info || !RecordingCapture(cb))
        return info;
    RenderPassInfo rp;
    if (!ResourceRegistry::Get().GetRenderPass(info->renderPass, rp) || !rp.storeAll)
        return info;
    t_beginCopy = *info;
...
static const VkRenderingInfo* StoreAllRendering(VkCommandBuffer cb, const VkRenderingInfo* info)
{
    ...
    for (uint32_t i = 0; i < info->colorAttachmentCount; ++i)
        needed |= info->pColorAttachments[i].storeOp == VK_ATTACHMENT_STORE_OP_DONT_CARE;
    if (info->pDepthAttachment)
        needed |= info->pDepthAttachment->storeOp == VK_ATTACHMENT_STORE_OP_DONT_CARE;
    if (info->pStencilAttachment)
        needed |= info->pStencilAttachment->storeOp == VK_ATTACHMENT_STORE_OP_DONT_CARE;
    if (!needed)
        return info;
    t_renderingCopy = *info;
    t_renderingColors.assign(info->pColorAttachments, info->pColorAttachments + info->colorAttachmentCount);
    for (auto& a : t_renderingColors)
        if (a.storeOp == VK_ATTACHMENT_STORE_OP_DONT_CARE)
            a.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
    t_renderingCopy.pColorAttachments = t_renderingColors.data();
    if (info->pDepthAttachment)
    {
        t_renderingDepth = *info->pDepthAttachment;
        if (t_renderingDepth.storeOp == VK_ATTACHMENT_STORE_OP_DONT_CARE)
            t_renderingDepth.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
        t_renderingCopy.pDepthAttachment = &t_renderingDepth;
    }
    if (info->pStencilAttachment)
    {
        t_renderingStencil = *info->pStencilAttachment;
        if (t_renderingStencil.storeOp == VK_ATTACHMENT_STORE_OP_DONT_CARE)
            t_renderingStencil.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
...
static void AttachPendingData(VkCommandBuffer cb)
{
    ...
    if (!rec || (rec->pendingImageData.empty() && rec->pendingBufferData.empty()))
        return;
    ...
    std::string extra;
    if (!rec->pendingImageData.empty())
        extra += list("imageData", rec->pendingImageData);
    if (!rec->pendingBufferData.empty())
        extra += list("bufferData", rec->pendingBufferData);
...
static void SnapshotRenderingLoads(VkCommandBuffer cb, const VkRenderingInfo* info)
{
    ...
    if (!rec || !info || (info->flags & VK_RENDERING_RESUMING_BIT))
        return;
    CaptureManager::Get().OnEndComputePass(dev, rec);
    ...
    for (uint32_t i = 0; i < info->colorAttachmentCount; ++i)
        begin(&info->pColorAttachments[i], VK_IMAGE_ASPECT_COLOR_BIT);
...
{
    ImageInfo img;
    if (!ResourceRegistry::Get().GetImage(image, img) || mip >= img.mipLevels)
        return false;
    ...
    void (*area)(const Region&, VkOffset3D&, VkExtent3D&))
{
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        if (CoversMip(dst, d.mipLevel, offset, extent))
            cm.NoteImageWrite(dst, d.aspectMask, d.mipLevel, 1, d.baseArrayLayer, d.layerCount);
    }
}
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        if (known)
        {
            ...
            const uint32_t layers = r.imageSubresource.layerCount == VK_REMAINING_ARRAY_LAYERS
                ? img.arrayLayers - std::min(img.arrayLayers, r.imageSubresource.baseArrayLayer)
        ...
        if (CoversMip(dst, d.mipLevel, r.imageOffset, r.imageExtent))
            cm.NoteImageWrite(dst, d.aspectMask, d.mipLevel, 1, d.baseArrayLayer, d.layerCount);
    }
}
...
{
    if (const VkCopyImageToBufferInfo2* i = pCopyImageToBufferInfo)
        BeforeImageToBuffer(commandBuffer, i->srcImage, i->srcImageLayout, i->regionCount, i->pRegions);
}
...
{
    if (const VkCopyBufferInfo2* i = pCopyBufferInfo)
        BeforeBufferCopy(commandBuffer, i->srcBuffer, i->regionCount, i->pRegions);
}
...
{
    if (const VkCopyBufferToImageInfo2* i = pCopyBufferToImageInfo)
        BeforeBufferToImage(commandBuffer, i->srcBuffer, i->dstImage, i->regionCount, i->pRegions);
}
...
static void BeforeDispatch(VkCommandBuffer commandBuffer)
{
    DeviceData* dev = GetDeviceData(commandBuffer);
    if (CommandRecorder* rec = dev->RecorderFor(commandBuffer))
        CaptureManager::Get().OnBeforeDispatch(dev, rec);
}
...
{
    DeviceData* dev = GetDeviceData(commandBuffer);
    if (!dev || (dev->appOcclusionQueries.load(std::memory_order_relaxed) && dev->appStatisticsQueries.load(std::memory_order_relaxed)))
        return;
    VkQueryType type;
    {
        std::shared_lock lock(dev->queryPoolsMutex);
        auto it = dev->appQueryPools.find(pool);
        if (it == dev->appQueryPools.end())
            return;
        type = it->second;
    }
...
static void SubmitEnd(VkQueue queue)
{
    ...
    if (DeviceData* dev = GetDeviceData(queue))
        dev->submitNanos.fetch_add((uint64_t)ns, std::memory_order_relaxed);
}
...
static const VkTimelineSemaphoreSubmitInfo* TimelineOf(const VkSubmitInfo& s, bool& other)
{
    ...
    for (auto* p = static_cast<const VkBaseInStructure*>(s.pNext); p; p = p->pNext)
    {
        if (p->sType == VK_STRUCTURE_TYPE_TIMELINE_SEMAPHORE_SUBMIT_INFO)
            timeline = reinterpret_cast<const VkTimelineSemaphoreSubmitInfo*>(p);
        else
            other = true;
    }
    return timeline;
...
static void BuildPart(uint32_t count, const VkSubmitInfo* submits, size_t first, size_t end, size_t total, SubmitPart& out)
{
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        base = hi;
        if (!PartRange(lo, hi, first, end, total, from, to, head, tail))
            continue;
        VkSubmitInfo part = s;
        part.pNext = nullptr;
        ...
        if (!head)
        {
            part.waitSemaphoreCount = 0;
            part.pWaitSemaphores = nullptr;
            part.pWaitDstStageMask = nullptr;
        }
        if (!tail)
        {
            part.signalSemaphoreCount = 0;
            part.pSignalSemaphores = nullptr;
        }
        if (const VkTimelineSemaphoreSubmitInfo* t = TimelineOf(s, other))
        {
            ...
            if (!head)
            {
                tp.waitSemaphoreValueCount = 0;
                tp.pWaitSemaphoreValues = nullptr;
            }
            if (!tail)
            {
                tp.signalSemaphoreValueCount = 0;
                tp.pSignalSemaphoreValues = nullptr;
            }
...
static void BuildPart(uint32_t count, const VkSubmitInfo2* submits, size_t first, size_t end, size_t total, SubmitPart& out)
{
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        if (!head)
        {
            part.waitSemaphoreInfoCount = 0;
            part.pWaitSemaphoreInfos = nullptr;
        }
        if (!tail)
        {
            part.signalSemaphoreInfoCount = 0;
            part.pSignalSemaphoreInfos = nullptr;
        }
...
static std::vector<size_t> SplitPoints(DeviceData* dev, const std::vector<VkCommandBuffer>& cbs, bool splittable)
{
    ...
    if (!dev || !splittable || cbs.size() < 2 || !CaptureManager::Get().IsCapturing())
        return cuts;
    for (size_t k = 0; k + 1 < cbs.size(); ++k)
        if (CaptureManager::Get().NeedsSubmitReadBack(dev, cbs[k]))
            cuts.push_back(k);
    return cuts;
}
...
{
    t_split = SplitSubmit{};
    if (!pSubmits || submitCount == 0 || !CaptureManager::Get().IsCapturing())
        return SubmitBegin();
...
void Hook_vkCreateBuffer(VkDevice device, const VkBufferCreateInfo* pCreateInfo,
    const VkAllocationCallbacks* pAllocator, VkBuffer* pBuffer)
{
    if (!pCreateInfo || !pBuffer || !*pBuffer)
        return;
    BufferInfo info;
    info.device = device;
...
{
    if (!pCreateInfo || !pFramebuffer || !*pFramebuffer)
        return;
...
static VkRenderPass StoreAllRenderPass(VkDevice device, const VkRenderPassCreateInfo* ci)
{
    ...
    for (auto& a : atts)
        needed |= StoreAll(a);
    DeviceData* dev = GetDeviceData(device);
    if (!needed || !dev || !dev->dispatch.CreateRenderPass)
...
{
    RenderPassInfo info;
    if (!renderPass || !ResourceRegistry::Get().GetRenderPass(renderPass, info) || !info.storeAll)
        return;
    DeviceData* dev = GetDeviceData(device);
    if (dev && dev->dispatch.DestroyRenderPass)
        dev->dispatch.DestroyRenderPass(device, info.storeAll, nullptr);
...
{
    if (!pCreateInfo || !pRenderPass || !*pRenderPass)
        return;
    RenderPassInfo info;
    ...
    for (uint32_t s = 0; s < pCreateInfo->subpassCount; ++s)
    {
        ...
        for (uint32_t i = 0; i < sp.colorAttachmentCount; ++i)
            color.push_back(sp.pColorAttachments[i].attachment);
    ...
    {
        if (n->sType != VK_STRUCTURE_TYPE_RENDER_PASS_MULTIVIEW_CREATE_INFO)
            continue;
        auto* mv = reinterpret_cast<const VkRenderPassMultiviewCreateInfo*>(n);
        for (uint32_t s = 0; s < mv->subpassCount; ++s)
            info.viewLayers = std::max(info.viewLayers, ViewMaskLayers(mv->pViewMasks[s]));
    }
    ...
    const VkAllocationCallbacks* pAllocator, VkSwapchainKHR* pSwapchain)
{
    ...
    if (info.refreshMs > 0)
        Log("swapchain refresh period %.3f ms (%s)", info.refreshMs, RefreshSourceName(source));
    ...
    VkImage* pSwapchainImages)
{
    ...
    for (uint32_t i = 0; i < *pSwapchainImageCount; ++i)
        if (pSwapchainImages[i])
            ResourceRegistry::Get().AddImage(pSwapchainImages[i], info);
}
...
static std::string SpirvStages(const uint32_t* words, size_t count)
{
    ...
    for (size_t i = 5; i < count;)
    {
        ...
        if (!len || i + len > count)
            break;
        if (op == 54)
            break;   // OpFunction: declarations are over
        if (op == 15 && len >= 3)
        {
            ...
            switch (words[i + 1])
            {
                ...
                case 5267:
                case 5364: name = "task"; break;
                case 5268:
                case 5365: name = "mesh"; break;
...
{
    if (!pCreateInfo || !pCreateInfo->pCode || !pShaderModule || !*pShaderModule)
        return;
...
void Hook_vkEnumeratePhysicalDevices(VkInstance instance, uint32_t* pPhysicalDeviceCount, VkPhysicalDevice* pPhysicalDevices)
{
    ...
    for (uint32_t i = 0; i < *pPhysicalDeviceCount; ++i)
    {
        ...
        if (familyCount)
            d.GetPhysicalDeviceQueueFamilyProperties(pd, &familyCount, families.data());
        w.Key("queueFamilies");
        w.BeginArray();
    ...
    const VkAllocationCallbacks* pAllocator, VkPipeline* pPipelines)
{
    ...
    for (uint32_t i = 0; i < createInfoCount; ++i)
    {
        ...
        for (uint32_t s = 0; s < ci.stageCount && ci.pStages; ++s)
            AttachStage(pPipelines[i], ci.pStages[s]);
...
{
    (void)device;
    if (!pipeline || !pData || !dataSize || !groupCount)
        return;
    ...
    w.Key("id");
    w.Uint(id);
    w.Key("shaderGroupHandles");
    w.BeginObject();
    w.Key("count");
    w.Uint(groupCount);
    w.Key("handleSize");
    w.Uint(dataSize / groupCount);
    w.EndObject();
    w.EndObject();
    ...
    uint32_t width, uint32_t height, uint32_t depth)
{
    ...
    DeviceData* dev = GetDeviceData(commandBuffer);
    CommandRecorder* rec = dev ? dev->RecorderFor(commandBuffer) : nullptr;
    if (!rec || !CaptureManager::Get().IsCapturing())
        return;
    const struct
    {
        const char* name;
    ...
    for (const auto& r : regions)
    {
        ...
        if (!ResourceRegistry::Get().ResolveAddress(r.region->deviceAddress, buffer, offset, remaining))
            continue;
...
void Hook_vkGetBufferDeviceAddress(VkDevice device, const VkBufferDeviceAddressInfo* pInfo, VkDeviceAddress result)
{
    (void)device;
    if (pInfo)
        ResourceRegistry::Get().NoteBufferAddress(pInfo->buffer, result);
}
    ...
    DeviceData* dev, CommandRecorder* rec)
{
    ...
    if (address && ResourceRegistry::Get().ResolveAddress(address, buffer, offset, remaining))
    {
        ...
        out.offset = offset;
        w.Key("buffer");
        w.Handle(HT_VkBuffer, "VkBuffer", (uint64_t)(uintptr_t)buffer);
        w.Key("offset");
        w.Uint(offset);
        ...
        if (rec && size)
            out.capture = CaptureManager::Get().QueueBufferCapture(dev, rec, buffer, offset, size, true);
        if (out.capture)
        {
            w.Key("capture");
            w.Uint(out.capture);
    ...
    VkCommandBuffer commandBuffer)
{
    ...
    for (uint32_t i = 0; i < infoCount; ++i)
    {
        ...
        for (uint32_t g = 0; ranges && ranges[i] && g < infos[i].geometryCount; ++g)
        {
            ...
            buildRanges += (g ? ",{" : "{") + std::string("\"primitiveCount\":") + std::to_string(r.primitiveCount) + ",\"primitiveOffset\":" + std::to_string(r.primitiveOffset) + ",\"firstVertex\":" + std::to_string(r.firstVertex) + ",\"transformOffset\":" + std::to_string(r.transformOffset) + "}";
        }
        buildRanges += "]";
    }
    for (uint32_t i = 0; i < infoCount; ++i)
    {
        ...
        w.String(method);
        w.Key("type");
        w.Enum(ToString_VkAccelerationStructureTypeKHR(info.type), (int64_t)info.type);
        w.Key("mode");
        w.Enum(ToString_VkBuildAccelerationStructureModeKHR(info.mode), (int64_t)info.mode);
        w.Key("flags");
        Flags_VkBuildAccelerationStructureFlagsKHR(w, info.flags);
        ...
        auto note = [&](const char* what, const ResolvedAddress& r) {
            if (!r.buffer)
                return;
            if (r.size)
                inputs.push_back({what, g, r.address, r.size});
            captures += captured++ ? "," : "";
            captures += "{\"info\":" + std::to_string(i) + ",\"geometry\":" + std::to_string(g) + ",\"field\":\"" + what + "\",\"buffer\":" + std::to_string(r.buffer) + ",\"offset\":" + std::to_string(r.offset);
            if (r.capture)
                captures += ",\"capture\":" + std::to_string(r.capture);
            captures += "}";
        };
        for (; g < info.geometryCount; ++g)
        {
            ...
            const uint32_t count = ranges && ranges[i]        ? ranges[i][g].primitiveCount
                : maxPrimitiveCounts && maxPrimitiveCounts[i] ? maxPrimitiveCounts[i][g]
                                                              : 0;
            primitives += count;
            counts[g] = count;
            w.BeginObject();
            w.Key("geometryType");
            w.Enum(ToString_VkGeometryTypeKHR(geometry->geometryType), (int64_t)geometry->geometryType);
            w.Key("flags");
            Flags_VkGeometryFlagsKHR(w, geometry->flags);
            ...
            if (geometry->geometryType == VK_GEOMETRY_TYPE_TRIANGLES_KHR)
            {
                ...
                w.Key("vertexFormat");
                w.Enum(ToString_VkFormat(tri.vertexFormat), (int64_t)tri.vertexFormat);
                w.Key("vertexStride");
                w.Uint(tri.vertexStride);
                w.Key("maxVertex");
                w.Uint(tri.maxVertex);
                w.Key("indexType");
                w.Enum(ToString_VkIndexType(tri.indexType), (int64_t)tri.indexType);
                // maxVertex is the highest index the build may read, so the array holds one more.
                note("vertexData", WriteBuildAddress(w, "vertexData", tri.vertexData.deviceAddress, (VkDeviceSize)(tri.maxVertex + 1) * tri.vertexStride, dev, rec));
                if (tri.indexType != VK_INDEX_TYPE_NONE_KHR)
                {
                    const VkDeviceSize indexSize = tri.indexType == VK_INDEX_TYPE_UINT16 ? 2 : 4;
                    note("indexData", WriteBuildAddress(w, "indexData", tri.indexData.deviceAddress, (VkDeviceSize)count * 3 * indexSize, dev, rec));
                }
                if (tri.transformData.deviceAddress)
                {
                    note("transformData", WriteBuildAddress(w, "transformData", tri.transformData.deviceAddress, sizeof(VkTransformMatrixKHR), dev, rec));
                }
            }
            ...
            else if (geometry->geometryType == VK_GEOMETRY_TYPE_INSTANCES_KHR)
            {
                ...
                w.Key("arrayOfPointers");
                w.Boolean(instances.arrayOfPointers == VK_TRUE);
        ...
        if (dev && dev->dispatch.GetAccelerationStructureBuildSizesKHR)
        {
            ...
            {
                w.Key("resultSize");
                w.Uint(sizes.accelerationStructureSize);
                w.Key("scratchSize");
                w.Uint(info.mode == VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR ? sizes.updateScratchSize : sizes.buildScratchSize);
            }
        }
        w.EndObject();
        w.EndObject();
...
{
    if (!pCreateInfos || !pShaders)
        return;
    for (uint32_t i = 0; i < createInfoCount; ++i)
    {
        const VkShaderCreateInfoEXT& ci = pCreateInfos[i];
        if (!pShaders[i] || ci.codeType != VK_SHADER_CODE_TYPE_SPIRV_EXT || !ci.pCode || !ci.codeSize)
            continue;
...
static void SendBinding(HandleType type, uint64_t handle, VkDeviceMemory memory, VkDeviceSize offset)
{
    ...
    w.Uint(id);
    w.Key("memory");
    w.Handle(HT_VkDeviceMemory, "VkDeviceMemory", (uint64_t)(uintptr_t)memory);
    w.Key("memoryOffset");
    w.Uint(offset);
    w.EndObject();
...
static void NotePassFinalLayouts(VkCommandBuffer commandBuffer, const VkRenderPassBeginInfo* info)
{
    ...
    FramebufferInfo fb;
    RenderPassInfo rp;
    if (!reg.GetFramebuffer(info->framebuffer, fb) || !reg.GetRenderPass(info->renderPass, rp))
        return;
    const std::vector<VkImageView> views = PassAttachmentViews(info, fb);
    for (size_t i = 0; i < views.size() && i < rp.attachments.size(); ++i)
    {
...
void Hook_vkQueueSubmit(VkQueue queue, uint32_t submitCount, const VkSubmitInfo* pSubmits, VkFence fence)
{
    ...
    if (!CaptureManager::Get().IsCapturing())
        return;
    JsonWriter w(&Tracker::Get());
    ...
    VkSubpassContents contents)
{
    ...
    if (CommandRecorder* rec = dev->RecorderFor(commandBuffer))
        CaptureManager::Get().OnBeginRenderPass(dev, rec, pRenderPassBegin);
}
...
void Hook_vkCmdBeginRendering(VkCommandBuffer commandBuffer, const VkRenderingInfo* pRenderingInfo)
{
    ...
    if (CommandRecorder* rec = dev->RecorderFor(commandBuffer))
        CaptureManager::Get().OnBeginRendering(dev, rec, pRenderingInfo);
}
...
static void EndPass(VkCommandBuffer commandBuffer)
{
    DeviceData* dev = GetDeviceData(commandBuffer);
    if (CommandRecorder* rec = dev->RecorderFor(commandBuffer))
        CaptureManager::Get().OnEndPass(dev, rec);
}
...
{
    if (pSetLayout && *pSetLayout)
        DescriptorTracker::Get().OnCreateLayout(*pSetLayout, pCreateInfo);
}
...
static VkPipelineBindPoint BindPointFromStages(VkShaderStageFlags stages)
{
    ...
    if (stages & (VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_ANY_HIT_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR | VK_SHADER_STAGE_INTERSECTION_BIT_KHR | VK_SHADER_STAGE_CALLABLE_BIT_KHR))
    ...
    const uint32_t* dynamicOffsets)
{
    ...
    w.BeginObject();
    w.Key("bindPoint");
    w.Enum(ToString_VkPipelineBindPoint(bindPoint), (int64_t)bindPoint);
    w.Key("sets");
    w.BeginArray();
    ...
    for (uint32_t i = 0; i < setCount; ++i)
    {
        ...
        if (!DescriptorTracker::Get().GetSet(sets[i], c))
        {
            ...
            w.Uint(firstSet + i);
            w.Key("descriptorSet");
            w.Handle(HT_VkDescriptorSet, "VkDescriptorSet", (uint64_t)(uintptr_t)sets[i]);
            w.Key("bindings");
            w.BeginArray();
            w.EndArray();
            w.EndObject();
...
{
    DeviceData* dev = GetDeviceData(commandBuffer);
    if (!dev->RecorderFor(commandBuffer))
        return;
...
static void SnapshotPushedTemplate(VkCommandBuffer commandBuffer, VkDescriptorUpdateTemplate tmpl, uint32_t set, const void* data)
{
    ...
    if (DescriptorTracker::Get().FromTemplate(tmpl, data, c, bindPoint))
        SnapshotPushedContents(commandBuffer, bindPoint, set, c);
}
...
{
    if (pPipelineLayout)
        DescriptorTracker::Get().OnCreatePipelineLayout(*pPipelineLayout, pCreateInfo);
}
...
{
    if (ppData && *ppData)
        ResourceRegistry::Get().NoteMemoryMapped(memory, *ppData, offset, size);
}
...
void Hook_vkCmdPushDescriptorSetWithTemplate2(VkCommandBuffer commandBuffer, const VkPushDescriptorSetWithTemplateInfo* pPushDescriptorSetWithTemplateInfo)
{
    ...
    if (i)
        SnapshotPushedTemplate(commandBuffer, i->descriptorUpdateTemplate, i->set, i->pData);
}
    ...
    const VkDeviceSize* offsets, const VkDeviceSize* sizes, VkDeviceSize fixedSize)
{
    DeviceData* dev = GetDeviceData(commandBuffer);
    CommandRecorder* rec = dev->RecorderFor(commandBuffer);
    if (!rec || !buffers || !CaptureManager::Get().IsCapturing())
        return;
...
{
    if (GetDeviceData(commandBuffer)->RecorderFor(commandBuffer))
        NoteClears(image, rangeCount, pRanges);
}
