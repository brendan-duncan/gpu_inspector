...
{
    if (handleType < 0 || handleType >= HT_Count)
        return 0;
...
std::string Tracker::AddObjectMessage(const TrackedObject& o) const
{
    ...
    w.Uint(o.parentId);
    w.Key("type");
    w.String(kHandleTypeNames[o.type]);
    w.Key("cmd");
    w.String(kVkCommandNames[(int)o.cmd]);
    w.Key("index");
    w.Uint(o.index);
    w.Key("handle");
    w.Pointer((const void*)(uintptr_t)o.handle);
    w.Key("label");
    if (o.label.empty())
        w.Null();
    else
    ...
    VkCmdId cmd, uint32_t index, const std::string& args)
{
    ...
    if (existing != m.end())
    {
        ...
        if (it == _byId.end() || !parentId || it->second.parentId == parentId)
...
void Tracker::SendLeakReport(HandleType type, uint64_t handle)
{
    ...
    while (!stack.empty())
    {
        ...
        for (uint64_t c : o.children)
            stack.push_back(c);
        if (o.type == HT_VkQueue || o.type == HT_VkPhysicalDevice || o.type == HT_VkDescriptorSet || o.type == HT_VkCommandBuffer)
            continue;
        if (o.type == HT_VkImage && o.cmd == VkCmdId::GetSwapchainImagesKHR)
            continue;
        leaked.push_back(&o);
    }
    if (leaked.empty())
    ...
    for (auto* o : leaked)
        byType[kHandleTypeNames[o->type]]++;
    ...
    w.Uint(owner.id);
    w.Key("ownerClass");
    w.String(kHandleTypeNames[owner.type]);
    w.Key("count");
    w.Uint(leaked.size());
    w.Key("byType");
    w.BeginObject();
    for (auto& [name, n] : byType)
    {
        w.Key(name.c_str());
        w.Uint(n);
    }
...
void Tracker::Update(uint64_t id, const std::string& key, const std::string& messageJson)
{
    ...
    if (!replaced)
        it->second.updates.emplace_back(key, messageJson);
    if (_live)
        Transport::Get().SendJson(messageJson);
}
...
std::shared_ptr<std::vector<uint8_t>> Tracker::GetBlob(uint64_t id, uint32_t index)
{
    std::shared_lock lock(_mutex);
    auto it = _byId.find(id);
    if (it == _byId.end() || index >= it->second.blobs.size())
        return nullptr;
...
void Tracker::SendSnapshot()
{
    ...
    {
        Transport::Get().SendJson(AddObjectMessage(*o));
        for (auto& [k, v] : o->updates)
            Transport::Get().SendJson(v);
    }
    _live = true;
