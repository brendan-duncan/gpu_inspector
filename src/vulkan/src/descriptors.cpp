...
static DescriptorEntry* EntryAt(DescriptorSetContents& set, size_t& bindingIndex, uint32_t& arrayElement)
{
    while (bindingIndex < set.bindings.size())
    {
        ...
        if (arrayElement < b.entries.size())
            return &b.entries[arrayElement];
...
static size_t FindBinding(const DescriptorSetContents& set, uint32_t binding)
{
    for (size_t i = 0; i < set.bindings.size(); ++i)
        if (set.bindings[i].binding == binding)
            return i;
    ...
    const VkAccelerationStructureKHR* structures)
{
    ...
    for (uint32_t k = 0; k < count; ++k)
    {
        ...
        if (IsImageType(type) && images)
            base = reinterpret_cast<const uint8_t*>(images) + k * stride;
        else if (IsBufferType(type) && buffers)
            base = reinterpret_cast<const uint8_t*>(buffers) + k * stride;
        else if (IsTexelType(type) && views)
            base = reinterpret_cast<const uint8_t*>(views) + k * stride;
        else if (IsStructureType(type) && structures)
            base = reinterpret_cast<const uint8_t*>(structures) + k * stride;
        ...
        else if (base)
        {
            if (IsImageType(type))
            {
                ...
                if (!set.bindings[bi].immutableSamplers)
    ...
    const VkCopyDescriptorSet* copies)
{
    ...
    for (uint32_t i = 0; writes && i < writeCount; ++i)
    {
        ...
        size_t stride = IsImageType(w.descriptorType) ? sizeof(VkDescriptorImageInfo)
            ...
            : IsStructureType(w.descriptorType)       ? sizeof(VkAccelerationStructureKHR)
                                                      : sizeof(VkBufferView);
    ...
    for (uint32_t i = 0; copies && i < copyCount; ++i)
    {
        ...
        if (src == _sets.end() || dst == _sets.end())
            continue;
...
void DescriptorTracker::OnUpdateWithTemplate(VkDescriptorSet set, VkDescriptorUpdateTemplate tmpl, const void* data)
{
    ...
    if (tit == _templates.end() || sit == _sets.end())
        return;
...
void DescriptorTracker::OnCreatePipelineLayout(VkPipelineLayout layout, const VkPipelineLayoutCreateInfo* info)
{
    ...
    for (uint32_t i = 0; info->pSetLayouts && i < info->setLayoutCount; ++i)
        sets[i] = info->pSetLayouts[i];
    std::unique_lock lock(_mutex);
...
VkDescriptorSetLayout DescriptorTracker::SetLayoutOf(VkPipelineLayout layout, uint32_t set) const
{
    ...
    if (it == _pipelineLayouts.end() || set >= it->second.size())
...
DescriptorSetContents DescriptorTracker::FromWrites(uint32_t writeCount, const VkWriteDescriptorSet* writes)
{
    ...
    for (uint32_t i = 0; writes && i < writeCount; ++i)
    {
        ...
        for (uint32_t k = 0; k < count; ++k)
        {
            ...
            {
                if (const VkAccelerationStructureKHR* s = StructuresOf(w))
                    e.accelerationStructure = s[k];
            }
        }
    ...
    VkPipelineBindPoint& bindPoint) const
{
    ...
    for (const VkDescriptorUpdateTemplateEntry& t : it->second.entries)
    {
        ...
        if (b->entries.size() < t.dstArrayElement + count)
            b->entries.resize(t.dstArrayElement + count);
        for (uint32_t k = 0; k < count; ++k)
        {
...
{
    if (e.range != VK_WHOLE_SIZE)
        return e.range;
    BufferInfo bi;
    if (!e.buffer || !ResourceRegistry::Get().GetBuffer(e.buffer, bi))
        return 0;
    ...
    const std::vector<std::vector<uint32_t>>* dataIds)
{
    ...
    w.Uint(setIndex);
    w.Key("descriptorSet");
    w.Handle(HT_VkDescriptorSet, "VkDescriptorSet", (uint64_t)(uintptr_t)set);
    w.Key("layout");
    ...
    const std::vector<std::vector<uint32_t>>* dataIds)
{
    ...
    for (size_t bi = 0; bi < contents.bindings.size(); ++bi)
    {
        ...
        w.Uint(b.binding);
        w.Key("type");
        w.Enum(ToString_VkDescriptorType(b.type), (int64_t)b.type);
        if (b.stages)
        {
            w.Key("stages");
            Flags_VkShaderStageFlags(w, b.stages);
        }
        w.Key("descriptors");
        w.BeginArray();
        for (size_t k = 0; k < b.entries.size(); ++k)
        {
            ...
            {
                w.Key("buffer");
                w.Handle(HT_VkBuffer, "VkBuffer", (uint64_t)(uintptr_t)e.buffer);
                w.Key("offset");
                w.Uint(e.offset);
                w.Key("range");
                w.Uint(DescriptorBufferRange(e));
                if (IsDynamicType(b.type))
                {
                    w.Key("dynamicOffset");
                    w.Uint(dynamicOffset);
                }
            ...
            else if (IsImageType(b.type))
            {
                ...
                {
                    w.Key("imageView");
                    w.Handle(HT_VkImageView, "VkImageView", (uint64_t)(uintptr_t)e.imageView);
                    w.Key("imageLayout");
                    w.Enum(ToString_VkImageLayout(e.imageLayout), (int64_t)e.imageLayout);
                ...
                {
                    w.Key("sampler");
                    w.Handle(HT_VkSampler, "VkSampler", (uint64_t)(uintptr_t)e.sampler);
            ...
            {
                w.Key("bufferView");
                w.Handle(HT_VkBufferView, "VkBufferView", (uint64_t)(uintptr_t)e.bufferView);
            }
