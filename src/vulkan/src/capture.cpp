...
void CaptureManager::Request(const CaptureOptions& options)
{
    ...
    if (options.atFrame == UINT64_MAX)
        ...
        Log("capture armed (%u frames) for frame %llu", _framesLeft, (unsigned long long)options.atFrame);
}
...
CaptureManager::DeviceCapture* CaptureManager::CaptureFor(DeviceData* dev)
{
    ...
    if (!slot)
    {
        ...
        if (_options.profilePasses)
            CreateQueryPools(*slot);
        if (dev->device != _homeDevice)
            Log("capture: device %p takes part as well (frame %llu)", (void*)dev->device, (unsigned long long)dev->frameIndex);
    }
    return slot.get();
}
...
{
    if (dev->device == _homeDevice)
        return (uint32_t)(dev->frameIndex - _frameIndex);
...
void CaptureManager::RetainCapturedCommands(DeviceData* dev)
{
    ...
    for (auto& [cb, rec] : dev->recorders)
    {
        ...
        if (!id)
            continue;   // freed already: the UI has no command of it to point at either
...
void CaptureManager::OnBeginCommandBuffer(DeviceData* dev, VkCommandBuffer cb, VkCommandBufferUsageFlags flags)
{
    ...
    if (!IsCapturing() && !RecordAlways())
        return;
    std::unique_lock lock(dev->recorderMutex);
    auto& slot = dev->recorders[cb];
    if (!slot)
        slot = std::make_unique<CommandRecorder>(dev->device, cb, &Tracker::Get());
...
bool CaptureManager::NeedsSubmitReadBack(DeviceData* dev, VkCommandBuffer cb)
{
    ...
    for (const RecordedPass& pass : rec->passes())
        if (!pass.readBack && !pass.attachments.empty())
            return true;
    return false;
}
    ...
    const std::vector<VkCommandBuffer>& readBack)
{
    ...
    {
        if (bc.frame != UINT32_MAX || !bc.recorded)
            continue;
        for (auto& scb : sub.commandBuffers)
        {
...
{
    if (info)
        _presentSeen.store(true, std::memory_order_relaxed);
    ...
    if (_state == State::Armed)
    {
        ...
        if (!info && _presentSeen.load(std::memory_order_relaxed))
            return;
        // frameIndex was advanced by this frame end: the frame that starts now is `frameIndex`.
        if (_options.atFrame == UINT64_MAX || dev->frameIndex >= _options.atFrame)
            Start(dev);
        return;
    }
...
{
    if (!IsCapturing() || !FindCapture(dev->device))
        return;
    if (dev->device == _homeDevice)
    {
    ...
    }
    {
        ...
        for (auto& tc : _textures)
        {
            if (tc.device != dev->device || tc.failed)
                continue;
            tc.failed = true;
        ...
        for (auto& bc : _buffers)
        {
            if (bc.device != dev->device || bc.failed)
                continue;
            bc.failed = true;
...
void CaptureManager::Finish(DeviceData* dev)
{
    ...
    if (uint32_t n = _storeAllPasses.exchange(0, std::memory_order_relaxed))
        Log("capture: %u render passes ran with their DONT_CARE store ops forced to STORE, so those attachments read back", n);
    ...
    std::vector<DeviceData*> devices;
    {
        ...
        for (auto& [device, dc] : _devices)
            devices.push_back(dc->dev);
    }
    for (DeviceData* d : devices)
        d->dispatch.DeviceWaitIdle(d->device);
    if (devices.size() > 1)
    ...
    int64_t slot)
{
    ...
    if (!extra.empty())
        w.str() += extra;  // extra is a pre-separated member list: ,"children":[...]
    w.EndObject();
}
...
void CaptureManager::SendCommands()
{
    ...
    for (auto& s : submissions)
    {
        ...
        for (auto& cb : s.commandBuffers)
            total += cb.commands ? cb.commands->size() : 1;
    }
...
void CaptureManager::ReleaseDevice(DeviceCapture& dc)
{
    ...
    if (dc.queryPool)
        d.DestroyQueryPool(dev->device, dc.queryPool, nullptr);
    if (dc.statsPool)
        d.DestroyQueryPool(dev->device, dc.statsPool, nullptr);
    if (dc.occlusionPool)
        d.DestroyQueryPool(dev->device, dc.occlusionPool, nullptr);
    dc.queryPool = dc.statsPool = dc.occlusionPool = VK_NULL_HANDLE;
    std::lock_guard lock(_mutex);
    for (auto& c : dc.staging)
    {
        if (c.mapped)
            d.UnmapMemory(dev->device, c.memory);
        d.DestroyBuffer(dev->device, c.buffer, nullptr);
        d.FreeMemory(dev->device, c.memory, nullptr);
    }
    dc.staging.clear();
    for (VkImageView v : dc.resolveViews)
...
{
    if (!IsCapturing() || !_options.profilePasses)
        return UINT32_MAX;
    if (rec->renderPassContinue())
        return UINT32_MAX;   // secondaries inside a pass: the primary times the pass
    DeviceCapture* dc = CaptureFor(dev);
    if (!dc || !dc->queryPool)
        return UINT32_MAX;
    uint32_t q = dc->queriesUsed.fetch_add(2, std::memory_order_relaxed);
    if (q + 2 > dc->queryCount)
        return UINT32_MAX;      // pool exhausted: later passes go untimed
...
uint32_t CaptureManager::BeginPipelineStatistics(DeviceData* dev, CommandRecorder* rec)
{
    ...
    if (!dc || !dc->statsPool)
        return UINT32_MAX;
    if (rec->renderPassContinue())
        return UINT32_MAX;   // the primary brackets the pass
    // Two pipeline statistics queries cannot be active at once, and the application's come first.
    if (dev->appStatisticsQueries.load(std::memory_order_relaxed))
        return UINT32_MAX;
    uint32_t q = dc->statsUsed.fetch_add(1, std::memory_order_relaxed);
    if (q >= dc->statsCount)
        return UINT32_MAX;         // pool exhausted: later passes go uncounted
    VkCommandBuffer cb = rec->commandBuffer();
...
void CaptureManager::OnBeginRendering(DeviceData* dev, CommandRecorder* rec, const VkRenderingInfo* info)
{
    ...
    for (uint32_t bit = 0; bit < 32; ++bit)
        if (info->viewMask & (1u << bit))
            p.layerCount = std::max(p.layerCount, bit + 1);   // multiview
    p.passIndex = rec->NextPassIndex();
    p.query = rec->pendingQuery;
    p.statsQuery = rec->pendingStatsQuery;
    p.occlusionQuery = rec->pendingOcclusionQuery;
    ...
    for (uint32_t i = 0; i < info->colorAttachmentCount; ++i)
        add(&info->pColorAttachments[i]);
    add(info->pDepthAttachment);
    if (info->pStencilAttachment && (!info->pDepthAttachment || info->pStencilAttachment->imageView != info->pDepthAttachment->imageView))
...
void CaptureManager::OnEndPass(DeviceData* dev, CommandRecorder* rec)
{
    ...
    if (p.query != UINT32_MAX && dc && dc->queryPool)
    {
        ...
        if (p.statsQuery != UINT32_MAX && dc->statsPool)
...
{
    {
        ...
        if (it != _imageCaptureByView.end())
            return it->second;
    }
    ResourceRegistry& reg = ResourceRegistry::Get();
    ImageViewInfo vi;
    ImageInfo img;
    if (!reg.GetImageView(view, vi) || !reg.GetImage(vi.image, img))
        return 0;
    ...
    uint32_t mips, VkImageLayout layout, std::vector<PendingImageCopy>* finish)
{
    ...
    if (!img.transferSrc)
        return fail("image lacks TRANSFER_SRC usage");
    ...
    if (layout == VK_IMAGE_LAYOUT_UNDEFINED || layout == VK_IMAGE_LAYOUT_PREINITIALIZED)
        return fail("unknown image layout");
    FormatBlock block = FormatBlockInfo(img.format, tc.aspect);
    if (block.bytes == 0)
        return fail("unsupported format for readback");
    if (tc.mip >= img.mipLevels)
        mips = 1;
    else if (tc.mip + mips > img.mipLevels)
        mips = img.mipLevels - tc.mip;
    ...
    if (overBudget)
        return fail("image capture budget exceeded");
    uint32_t chunkIndex = 0;
    VkDeviceSize offset = 0;
    VkBuffer staging = VK_NULL_HANDLE;
    if (!AllocateStaging(dev, tc.size, chunkIndex, offset, &staging))
...
{
    if (!rec || !image || !IsCapturing() || !_options.captureImages || rec->InsidePass())
        return;
    ImageInfo img;
    if (!ResourceRegistry::Get().GetImage(image, img) || img.samples != VK_SAMPLE_COUNT_1_BIT)
        return;
    if (baseMip >= img.mipLevels || baseLayer >= img.arrayLayers)
        return;
    if (mipCount == VK_REMAINING_MIP_LEVELS || baseMip + mipCount > img.mipLevels)
        mipCount = img.mipLevels - baseMip;
    if (layerCount == VK_REMAINING_ARRAY_LAYERS || baseLayer + layerCount > img.arrayLayers)
        layerCount = img.arrayLayers - baseLayer;
    ...
    {
        if (!(aspect & one) || !(FormatAspects(img.format) & one))
            continue;
    ...
    uint32_t layerCount)
{
    if (!image || !IsCapturing())
        return;
    ImageInfo img;
    if (!ResourceRegistry::Get().GetImage(image, img) || baseMip >= img.mipLevels || baseLayer >= img.arrayLayers)
        return;
    if (mipCount == VK_REMAINING_MIP_LEVELS || baseMip + mipCount > img.mipLevels)
        mipCount = img.mipLevels - baseMip;
    if (layerCount == VK_REMAINING_ARRAY_LAYERS || baseLayer + layerCount > img.arrayLayers)
...
{
    if (!rec || !buffer || !IsCapturing() || !_options.captureBuffers)
        return 0;
...
{
    BufferInfo bi;
    if (!ResourceRegistry::Get().GetBuffer(buffer, bi))
        return 0;
    if (offset >= bi.size)
        return 0;
    VkDeviceSize avail = bi.size - offset;
    if (size == VK_WHOLE_SIZE || size > avail)
        size = avail;
    if (size == 0)
        return 0;
    ...
    if (reuse)
    {
        ...
        {
            if (p.buffer == buffer && p.offset == offset && p.size >= bc.size)
                return p.captureId;
        }
    }
    auto fail = [&](const char* why) {
    ...
    if (!bi.transferSrc)
        return fail("buffer lacks TRANSFER_SRC usage");

    uint32_t chunkIndex = 0;
    VkDeviceSize stagingOffset = 0;
    VkBuffer staging = VK_NULL_HANDLE;
    if (!AllocateStaging(dev, bc.size, chunkIndex, stagingOffset, &staging))
        return fail("staging allocation failed");
...
void CaptureManager::SendBuffers(DeviceData* dev)
{
    ...
    for (auto& bc : buffers)
    {
        ...
        w.Uint(bc.frame);
        w.Key("commandBuffer");
        w.Uint(bc.commandBufferId);
        w.Key("offset");
        w.Uint(bc.offset);
        w.Key("size");
        w.Uint(bc.failed ? 0 : bc.size);
        if (bc.originalSize)
        {
            w.Key("originalSize");
            w.Uint(bc.originalSize);
        }
        if (bc.failed)
        {
            w.Key("error");
...
void CaptureManager::MapStaging()
{
    ...
    for (DeviceCapture* dc : captures)
    {
        ...
        {
            if (!c.mapped)
                d.MapMemory(dc->dev->device, c.memory, 0, VK_WHOLE_SIZE, 0, &c.mapped);
    ...
    VkBuffer* bufferOut)
{
    ...
    if (dev->dispatch.CreateBuffer(dev->device, &bci, nullptr, &chunk.buffer) != VK_SUCCESS)
        return false;
    ...
    VkDeviceMemory* memory)
{
    ...
    *memory = VK_NULL_HANDLE;
    ...
    if (FormatAspects(img.format) & VK_IMAGE_ASPECT_DEPTH_BIT)
        ici.usage |= VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
    ici.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    ici.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
    if (d.CreateImage(dev->device, &ici, nullptr, image) != VK_SUCCESS)
        return false;
    VkMemoryRequirements req;
...
bool CaptureManager::AllocateResolveImage(DeviceData* dev, const ImageInfo& img, uint32_t mip, uint32_t layers, VkImage* out)
{
    ...
    if (!dc)
        return false;
    ResolveImage ri;
    if (!CreateResolveImage(dev, img, mip, layers, &ri.image, &ri.memory))
        return false;
    std::lock_guard lock(_mutex);
...
{
    DeviceCapture* dc = CaptureFor(dev);
    if (!dc || !CanResolveDepth(dev))
        return false;
    if (!CreateDepthResolveViews(dev, p, &p.srcView, &p.dstView))
        return false;
    std::lock_guard lock(_mutex);
...
std::vector<VkImageAspectFlagBits> CaptureManager::ReadBackAspects(VkImageView view)
{
    ...
    if (aspects & VK_IMAGE_ASPECT_DEPTH_BIT)
        out.push_back(VK_IMAGE_ASPECT_DEPTH_BIT);
    if (aspects & VK_IMAGE_ASPECT_STENCIL_BIT)
        out.push_back(VK_IMAGE_ASPECT_STENCIL_BIT);
    if (out.empty())
        out.push_back(VK_IMAGE_ASPECT_COLOR_BIT);
    return out;
}
    ...
    VkImageView view, VkImageLayout layout, bool resolveTarget)
{
    ...
    for (VkImageAspectFlagBits aspect : ReadBackAspects(view))
    {
        TextureCapture tc;
        PendingImageCopy p;
        if (!PrepareAttachment(dev, cbId, rec->pass().passIndex, rec->pass().layerCount, attachmentIndex, view, layout, resolveTarget, aspect, tc, p))
            continue;
    ...
    VkImageAspectFlagBits aspect, TextureCapture& tc, PendingImageCopy& p)
{
    ...
    if (layout == VK_IMAGE_LAYOUT_UNDEFINED)
        return fail("unknown final layout");
    ...
    p.resolve = resolve;
    p.format = img.format;
    if (resolve && depthStencil && !PrepareDepthResolve(dev, p))
        return fail("depth resolve views could not be created");
    return true;
}
...
void CaptureManager::ReadBackAfterSubmit(DeviceData* dev, VkQueue queue, CommandRecorder* rec, uint64_t commandBufferId, uint32_t frame)
{
    ...
    for (const RecordedPass& pass : rec->passes())
    {
        ...
        auto prepare = [&](uint32_t i, VkImageView view, VkImageLayout recordedLayout, bool resolveTarget) {
            ...
            if (ResourceRegistry::Get().GetImageView(view, vi) && LayoutTracker::Get().GetLayout(vi.image, tracked) && tracked != VK_IMAGE_LAYOUT_UNDEFINED)
                layout = tracked;
            for (VkImageAspectFlagBits aspect : ReadBackAspects(view))
            {
                TextureCapture tc;
        ...
        for (uint32_t i = 0; i < pass.attachments.size(); ++i)
        {
            ...
            if (i < pass.resolveViews.size() && pass.resolveViews[i])
                prepare(i, pass.resolveViews[i], pass.resolveLayouts[i], true);
        }
    }
    if (copies.empty())
        return;
    ...
    const VkResult res = SubmitReadBack(dev, queue, [&](VkCommandBuffer cb) {
        for (auto& c : copies)
            RecordImageCopy(dev, cb, c.second);
    });
    std::lock_guard lock(_mutex);
    for (auto& c : copies)
    {
        ...
        {
            c.first.failed = true;
            c.first.note = "read-back after submission failed";
        }
        _textures.push_back(c.first);
    }
    if (res == VK_SUCCESS)
        _postSubmitReadbacks.fetch_add((uint32_t)copies.size(), std::memory_order_relaxed);
}
...
VkResult CaptureManager::SubmitReadBack(DeviceData* dev, VkQueue queue, const std::function<void(VkCommandBuffer)>& record)
{
    ...
    if (!pool)
    {
        ...
        if (d.CreateCommandPool(dev->device, &pci, nullptr, &pool) != VK_SUCCESS)
            return VK_ERROR_INITIALIZATION_FAILED;
        std::lock_guard lock(dev->queueMutex);
    ...
    *reinterpret_cast<void**>(cb) = *reinterpret_cast<void**>(dev->device);
    ...
    if (res == VK_SUCCESS)
        res = d.WaitForFences(dev->device, 1, &fence, VK_TRUE, 5000000000ull);
...
{
    if (!_options.captureBuffers)
        return;
    DeviceCapture* dc = CaptureFor(dev);
    if (!dc || dc->earlierStructuresRead.exchange(true))
        return;
    ResourceRegistry& reg = ResourceRegistry::Get();
    Tracker& t = Tracker::Get();
    ...
    for (const auto& [id, inputs] : reg.StructureInputs(dev->device, CaptureSerial()))
    {
        ...
        for (const ResourceRegistry::StructureInput& in : inputs)
        {
            ...
            if (!reg.ResolveAddress(in.address, buffer, offset, remaining))
                continue;
            PendingBufferCopy copy;
            ...
            list += count++ ? "," : "";
            list += "{\"info\":0,\"geometry\":" + std::to_string(in.geometry) + ",\"field\":\"" + in.field + "\",\"buffer\":" + std::to_string(t.Resolve(HT_VkBuffer, (uint64_t)(uintptr_t)buffer)) + ",\"offset\":" + std::to_string(offset);
            if (capture)
                list += ",\"capture\":" + std::to_string(capture);
            list += "}";
        }
        if (count)
            updates.emplace_back(id, std::move(list));
    }
    if (updates.empty())
        return;
    VkResult res = VK_SUCCESS;
    if (!copies.empty())
        res = SubmitReadBack(dev, queue, [&](VkCommandBuffer cb) { RecordBufferCopies(dev, cb, copies); });
    {
        std::lock_guard lock(_mutex);
        for (const PendingBufferCopy& p : copies)
        {
            ...
            {
                bc.failed = true;
                bc.note = "read-back of a structure's build inputs failed";
            }
        }
    }
...
void CaptureManager::SendTextures(DeviceData* dev)
{
    ...
    for (auto& tc : textures)
    {
        ...
        w.Uint(tc.frame);
        w.Key("commandBuffer");
        w.Uint(tc.commandBufferId);
        w.Key("passIndex");
        w.Uint(tc.passIndex);
        w.Key("attachment");
        w.Uint(tc.attachment);
        w.Key("format");
        w.Enum(ToString_VkFormat(tc.format), (int64_t)tc.format);
        w.Key("aspect");
        w.String(tc.aspect == VK_IMAGE_ASPECT_DEPTH_BIT ? "depth" : tc.aspect == VK_IMAGE_ASPECT_STENCIL_BIT ? "stencil"
                                                                                                             : "color");
        w.Key("width");
        w.Uint(tc.width);
        w.Key("height");
        ...
        {
            w.Key("kind");
            w.String(tc.initial ? "initial" : "sampled");
            w.Key("capture");
            w.Uint(tc.captureId);
            if (tc.viewId)
            {
    ...
    for (auto& tc : textures)
    {
        ...
        h.Uint(tc.frame);
        h.Key("commandBuffer");
        h.Uint(tc.commandBufferId);
        h.Key("passIndex");
        h.Uint(tc.passIndex);
        h.Key("attachment");
        h.Uint(tc.attachment);
        // A depth-stencil attachment has an entry per aspect under the same attachment index.
        h.Key("aspect");
        h.String(tc.aspect == VK_IMAGE_ASPECT_DEPTH_BIT ? "depth" : tc.aspect == VK_IMAGE_ASPECT_STENCIL_BIT ? "stencil"
                                                                                                             : "color");
        if (tc.sampled || tc.initial)
        {
            h.Key("capture");
...
void CaptureManager::SendPassTimings()
{
    ...
    w.Double((home ? home : captures[0])->dev->properties.limits.timestampPeriod);
    w.Key("passes");
    w.BeginArray();
    uint32_t sent = 0;
    uint32_t counted = 0;
    ...
    for (DeviceCapture* dc : captures)
        SendPassTimings(*dc, w, sent, counted, total, dc == home ? &originTicks : nullptr);
    w.EndArray();
    w.Key("count");
    w.Uint(sent);
    if (originTicks)
    ...
    uint64_t* originTicks)
{
    ...
    for (auto& pt : timings)
    {
        if (pt.frame == UINT32_MAX || pt.query + 1 >= used)
            continue;
    ...
    if (originTicks && earliest != UINT64_MAX)
    ...
    for (auto& pt : timings)
    {
        ...
        if (!results[(size_t)pt.query * 2 + 1] || !results[((size_t)pt.query + 1) * 2 + 1] || end < begin)
            continue;
        w.BeginObject();
        w.Key("frame");
        w.Uint(pt.frame);
        w.Key("commandBuffer");
        w.Uint(pt.commandBufferId);
        w.Key("passIndex");
        w.Uint(pt.passIndex);
        w.Key("kind");
        w.String(pt.compute ? "compute" : "render");
        w.Key("startMs");
        w.Double((double)(begin - earliest) * period / 1e6);
        w.Key("durationMs");
        w.Double((double)(end - begin) * period / 1e6);
        const uint64_t* v = pt.statsQuery != UINT32_MAX && pt.statsQuery < statsUsed && !stats.empty()
            ? &stats[(size_t)pt.statsQuery * kStatsStride]
            : nullptr;
        const bool hasStats = v && v[kPipelineStatisticCount];   // the pass ran and its counters are readable
        const bool hasOcclusion = pt.occlusionQuery != UINT32_MAX && pt.occlusionQuery < occlusionUsed && !occlusion.empty() && occlusion[(size_t)pt.occlusionQuery * 2 + 1];
