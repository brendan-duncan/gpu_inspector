...
void DescriptorBufferTracker::OnGetDescriptor(const VkDescriptorGetInfoEXT* info, size_t size, const void* data) {
    ...
    auto fromAddress = [&](const VkDescriptorAddressInfoEXT* a) {
        ...
        if (!reg.ResolveAddress(a->address, buffer, offset, remaining)) return;
        e.buffer = buffer;
        e.offset = offset;
        e.range = a->range;
    };
    auto fromImage = [&](const VkDescriptorImageInfo* i, bool withSampler, bool withView) {
        if (!i) return;
        if (withView) e.imageView = i->imageView;
        if (withSampler) e.sampler = i->sampler;
        e.imageLayout = i->imageLayout;
    };

    switch (info->type) {
        case VK_DESCRIPTOR_TYPE_SAMPLER:
            if (info->data.pSampler) e.sampler = *info->data.pSampler;
            break;
    ...
    std::unique_lock lock(_mutex);
    if (_descriptors.size() >= kMaxDescriptors && _descriptors.find(key) == _descriptors.end()) return;
...
void DescriptorBufferTracker::OnSetOffsets(VkCommandBuffer cb, VkPipelineBindPoint point, uint32_t firstSet,
    ...
    if (sets.size() < (size_t)firstSet + setCount) sets.resize((size_t)firstSet + setCount);
...
bool DescriptorBufferTracker::SetSource(VkCommandBuffer cb, VkPipelineBindPoint point, uint32_t set,
    ...
    if (c == _commandBuffers.end()) return false;
    auto s = c->second.sets.find((int)point);
    if (s == c->second.sets.end() || set >= s->second.size() || !s->second[set].set) return false;
    const SetOffset& so = s->second[set];
    if (so.bufferIndex >= c->second.buffers.size()) return false;
...
bool DescriptorBufferTracker::LayoutSize(DeviceData* dev, VkDescriptorSetLayout layout, VkDeviceSize& out) const {
    if (!dev || !layout || !dev->dispatch.GetDescriptorSetLayoutSizeEXT) return false;
...
bool DescriptorBufferTracker::Decode(DeviceData* dev, VkDescriptorSetLayout layout, const uint8_t* bytes, size_t size,
    ...
    if (!dev || !bytes || !size) return false;
    if (!DescriptorTracker::Get().GetLayout(layout, out)) return false;
    auto& self = const_cast<DescriptorBufferTracker&>(*this);
    const DescriptorSizes& sizes = self.Sizes(dev);
    if (!sizes.known || !dev->dispatch.GetDescriptorSetLayoutBindingOffsetEXT) return false;
    out.layout = layout;

    std::shared_lock lock(_mutex);
    for (DescriptorBinding& b : out.bindings) {
        const size_t stride = sizes.For(b.type);
        if (!stride) continue;   // a type that holds no descriptor here (inline uniform data)
        VkDeviceSize at = 0;
        ...
        for (size_t k = 0; k < b.entries.size(); ++k) {
            ...
            if (start + stride > size) break;
            // Memory nothing was ever written into reads as zero, which is not a descriptor.
            bool anySet = false;
            for (size_t j = 0; j < stride && !anySet; ++j) anySet = bytes[start + j] != 0;
            if (!anySet) continue;
            ...
            if (it == _descriptors.end()) continue;
...
void DescriptorBufferTracker::OnDestroy(HandleType type, uint64_t handle) {
    std::unique_lock lock(_mutex);
    if (type == HT_VkCommandBuffer) _commandBuffers.erase(handle);
    else if (type == HT_VkDevice) _sizes.erase(handle);
}
