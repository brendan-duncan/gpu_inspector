...
static void EnsureStarted()
{
    ...
    // The HUD's capture hotkey, armed with the HUD itself (hud_hotkey.h). Read here rather than
    // where it is polled so a misspelled key is reported at start-up, not on the frame it fails.
    const std::string hotkey = ConfigValue("VKINSP_HOTKEY");
    if (!gpuhud::CaptureHotkey::Get().SetBinding(hotkey.c_str()))
        Log("VKINSP_HOTKEY=\"%s\" is not a key this understands (\"F11\", \"CTRL+F9\", \"off\"); "
...
{
    return vkinsp::RequestInspectorCapture(frameCount ? frameCount : 1u, label, "the application") ? 1 : 0;
}
