...
{
    if (!image || newLayout == VK_IMAGE_LAYOUT_UNDEFINED)
        return;
    std::lock_guard lock(_mutex);
    ...
    uint32_t depth, uint64_t size, const char* error)
{
    ...
    {
        w.Key("format");
        w.Enum(ToString_VkFormat(img->format), (int64_t)img->format);
    }
    else
    {
        w.Key("format");
        w.String("VK_FORMAT_UNDEFINED");
    }
    w.Key("aspect");
    w.String(aspect == VK_IMAGE_ASPECT_DEPTH_BIT ? "depth" : aspect == VK_IMAGE_ASPECT_STENCIL_BIT ? "stencil"
                                                                                                   : "color");
    w.Key("size");
    w.Uint(size);
    if (error)
...
static int FindHostMemoryType(DeviceData* dev, uint32_t typeBits)
{
    for (int pass = 0; pass < 2; ++pass)
    {
        ...
        {
            if ((typeBits & (1u << i)) && (dev->memoryProperties.memoryTypes[i].propertyFlags & want) == want)
                return (int)i;
        }
    }
    return -1;
...
void ImageReadback::Serve(DeviceData* dev, VkQueue queue, const PendingRequest& r)
{
    TrackedObject obj;
    if (!Tracker::Get().FindById(r.imageId, obj) || obj.type != HT_VkImage)
        return Fail(r, "image no longer exists");
    VkImage image = (VkImage)(uintptr_t)obj.handle;
    ImageInfo img;
    if (!ResourceRegistry::Get().GetImage(image, img))
        return Fail(r, "image is not tracked");
    if (!img.transferSrc)
        return Fail(r, "image lacks TRANSFER_SRC usage");

    VkImageLayout layout = VK_IMAGE_LAYOUT_UNDEFINED;
    if (!LayoutTracker::Get().GetLayout(image, layout))
    {
        if (img.swapchainImage)
            layout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
        else
            return Fail(r, "image layout unknown (no transition seen yet)");
    }
    ...
    if (block.bytes == 0)
        return Fail(r, "unsupported format for readback");
    if (img.samples != VK_SAMPLE_COUNT_1_BIT && (aspect == VK_IMAGE_ASPECT_DEPTH_BIT || aspect == VK_IMAGE_ASPECT_STENCIL_BIT) && !CanResolveDepth(dev))
    ...
    if (size > (256ull << 20))
        return Fail(r, "image is larger than 256 MB");
    ...
    uint32_t family = 0;
    {
        ...
        if (it == dev->queueFamilies.end())
            return Fail(r, "unknown queue family for the present queue");
        family = it->second;
    }
    ...
    if (!pool)
    {
        ...
        if (d.CreateCommandPool(dev->device, &pci, nullptr, &pool) != VK_SUCCESS)
            return Fail(r, "command pool creation failed");
        std::lock_guard lock(dev->queueMutex);
    ...
    if (d.CreateBuffer(dev->device, &bci, nullptr, &buffer) != VK_SUCCESS)
        return Fail(r, "staging buffer creation failed");
    ...
    auto cleanup = [&]() {
        ...
        if (dstView)
            d.DestroyImageView(dev->device, dstView, nullptr);
        if (resolve)
            d.DestroyImage(dev->device, resolve, nullptr);
        if (resolveMemory)
            d.FreeMemory(dev->device, resolveMemory, nullptr);
    };
