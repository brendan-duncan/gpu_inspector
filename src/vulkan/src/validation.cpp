...
bool ValidationLog::EnsureDebugUtils(PFN_vkGetInstanceProcAddr nextGipa, VkInstanceCreateInfo& info, std::vector<const char*>& names)
{
    ...
    {
        if (strcmp(info.ppEnabledExtensionNames[i], VK_EXT_DEBUG_UTILS_EXTENSION_NAME) == 0)
            return true;
    }
...
{
    if (!inst || inst->messenger == VK_NULL_HANDLE || !inst->dispatch.DestroyDebugUtilsMessengerEXT)
        return;
...
static std::string CommandName(const char* message)
{
    ...
    if (const char* c = strstr(message, "command: vk"))
    {
        c += 9;
        size_t len = 0;
        while (isalnum((unsigned char)c[len]))
            ++len;
    ...
    {
        size_t len = 0;
        while (isalnum((unsigned char)p[len]))
            ++len;
        if (p[len] == '(' && p[len + 1] == ')')
...
static bool CommandFromText(const char* message, const std::shared_ptr<const CommandList>& commands, int64_t& slot)
{
    ...
    if (seq > 0 && (size_t)seq <= commands->size())
    {
        slot = seq - 1;
        return true;
    }
    ...
    if (family)
    {
        ...
        {
            if (std::string(kVkCommandNames[(int)(*commands)[i].id]).rfind(family, 0) != 0)
                continue;
            if (seen++ == action)
            {
                slot = (int64_t)i;
    ...
    for (size_t i = 0; i < commands->size(); ++i)
    {
        if (name == kVkCommandNames[(int)(*commands)[i].id])
        {
            slot = (int64_t)i;
            return true;
        }
...
ValidationLog::CommandRef ValidationLog::CurrentCommand(const VkDebugUtilsMessengerCallbackDataEXT* data)
{
    ...
    if (!capturing && !CaptureManager::Get().HasCapturedCommands())
        return ref;
    std::vector<uint64_t> handles;
    for (uint32_t i = 0; i < data->objectCount; ++i)
    {
        const VkDebugUtilsObjectNameInfoEXT& o = data->pObjects[i];
        if (o.objectType == VK_OBJECT_TYPE_COMMAND_BUFFER && o.objectHandle)
            handles.push_back(o.objectHandle);
    }
    if (handles.empty())
        handles = CommandBuffersInText(data->pMessage);
    ...
    const VkDebugUtilsMessengerCallbackDataEXT* data)
{
    ...
    for (uint32_t i = 0; i < data->objectCount; ++i)
    {
        ...
        if (o.pObjectName)
            ref.name = o.pObjectName;
        e.objects.push_back(std::move(ref));
    }
    for (uint32_t i = 0; i < data->queueLabelCount; ++i)
        if (data->pQueueLabels[i].pLabelName)
            e.queueLabels.push_back(data->pQueueLabels[i].pLabelName);
    for (uint32_t i = 0; i < data->cmdBufLabelCount; ++i)
        if (data->pCmdBufLabels[i].pLabelName)
            e.cmdBufLabels.push_back(data->pCmdBufLabels[i].pLabelName);
    e.count = 1;
    e.cmdBufferId = cmd.cmdBufferId;
    e.cmdSlot = cmd.slot;
...
std::string ValidationLog::MessageJson(const Entry& e) const
{
    ...
    for (auto& o : e.objects)
    {
        ...
        w.Key("object");
        if (o.handleType >= 0)
            w.Handle(o.handleType, o.className.c_str(), o.handle);
        else
            w.Null();
        w.Key("class");
        w.String(o.className);
        char buf[32];
...
{
    if (!_anyDirty || !Transport::Get().Connected())
        return;
    std::lock_guard<std::mutex> lock(_mutex);
    if (!_anyDirty)
        return;
