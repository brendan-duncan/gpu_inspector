...
void PlanBreadcrumbs(InstanceData* inst, VkPhysicalDevice physicalDevice, VkDeviceCreateInfo& info, BreadcrumbSetup& setup) {
    if (!ConfigFlag("VKINSP_BREADCRUMBS")) return;
    ...
    if (!already) setup.extensionNames.push_back(VK_AMD_BUFFER_MARKER_EXTENSION_NAME);
...
void CreateBreadcrumbs(DeviceData* dev, const BreadcrumbSetup& setup) {
    if (!setup.added || !dev->dispatch.CmdWriteBufferMarkerAMD) {
        if (setup.wanted && !setup.added) Log("device-lost breadcrumbs: not enabled on this device");
        return;
    }
    ...
    for (uint32_t i = 0; i < dev->memoryProperties.memoryTypeCount; ++i) {
        ...
        if ((dev->memoryProperties.memoryTypes[i].propertyFlags & want) != want) continue;
        typeIndex = i;
        if (!(dev->memoryProperties.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)) break;
    }
    ...
    if (dev->dispatch.AllocateMemory(dev->device, &ai, nullptr, &crumbs->memory) != VK_SUCCESS ||
        ...
        if (crumbs->memory) dev->dispatch.FreeMemory(dev->device, crumbs->memory, nullptr);
        dev->dispatch.DestroyBuffer(dev->device, crumbs->buffer, nullptr);
        return;
    }
    void* mapped = nullptr;
...
void DestroyBreadcrumbs(DeviceData* dev) {
    ...
    if (crumbs->memory) dev->dispatch.FreeMemory(dev->device, crumbs->memory, nullptr);
    if (crumbs->buffer) dev->dispatch.DestroyBuffer(dev->device, crumbs->buffer, nullptr);
    delete crumbs;
}
...
bool SimulateDeviceLost(DeviceData* dev, bool* pretendIncomplete) {
    Breadcrumbs* crumbs = dev ? dev->breadcrumbs : nullptr;
    if (!crumbs || !crumbs->simulateAfter) return false;
    if (crumbs->submissions.fetch_add(1) + 1 != crumbs->simulateAfter) return false;
    if (pretendIncomplete) *pretendIncomplete = crumbs->simulateHung;
    return true;
}
...
std::string DescribeOrdinal(Breadcrumbs* crumbs, uint32_t ordinal, uint32_t newest) {
    if (!ordinal) return "nothing (the GPU had not reached a recorded action)";
...
void OnDeviceLost(DeviceData* dev, const char* call, bool pretendIncomplete) {
    ...
    if (!crumbs) {
        if (reportedWithout.exchange(true)) return;
        Log("VK_ERROR_DEVICE_LOST from %s. The GPU stopped responding; the command that hung is not "
            ...
            "have the GPU record how far it got.", call);
        JsonWriter w;
        w.BeginObject();
        w.Key("action"); w.String("DeviceLost");
        w.Key("call"); w.String(call);
        w.Key("breadcrumbs"); w.Boolean(false);
        w.Key("message"); w.String("The GPU stopped responding. Run with device-lost breadcrumbs on to learn which command it was executing.");
        w.EndObject();
        Transport::Get().SendJson(std::move(w.str()));
        return;
    }
    if (crumbs->reported.exchange(true)) return;
    ...
    w.Key("action"); w.String("DeviceLost");
    w.Key("call"); w.String(call);
    w.Key("breadcrumbs"); w.Boolean(true);
    w.Key("lastBegun"); w.Uint(begun);
    w.Key("lastCompleted"); w.Uint(ended);
    w.Key("hungCommand"); w.String(hung ? begunText : std::string());
    w.Key("lastCompletedCommand"); w.String(endedText);
