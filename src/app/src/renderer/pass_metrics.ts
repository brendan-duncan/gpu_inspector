...
// counter sets (src/metal/src/capture.mm) and the Vulkan layer a pipeline statistics query beside its
// timestamps (src/vulkan/src/pipeline_stats.h). Those are raw totals. What a profiling session asks is
...
export function collectPassMetrics(data: CaptureData, db: ObjectLookup): FrameMetrics {
  ...
  // src/metal/src/capture.mm). Vulkan's PASS_BEGIN is render-only and its runs of dispatches are
