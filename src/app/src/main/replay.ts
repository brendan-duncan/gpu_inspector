 ...
 * packaged GPU Inspector (`layerDirs`, where src/app/tools/stage_layer.mjs puts it).
 */
...
/** The Metal replay tool (src/metal/replay/): it replays a capture to compare it and to export it, and serves no analyses. */
export const METAL_REPLAY_TOOL = "mtlinsp_replay";

/** mtlinsp_replay: INSPECTOR_METAL_REPLAY, a checkout's build tree, or beside the layer of a packaged GPU Inspector. macOS only. */
export function findMetalReplayTool(roots: string[], layerDirs: string[]): string | null {
  if (process.platform !== "darwin") return null;
  const candidates = [
    process.env.INSPECTOR_METAL_REPLAY,
    ...roots.flatMap((root) => ["Release", "RelWithDebInfo", "Debug", ""].map((config) => path.join(root, "build", "bin", config, METAL_REPLAY_TOOL))),
    ...layerDirs.map((dir) => path.join(dir, METAL_REPLAY_TOOL)),
  ].filter((f): f is string => !!f);
  return candidates.find((f) => fs.existsSync(f)) ?? null;
}

export const NO_METAL_REPLAY_TOOL = process.platform === "darwin"
  ? `${METAL_REPLAY_TOOL} not found. Build it (cmake --build build --target mtlinsp_replay), or set INSPECTOR_METAL_REPLAY to its path.`
  : "a Metal capture replays on macOS only.";
...
function analysisArgs(analysis: ReplayAnalysis, out: string, input: string | null): string[] {
  ...
  if (analysis.kind === "counters") {
    return [...(analysis.counters ?? []).flatMap((c) => ["--counter", c]),
...
/** One answer of `vkinsp_replay --serve` (Serve in src/replay/src/main.cpp). */
