...
const char* StemOf(const std::string& type) {
    if (type == "MTLTexture") return "texture";
    if (type == "MTLBuffer") return "buffer";
    if (type == "MTLLibrary") return "library";
    if (type == "MTLFunction") return "function";
    if (type == "MTLRenderPipelineState") return "pipeline";
    if (type == "MTLComputePipelineState") return "compute_pipeline";
    if (type == "MTLDepthStencilState") return "depth_stencil";
    if (type == "MTLSamplerState") return "sampler";
    if (type == "MTLHeap") return "heap";
    if (type == "MTLFence") return "fence";
    if (type == "MTLEvent" || type == "MTLSharedEvent") return "event";
    if (type == "MTLIndirectCommandBuffer") return "icb";
    if (type == "MTLAccelerationStructure") return "acceleration_structure";
    if (type == "MTLCommandQueue") return "queue";
    return "object";
}

/** The Objective-C type a global of this kind is declared with. */
const char* TypeOf(const std::string& type) {
    if (type == "MTLTexture") return "id<MTLTexture>";
    if (type == "MTLBuffer") return "id<MTLBuffer>";
    if (type == "MTLLibrary") return "id<MTLLibrary>";
    if (type == "MTLFunction") return "id<MTLFunction>";
    if (type == "MTLRenderPipelineState") return "id<MTLRenderPipelineState>";
    if (type == "MTLComputePipelineState") return "id<MTLComputePipelineState>";
    if (type == "MTLDepthStencilState") return "id<MTLDepthStencilState>";
    if (type == "MTLSamplerState") return "id<MTLSamplerState>";
    if (type == "MTLHeap") return "id<MTLHeap>";
    if (type == "MTLFence") return "id<MTLFence>";
    if (type == "MTLEvent") return "id<MTLEvent>";
    if (type == "MTLSharedEvent") return "id<MTLSharedEvent>";
    if (type == "MTLIndirectCommandBuffer") return "id<MTLIndirectCommandBuffer>";
    if (type == "MTLAccelerationStructure") return "id<MTLAccelerationStructure>";
    return "id";
}
...
bool ConstantType(const std::string& name, MTLDataType* type, uint32_t* count) {
    static const struct { const char* base; MTLDataType first; } kFamilies[] = {
        {"float", MTLDataTypeFloat}, {"half", MTLDataTypeHalf}, {"int", MTLDataTypeInt},
        {"uint", MTLDataTypeUInt},   {"short", MTLDataTypeShort}, {"ushort", MTLDataTypeUShort},
    ...
    for (const auto& f : kFamilies) {
        ...
        if (name.compare(0, length, f.base) != 0) continue;
        const std::string rest = name.substr(length);
        uint32_t components = 1;
        if (!rest.empty()) {
            if (rest.size() != 1 || rest[0] < '2' || rest[0] > '4') continue;
...
void MtlReplayer::Problem(const std::string& message) {
    if (_env.problems.size() < 10000) _env.problems.push_back(message);
}
...
void MtlReplayer::LeftOut(uint32_t index, const std::string& method, const std::string& why) {
    ...
    if (_x) _x->LeftOut(index, method, why);
}
    ...
    @autoreleasepool {
        ...
        if (const JValue* objects = _capture.Objects()) {
            for (uint32_t i = 0; i < objects->count; ++i) {
                ...
                if (id) _records[id->Uint()] = &o;
                if (Text(o.Get("type")) == "MTLDevice") report.capturedDevice = Text(o.Get("args") ? o.Get("args")->Get("name") : nullptr);
            }
        }
        if (const JValue* buffers = _capture.Buffers()) {
            for (uint32_t i = 0; i < buffers->count; ++i) {
                const JValue* info = buffers->items[i].Get("info");
                if (info && info->Get("id")) _bufferData[info->Get("id")->Uint()] = &buffers->items[i];
            }
        }
        if (const JValue* textures = _capture.Textures()) {
            for (uint32_t i = 0; i < textures->count; ++i) {
                ...
                if (kind == "sampled" || kind == "initial") {
                    if (info->Get("capture")) _textureData[info->Get("capture")->Uint()] = &textures->items[i];
        ...
        for (const std::string& p : _env.problems) report.problems.push_back(p);
        if (_env.unresolved) {
...
void MtlReplayer::CreateObjects() {
    ...
    if (!objects) return;
    if (_x) _x->Comment(MtlExporter::Create, "The capture's objects, in the order it created them.");
    for (uint32_t i = 0; i < objects->count; ++i) {
        @autoreleasepool { CreateObject(objects->items[i]); }
    }
}
...
void MtlReplayer::CreateObject(const JValue& object) {
    ...
    if (_options.trace) std::fprintf(stderr, "object %" PRIu64 " %s\n", captureId, type.c_str());
    // The capture's device and its queues are the replay's own, which are already made.
    if (type == "MTLDevice") { _objects[captureId] = _device; return; }
    if (type == "MTLCommandQueue") { _objects[captureId] = _queue; return; }
    if (type == "MTLCommandBuffer") return;   // made as the frame is replayed
    ...
    if (type == "MTLTexture") made = CreateTexture(object, d, captureId);
    else if (type == "MTLBuffer") made = CreateBuffer(object, d, captureId);
    else if (type == "MTLLibrary") made = CreateLibrary(object, d, captureId);
    else if (type == "MTLFunction") made = CreateFunction(object, d, captureId);
    else if (type == "MTLRenderPipelineState") made = CreateRenderPipeline(object, d, captureId);
    else if (type == "MTLComputePipelineState") made = CreateComputePipeline(object, d, captureId);
...
id<MTLTexture> MtlReplayer::CreateTexture(const JValue& object, const Decoder& d, uint64_t captureId) {
    ...
    if (drawable) {
        ...
        if (_x) _x->Note("the drawable's texture is an ordinary render target here: an exported frame has no window");
    }
    ...
    if (descriptor.storageMode == MTLStorageModeMemoryless) descriptor.storageMode = MTLStorageModePrivate;
    ...
    if (!texture) texture = [_device newTextureWithDescriptor:descriptor];
    ...
    if (_x) {
        ...
        _x->Block(MtlExporter::Create, LabelOf(captureId), [&](Source& w) {
            ...
            if (!label.empty()) w.Line(name + ".label = " + Source::NSString(label, true) + ";");
        });
    }
    return texture;
}
...
id<MTLBuffer> MtlReplayer::CreateBuffer(const JValue& object, const Decoder& d, uint64_t captureId) {
    ...
    id<MTLHeap> heap = d.Object("heap");
    if (heap) buffer = [heap newBufferWithLength:(NSUInteger)length options:options];
    if (!buffer) buffer = [_device newBufferWithLength:(NSUInteger)length options:options];
    if (!buffer) return nil;
    const std::string label = LabelOf(captureId);
    if (!label.empty()) buffer.label = [NSString stringWithUTF8String:label.c_str()];
...
id<MTLLibrary> MtlReplayer::CreateLibrary(const JValue& object, const Decoder& d, uint64_t captureId) {
    ...
    if (_capture.Blob(object, "Metal Shading Language", bytes, size) && size) {
        ...
        if (_x) shaderFile = _x->Shader("library_" + std::to_string(captureId),
...
id<MTLFunction> MtlReplayer::CreateFunction(const JValue& object, const Decoder& d, uint64_t captureId) {
    ...
    if (constants && constants->IsArray() && constants->count) {
        ...
        for (uint32_t i = 0; i < constants->count; ++i) {
            ...
            uint32_t count = 0;
            if (!ConstantType(c.Str("type"), &type, &count)) continue;
            ...
            for (uint32_t n = 0; n < count; ++n) {
                ...
                if (base.compare(0, 5, "float") == 0) {
                    ...
                    const size_t width = base.compare(0, 5, "short") == 0 || base.compare(0, 6, "ushort") == 0 ? 2
                                       : base.compare(0, 4, "char") == 0 || base.compare(0, 5, "uchar") == 0 ? 1 : 4;
    ...
    if (_x) {
        ...
        _x->Block(MtlExporter::Create, std::string(fname.UTF8String), [&](Source& w) {
            ...
            for (uint32_t i = 0; i < constants->count; ++i) {
                ...
                uint32_t count = 0;
                const std::string base = c.Str("type");
                if (!ConstantType(base, &type, &count)) continue;
...
id MtlReplayer::CreateRenderPipeline(const JValue& object, const Decoder& d, uint64_t captureId) {
    ...
    if (_x) {
        ...
        _x->Block(MtlExporter::Create, LabelOf(captureId), [&](Source& w) {
            ...
            EmitVisitor emit(w, var + ".");
            Reflect(emit, descriptor, defaults);
            if (descriptor.vertexFunction) w.Line(var + ".vertexFunction = " + w.Object(descriptor.vertexFunction) + ";");
            if (descriptor.fragmentFunction) w.Line(var + ".fragmentFunction = " + w.Object(descriptor.fragmentFunction) + ";");
            if (vertexDescriptor) {
...
id MtlReplayer::CreateDepthStencil(const JValue& object, const Decoder& d, uint64_t captureId) {
    ...
    if (front.Json()) { FillVisitor f(front); Reflect(f, descriptor.frontFaceStencil, defaults.frontFaceStencil); }
    if (back.Json()) { FillVisitor f(back); Reflect(f, descriptor.backFaceStencil, defaults.backFaceStencil); }
...
void MtlReplayer::WriteBuffer(id<MTLBuffer> buffer, uint64_t offset, const void* data, uint64_t size,
    ...
    id<MTLBuffer> staging = [_device newBufferWithBytes:data length:(NSUInteger)size
                                                options:MTLResourceStorageModeShared];
    id<MTLCommandBuffer> commands = [_queue commandBuffer];
    id<MTLBlitCommandEncoder> blit = [commands blitCommandEncoder];
    [blit copyFromBuffer:staging sourceOffset:0 toBuffer:buffer destinationOffset:(NSUInteger)offset
...
void MtlReplayer::UploadTexture(id<MTLTexture> texture, const JValue& info, const uint8_t* data, size_t size) {
    ...
    id<MTLBuffer> staging = [_device newBufferWithBytes:data length:(NSUInteger)imageBytes
                                                options:MTLResourceStorageModeShared];
    id<MTLCommandBuffer> commands = [_queue commandBuffer];
    id<MTLBlitCommandEncoder> blit = [commands blitCommandEncoder];
...
void MtlReplayer::UploadContents() {
    ...
    for (const auto& [captureId, entry] : _textureData) {
        @autoreleasepool {
            ...
            if (!info || !info->Get("id")) continue;
            const uint64_t textureId = info->Get("id")->Uint();
            if (!_uploadedTextures.insert(captureId).second) continue;
            ...
            size_t size = 0;
            if (!_capture.Payload(entry->Get("payload"), data, size) || !size) continue;
...
void MtlReplayer::ReplayCommands() {
    ...
    for (uint32_t i = 0; i < commands->count; ++i) {
        @autoreleasepool {
            ...
            if (_options.trace) std::fprintf(stderr, "[%u] %s\n", index, method.c_str());

            if (owner && owner != _commandBufferId) {
                if (_commandBuffer) Commit(false);
    ...
    if (_commandBuffer) Commit(true);
    for (id<MTLCommandBuffer> committed : _committed) [committed waitUntilCompleted];
...
void MtlReplayer::IssueCommand(uint32_t index, const std::string& method, const Decoder& d,
    ...
    if (CommandBufferCommand(method, d, index)) { ++_commandCount; return; }
    if (!_encoder) {
        LeftOut(index, method, "no encoder is open");
        return;
    }
    if (CommonCommand(method, d, index)) { ++_commandCount; return; }
    bool handled = false;
