...
{
    if (const char* name = mtlinsp::EnumName(table.entries, table.count, value))
        return name;
...
std::string Source::Flags(const EnumTable& table, uint64_t value)
{
    ...
    if (const char* exact = mtlinsp::EnumName(table.entries, table.count, (int64_t)value))
        return exact;
    std::string out;
    uint64_t rest = value;
