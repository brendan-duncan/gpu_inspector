...
std::vector<id> ObjectArray(const Decoder& d, const char* key)
{
    ...
    for (uint32_t i = 0; i < array->count; ++i)
        out.push_back(d.Resolve(IdOf(&array->items[i])));
    return out;
}
...
std::vector<uint64_t> UintArray(const Decoder& d, const char* key)
{
    ...
    out.reserve(array->count);
    for (uint32_t i = 0; i < array->count; ++i)
        out.push_back(array->items[i].Uint());
    return out;
}
...
MTLRenderPassDescriptor* MtlReplayer::BuildRenderPass(const Decoder& d)
{
    ...
    if (colors && colors->IsArray())
    {
        for (uint32_t i = 0; i < colors->count; ++i)
        {
            ...
            if (a.Has("clearColor"))
                pass.colorAttachments[slot].clearColor = a.ClearColor("clearColor");
    ...
    if (depth.Json())
    {
        ...
        if (depth.Has("clearDepth"))
            pass.depthAttachment.clearDepth = depth.Double("clearDepth");
    ...
    if (stencil.Json())
    {
        ...
        if (stencil.Has("clearStencil"))
            pass.stencilAttachment.clearStencil = (uint32_t)stencil.Uint("clearStencil");
...
bool MtlReplayer::CommandBufferCommand(const std::string& m, const Decoder& d, uint32_t index)
{
    ...
    if (m == "encodeSignalEvent:value:" || m == "encodeWaitForEvent:value:")
    {
        ...
        if (signal)
            [_commandBuffer encodeSignalEvent:event value:value];
        else
...
bool MtlReplayer::RenderCommand(const std::string& m, const Decoder& d, uint32_t index)
{
    ...
    if (m == "setRenderPipelineState:")
    {
        ...
        if (!p)
        {
            LeftOut(index, m, "the pipeline is not in the replay");
            return true;
        }
    ...
    if (m == "setViewports:count:" || m == "setScissorRects:count:")
    {
        ...
        if (list && list->IsArray())
        {
            for (uint32_t i = 0; i < list->count; ++i)
            {
                ...
                if (viewports)
                    vs.push_back(one.Viewport("viewport"));
                else
                    rs.push_back(one.Scissor("rect"));
            }
        }
        if (viewports)
            [e setViewports:vs.data() count:vs.size()];
        else
            [e setScissorRects:rs.data() count:rs.size()];
        emitWith([&](Source& w) {
    ...
    for (const char* stage : {"Vertex", "Fragment"})
    {
        ...
        if (rest == "Buffer:offset:atIndex:")
        {
            ...
            if (vertex)
                [e setVertexBuffer:b offset:offset atIndex:slot];
            else
                [e setFragmentBuffer:b offset:offset atIndex:slot];
        ...
        if (rest == "BufferOffset:atIndex:")
        {
            const uint64_t offset = d.Uint("offset"), slot = d.Uint("index");
            if (vertex)
                [e setVertexBufferOffset:offset atIndex:slot];
            else
                [e setFragmentBufferOffset:offset atIndex:slot];
        ...
        if (rest == "Bytes:length:atIndex:")
        {
            ...
            if (bytes.empty())
            {
                LeftOut(index, m, "the capture holds none of the inline bytes");
                return true;
            }
            if (vertex)
                [e setVertexBytes:bytes.data() length:(NSUInteger)size atIndex:slot];
            else
                [e setFragmentBytes:bytes.data() length:(NSUInteger)size atIndex:slot];
            if (_x)
            {
        ...
        if (rest == "Texture:atIndex:")
        {
            ...
            const uint64_t slot = d.Uint("index");
            if (vertex)
                [e setVertexTexture:t atIndex:slot];
            else
                [e setFragmentTexture:t atIndex:slot];
        ...
        if (rest == "SamplerState:atIndex:")
        {
            ...
            const uint64_t slot = d.Uint("index");
            if (vertex)
                [e setVertexSamplerState:s atIndex:slot];
            else
                [e setFragmentSamplerState:s atIndex:slot];
        ...
        if (rest == "SamplerState:lodMinClamp:lodMaxClamp:atIndex:")
        {
            ...
            const uint64_t slot = d.Uint("index");
            if (vertex)
                [e setVertexSamplerState:s lodMinClamp:lo lodMaxClamp:hi atIndex:slot];
            else
                [e setFragmentSamplerState:s lodMinClamp:lo lodMaxClamp:hi atIndex:slot];
        ...
        if (rest == "Buffers:offsets:withRange:" || rest == "Textures:withRange:" || rest == "SamplerStates:withRange:")
        {
            ...
            const char* key = rest[0] == 'B' ? "buffers" : rest[0] == 'T' ? "textures"
                                                                          : "samplers";
            ...
            const char* elementType = rest[0] == 'B' ? "id<MTLBuffer>" : rest[0] == 'T' ? "id<MTLTexture>"
                                                                                        : "id<MTLSamplerState>";
            if (rest[0] == 'B')
            {
                std::vector<NSUInteger> ns(offsets.begin(), offsets.end());
                if (vertex)
                    [e setVertexBuffers:(const id<MTLBuffer>*)objects.data() offsets:ns.data() withRange:range];
                else
                    [e setFragmentBuffers:(const id<MTLBuffer>*)objects.data() offsets:ns.data() withRange:range];
            }
            else if (rest[0] == 'T')
            {
                if (vertex)
                    [e setVertexTextures:(const id<MTLTexture>*)objects.data() withRange:range];
                else
                    [e setFragmentTextures:(const id<MTLTexture>*)objects.data() withRange:range];
            }
            else
            {
                if (vertex)
                    [e setVertexSamplerStates:(const id<MTLSamplerState>*)objects.data() withRange:range];
                else
                    [e setFragmentSamplerStates:(const id<MTLSamplerState>*)objects.data() withRange:range];
            }
            emitWith([&](Source& w) {
                const std::string name = w.Local("bindings");
                std::string items;
                for (size_t i = 0; i < objects.size(); ++i)
                    items += (i ? ", " : "") + w.Object(objects[i]);
                ...
                if (rest[0] == 'B')
                {
                    ...
                    for (size_t i = 0; i < offsets.size(); ++i)
                        values += (i ? ", " : "") + Source::Uint(offsets[i]);
    ...
    if (m.compare(0, 14, "drawPrimitives") == 0)
    {
        ...
        if (m == "drawPrimitives:indirectBuffer:indirectBufferOffset:")
        {
            ...
            if (!b)
            {
                LeftOut(index, m, "the indirect buffer is not in the replay");
                return true;
            }
    ...
    if (m.compare(0, 21, "drawIndexedPrimitives") == 0)
    {
        ...
        if (!indexBuffer)
        {
            LeftOut(index, m, "the index buffer is not in the replay");
            return true;
        }
    ...
    if (m.compare(0, 11, "drawPatches") == 0 || m.compare(0, 18, "drawIndexedPatches") == 0)
    {
        ...
        if (m.find("indirectBuffer") != std::string::npos)
        {
            ...
            if (indexed)
            {
                [e drawIndexedPatches:points
                                 patchIndexBuffer:patchIndex
                           patchIndexBufferOffset:patchIndexOffset
                          controlPointIndexBuffer:controlPoints
                    controlPointIndexBufferOffset:controlPointsOffset
                                   indirectBuffer:b
                             indirectBufferOffset:offset];
...
bool MtlReplayer::ComputeCommand(const std::string& m, const Decoder& d, uint32_t index)
{
    ...
    if (m == "setTexture:atIndex:")
    {
        ...
        if (t != nil && (t.usage & MTLTextureUsageShaderWrite) != 0)
        {
            ...
            if (access == "readWrite")
                _encoderAccumulates = true;
            if (access != "readOnly")
                _encoderWrites.push_back(d.ObjectId("texture"));
        }
    ...
    {
        const NSRange range = d.Range("range");
        const char* key = m[3] == 'B' ? "buffers" : m[3] == 'T' ? "textures"
                                                                : "samplers";
        std::vector<id> objects = ObjectArray(d, key);
        std::vector<uint64_t> offsets = UintArray(d, "offsets");
        objects.resize(range.length, nil);
        offsets.resize(range.length, 0);
        const char* elementType = m[3] == 'B' ? "id<MTLBuffer>" : m[3] == 'T' ? "id<MTLTexture>"
                                                                              : "id<MTLSamplerState>";
        if (m[3] == 'B')
        {
            std::vector<NSUInteger> ns(offsets.begin(), offsets.end());
    ...
    if (m == "setAccelerationStructure:atBufferIndex:")
    {
        ...
        if (structure == nil)
        {
            LeftOut(index, m, "the acceleration structure is not in the replay");
            return true;
        }
        if (@available(macOS 11.0, *))
        {
    ...
    if (m == "setIntersectionFunctionTable:atBufferIndex:" || m == "setVisibleFunctionTable:atBufferIndex:")
    {
        ...
        if (table == nil)
        {
            LeftOut(index, m, "the function table is not in the replay");
            return true;
        }
        if (@available(macOS 11.0, *))
        {
    ...
    if (m == "setIntersectionFunctionTables:withBufferRange:" || m == "setVisibleFunctionTables:withBufferRange:")
    {
        ...
        if (tables.empty())
        {
            LeftOut(index, m, "none of the function tables are in the replay");
            return true;
        }
        if (@available(macOS 11.0, *))
        {
    ...
    if (m == "memoryBarrierWithResources:count:" || m == "memoryBarrierWithResources:count:afterStages:beforeStages:")
    {
        ...
        if (resources.empty())
        {
            LeftOut(index, m, "none of the resources are in the replay");
            return true;
        }
...
bool MtlReplayer::AccelerationCommand(const std::string& m, const Decoder& d, uint32_t index)
{
    ...
    if (build || refit)
    {
        ...
        if (destination == nil)
        {
            LeftOut(index, m, "the structure being built is not in the replay");
            return true;
        }
        std::string error;
        ...
        if (scratch == nil)
        {
            LeftOut(index, m, "no memory for the build's scratch buffer");
            return true;
        }
        ...
        else
        {
            ...
            if (source == nil)
            {
                LeftOut(index, m, "the structure being refit is not in the replay");
                return true;
            }
...
bool MtlReplayer::BlitCommand(const std::string& m, const Decoder& d, uint32_t index)
{
    ...
    if (m == "copyFromBuffer:sourceOffset:toBuffer:destinationOffset:size:")
    {
        ...
        if (!from || !to)
        {
            LeftOut(index, m, "a buffer is not in the replay");
            return true;
        }
    ...
    if (m == "copyFromTexture:toTexture:")
    {
        ...
        if (!from || !to)
        {
            LeftOut(index, m, "a texture is not in the replay");
            return true;
        }
    ...
    if (m == "copyFromBuffer:sourceOffset:sourceBytesPerRow:sourceBytesPerImage:sourceSize:toTexture:")
    {
        ...
        if (!from || !to)
        {
            LeftOut(index, m, "a resource is not in the replay");
            return true;
        }
        [e copyFromBuffer:from
                   sourceOffset:offset
    ...
    if (m == "fillBuffer:range:value:")
    {
        ...
        if (!b)
        {
            LeftOut(index, m, "the buffer is not in the replay");
            return true;
        }
    ...
    if (m == "generateMipmapsForTexture:")
    {
        ...
        if (!t)
        {
            LeftOut(index, m, "the texture is not in the replay");
            return true;
        }
    ...
    if (m.compare(0, 24, "optimizeContentsForCPU") == 0 || m.compare(0, 24, "optimizeContentsForGPU") == 0)
    {
        ...
        if (m.find("slice") != std::string::npos)
        {
            ...
            if (cpu)
                [e optimizeContentsForCPUAccess:t slice:slice level:level];
            else
                [e optimizeContentsForGPUAccess:t slice:slice level:level];
        ...
        {
            if (cpu)
                [e optimizeContentsForCPUAccess:t];
            else
                [e optimizeContentsForGPUAccess:t];
...
bool MtlReplayer::ResidencyCommand(const std::string& m, const Decoder& d, uint32_t index, const std::string& var)
{
    ...
    id<MTLBlitCommandEncoder> blit = [_encoder conformsToProtocol:@protocol(MTLBlitCommandEncoder)]
        ? (id<MTLBlitCommandEncoder>)_encoder
        : nil;
    ...
    if (m.compare(0, 11, "useResource") == 0)
    {
        ...
        if (m.compare(0, 12, "useResources") == 0)
            resources = ObjectArray(d, "resources");
        else if (id one = d.Object("resource"))
        ...
        {
            if (render)
                [render useResource:(id<MTLResource>)resource usage:usage];
            else if (compute)
                [compute useResource:(id<MTLResource>)resource usage:usage];
        }
        emitWith([&](Source& w) {
            for (id resource : resources)
            {
    ...
    {
        std::vector<id> heaps;
        if (m.compare(0, 8, "useHeaps") == 0)
            heaps = ObjectArray(d, "heaps");
        else if (id one = d.Object("heap"))
            heaps.push_back(one);
        heaps.erase(std::remove(heaps.begin(), heaps.end(), nil), heaps.end());
        if (heaps.empty())
        {
            LeftOut(index, m, "none of the heaps are in the replay");
            return true;
        }
        for (id heap : heaps)
        {
            if (render)
                [render useHeap:(id<MTLHeap>)heap];
            else if (compute)
                [compute useHeap:(id<MTLHeap>)heap];
        }
        emitWith([&](Source& w) {
            for (id heap : heaps)
                w.Line("[" + var + " useHeap:" + w.Object(heap) + "];");
        });
        return true;
    }
    if (m.compare(0, 11, "updateFence") == 0 || m.compare(0, 12, "waitForFence") == 0)
    {
        ...
        if (!fence)
        {
            LeftOut(index, m, "the fence is not in the replay");
            return true;
        }
        ...
        if (render)
        {
            ...
            if (update)
                [render updateFence:fence afterStages:stages];
            else
                [render waitForFence:fence beforeStages:stages];
...
void MtlReplayer::QueuePassReadbacks()
{
    ...
    if (it == _targets.end() || !_commandBuffer)
        return;
...
{
    (void)report;
    if (!_options.compareTargets || _writtenTextures.empty())
        return;
    ...
    for (uint32_t i = 0; i < (_capture.Textures() ? _capture.Textures()->count : 0); ++i)
    {
        ...
        if (written == _writtenTextures.end())
            continue;
        if (info->Get("error") || !entry->Get("payload"))
            continue;
        // Only the top level: a kernel writes the level it was bound, and the capture reads that.
        if (info->Get("mip") && info->Get("mip")->Uint() != 0)
            continue;
        ...
        readback.staging = [_device newBufferWithLength:(NSUInteger)imageBytes
                                                options:MTLResourceStorageModeShared];
        _capture.Payload(entry->Get("payload"), readback.captured, readback.capturedSize);
        if (!readback.staging || !readback.captured)
            continue;
    ...
    for (Readback& r : _readbacks)
    {
        ...
        if (r.staging && r.captured && r.capturedSize)
        {
            ...
            for (size_t i = 0; i < size; i += texelBytes)
            {
                ...
                if (std::memcmp(replayed + i, r.captured + i, width) == 0)
                    continue;
                ++c.differingTexels;
