...
Input ReadInput(const Decoder& d, const char* key) {
    ...
    if (!nested.Json()) return in;
    if (nested.ObjectId("buffer") == 0) return in;
...
void FillCommonGeometry(MTLAccelerationStructureGeometryDescriptor* g, const Decoder& d) {
    ...
    if (@available(macOS 12.0, *)) {
        if (NSString* label = d.NSStr("label")) g.label = label;
    }
...
MTLAccelerationStructureGeometryDescriptor* TriangleGeometry(const Decoder& d, bool motion,
    ...
    if (motion) {
        if (@available(macOS 13.0, *)) {
            ...
            if (index.buffer != nil) { g.indexBuffer = index.buffer; g.indexBufferOffset = index.offset; }
...
MTLAccelerationStructureGeometryDescriptor* BoundingBoxGeometry(const Decoder& d, bool motion,
    ...
    if (stride != 0) g.boundingBoxStride = stride;
    FillCommonGeometry(g, d);
    return g;
}
...
MTLAccelerationStructureGeometryDescriptor* CurveGeometry(const Decoder& d, bool motion,
    ...
    if (@available(macOS 14.0, *)) {
        ...
        if (radius.buffer != nil) { g.radiusBuffer = radius.buffer; g.radiusBufferOffset = radius.offset; }
        g.segmentCount = segmentCount;
        g.segmentControlPointCount = segmentControlPointCount;
        g.controlPointCount = controlPointCount;
        g.controlPointStride = controlPointStride;
...
MTLAccelerationStructureGeometryDescriptor* Geometry(const Decoder& d, std::string& error) {
    ...
    if (kind == "triangles" || kind == "motionTriangles") return TriangleGeometry(d, motion, error);
    if (kind == "boundingBoxes" || kind == "motionBoundingBoxes") return BoundingBoxGeometry(d, motion, error);
    if (kind == "curves" || kind == "motionCurves") return CurveGeometry(d, motion, error);
...
MTLAccelerationStructureDescriptor* InstanceDescriptor(const Decoder& d, std::string& error) {
    ...
    if (d.Bool("indirect")) {
        if (@available(macOS 14.0, *)) {
            ...
            if (stride != 0) indirect.instanceDescriptorStride = stride;
    ...
    if (stride != 0) top.instanceDescriptorStride = stride;
    if (@available(macOS 12.0, *)) {
...
MTLAccelerationStructureDescriptor* BuildDescriptor(const Decoder& d, std::string& error) {
    ...
    if (kind == "instance") descriptor = InstanceDescriptor(d, error);
    else if (kind == "primitive") descriptor = PrimitiveDescriptor(d, error);
    else error = "a descriptor of kind '" + kind + "' is not rebuilt";
...
id LinkedFunctions(const Decoder& d, std::vector<std::pair<std::string, id>>& out) {
    ...
    auto read = [&](const char* key, void (^assign)(NSArray*)) {
        ...
        if (!list || !list->IsArray() || list->count == 0) return;
        ...
        for (uint32_t i = 0; i < list->count; ++i) {
            ...
            if (!name.empty()) out.push_back({name, function});
        }
        if (functions.count == 0) return;
        assign(functions);
        any = true;
    };
    read("functions", ^(NSArray* f) { result.functions = f; });
    read("binaryFunctions", ^(NSArray* f) { result.binaryFunctions = f; });
    if (@available(macOS 12.0, *)) {
        read("privateFunctions", ^(NSArray* f) { result.privateFunctions = f; });
    }
...
std::string WriteLinkedFunctionsSource(Source& w, const Decoder& d) {
    ...
    auto read = [&](const char* key) {
        ...
        std::string items;
        for (size_t i = 0; i < names.size(); ++i) items += (i ? ", " : "") + names[i];
    ...
    for (const auto& [property, local] : lists) w.Line(var + "." + property + " = " + local + ";");
    return var;
}
...
bool WriteGeometrySource(Source& w, const std::string& var, const Decoder& d, std::string& error) {
    ...
    if (kind == "triangles") {
        ...
        if (d.Uint("boundingBoxStride") != 0) set("boundingBoxStride", uint("boundingBoxStride"));
    } else {
...
bool WriteDescriptorSource(Source& w, const std::string& var, const Decoder& d, std::string& error) {
    ...
    if (kind == "primitive") {
        ...
        for (uint32_t i = 0; list && i < list->count; ++i) {
            ...
            if (!WriteGeometrySource(w, g, d.At(&list->items[i]), error)) return false;
