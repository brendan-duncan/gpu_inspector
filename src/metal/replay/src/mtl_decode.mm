...
int64_t Decoder::ParseEnum(const JValue* v, const EnumTable& table, int64_t fallback)
{
    ...
    if (!v->IsString())
        return fallback;
    int64_t value = 0;
    if (mtlinsp::EnumValue(table.entries, table.count, v->Str(), value))
        return value;
...
uint64_t Decoder::ParseFlags(const JValue* v, const EnumTable& table, uint64_t fallback)
{
    ...
    while (!s.empty())
    {
        ...
        {
            int64_t bit = 0;
            if (mtlinsp::EnumValue(table.entries, table.count, token, bit))
                value |= (uint64_t)bit;
            else
...
MTLClearColor Decoder::ClearColor(const char* name) const
{
    ...
    for (uint32_t i = 0; i < v->count && i < 4; ++i)
        *channels[i] = v->items[i].Double();
    return c;
}
