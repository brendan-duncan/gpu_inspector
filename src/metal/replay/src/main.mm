...
std::string JsonString(const std::string& s) {
    ...
    for (unsigned char c : s) {
        ...
        else if (c < 0x20) { char esc[8]; std::snprintf(esc, sizeof(esc), "\\u%04x", c); out += esc; }
        else out += (char)c;
    }
    return out + "\"";
}
...
int main(int argc, const char** argv) {
    @autoreleasepool {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            else if (arg == "--dump" && i + 1 < argc) { dumpDir = argv[++i]; options.keepPixels = true; }
            else if (arg == "--export" && i + 1 < argc) options.exportDir = argv[++i];
            else if (arg == "--export-data" && i + 1 < argc) exportData = argv[++i];
            else if (!arg.empty() && arg[0] == '-') { Usage(); return 2; }
            else if (capturePath.empty()) capturePath = arg;
            else { Usage(); return 2; }
        }
        if (capturePath.empty()) { Usage(); return 2; }
        ...
        if (options.validate) setenv("METAL_DEVICE_WRAPPER_TYPE", "1", 1);
        ...
        if (!replayer.Run(report)) {
            for (const std::string& p : report.problems) std::fprintf(stderr, "%s\n", p.c_str());
            return 2;
        }
        ...
        for (const MtlTargetComparison& t : report.comparisons) {
            ...
            if (!dumpDir.empty()) DumpTarget(dumpDir, t);
        }
        if (!dumpDir.empty()) std::printf("wrote the compared targets to %s/\n", dumpDir.c_str());
        ...
        if (options.validate) std::printf("Metal's API validation is on; its messages are on stderr above\n");
