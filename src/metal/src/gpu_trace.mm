...
void SendResult(bool ok, const std::string& path, const std::string& error, uint64_t frame)
{
    ...
    if (ok)
        Log("gpu trace: frame %llu written to %s", (unsigned long long)frame, path.c_str());
    else
        Log("gpu trace: %s", error.c_str());
}
        ...
        if (@available(macOS 10.15, *))
        {
            if (![manager supportsDestination:MTLCaptureDestinationGPUTraceDocument])
            {
                SendResult(false, path,
                    ...
                    "with METAL_CAPTURE_ENABLED=1 (the inspector's launch does that)",
                    frame);
                return;
            }
