...
uint64_t OnFrameEnded()
{
    ...
    const uint64_t workingSetBytes = device != nil ? device.recommendedMaxWorkingSetSize : 0;
    {
        ...
        // again every ~2 s.
        if (g_frame % 120 == 1)
            g_displayRefreshMs = QueryDisplayRefreshMs();
        if (g_lastFrame.time_since_epoch().count() != 0)
        {
            ...
            if (g_intervals.size() < kWindow)
                g_intervals.push_back(ms);
            else
                g_intervals[g_intervalNext] = ms;
            ...
            if (sinceReport >= 100.0 && Transport::Get().Connected())
            {
                ...
                w.Uint(g_count);
                w.Key("submitMs");
                w.Double((double)g_submitNanos / 1e6 / g_count);
                w.Key("refreshMs");
                w.Double(g_refreshMs);
                w.Key("refreshSource");
                w.String(g_refreshMs > 0 ? g_refreshSource : "");
                w.Key("displayRefreshMs");
                w.Double(g_displayRefreshMs);
                w.Key("frameBoundary");
                w.String("present");
                // What Metal has set aside for this process, and the size it recommends staying
                // under: the memory meter's total.
                w.Key("allocatedBytes");
                w.Uint(allocatedBytes);
                w.Key("workingSetBytes");
                w.Uint(workingSetBytes);
                w.EndObject();
