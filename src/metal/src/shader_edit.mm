...
std::string CompileError(NSError *error) {
    if (error == nil) return "the compiler reported no error and produced no library";
    NSString *log = error.userInfo[@"MTLCompilerErrorLog"];
    if (log == nil) log = error.localizedDescription;
    return Utf8(log);
}
...
id CompileLibrary(id<MTLDevice> device, const std::string &source, std::string &error) {
    ...
    [options release];
    if (library == nil) error = CompileError(compileError);
    return library;
}
...
id FunctionLike(id library, id<MTLFunction> original, std::string &error, std::string &note) {
    ...
    if (function == nil) {
        error = "specializing '" + Utf8(name) + "' with the function constants the application used failed: "
                + CompileError(specializeError);
        return nil;
    }
    if (note.empty()) note = "specialized with the function constants the original was built with";
    return function;
}
...
id BuildReplacement(id original, const Edit &edit, std::string &error, std::string &note) {
    ...
    if (descriptor == nil) {
        ...
        if (it != g_computeFunctions.end()) computeFunction = [it->second retain];
    }
    ...
    if (IsRenderPipeline(original)) {
        ...
        state = [device newRenderPipelineStateWithDescriptor:(MTLRenderPipelineDescriptor *)descriptor
                                                 ...
                                                 reflection:&reflection
                                                      error:&buildError];
        if (state != nil) args = RenderPipelineArgs((MTLRenderPipelineDescriptor *)descriptor, reflection);
    } else {
...
bool ReplaceShader(uint64_t pipelineId, const std::string &stage, const std::string &source,
    ...
    Edit next;
    {
        ...
        if (it != g_edits.end()) next.stages = it->second.stages;
    }
...
id SubstitutePipelineState(id state) {
    if (state == nil || g_editCount.load(std::memory_order_acquire) == 0) return state;
    std::lock_guard<std::mutex> lock(g_mutex);
...
void RememberComputePipeline(id state, id function) {
    if (state == nil || function == nil || IsInternal()) return;
