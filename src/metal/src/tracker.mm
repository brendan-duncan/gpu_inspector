...
std::string ObjectUpdateMessage(uint64_t id, const std::string& argsJson)
{
    ...
    if (argsJson.size() > 2)
        message += "," + argsJson.substr(1, argsJson.size() - 2);
    return message + "}";
}
...
void CompactOrder()
{
    if (g_order.size() < 2 * g_byId.size() + 1024)
        return;
    std::vector<uint64_t> live;
    live.reserve(g_byId.size());
    for (uint64_t id : g_order)
    {
        if (g_byId.count(id) != 0)
            live.push_back(id);
    }
    g_order.swap(live);
}
...
{
    if (object == nil || data == nullptr || size == 0)
        return;
...
void SendStacktraces(const std::vector<uint64_t>& ids)
{
    ...
    std::vector<std::pair<uint64_t, StackTrace>> stacks;
    {
        ...
        {
            auto it = g_byId.find(id);
            if (it != g_byId.end() && !it->second.stack.empty())
                stacks.emplace_back(id, it->second.stack);
        }
    }
    vkinsp::JsonWriter w;
    w.BeginObject();
...
void TrackLabel(id object)
{
    ...
    std::string message;
    {
        ...
        if (it == g_byPointer.end())
            return;
        auto tracked = g_byId.find(it->second);
        if (tracked == g_byId.end() || tracked->second.label == label)
            return;
...
void UpdateObject(id object, const char* key, const std::string& argsJson)
{
    ...
    std::string message;
    {
        ...
        if (!replaced)
            updates.emplace_back(key, argsJson);
        if (!g_live)
            return;
    ...
    std::vector<std::string> messages;
    {
        ...
        for (uint64_t id : g_order)
        {
            ...
            for (const auto& u : it->second.updates)
                messages.push_back(ObjectUpdateMessage(id, u.second));
        }
        g_live = true;
    }
...
void SendLeakReport()
{
    std::string message;
    {
        ...
        for (uint64_t id : g_order)
        {
            ...
            if (o.type == "MTLCommandQueue" || o.type == "MTLCommandBuffer")
                continue;
            leaked.push_back(&o);
        }
        if (leaked.empty())
