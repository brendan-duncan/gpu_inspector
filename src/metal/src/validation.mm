...
std::string MessageJson(const Entry& e)
{
    ...
    if (!e.objectClass.empty())
    {
        ...
        {
            w.Key("__handle");
            w.String(e.objectHandle);
        }
        w.Key("__class");
        w.String(e.objectClass);
        w.EndObject();
        w.Key("class");
        w.String(e.objectClass);
        w.Key("handle");
...
void SendDeviceLost(id<MTLCommandBuffer> cb, NSError* error, const std::string& message)
{
    ...
    w.Key("call");
    w.String("MTLCommandBuffer completion");
    w.Key("breadcrumbs");
    w.Boolean(breadcrumbs);
    if (!breadcrumbs)
    {
    ...
    {
        w.Key("hungCommand");
        w.String("encoder \"" + faulted + "\"");
    }
    if (!lastCompleted.empty())
    {
        w.Key("lastCompletedCommand");
        w.String("encoder \"" + lastCompleted + "\"");
    }
    w.Key("message");
    w.String(message);
    w.EndObject();
    ...
    if (@available(macOS 11.0, *))
    {
        ...
        if (logs != nil)
        {
            for (id<MTLFunctionLog> log in logs)
            {
                ...
                if (log.encoderLabel != nil)
                    message = "encoder \"" + Utf8(log.encoderLabel) + "\": " + message;
...
bool LooksLikeMetalValidation(NSString* text)
{
    ...
    {
        if ([text rangeOfString:kMarks[i]].location != NSNotFound)
            return true;
    }
    return false;
}
...
{
    if (text == nil || !LooksLikeMetalValidation(text))
        return;
    ...
    if (open.location != NSNotFound)
    {
        ...
        if (close.location != NSNotFound)
            idName = Utf8([text substringWithRange:NSMakeRange(open.location, close.location - open.location + 1)]);
    }
    ...
    const char* objectClass)
{
    ...
    std::string toSend;
    {
        ...
        if (Transport::Get().Connected())
            toSend = MessageJson(g_entries.back());
    }
    if (!toSend.empty())
        Transport::Get().SendJson(std::move(toSend));
}

void WatchCommandBuffer(id commandBuffer)
{
    if (commandBuffer == nil || !Transport::Get().Connected())
        return;
    Internal internal;
...
{
    if (!g_anyDirty || !Transport::Get().Connected())
        return;
    std::string counts;
    {
        std::lock_guard<std::mutex> lock(g_mutex);
...
void SendValidationSnapshot()
{
    ...
    for (std::string& m : messages)
        Transport::Get().SendJson(std::move(m));
}
