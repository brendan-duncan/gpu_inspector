...
bool ScalarOf(MTLDataType type, Scalar *scalar, uint32_t *count) {
    ...
    static const Family kFamilies[] = {
        {MTLDataTypeFloat, {"float", 32}},  {MTLDataTypeHalf, {"half", 16}},
        {MTLDataTypeInt, {"int", 32}},      {MTLDataTypeUInt, {"uint", 32}},
        {MTLDataTypeShort, {"short", 16}},  {MTLDataTypeUShort, {"ushort", 16}},
        {MTLDataTypeChar, {"char", 8}},     {MTLDataTypeUChar, {"uchar", 8}},
...
size_t ValueBytes(MTLDataType type) {
    Scalar scalar;
    uint32_t count = 0;
    if (!ScalarOf(type, &scalar, &count)) return 0;
...
std::string TypeName(MTLDataType type) {
    Scalar scalar;
    uint32_t count = 0;
    if (!ScalarOf(type, &scalar, &count)) return "";
...
double Float16(uint16_t h) {
    ...
    if (exponent == 0) return sign * ldexp((double)fraction / 1024.0, -14);
...
void Remember(id self, MTLDataType type, const void *bytes, uint32_t index, bool hasIndex, NSString *name) {
    ...
    if (self == nil || bytes == nullptr || size == 0) return;
    ConstantValue value;
    value.hasIndex = hasIndex;
    value.index = index;
    if (name != nil && name.UTF8String != nullptr) value.name = name.UTF8String;
    value.type = type;
    ...
    if (list.size() < 512) list.push_back(std::move(value));
}
...
void FCV_setConstantValueAtIndex(id self, SEL _cmd, const void *value, MTLDataType type,
    ...
    if (reentry.outermost()) Remember(self, type, value, (uint32_t)index, true, nil);
...
void FCV_setConstantValueWithName(id self, SEL _cmd, const void *value, MTLDataType type,
                                  NSString *name) {
    Reentry reentry(self, _cmd);
    if (reentry.outermost()) Remember(self, type, value, 0, false, name);
...
void HookValuesClass(Class cls) {
    if (cls == nullptr || !FirstSighting(cls)) return;
...
id FCV_alloc(id self, SEL _cmd) {
    ...
    if (values != nil) HookValuesClass(object_getClass(values));
    return values;
}
...
std::string FunctionConstantsJson(id values) {
    ...
    std::vector<ConstantValue> list;
    {
        ...
        if (it == g_values.end() || it->second.empty()) return "";
        list = it->second;
    }
    vkinsp::JsonWriter w;
    w.BeginArray();
    for (const ConstantValue &c : list) {
        Scalar scalar;
        uint32_t count = 0;
        if (!ScalarOf(c.type, &scalar, &count)) continue;
        w.BeginObject();
        if (c.hasIndex) { w.Key("index"); w.Uint(c.index); }
        if (!c.name.empty()) { w.Key("name"); w.String(c.name); }
        w.Key("type"); w.String(TypeName(c.type));
        w.Key("value");
        if (count == 1) {
