...
enum class Run : int { Cover = 0, Passed = 1, Wireframe = 2, Stencil = 3, BackFace = 4, Count = 5 };
...
public:
    OverlayRun(id<MTLDevice> device, id<MTLFunction> mask, Run run, uint32_t drawIndex,
        ...
        : device_(device), mask_(mask), run_(run), wanted_(drawIndex),
          depthFormat_(depthFormat), stencilFormat_(stencilFormat), out_(out) {}
    ...
    void BindPipeline(id<MTLRenderCommandEncoder> encoder, id state) override {
        ...
        if (mask.pipeline == nil) {
            if (out_.note.empty() && !mask.error.empty()) out_.note = "the draw was not drawn: " + mask.error;
            return;
        }
        Keep(mask.pipeline);
        bound_ = true;
...
private:
    ...
    void Keep(id object) {
        if (kept_.insert((__bridge const void *)object).second) out_.keep.emplace_back(object);
    }
...
id ReadBack(id<MTLCommandBuffer> commandBuffer, id<MTLTexture> target, uint32_t width, uint32_t height) {
    ...
    [blit copyFromTexture:target sourceSlice:0 sourceLevel:0
       ...
       destinationBytesPerRow:width destinationBytesPerImage:(NSUInteger)width * height];
    [blit endEncoding];
    return staging;
}
...
void MeasureDrawOverlay(OverdrawPass &pass) {
  @autoreleasepool {
    ...
    // drawn, so a run that fails still names the draw it was about.
    {
        ...
        for (const OverdrawPass::Segment &segment : pass.segments) {
            for (const LoggedOp &logged : segment.ops) {
                if (logged.key.policy != OpPolicy::Draw) continue;
                if (ordinal++ != out.drawIndex) continue;
    ...
    const bool hasDepthStencil = !pass.multisampled
                                 && (pass.depthFormat != MTLPixelFormatInvalid
                                     || pass.stencilFormat != MTLPixelFormatInvalid);
    out.measured = true;
    out.wireframe = true;
    ...
    for (int run = 0; run < kRuns; run++) {
        ...
        if (tests && !hasDepthStencil) continue;
        ...
        if (kind == Run::Stencil && pass.stencilFormat == MTLPixelFormatInvalid) continue;
        ...
        if (tests) {
            if (pass.depthFormat != MTLPixelFormatInvalid) {
                ...
                if (depth != nil) out.keep.emplace_back(depth);
            ...
            if (pass.combined) {
                ...
                if (stencil != nil) out.keep.emplace_back(stencil);
        ...
        for (OverdrawPass::Segment &segment : pass.segments) {
            ...
            replay.BeginEncoder();
            for (LoggedOp &logged : segment.ops) logged.op(encoder, replay);
            [encoder endEncoding];
            first = false;
        }
        if (!ok) {
        ...
        if (kind == Run::Passed) out.depthTested = replay.drawn();
        if (kind == Run::Stencil) out.stencilTested = replay.drawn();
    ...
    for (PendingOverlay &p : pending) {
        ...
        if (p.measured) {
            ...
            for (size_t i = 0; i < texels; i++) {
                ...
                if (test != nullptr ? test[i] != 0 : isCovered) bits |= kPassed;
                if (lines != nullptr && lines[i] != 0) bits |= kWireframe;
                if (stencilRun != nullptr && stencilRun[i] != 0) bits |= kStencilPassed;
                // Only where the culling actually removed what would have been drawn: every pixel
                // of a closed mesh has a back face behind it.
                if (back != nullptr && back[i] != 0 && !isCovered) bits |= kBackFacing;
                merged[i] = bits;
                if (bits & kCovered) covered++;
                if (bits & kPassed) passed++;
                if (bits & kWireframe) wire++;
                if (bits & kStencilPassed) stencilPassed++;
                if (bits & kBackFacing) backFacing++;
            }
        ...
        w.Key("command"); w.Uint(p.command);
        w.Key("method"); w.String(p.method);
        w.Key("frame"); w.Uint(p.frame);
        w.Key("commandBuffer"); w.Uint(p.commandBufferId);
        w.Key("passIndex"); w.Uint(p.passIndex);
        w.Key("drawIndex"); w.Uint(p.drawIndex);
        w.Key("measured"); w.Boolean(p.measured);
        w.Key("width"); w.Uint(p.width);
        w.Key("height"); w.Uint(p.height);
        w.Key("fragments"); w.Uint(fragments);
        w.Key("pixelsCovered"); w.Uint(covered);
        w.Key("pixelsPassed"); w.Uint(passed);
        w.Key("pixelsRejected"); w.Uint(p.depthTested && covered > passed ? covered - passed : 0);
        w.Key("pixelsStencilRejected"); w.Uint(p.stencilTested && covered > stencilPassed ? covered - stencilPassed : 0);
        w.Key("pixelsBackFacing"); w.Uint(backFacing);
        w.Key("depthTested"); w.Boolean(p.depthTested);
        w.Key("wireframe"); w.Boolean(p.wireframe && wire != 0);
        w.Key("stencilTested"); w.Boolean(p.stencilTested);
        w.Key("backFaceTested"); w.Boolean(p.backFaceTested);
        w.Key("size"); w.Uint(merged.size());
        if (!p.note.empty()) { w.Key("note"); w.String(p.note); }
        w.EndObject();
        ...
        if (!merged.empty()) {
            ...
            h.Key("command"); h.Uint(p.command);
            h.Key("size"); h.Uint(merged.size());
            h.EndObject();
