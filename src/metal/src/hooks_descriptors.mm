...
{
    if (object == nil || ![object respondsToSelector:@selector(label)])
        return "";
...
void WriteLinkedFunctions(Args& a, id descriptor)
{
    ...
    if (descriptor == nil || ![descriptor respondsToSelector:sel])
        return;
    ...
    auto list = [&](const char* key) {
        ...
        if (![linked respondsToSelector:each])
            return;
...
void WriteFunction(Args& a, const char* key, id<MTLFunction> function)
{
    ...
    if (IdOf(function) != 0)
        a.ref(key, function, "MTLFunction");
    else
        a.s(key, function.name);
    ...
    NSUInteger index)
{
    ...
    w.Uint(index);
    w.Key("pixelFormat");
    WriteEnum(w, PixelFormatEnumName(a.pixelFormat), (uint64_t)a.pixelFormat);
    w.Key("blendingEnabled");
    w.Boolean(a.blendingEnabled);
    if (a.blendingEnabled)
    {
        w.Key("sourceRGBBlendFactor");
        w.Uint((uint64_t)a.sourceRGBBlendFactor);
        w.Key("destinationRGBBlendFactor");
        w.Uint((uint64_t)a.destinationRGBBlendFactor);
        w.Key("rgbBlendOperation");
        w.Uint((uint64_t)a.rgbBlendOperation);
        w.Key("sourceAlphaBlendFactor");
        w.Uint((uint64_t)a.sourceAlphaBlendFactor);
        w.Key("destinationAlphaBlendFactor");
        w.Uint((uint64_t)a.destinationAlphaBlendFactor);
        w.Key("alphaBlendOperation");
        w.Uint((uint64_t)a.alphaBlendOperation);
    }
    w.Key("writeMask");
    w.Uint((uint64_t)a.writeMask);
    w.EndObject();
}
...
void WriteVertexDescriptor(vkinsp::JsonWriter& w, MTLVertexDescriptor* v)
{
    ...
    for (NSUInteger i = 0; i < 31; i++)
    {
        ...
        switch (layout.stepFunction)
        {
            ...
            case MTLVertexStepFunctionPerPatchControlPoint:
                w.String("MTLVertexStepFunctionPerPatchControlPoint");
                break;
            default: w.Uint((uint64_t)layout.stepFunction); break;
        }
        w.Key("stepRate");
        w.Uint(layout.stepRate);
        w.EndObject();
    }
    w.EndArray();
    w.Key("attributes");
    ...
    for (NSUInteger i = 0; i < 31; i++)
    {
        ...
        if (attribute == nil || attribute.format == MTLVertexFormatInvalid)
            continue;
        w.BeginObject();
        w.Key("index");
        w.Uint(i);
        w.Key("format");
        WriteEnum(w, VertexFormatEnumName(attribute.format), (uint64_t)attribute.format);
        w.Key("vkFormat");
        w.String(VertexFormatCanonicalName(attribute.format));
        w.Key("offset");
        w.Uint(attribute.offset);
        w.Key("bufferIndex");
        w.Uint(attribute.bufferIndex);
        w.EndObject();
    }
    w.EndArray();
    w.EndObject();
...
void WriteStencil(vkinsp::JsonWriter& w, MTLStencilDescriptor* s)
{
    ...
    w.BeginObject();
    w.Key("stencilCompareFunction");
    w.Uint((uint64_t)s.stencilCompareFunction);
    w.Key("stencilFailureOperation");
    w.Uint((uint64_t)s.stencilFailureOperation);
    w.Key("depthFailureOperation");
    w.Uint((uint64_t)s.depthFailureOperation);
    w.Key("depthStencilPassOperation");
    w.Uint((uint64_t)s.depthStencilPassOperation);
    w.Key("readMask");
    w.Uint(s.readMask);
    w.Key("writeMask");
    w.Uint(s.writeMask);
...
{
    w.Key("texture");
    WriteRef(w, a.texture, "MTLTexture");
    w.Key("level");
    w.Uint(a.level);
    w.Key("slice");
    w.Uint(a.slice);
    ...
    w.Key("loadAction");
    WriteEnum(w, LoadActionEnumName(a.loadAction), (uint64_t)a.loadAction);
    w.Key("storeAction");
    WriteEnum(w, StoreActionEnumName(a.storeAction), (uint64_t)a.storeAction);
    if (a.storeActionOptions != MTLStoreActionOptionNone)
    {
        w.Key("storeActionOptions");
        w.Uint((uint64_t)a.storeActionOptions);
    }
    if (a.resolveTexture != nil)
    {
        w.Key("resolveTexture");
        WriteRef(w, a.resolveTexture, "MTLTexture");
        w.Key("resolveLevel");
        w.Uint(a.resolveLevel);
        w.Key("resolveSlice");
        w.Uint(a.resolveSlice);
    }
}
...
std::string TextureDescriptorArgs(MTLTextureDescriptor* d, id texture)
{
    ...
    a.e("textureType", TextureTypeEnumName(d.textureType), (uint64_t)d.textureType)
        ...
        .u("height", d.height)
        .u("depth", d.depth)
        .u("mipmapLevelCount", d.mipmapLevelCount)
        .u("sampleCount", d.sampleCount)
...
std::string TextureObjectArgs(id<MTLTexture> t)
{
    ...
    a.e("textureType", TextureTypeEnumName(t.textureType), (uint64_t)t.textureType)
        ...
        .u("height", t.height)
        .u("depth", t.depth)
        .u("mipmapLevelCount", t.mipmapLevelCount)
        .u("sampleCount", t.sampleCount)
    ...
    NSRange levels, NSRange slices)
{
    ...
    if (view != nil)
    {
        a.u("width", view.width).u("height", view.height).u("depth", view.depth).u("mipmapLevelCount", view.mipmapLevelCount).u("arrayLength", view.arrayLength).c("usage", UsageFlags(view.usage).c_str());
        WriteGpuIds(a, view);
    }
    return a.str();
}
...
std::string RenderPipelineArgs(MTLRenderPipelineDescriptor* d, MTLRenderPipelineReflection* reflection)
{
    ...
    for (NSUInteger i = 0; i < 8; i++)
    {
        ...
        if (c == nil || c.pixelFormat == MTLPixelFormatInvalid)
            continue;
...
std::string FunctionArgs(id<MTLFunction> function, id constantValues)
{
    ...
    if (!constants.empty())
        a.raw("constantValues", constants);
...
std::string SamplerArgs(MTLSamplerDescriptor* d, id sampler)
{
    Args a;
    a.s("label", d.label)
        .u("minFilter", (uint64_t)d.minFilter)
        .u("magFilter", (uint64_t)d.magFilter)
        .u("mipFilter", (uint64_t)d.mipFilter)
        .u("maxAnisotropy", d.maxAnisotropy)
        .u("sAddressMode", (uint64_t)d.sAddressMode)
        .u("tAddressMode", (uint64_t)d.tAddressMode)
        ...
        .d("lodMinClamp", d.lodMinClamp)
        .d("lodMaxClamp", d.lodMaxClamp)
...
std::string RenderPassArgs(MTLRenderPassDescriptor* d)
{
    ...
    for (NSUInteger i = 0; i < 8; i++)
    {
        ...
        if (c.loadAction == MTLLoadActionClear)
        {
            ...
            w.BeginArray();
            w.Double(color.red);
            w.Double(color.green);
            w.Double(color.blue);
            w.Double(color.alpha);
            w.EndArray();
        }
        w.EndObject();
    }
    ...
    else
    {
        ...
        w.Key("clearDepth");
        w.Double(d.depthAttachment.clearDepth);
        w.EndObject();
    }
    ...
    else
    {
        ...
        w.Key("clearStencil");
        w.Uint(d.stencilAttachment.clearStencil);
        w.EndObject();
    }
    ...
    if (d.renderTargetArrayLength != 0)
        a.u("renderTargetArrayLength", d.renderTargetArrayLength);
    ...
    if (d.defaultRasterSampleCount != 0)
        a.u("defaultRasterSampleCount", d.defaultRasterSampleCount);
    if (d.imageblockSampleLength != 0)
        a.u("imageblockSampleLength", d.imageblockSampleLength);
    if (d.threadgroupMemoryLength != 0)
        a.u("threadgroupMemoryLength", d.threadgroupMemoryLength);
