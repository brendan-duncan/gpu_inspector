...
void Send(id object, const char* selector, NSUInteger value)
{
    ...
    if ([object respondsToSelector:sel])
        ((void (*)(id, SEL, NSUInteger))objc_msgSend)(object, sel, value);
}
...
void SendBool(id object, const char* selector, BOOL value)
{
    SEL sel = sel_registerName(selector);
    if ([object respondsToSelector:sel])
        ((void (*)(id, SEL, BOOL))objc_msgSend)(object, sel, value);
}
...
public:
    ...
    void BindPipeline(id<MTLRenderCommandEncoder> encoder, id state) override
    {
        ...
        {
            if (out_.note.empty() && !copy.error.empty())
                out_.note = "draws not counted: " + copy.error;
            return;
        }
        [encoder setRenderPipelineState:(id<MTLRenderPipelineState>)copy.pipeline];
        if (kept_.insert((__bridge const void*)copy.pipeline).second)
            out_.keep.emplace_back(copy.pipeline);
        bound_ = true;
    }
...
void MeasurePass(OverdrawPass& pass)
{
    @autoreleasepool
    {
        ...
        for (int mode = 0; mode < 2; mode++)
        {
            ...
            if (m.depthTested && !hasDepthStencil)
                m.note = "the pass has no depth or stencil attachment";
            ...
            if (pass.layered)
                m.note += std::string(m.note.empty() ? "" : "; ") + "a layered pass: every layer's fragments land in one count";
            ...
            for (OverdrawPass::Segment& segment : pass.segments)
            {
                ...
                for (LoggedOp& op : segment.ops)
                    op.op(encoder, measurement);
                [encoder endEncoding];
                first = false;
            }
            if (m.measured && !first)
    ...
    MTLPixelFormat depthFormat, MTLPixelFormat stencilFormat)
{
    ...
    else
    {
        ...
        if ([descriptor respondsToSelector:rasterization] && !((BOOL(*)(id, SEL))objc_msgSend)(descriptor, rasterization))
        {
        ...
        else
        {
            ...
            if (variant == PipelineVariant::OverdrawCount || variant == PipelineVariant::HistoryCover || variant == PipelineVariant::OverlayMask)
            {
            ...
            if ([descriptor respondsToSelector:archives])
                ((void (*)(id, SEL, id))objc_msgSend)(descriptor, archives, nil);
...
{
    if (state == nil)
        variant = DepthStencilVariant::None;
    ...
    MTLDepthStencilDescriptor* descriptor = nil;
    {
        ...
        if (variant != DepthStencilVariant::None)
        {
            ...
            if (it == g_depthStencilDescriptors.end())
                return nil;
            descriptor = [it->second copy];
        }
    }
    if (descriptor == nil)
        descriptor = [[MTLDepthStencilDescriptor alloc] init];   // Always, no writes, no stencil
...
{
    if (!OverdrawActive() || commandBuffer == nil || descriptor == nil)
        return nullptr;
    bool overdraw = false;
    {
        std::lock_guard<std::mutex> lock(g_mutex);
    ...
    for (NSUInteger i = 0; i < 8; i++)
    {
        ...
        if (c.texture.sampleCount > 1)
            pass->multisampled = true;
    }
    ...
    if (depth.texture != nil)
    {
        ...
        if (depth.texture.sampleCount > 1)
            pass->multisampled = true;
    }
    if (stencil.texture != nil)
    {
        ...
        if (stencil.texture.sampleCount > 1)
            pass->multisampled = true;
    }
    ...
    if (copyDepth)
        pass->depthStart = copy(depth);
    if (copyStencil)
        pass->stencilStart = copy(stencil);
    [blit endEncoding];
    // A copy that could not be made starts from the clear value instead.
    if (pass->depthStart == nil)
        pass->depthLoads = false;
    if (pass->stencilStart == nil && !pass->combined)
        pass->stencilLoads = false;
    if (pass->combined && pass->depthStart == nil)
        pass->stencilLoads = false;
    return pass;
}
...
void NotePassBeginCommand(id encoder, uint32_t command)
{
    ...
    if (it != g_passes.end())
        it->second->beginCommand = command;
}

void NoteOverdrawSubEncoder(id parent, id encoder)
{
    if (!OverdrawActive() || parent == nil || encoder == nil)
        return;
    std::lock_guard<std::mutex> lock(g_mutex);
...
void EndOverdrawPass(id encoder)
{
    ...
    std::shared_ptr<OverdrawPass> pass;
    {
        ...
        {
            if (s->second == (__bridge const void*)encoder)
                s = g_subEncoders.erase(s);
            else
                ++s;
        }
    }
...
void ForgetRenderPipeline(id object)
{
    ...
    {
        std::lock_guard<std::mutex> lock(g_stateMutex);
        if (g_descriptors.empty() && g_depthStencilDescriptors.empty())
            return;
        ...
        if (auto it = g_descriptors.find(key); it != g_descriptors.end())
        {
            ...
            for (auto c = g_derived.begin(); c != g_derived.end();)
            {
                ...
                {
                    if (c->second.pipeline != nil)
                        released.push_back(c->second.pipeline);
                    c = g_derived.erase(c);
                }
                else
                {
...
void StartOverdrawCapture(bool overdraw, bool recordPasses, uint64_t maxDataSize)
{
    ...
    for (PendingMeasurement& m : pending)
        [m.staging release];
    if (overdraw)
...
void SendOverdraw()
{
    ...
    for (size_t i = 0; i < pending.size(); i++)
    {
        ...
        if (!m.measured)
            continue;                 // no counts: the note says why
        if (m.empty)
        {
