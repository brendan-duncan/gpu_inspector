        ...
        if (texture != nil)
        {
            if (IdOf(texture) == 0)
            {
                ...
                a.e("pixelFormat", PixelFormatEnumName(texture.pixelFormat), (uint64_t)texture.pixelFormat)
                    ...
                    .u("width", texture.width)
                    .u("height", texture.height)
                    .u("mipmapLevelCount", texture.mipmapLevelCount)
                    .u("arrayLength", texture.arrayLength)
...
{
    if (Hud::Get().Enabled())
        Hud::Get().NoteUnsupportedPresentPath();
...
{
    Reentry reentry(self, _cmd);
    ORIG(void (*)(id, SEL, CFTimeInterval))
    (self, _cmd, time);
    if (reentry.outermost() && OnDrawablePresent(self))
    {
...
void FT_setFunction(id self, SEL _cmd, id handle, NSUInteger index)
{
    ...
    (self, _cmd, handle, index);
    if (reentry.outermost())
        NoteTableFunction(self, index, handle);
}
...
void FT_setOpaqueTriangleFunction(id self, SEL _cmd, NSUInteger signature, NSUInteger index)
{
    ...
    (self, _cmd, signature, index);
    if (reentry.outermost())
        NoteTableOpaqueFunction(self, index, signature, "triangle");
}

void FT_setOpaqueTriangleFunctionRange(id self, SEL _cmd, NSUInteger signature, NSRange range)
{
    Reentry reentry(self, _cmd);
    ORIG(void (*)(id, SEL, NSUInteger, NSRange))
    (self, _cmd, signature, range);
    if (!reentry.outermost())
        return;
    for (NSUInteger i = 0; i < range.length; i++)
    {
...
void FT_setOpaqueCurveFunction(id self, SEL _cmd, NSUInteger signature, NSUInteger index)
{
    ...
    (self, _cmd, signature, index);
    if (reentry.outermost())
        NoteTableOpaqueFunction(self, index, signature, "curve");
}
...
void FT_setBuffer(id self, SEL _cmd, id buffer, NSUInteger offset, NSUInteger index)
{
    ...
    if (reentry.outermost())
        NoteTableBuffer(self, index, buffer, offset);
}
...
void FT_setVisibleFunctionTable(id self, SEL _cmd, id table, NSUInteger index)
{
    Reentry reentry(self, _cmd);
    ORIG(void (*)(id, SEL, id, NSUInteger))
    (self, _cmd, table, index);
    if (reentry.outermost())
        NoteTableVisibleTable(self, index, table);
}
...
id D_newFence(id self, SEL _cmd)
{
    ...
    if (reentry.outermost())
        Track(fence, "MTLFence", "newFence", self, {});
    return fence;
}
...
id D_newEvent(id self, SEL _cmd)
{
    ...
    if (reentry.outermost())
        Track(event, "MTLEvent", "newEvent", self, {});
    return event;
}
...
id D_newSharedEvent(id self, SEL _cmd)
{
    ...
    id event = ORIG(id(*)(id, SEL))(self, _cmd);
    if (reentry.outermost())
        Track(event, "MTLSharedEvent", "newSharedEvent", self, {});
    return event;
}
...
id D_newSharedEventWithHandle(id self, SEL _cmd, id handle)
{
    ...
    if (reentry.outermost())
        Track(event, "MTLSharedEvent", "newSharedEventWithHandle:", self, {});
    return event;
}
...
id D_newArgumentEncoderWithArguments(id self, SEL _cmd, NSArray* arguments)
{
    ...
    if (reentry.outermost() && encoder != nil)
    {
        ...
        Args a;
        a.u("argumentCount", arguments.count).u("encodedLength", e.encodedLength).u("alignment", e.alignment);
    ...
    NSUInteger maxCount, MTLResourceOptions options)
{
    ...
    if (reentry.outermost())
    {
        ...
        a.u("commandTypes", (uint64_t)descriptor.commandTypes)
            ...
            .u("maxCommandCount", maxCount)
            .u("options", (uint64_t)options);
    ...
    if (reentry.outermost())
    {
        ...
        if (library != nil && data != nil)
        {
            ...
            if (bytes != nullptr && size != 0)
                AddBlob(library, "metallib", bytes, size);
            if (contiguous != nil)
...
void AddLibraryFileBlob(id library, NSURL* url)
{
    ...
    if (data.length != 0)
        AddBlob(library, "metallib", data.bytes, data.length);
}
...
id D_newLibraryWithFile(id self, SEL _cmd, NSString* path, NSError** error)
{
    ...
    if (reentry.outermost())
    {
        ...
        if (path != nil)
            AddLibraryFileBlob(library, [NSURL fileURLWithPath:path]);
    }
    HookLibraryClass(library);
    return library;
}
...
void TrackSourceLibrary(id device, id library, NSString* source, const char* cmd)
{
    ...
    if (utf8 != nullptr)
        AddBlob(library, "Metal Shading Language", utf8, strlen(utf8));
    HookLibraryClass(library);
}
...
void R_makeAliasable(id self, SEL _cmd)
{
    ...
    (self, _cmd);
    if (reentry.outermost())
        UpdateObject(self, "aliasable", Args().b("aliasable", true).str());
}
