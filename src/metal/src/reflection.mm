...
uint64_t WriteStruct(vkinsp::JsonWriter& w, MTLStructType* s, const char* name, uint64_t knownSize)
{
    ...
    w.String("struct");
    w.Key("name");
    w.String(name != nullptr ? name : "struct");
    w.Key("members");
    w.BeginArray();
    for (MTLStructMember* m in s.members)
    {
        w.BeginObject();
        w.Key("name");
        w.String(m.name == nil ? "" : m.name.UTF8String);
        w.Key("offset");
        w.Uint(m.offset);
        w.Key("type");
    ...
    MTLArrayType* arrayType, MTLPointerType* pointerType, uint64_t knownSize)
{
    ...
    if (type == MTLDataTypeArray && arrayType != nil)
    {
        ...
        w.Key("count");
        w.Uint(arrayType.arrayLength);
        w.Key("stride");
        w.Uint(stride);
        w.Key("size");
        w.Uint(stride * arrayType.arrayLength);
        w.EndObject();
    ...
    if (type == MTLDataTypePointer && pointerType != nil)
    {
        ...
        if (pointerType.elementType == MTLDataTypeStruct)
            name += "struct";
        else if (ScalarOf(pointerType.elementType, &s, &n))
            name += std::string(s.base) + (n > 1 ? std::to_string(n) : "");
        else
            name += "T";
        name += " *";
        w.Key("name");
        ...
        w.Key("size");
        w.Uint(8);
        if (pointerType.elementIsArgumentBuffer)
        {
            w.Key("argumentBuffer");
            w.Boolean(true);
        }
...
void WriteStage(vkinsp::JsonWriter& w, NSArray* bindings)
{
    ...
    {
        if (UIntProperty(b, @selector(type)) != kBindingBuffer)
            continue;
        w.BeginObject();
        w.Key("index");
        w.Uint(UIntProperty(b, @selector(index)));
        NSString* name = ObjectProperty(b, @selector(name));
        w.Key("name");
        w.String(name == nil ? "" : name.UTF8String);
        w.Key("access");
        w.String(AccessName(UIntProperty(b, @selector(access))));
        ...
        w.Uint(dataSize);
        w.Key("alignment");
        w.Uint(UIntProperty(b, @selector(bufferAlignment)));
    ...
    for (id b in bindings)
    {
        if (UIntProperty(b, @selector(type)) != kBindingTexture)
            continue;
        w.BeginObject();
        w.Key("index");
        w.Uint(UIntProperty(b, @selector(index)));
        ...
        w.Key("textureType");
        w.String(TextureTypeEnumName(textureType));
        Scalar scalar;
        uint32_t count = 0;
        const MTLDataType dataType = (MTLDataType)UIntProperty(b, @selector(textureDataType));
        w.Key("dataType");
        if (ScalarOf(dataType, &scalar, &count))
            w.String(scalar.base);
        else
            w.Uint((uint64_t)dataType);
        ...
        w.Boolean(depth);
        w.Key("arrayLength");
        w.Uint(UIntProperty(b, @selector(arrayLength)));
        w.EndObject();
    }
    w.EndArray();
    ...
    for (id b in bindings)
    {
        if (UIntProperty(b, @selector(type)) != kBindingSampler)
            continue;
        w.BeginObject();
        w.Key("index");
        w.Uint(UIntProperty(b, @selector(index)));
    ...
    for (id b in bindings)
    {
        if (UIntProperty(b, @selector(type)) != kBindingThreadgroupMemory)
            continue;
        w.BeginObject();
        w.Key("index");
        w.Uint(UIntProperty(b, @selector(index)));
        ...
        w.String(name == nil ? "" : name.UTF8String);
        w.Key("dataSize");
        w.Uint(UIntProperty(b, @selector(threadgroupMemoryDataSize)));
        w.Key("alignment");
        w.Uint(UIntProperty(b, @selector(threadgroupMemoryAlignment)));
        w.EndObject();
    }
    w.EndArray();
    ...
    for (id b in bindings)
    {
        ...
        if (type == kBindingBuffer || type == kBindingTexture || type == kBindingSampler || type == kBindingThreadgroupMemory)
            continue;
        w.BeginObject();
        w.Key("index");
        w.Uint(UIntProperty(b, @selector(index)));
...
NSArray* StageBindings(id reflection, const char* bindingsSelector, const char* argumentsSelector)
{
    ...
    if (bindings == nil)
        bindings = ObjectProperty(reflection, sel_registerName(argumentsSelector));
    return bindings;
}

void WriteStageIfAny(vkinsp::JsonWriter& w, const char* stage, NSArray* bindings)
{
    if (bindings == nil || bindings.count == 0)
        return;
    w.Key(stage);
