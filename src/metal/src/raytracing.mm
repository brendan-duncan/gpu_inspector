    ...
    std::vector<uint64_t>* captured)
{
    ...
    w.EndObject();
    if (inputs != nullptr)
        inputs->push_back({key, geometry, IdOf(buffer), offset, size});
}
...
void WriteMotionKeyframes(JsonWriter& w, const char* key, id keyframes, uint64_t stride, uint64_t count,
    ...
    std::vector<uint64_t>* captured)
{
    if (![keyframes isKindOfClass:[NSArray class]])
        return;
    w.Key(key);
    w.BeginArray();
    ...
    std::vector<StructureInput>* inputs, std::vector<uint64_t>* captured)
{
    ...
    if (label != nil)
        w.Key("label"), w.String([(NSString*)label UTF8String]);
    w.Key("intersectionFunctionTableOffset");
    w.Uint(UIntProp(g, "intersectionFunctionTableOffset"));
    w.Key("opaque");
    w.Boolean(BoolProp(g, "opaque"));
        ...
        IsKind(g, "MTLAccelerationStructureMotionTriangleGeometryDescriptor"))
    {
        ...
        const NSUInteger format = [g respondsToSelector:sel_registerName("vertexFormat")]
            ? UIntProp(g, "vertexFormat")
            : (NSUInteger)MTLAttributeFormatFloat3;
        ...
        w.Uint(stride);
        w.Key("vertexFormat");
        WriteEnum(w, VertexFormatEnumName((MTLVertexFormat)format), format);
        ...
        w.Key("vkFormat");
        w.String(VertexFormatCanonicalName((MTLVertexFormat)format));
        w.Key("indexType");
        WriteEnum(w, IndexTypeEnumName((MTLIndexType)indexType), indexType);
        ...
        IsKind(g, "MTLAccelerationStructureMotionBoundingBoxGeometryDescriptor"))
    {
        ...
        w.Key("boundingBoxCount");
        w.Uint(primitives);
        w.Key("boundingBoxStride");
        w.Uint(stride);
        if (motion)
        {
        ...
        IsKind(g, "MTLAccelerationStructureMotionCurveGeometryDescriptor"))
    {
        ...
        w.Key("segmentCount");
        w.Uint(primitives);
        w.Key("segmentControlPointCount");
        w.Uint(UIntProp(g, "segmentControlPointCount"));
        w.Key("controlPointCount");
        w.Uint(UIntProp(g, "controlPointCount"));
        w.Key("controlPointStride");
        w.Uint(UIntProp(g, "controlPointStride"));
        const NSUInteger cpFormat = UIntProp(g, "controlPointFormat");
        w.Key("controlPointFormat");
        WriteEnum(w, VertexFormatEnumName((MTLVertexFormat)cpFormat), cpFormat);
        const NSUInteger curveType = UIntProp(g, "curveType");
        w.Key("curveType");
        WriteEnum(w, CurveTypeEnumName(curveType), curveType);
        const NSUInteger basis = UIntProp(g, "curveBasis");
        w.Key("curveBasis");
        WriteEnum(w, CurveBasisEnumName(basis), basis);
        const NSUInteger caps = UIntProp(g, "curveEndCaps");
        w.Key("curveEndCaps");
        WriteEnum(w, CurveEndCapsEnumName(caps), caps);
        w.Key("indexType");
        WriteEnum(w, IndexTypeEnumName((MTLIndexType)indexType), indexType);
        if (motion)
        {
    ...
    if (primitiveData != nil)
    {
        ...
        w.Key("primitiveDataStride");
        w.Uint(stride);
        w.Key("primitiveDataElementSize");
        w.Uint(UIntProp(g, "primitiveDataElementSize"));
    ...
    std::vector<uint64_t>* captured)
{
    ...
    w.Key("usage");
    w.String(AccelerationStructureUsageFlags(usage));
    uint64_t primitives = 0;
    ...
    if (IsKind(descriptor, "MTLInstanceAccelerationStructureDescriptor"))
    {
        ...
        {
            for (id s in (NSArray*)instanced)
                WriteRef(w, s, "MTLAccelerationStructure");
        }
        w.EndArray();
        ...
        {
            w.Key("motionTransformCount");
            w.Uint(UIntProp(descriptor, "motionTransformCount"));
    ...
    else
    {
        ...
        if ([descriptor respondsToSelector:sel_registerName("motionKeyframeCount")])
        {
            ...
            if (keyframes > 1)
            {
                ...
                w.Key("motionStartTime");
                w.Double(FloatProp(descriptor, "motionStartTime"));
                w.Key("motionEndTime");
                w.Double(FloatProp(descriptor, "motionEndTime"));
            }
        }
        w.Key("geometries");
        w.BeginArray();
        ...
        {
            uint32_t index = 0;
            for (id g in (NSArray*)geometries)
                primitives += WriteGeometry(w, g, index++, encoder, inputs, captured);
        }
        w.EndArray();
    }
    w.Key("primitiveCount");
    ...
    const std::vector<uint64_t>& captureIds)
{
    ...
    for (size_t i = 0; i < inputs.size(); i++)
    {
        w.BeginObject();
        w.Key("geometry");
        w.Uint(inputs[i].geometry);
        w.Key("field");
        w.String(inputs[i].field);
        w.Key("buffer");
        w.Uint(inputs[i].bufferId);
        w.Key("offset");
        w.Uint(inputs[i].offset);
        if (i < captureIds.size() && captureIds[i] != 0)
        {
            w.Key("capture");
            w.Uint(captureIds[i]);
        }
    ...
    MTLAccelerationStructureDescriptor* descriptor)
{
    ...
    if (structureId == 0 || descriptor == nil)
        return;
    ...
    id scratch, NSUInteger scratchOffset, id source)
{
    ...
    if (scratch != nil)
        a.ref("scratchBuffer", scratch, "MTLBuffer").u("scratchBufferOffset", scratchOffset);
    ...
    if (structureId != 0)
    {
        ...
        w.String(method);
        w.Key("mode");
        w.String(refit ? "REFIT" : "BUILD");
        w.Key("resultSize");
        w.Uint(UIntProp(destination, "size"));
        w.Key("primitiveCount");
        w.Uint(primitives);
        w.Key("descriptor");
        w.Raw(DescriptorJson(descriptor, nil, nullptr, nullptr, nullptr));
        w.EndObject();
        w.EndObject();
    ...
    id buffer, NSUInteger offset)
{
    Args a;
    if (destination != nil)
        a.ref("destinationAccelerationStructure", destination, "MTLAccelerationStructure");
    a.ref("sourceAccelerationStructure", source, "MTLAccelerationStructure");
    if (buffer != nil)
        a.ref("buffer", buffer, "MTLBuffer").u("offset", offset);
    ...
    if (from != 0 && to != 0)
    {
        ...
        w.String(method);
        w.Key("mode");
        w.String(copy.mode.empty() ? "BUILD" : copy.mode.c_str());
        w.Key("copiedFrom");
        w.Uint(from);
        w.Key("resultSize");
        w.Uint(UIntProp(destination, "size"));
        w.Key("primitiveCount");
        w.Uint(copy.primitives);
        w.Key("descriptor");
        w.Raw(copy.descriptor);
        w.EndObject();
        w.EndObject();
        UpdateObject(destination, "build", w.str());
        std::lock_guard<std::mutex> lock(g_mutex);
...
void ReadBackEarlierStructures(id encoder, uint64_t captureSerial)
{
    ...
    {
        std::lock_guard<std::mutex> lock(g_mutex);
        if (g_readBackSerial == captureSerial)
            return;
        ...
        for (const auto& entry : g_structures)
        {
            ...
            if (entry.second.capture == captureSerial)
                continue;
            if (entry.second.inputs.empty())
                continue;
    ...
    for (const auto& entry : pending)
    {
        ...
        for (const StructureInput& in : record.inputs)
        {
            ...
            const uint64_t captureId = buffer != nil
                ? QueueBufferCapture(encoder, buffer, in.offset, in.size, /*whole=*/true)
                : 0;
            captured.push_back(captureId);
            any = any || captureId != 0;
        }
        ...
        w.Key("method");
        w.String(record.method);
        w.Key("mode");
        w.String(record.mode.empty() ? "BUILD" : record.mode.c_str());
        w.Key("primitiveCount");
        w.Uint(record.primitives);
        w.Key("descriptor");
        w.Raw(record.descriptor);
        w.Key("inputs");
        w.Raw(InputsJson(record.inputs, captured));
        w.EndObject();
        w.EndObject();
...
void ReadBackTableBuffers(id encoder, uint64_t captureSerial)
{
    ...
    for (const auto& entry : pending)
    {
        ...
        for (const auto& bound : entry.second.buffers)
        {
            ...
            const uint64_t captureId = buffer != nil
                ? QueueBufferCapture(encoder, buffer, bound.second.second, 0, /*whole=*/true)
                : 0;
            if (captureId != 0)
                captured.push_back({bound.first, captureId});
        }
        if (captured.empty())
            continue;
        JsonWriter w;
    ...
    NSUInteger functionCount)
{
    ...
    if (pipeline != nil)
        a.ref("pipeline", pipeline, IdOf(pipeline) != 0 ? "MTLComputePipelineState" : "MTLComputePipelineState");
    WriteGpuIds(a, table);
...
void SendTable(id table, uint64_t tableId)
{
    ...
    for (size_t i = 0; i < record.entries.size(); i++)
    {
        ...
        else if (!e.opaque.empty())
        {
            ...
            w.String(e.opaque);
            w.Key("signature");
            w.String(IntersectionFunctionSignatureFlags(e.signature));
        }
        else
        {
            w.Key("function");
    ...
    for (const auto& b : record.buffers)
    {
        ...
        w.Uint(b.first);
        w.Key("buffer");
        w.Uint(b.second.first);
        w.Key("offset");
        w.Uint(b.second.second);
        w.EndObject();
    }
    w.EndArray();
...
{
    if (index >= 4096)
        return nullptr;   // a wild index, not a table this size
    if (index >= record.entries.size())
        record.entries.resize(index + 1);
...
void NoteTableBuffer(id table, NSUInteger index, id buffer, NSUInteger offset)
{
        ...
        return;
    {
        ...
        if (bufferId == 0)
            it->second.buffers.erase(index);
        else
            it->second.buffers[index] = {bufferId, offset};
    }
    SendTable(table, tableId);
}
...
void NoteTableVisibleTable(id table, NSUInteger index, id visible)
{
        ...
        return;
    {
        ...
        if (visibleId == 0)
            it->second.visibleTables.erase(index);
        else
            it->second.visibleTables[index] = visibleId;
    }
    SendTable(table, tableId);
}
