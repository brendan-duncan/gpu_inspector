...
Capturing Metal, the way `src/vulkan/` captures Vulkan. It discovers and hooks the Metal class tree,
...
```
cd src/app && npm start -- --launch=/path/to/Player.app --launch-args="-screen-width 800"
```
...
`src/app/src/main/metal.ts` handles the two things that make this different from registering a Vulkan
layer:
...
`src/app/src/renderer/command_sets.ts` defines the interface, `vulkan/command_sets.ts` and
...
interpreter that runs it is `src/app/src/renderer/msl/`; nothing in this directory executes shaders.
...
keyed by Vulkan names, so a Metal capture gets its own (`src/app/src/renderer/metal/frame_analysis.ts`),
...
argument buffer is then shown member by member (`src/app/src/renderer/metal/argument_buffer.ts`):
...
`transport.mm` also duplicates `src/vulkan/src/transport.cpp`; see the note at the top of
