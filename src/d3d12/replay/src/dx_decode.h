...
public:
    ...
    {
        if (const JValue* v = Get(n))
            f = (v->IsBool() ? v->boolean : v->Uint() != 0) ? TRUE : FALSE;
    }
    template <typename T>
    void Enum(const char* n, T& f, const dxinsp::EnumEntry* t, size_t c, const char*)
    {
        if (const JValue* v = Get(n))
            f = (T)ParseEnum(v, t, c);
    }
    template <typename T>
    void Flags(const char* n, T& f, const dxinsp::EnumEntry* t, size_t c, const char*)
    {
        if (const JValue* v = Get(n))
            f = (T)ParseFlags(v, t, c);
    }
    template <typename T>
    ...
    template <typename T>
    void Object(const char* n, T*& f, const char* cls)
    {
        const JValue* v = Get(n);
        if (!v)
            return;
    ...
    void Array(const char* n, const T*& p, const char*, C& count)
    {
        const JValue* v = Get(n);
        if (!v || !v->IsArray() || !v->count)
        {
            p = nullptr;
            count = 0;
            return;
    ...
    template <typename T, typename C>
    void Ints(const char* n, const T*& p, const char*, C& count, const char*)
    {
        const JValue* v = Get(n);
        if (!v || !v->IsArray() || !v->count)
        {
    ...
    template <typename T, typename C>
    void FixedEnums(const char* n, T* f, size_t size, C&, const dxinsp::EnumEntry* t, size_t c, const char*)
    {
        const JValue* v = Get(n);
        for (uint32_t i = 0; v && v->IsArray() && i < v->count && i < size; ++i)
            f[i] = (T)ParseEnum(&v->items[i], t, c);
    }
    ...
    void Bytecode(const char*, D3D12_SHADER_BYTECODE& f, const char* stage)
    {
        ...
        if (_env.bytecode && _env.bytecode(stage, data, size))
            f = {data, size};
    }
