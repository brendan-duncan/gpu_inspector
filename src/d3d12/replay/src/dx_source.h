...
public:
    ...
    template <typename T> static std::string Enum(const dxinsp::EnumEntry* table, size_t count, const char* type, T value) { return Enum(table, count, (int64_t)value, type); }
    template <typename T> static std::string Flags(const dxinsp::EnumEntry* table, size_t count, const char* type, T value) { return Flags(table, count, (uint64_t)value, type); }
...
public:
    ...
    template <typename T> void Int(const char* n, T& f) {
        if (!f) return;
        if (std::is_signed_v<T>) Set(n, Source::Int((int64_t)f)); else Set(n, Source::Uint((uint64_t)f));
    }
    void Float(const char* n, float& f) { if (f != 0.0f) Set(n, Source::Float(f)); }
    void Bool(const char* n, BOOL& f) { if (f) Set(n, "TRUE"); }
    template <typename T> void Enum(const char* n, T& f, const dxinsp::EnumEntry* t, size_t c, const char* type) {
        if ((int64_t)f) Set(n, Source::Enum(t, c, (int64_t)f, type));
    }
    template <typename T> void Flags(const char* n, T& f, const dxinsp::EnumEntry* t, size_t c, const char* type) {
        if (!(uint64_t)f) return;
    ...
    template <typename T> void Object(const char* n, T*& f, const char*) { if (f) Set(n, _w.Object(f)); }

    template <typename T, typename C> void Array(const char* n, const T*& p, const char* countName, C& count) {
        if (!p || !count) return;
    ...
    template <typename T, typename C> void Ints(const char* n, const T*& p, const char* countName, C& count, const char* type) {
        if (!p || !count) return;
        const std::string var = _w.Local(Stem(n));
        std::string items;
        for (size_t i = 0; i < (size_t)count; ++i) items += (i ? ", " : "") + Source::Uint((uint64_t)p[i]);
    ...
    void FixedFloats(const char* n, float* f, size_t size) {
        for (size_t i = 0; i < size; ++i)
            if (f[i] != 0.0f) _w.Line(_path + n + "[" + std::to_string(i) + "] = " + Source::Float(f[i]) + ";");
    }
    ...
    template <typename T, typename C> void FixedEnums(const char* n, T* f, size_t size, C& used, const dxinsp::EnumEntry* t, size_t c, const char* type) {
        for (size_t i = 0; i < size && i < (size_t)used; ++i)
            if ((int64_t)f[i]) _w.Line(_path + n + "[" + std::to_string(i) + "] = " + Source::Enum(t, c, (int64_t)f[i], type) + ";");
    }
    void String(const char* n, LPCSTR& f) { if (f) Set(n, Source::String(f)); }
    void Address(const char* n, D3D12_GPU_VIRTUAL_ADDRESS& f) { if (f) Set(n, _w.Address(f)); }
    void CpuHandle(const char* n, D3D12_CPU_DESCRIPTOR_HANDLE& f) { if (f.ptr) Set(n, _w.CpuHandle(f)); }
    ...
    void Bytecode(const char* n, D3D12_SHADER_BYTECODE& f, const char*) {
        if (f.pShaderBytecode && f.BytecodeLength) Set(n, "{" + _w.Data(f.pShaderBytecode, f.BytecodeLength) + ", " + std::to_string(f.BytecodeLength) + "}");
    }
...
private:
    ...
    static std::string Stem(const char* n) {
        std::string s = n;
        if (s.size() > 1 && s[0] == 'p' && s[1] >= 'A' && s[1] <= 'Z') s.erase(0, 1);
        if (!s.empty()) s[0] = (char)tolower((unsigned char)s[0]);
        return s;
    }
    template <typename T> static const char* IntegerName() {
        return sizeof(T) == 1 ? "UINT8" : sizeof(T) == 2 ? "UINT16" : sizeof(T) == 8 ? "UINT64" : "UINT";
    }
