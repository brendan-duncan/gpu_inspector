...
LONG WINAPI OnCrash(EXCEPTION_POINTERS* info)
{
    ...
    if (!g_exportDataPath.empty())
    {
        if (FILE* f = std::fopen(g_exportDataPath.c_str(), "wb"))
        {
            ...
            std::string dir;
            for (char ch : g_exportDir)
                dir += ch == '\\' ? '/' : ch;
            std::fprintf(f,
                "{\"format\":\"gpu-inspector-export-cpp\",\"version\":1,\"device\":\"\",\"directory\":\"%s\",\"ok\":false,\"error\":\"%s\","
...
bool ReadReplaceRequest(const std::string& path, std::vector<DxShaderReplacement>& out, std::string& error)
{
    ...
    for (uint32_t i = 0; list && list->IsArray() && i < list->count; ++i)
    {
        ...
        if (payload && payload->IsArray() && payload->count == 2)
        {
            ...
            if (base + offset + size <= bytes.size())
                r.code.assign(bytes.begin() + (ptrdiff_t)(base + offset), bytes.begin() + (ptrdiff_t)(base + offset + size));
        }
...
bool ReadAblationRequest(const std::string& path, DxAblationOptions& options, std::string& error)
{
    ...
    if (const vkreplay::JValue* rounds = root.Get("rounds"))
        options.rounds = std::clamp<uint32_t>((uint32_t)rounds->Uint(), 1, 64);
    ...
    for (uint32_t t = 0; targets && targets->IsArray() && t < targets->count; ++t)
    {
        ...
        if (const vkreplay::JValue* repeat = target.Get("repeat"))
        ...
        for (uint32_t v = 0; variants && variants->IsArray() && v < variants->count; ++v)
        {
            ...
            if (payload && payload->IsArray() && payload->count == 2)
            {
                ...
                const uint64_t size = payload->items[1].Uint();
                if (base + offset + size <= bytes.size())
                    vo.code.assign(bytes.begin() + (ptrdiff_t)(base + offset), bytes.begin() + (ptrdiff_t)(base + offset + size));
            }
...
int main(int argc, char** argv)
{
    ...
    {
        if (!std::strcmp(argv[i], "--debug-layer"))
            ...
            options.counters.enabled = options.counters.list = true;
    ...
    {
        std::fprintf(stderr,
            "usage: dxinsp_replay <capture.gpucap> [--debug-layer] [--trace] [--export <directory> [--export-data <file>]]\n"
            "       dxinsp_replay <capture.gpucap> --counters [--counter <name>]... [--counter-data <file>]\n"
            "       dxinsp_replay <capture.gpucap> --list-counters [--counter-data <file>]\n"
    ...
    if (report.counters.requested)
    {
        ...
        if (options.counters.list)
        {
            ...
            if (h.available.size() > 20)
                std::printf("  ... %zu more (--counter-data writes them all)\n", h.available.size() - 20);
        }
        ...
        for (size_t i = 0; i < h.notes.size() && i < 20; ++i)
            std::printf("  note: %s\n", h.notes[i].c_str());
    ...
    if (options.drawStats)
    {
        ...
        if (!drawData.empty())
            std::printf(WriteDrawData(report, drawData) ? "  wrote %s\n" : "  could not write %s\n", drawData.c_str());
    }
    if (options.ablation.enabled)
    {
        ...
        if (!ablateData.empty())
            std::printf(WriteAblationData(report, ablateData) ? "  wrote %s\n" : "  could not write %s\n", ablateData.c_str());
    }
    ...
    if (measuring || !replaceRequest.empty())
        return 0;
