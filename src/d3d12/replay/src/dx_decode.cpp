...
int64_t ParseEnum(const JValue* v, const dxinsp::EnumEntry* table, size_t count) {
    ...
    int64_t value = 0;
    if (v->IsString() && Lookup(table, count, v->Str(), value)) return value;
...
uint64_t ParseFlags(const JValue* v, const dxinsp::EnumEntry* table, size_t count) {
    ...
    while (!s.empty()) {
        ...
        int64_t bit = 0;
        if (!token.empty()) {
            if (Lookup(table, count, token, bit)) value |= (uint64_t)bit;
