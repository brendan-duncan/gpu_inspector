...
void DxReplayer::PrepareRaytracing()
{
    ...
    for (uint32_t i = 0; i < objects->count; ++i)
    {
        ...
        if (Str(o.Get("type")) != "ID3D12RaytracingAccelerationStructure")
            continue;
    ...
    for (uint32_t i = 0; commands && commands->IsArray() && i < commands->count; ++i)
    {
        ...
        if (Str(c.Get("method")) != "BuildRaytracingAccelerationStructure")
            continue;
        ...
        if (number && number->IsString())
            _structuresBuiltInFrame.insert(strtoull(Str(number).c_str(), nullptr, 0));
    }
}
...
D3D12_GPU_VIRTUAL_ADDRESS DxReplayer::RemapStructureAddress(uint64_t captured) const
{
    ...
    if (r == _resources.end() || !r->second.resource || !r->second.IsBuffer())
        return 0;
    ...
    for (uint32_t i = 0; i < subobjects->count && !failed; ++i)
    {
        ...
        auto exportsOf = [&](const JValue* list, UINT& count) -> const D3D12_EXPORT_DESC* {
            ...
            for (uint32_t k = 0; k < list->count; ++k)
            {
                ...
                if (!rename.empty())
                    out[k].ExportToRename = Wide(_arena, rename);
        ...
        else if (type.find("HIT_GROUP") != std::string::npos)
        {
            ...
            if (!anyHit.empty())
                group->AnyHitShaderImport = Wide(_arena, anyHit);
            if (!closestHit.empty())
                group->ClosestHitShaderImport = Wide(_arena, closestHit);
            if (!intersection.empty())
                group->IntersectionShaderImport = Wide(_arena, intersection);
        ...
        else if (type.find("DXIL_SUBOBJECT_TO_EXPORTS_ASSOCIATION") != std::string::npos)
        {
            ...
            if (list && list->IsArray() && list->count)
            {
                ...
                for (uint32_t k = 0; k < list->count; ++k)
                    names[k] = Wide(_arena, Str(&list->items[k]));
...
void DxReplayer::NoteStateObjectIdentifiers(uint64_t id, const JValue& object, ID3D12StateObject* stateObject)
{
    ...
    for (uint32_t i = 0; i < exports->count; ++i)
    {
        ...
        if (name.empty() || captured.size() != kIdentifierSize * 2)
            continue;
        const void* here = properties->GetShaderIdentifier(Wide(_arena, name));
        if (!here)
        {
            Problem("state object " + std::to_string(id) + ": this runtime gives no identifier for \"" + name + "\", so records of it in a binding table cannot be replayed");
            continue;
        }
        Identifier bytes{};
...
D3D12_GPU_VIRTUAL_ADDRESS DxReplayer::RemapInstancesFrom(const JValue* list, UINT count, InstanceSource* source)
{
    ...
    for (uint32_t i = 0; list && list->IsArray() && i < list->count; ++i)
    {
        ...
        if (Str(e.Get("field")) == "InstanceDescs" && e.Get("capture"))
            dataId = e.Get("capture")->Uint();
    }
    if (!dataId)
    {
    ...
    if (have < count)
    {
        Problem("a top level build reads " + std::to_string(count) + " instances and the capture holds " + std::to_string(have) + "; the rest are replayed as they were read back");
    }
    ...
    for (size_t i = 0; i < have; ++i)
    {
        ...
        else if (here && !_structuresBuiltInFrame.count(captured) && _reportedUnbuilt.insert(captured).second)
        {
            ...
            Problem(
                "an instance names a bottom level the captured frame does not build (it was built before the "
    ...
    UINT64 size, uint64_t stateObjectId)
{
    ...
    {
        const JValue& e = list->items[i];
        if (Str(e.Get("region")) != region)
            continue;
        if (e.Get("capture"))
            dataId = e.Get("capture")->Uint();
        if (e.Get("stride"))
            capturedStride = e.Get("stride")->Uint();
    }
    if (!dataId)
    {
...
ID3D12GraphicsCommandList4* DxReplayer::RaytracingList(ID3D12GraphicsCommandList* list)
{
    ...
    if (!list || FAILED(list->QueryInterface(IID_PPV_ARGS(&out))) || !out)
        return nullptr;
    ...
    ID3D12GraphicsCommandList* list, std::string& leftOut, uint32_t index)
{
    ...
    if (method == "DispatchRays")
    {
        ...
        if (!stateObjectId)
            stateObjectId = _boundStateObject;
    ...
    const uint8_t*& bytes, size_t& size)
{
    ...
    if (!info || info->Get("error") || !info->Get("buffer") || !input.Get("buffer") ||
        info->Get("buffer")->Uint() != input.Get("buffer")->Uint())
        return false;
...
void DxReplayer::BuildEarlierStructures()
{
    ...
    for (uint32_t i = 0; i < objects->count; ++i)
    {
        ...
        if (!build || !inputs || !inputs->IsArray() || !number || !number->IsString())
            continue;
        Earlier e;
        e.id = o.Get("id")->Uint();
        e.captured = strtoull(Str(number).c_str(), nullptr, 0);
        // A structure the frame builds itself is left to the frame.
        if (!e.captured || _structuresBuiltInFrame.count(e.captured))
            continue;
    ...
    for (const Earlier& e : earlier)
    {
        ...
        else
        {
            ...
            for (uint32_t k = 0; k < e.inputs->count; ++k)
            {
                ...
                if (g >= p.geometries.size())
                    continue;
                const uint8_t* bytes = nullptr;
                size_t size = 0;
                if (!ReadBack(*_capture, _bufferData, input, bytes, size))
                    continue;
                ...
                else if (field == "IndexBuffer")
                    geometry.Triangles.IndexBuffer = here;
                else if (field == "Transform3x4")
                    geometry.Triangles.Transform3x4 = here;
                else if (field == "AABBs")
                    geometry.AABBs.AABBs.StartAddress = here;
            }
            ...
            for (const auto& geometry : p.geometries)
            {
                if (geometry.Type == D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES)
                {
                    if (!geometry.Triangles.VertexBuffer.StartAddress)
                        complete = false;
        ...
        {
            Problem("acceleration structure " + std::to_string(e.id) +
                ": built before the capture began, and what it was built "
    ...
    {
        if (!p.topLevel)
            p.desc.Inputs.pGeometryDescs = p.geometries.data();
