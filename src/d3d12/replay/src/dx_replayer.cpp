...
std::string Narrow(const wchar_t* wide) {
    std::string out;
    for (; wide && *wide; ++wide) out += *wide < 0x80 ? (char)*wide : '?';
    return out;
}
...
std::string StateText(D3D12_RESOURCE_STATES s) {
    if (s == D3D12_RESOURCE_STATE_COMMON) return "D3D12_RESOURCE_STATE_COMMON";
...
std::string FormatText(DXGI_FORMAT f) {
    if (f == DXGI_FORMAT_UNKNOWN) return "DXGI_FORMAT_UNKNOWN";
...
bool IsWriteState(D3D12_RESOURCE_STATES s) {
    return (s & (D3D12_RESOURCE_STATE_RENDER_TARGET | D3D12_RESOURCE_STATE_UNORDERED_ACCESS | D3D12_RESOURCE_STATE_DEPTH_WRITE |
                 D3D12_RESOURCE_STATE_COPY_DEST | D3D12_RESOURCE_STATE_RESOLVE_DEST | D3D12_RESOURCE_STATE_STREAM_OUT)) != 0;
}
...
DxReplayer::DxReplayer() {
    ...
    _env.cpuHandle = [this](uint64_t heapId, uint32_t index) {
        ...
        if (it != _heaps.end() && index < it->second.desc.NumDescriptors) h.ptr = it->second.cpu.ptr + (SIZE_T)index * it->second.increment;
        return h;
    };
    _env.bytecode = [this](const char* stage, const uint8_t*& data, size_t& size) {
        ...
        for (uint32_t i = 0; blobs && blobs->IsArray() && i < blobs->count; ++i) {
            ...
            if (Str(b.Get("name")).rfind(prefix, 0) == 0) return _capture->Payload(b.Get("payload"), data, size);
        }
        return false;
    };
}
...
DxReplayer::~DxReplayer() {
    if (_queue && _fence) WaitForQueue(_queue);
    for (ID3D12Resource* r : _transients) r->Release();
    for (auto it = _created.rbegin(); it != _created.rend(); ++it) (*it)->Release();
...
void DxReplayer::CollectMessages() {
    ...
    for (UINT64 i = 0; i < count; ++i) {
        ...
        if (FAILED(_infoQueue->GetMessage(i, message, &size))) continue;
...
bool DxReplayer::CreateDevice() {
    ...
    if (const JValue* objects = _capture->Objects(); objects && objects->IsArray()) {
        for (uint32_t i = 0; i < objects->count; ++i) {
            ...
            if (Str(o.Get("type")) == "IDXGIAdapter" && args && capturedAdapter.empty()) {
                if (const JValue* desc = args->Get("Desc")) capturedAdapter = Str(desc->Get("Description"));
            }
    ...
    if (level < D3D_FEATURE_LEVEL_11_0) level = D3D_FEATURE_LEVEL_11_0;
    ...
    for (UINT i = 0;; ++i) {
        ...
        if (_factory->EnumAdapters1(i, &adapter) == DXGI_ERROR_NOT_FOUND) break;
        ...
        if (!first && !software) first = adapter; else adapter->Release();
    }
    if (!_adapter) _adapter = first;
    DXGI_ADAPTER_DESC1 desc{};
    if (_adapter) _adapter->GetDesc1(&desc);
    _report->device = Narrow(desc.Description);
    if (!capturedAdapter.empty() && capturedAdapter != _report->device) Problem("captured on " + capturedAdapter + ", replayed on " + _report->device);
    ...
    if (_options.debugLayer) _device->QueryInterface(IID_PPV_ARGS(&_infoQueue));
...
bool DxReplayer::RunOneTime(const std::function<void(ID3D12GraphicsCommandList*)>& record) {
    if (FAILED(_allocator->Reset()) || FAILED(_utility->Reset(_allocator, nullptr))) return false;
    record(_utility);
    if (FAILED(_utility->Close())) return false;
...
ID3D12Resource* DxReplayer::CreateResource(uint64_t id, const D3D12_HEAP_PROPERTIES& capturedHeap, D3D12_HEAP_FLAGS, const D3D12_RESOURCE_DESC& capturedDesc,
    ...
    if (_x) {
        ...
        if (buffer) _x->Buffer(resource, resource->GetGPUVirtualAddress(), desc.Width);
...
ID3D12PipelineState* DxReplayer::CreatePipeline(const JValue& object, const std::string& cmd, const JValue& args) {
    ...
    if (compute) {
        ...
        DecodeStruct(_env, args, "pDesc", desc);
        if (desc.SampleDesc.Count == 0) desc.SampleDesc.Count = 1;   // a stream without SAMPLE_DESC means one sample
    ...
    if (_env.unresolved != unresolved) return nullptr;
...
void DxReplayer::CreateObject(const JValue& o) {
    ...
    auto skip = [&](const char* why) {
        ...
        if (_x) _x->Comment(DxExporter::Create, type + " " + std::to_string(id) + ": left out on purpose: " + why);
    };
    ...
    if (type == "ID3D12Device") {
        ...
        if (SUCCEEDED(_device->CreateCommandQueue(&desc, IID_PPV_ARGS(&queue)))) made = queue;
        if (made && _x) {
        ...
        if (SUCCEEDED(_device->CreateCommandAllocator(listType, IID_PPV_ARGS(&allocator))) &&
            ...
            if (listType == D3D12_COMMAND_LIST_TYPE_BUNDLE) _bundles[id] = {IdOf(d.Get("pCommandAllocator")), IdOf(d.Get("pInitialState")), false};
        } else {
        ...
        if (SUCCEEDED(_device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&heap)))) {
            ...
            if (desc.Flags & D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE) rec.gpu = heap->GetGPUDescriptorHandleForHeapStart();
        ...
        if (cmd == "GetBuffer") {
            ...
            if (heapDesc) DecodeStruct(_env, *heapDesc, "Properties", heap);
        ...
        if (hasDesc) made = CreateResource(id, heap, D3D12_HEAP_FLAG_NONE, desc, hasClear ? &clear : nullptr, comment);
        if (!made) {
            if (!hasDesc) Problem("resource " + std::to_string(id) + " (" + cmd + "): the capture has no description of it");
            _report->objectsSkipped++;
            return;
        }
        _objects[id] = made;
        ...
        if (made) {
            ...
            if (!resolved()) Problem("pipeline " + std::to_string(id) + " was not created: it names objects the replay does not have");
            _report->objectsSkipped++;
        }
        return;
        ...
        desc.NodeMask = 0;
        ID3D12RootSignature* root = nullptr;
        if (d.Get("pRootSignature")) d.Object("pRootSignature", root, "ID3D12RootSignature");
        ID3D12CommandSignature* signature = nullptr;
        if (resolved() && SUCCEEDED(_device->CreateCommandSignature(&desc, root, IID_PPV_ARGS(&signature)))) made = signature;
        if (made && _x) {
        ...
        if (SUCCEEDED(_device->CreateQueryHeap(&desc, IID_PPV_ARGS(&heap)))) made = heap;
        if (made && _x) {
        ...
        _report->objectsSkipped++;
        if (_x) _x->Comment(DxExporter::Create, type + " " + std::to_string(id) + ": left out: the replay does not make objects of this type yet");
        return;
    }
    ...
    if (!made) {
        ...
        _report->objectsSkipped++;
        if (_x) _x->Comment(DxExporter::Create, type + " " + std::to_string(id) + ": left out: the replay could not create it");
        return;
    }
    _objects[id] = made;
    if (owned) {
...
void DxReplayer::NoteUse(uint64_t resourceId, uint32_t subresource, D3D12_RESOURCE_STATES state, bool writes) {
    ...
    for (uint32_t i = 0; i < initial.size(); ++i) {
        if (subresource != UINT_MAX && i != subresource) continue;
...
void DxReplayer::ComputeInitialStates() {
    ...
    auto address = [&](const JValue* v, D3D12_RESOURCE_STATES state, bool writes) {
        if (v && !v->IsNull()) NoteUse(IdOf(v->Get("buffer")), UINT_MAX, state, writes);
    };
    auto target = [&](const JValue* entry, bool depth) {
        ...
        if (depth) {
            ...
            if (viewFlags.find("READ_ONLY_DEPTH") != std::string::npos) state = D3D12_RESOURCE_STATE_DEPTH_READ;
        }
    ...
    for (uint32_t i = 0; i < commands->count; ++i) {
            ...
            for (uint32_t s = 0; sets && sets->IsArray() && s < sets->count; ++s) {
                ...
                for (uint32_t b = 0; bindings && bindings->IsArray() && b < bindings->count; ++b) {
                    ...
                    for (uint32_t k = 0; list && list->IsArray() && k < list->count; ++k) {
                        ...
                        if (!id) continue;
                        NoteUse(id, UINT_MAX, uav ? D3D12_RESOURCE_STATE_UNORDERED_ACCESS : cbv ? D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER : kShaderRead, uav);
                    }
                }
            }
        }
        ...
        if (m == "ResourceBarrier") {
            ...
            for (uint32_t b = 0; barriers && barriers->IsArray() && b < barriers->count; ++b) {
                ...
                if (Str(barriers->items[b].Get("Flags")).find("END_ONLY") != std::string::npos) continue;
                const uint64_t id = IdOf(t->Get("pResource"));
                Resource* r = ResourceOf(id);
                if (!r || r->heapType != D3D12_HEAP_TYPE_DEFAULT) continue;
                ...
                for (uint32_t k = 0; k < initial.size(); ++k) {
                    if ((sub != UINT_MAX && k != sub) || initial[k].fixed) continue;
            ...
            for (uint32_t k = 0; views && views->IsArray() && k < views->count; ++k) address(views->items[k].Get("BufferLocation"), D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, false);
        } else if (m == "IASetIndexBuffer") {
            if (const JValue* view = args->Get("pView"); view && !view->IsNull()) address(view->Get("BufferLocation"), D3D12_RESOURCE_STATE_INDEX_BUFFER, false);
            ...
            for (uint32_t k = 0; list && list->IsArray() && k < list->count; ++k) target(&list->items[k], false);
...
void DxReplayer::Transition(ID3D12GraphicsCommandList* list, Resource& r, uint32_t subresource, D3D12_RESOURCE_STATES to, const char* listName) {
    ...
    for (uint32_t i = 0; i < r.states.size(); ++i)
        if (subresource == UINT_MAX || i == subresource) r.states[i] = to;
...
void DxReplayer::NoteBarrier(const D3D12_RESOURCE_BARRIER& barrier) {
    if (barrier.Type != D3D12_RESOURCE_BARRIER_TYPE_TRANSITION || (barrier.Flags & D3D12_RESOURCE_BARRIER_FLAG_BEGIN_ONLY)) return;
    Resource* r = ResourceOf(IdOfResource(barrier.Transition.pResource));
    if (!r) return;
    for (uint32_t i = 0; i < r->states.size(); ++i)
        if (barrier.Transition.Subresource == D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES || i == barrier.Transition.Subresource) r->states[i] = barrier.Transition.StateAfter;
}
...
void DxReplayer::UploadTextures() {
    ...
    for (uint32_t n = textures->count; n-- > 0;) {
        ...
        size_t size = 0;
        if (!_capture->Payload(t.Get("payload"), data, size) || !size) continue;
        const uint64_t id = info->Get("id")->Uint();
        Resource* r = ResourceOf(id);
        if (!r || r->IsBuffer() || r->desc.SampleDesc.Count > 1) continue;
        ...
        if (stencilAspect) { format = dxinsp::FormatInfo{}; format.bytes = 1; }
        if (!format.bytes) {
        ...
        struct Region { UINT subresource; UINT64 rowBytes; UINT rows; UINT slices; };
        ...
        if (FAILED(_device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &bufferDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&staging)))) continue;
        void* mapped = nullptr;
        if (FAILED(staging->Map(0, nullptr, &mapped))) { staging->Release(); continue; }
...
void DxReplayer::MoveToInitialStates() {
    ...
    RunOneTime([&](ID3D12GraphicsCommandList* list) {
        for (auto& [id, r] : _resources) {
            if (r.heapType != D3D12_HEAP_TYPE_DEFAULT) continue;
            ...
            for (uint32_t i = 0; i < r.states.size(); ++i) {
                ...
                if (r.states[i] != want) Transition(list, r, i, want, "list");
            }
        }
    });
...
void DxReplayer::ApplyBufferData(const Group& group) {
    ...
    auto apply = [&](uint64_t dataId) {
        ...
        Resource* r = ResourceOf(bufferId);
        if (!r || !r->IsBuffer() || r->heapType == D3D12_HEAP_TYPE_READBACK) return;
        const uint64_t offset = info->Get("offset") ? info->Get("offset")->Uint() : 0;
        if (offset >= r->desc.Width) return;
        ...
        for (uint32_t s = 0; sets && sets->IsArray() && s < sets->count; ++s) {
            ...
            for (uint32_t b = 0; bindings && bindings->IsArray() && b < bindings->count; ++b) {
                ...
                for (uint32_t k = 0; list && list->IsArray() && k < list->count; ++k) {
                    ...
                    if (record.Get("buffer") && record.Get("data")) apply(record.Get("data")->Uint());
                }
            }
        }
    }
...
bool DxReplayer::WriteDescriptor(uint64_t heapId, uint32_t index, D3D12_DESCRIPTOR_RANGE_TYPE type, const JValue& record) {
    ...
    if (hit == _heaps.end() || index >= hit->second.desc.NumDescriptors || record.IsNull()) return false;
    ...
    if (type == D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER) {
        ...
        if (!DecodeStruct(_env, record, "samplerDesc", desc)) return false;
        _device->CreateSampler(&desc, handle);
        if (_x) _x->Block(DxExporter::Frame, "", [&](Source& s) { s.Line("device->CreateSampler(&" + EmitStruct(s, "sampler", desc) + ", " + handleText + ");"); });
    } else if (type == D3D12_DESCRIPTOR_RANGE_TYPE_CBV) {
        const uint64_t bufferId = IdOf(record.Get("buffer"));
        Resource* r = ResourceOf(bufferId);
        if (!r) { Problem(_env.where + ": a constant buffer view of a buffer the replay does not have"); return false; }
        ...
        Resource* r = ResourceOf(resourceId);
        if (!r) { Problem(_env.where + ": a view of resource " + std::to_string(resourceId) + ", which the replay does not have"); return false; }
        const bool hasView = d.Get("view") != nullptr;
        if (type == D3D12_DESCRIPTOR_RANGE_TYPE_SRV) {
            D3D12_SHADER_RESOURCE_VIEW_DESC desc{};
            if (hasView) d.Struct("view", desc);
            if (_env.unresolved != unresolved) return false;
            ...
            if (d.Get("counter")) d.Object("counter", counter, "ID3D12Resource");
            if (_env.unresolved != unresolved) return false;
...
bool DxReplayer::WriteTargetDescriptor(const JValue* entry, bool depth, PassTarget* out) {
    if (!entry || entry->IsNull()) return false;
    ...
    if (depth) {
        ...
        if (hasView) d.Struct("view", desc);
        if (desc.ViewDimension == D3D12_DSV_DIMENSION_TEXTURE2D) target.mip = desc.Texture2D.MipSlice;
        if (desc.ViewDimension == D3D12_DSV_DIMENSION_TEXTURE2DARRAY) { target.mip = desc.Texture2DArray.MipSlice; target.firstSlice = desc.Texture2DArray.FirstArraySlice; }
        if (fresh) {
        ...
        if (hasView) d.Struct("view", desc);
        if (desc.ViewDimension == D3D12_RTV_DIMENSION_TEXTURE2D) target.mip = desc.Texture2D.MipSlice;
        if (desc.ViewDimension == D3D12_RTV_DIMENSION_TEXTURE2DARRAY) { target.mip = desc.Texture2DArray.MipSlice; target.firstSlice = desc.Texture2DArray.FirstArraySlice; }
        if (fresh) {
    ...
    if (fresh) _report->descriptorsWritten++;
    if (out) *out = target;
    return true;
}
...
void Canonical(const JValue& v, std::string& out) {
    if (v.IsObject()) {
        ...
        for (uint32_t i = 0; i < v.count; ++i) {
            if (v.members[i].key == "data") continue;   // which read-back holds the contents: not what the descriptor is
            out += v.members[i].key;
            out += ':';
...
void DxReplayer::WriteTableDescriptors(const JValue& command, const JValue& args) {
    ...
    const JValue* sets = snapshot ? snapshot->Get("sets") : nullptr;
    if (!sets || !sets->IsArray() || !sets->count) return;
    ...
    if (!heapId || rit == _rootSignatures.end() || !rit->second) return;
    const JValue* versioned = rit->second;
    const JValue* desc = versioned->Get("Desc_1_1") ? versioned->Get("Desc_1_1") : versioned->Get("Desc_1_0") ? versioned->Get("Desc_1_0") : versioned->Get("Desc_1_2");
    const JValue* parameters = desc ? desc->Get("pParameters") : nullptr;
    if (!parameters || !parameters->IsArray() || parameter >= parameters->count) return;
    ...
    if (!ranges || !ranges->IsArray() || !bindings || !bindings->IsArray()) return;
    uint32_t append = 0;
    for (uint32_t k = 0; k < ranges->count && k < bindings->count; ++k) {
        ...
        for (uint32_t i = 0; list && list->IsArray() && i < list->count; ++i) {
            ...
            if (hit->second.written[slot] == content) continue;
            ...
            if (WriteDescriptor(heapId, slot, type, record)) hit->second.written[slot] = content;
        }
...
void DxReplayer::BuildGroups() {
    ...
    for (uint32_t i = 0; i < commands->count; ++i) {
        const JValue& c = commands->items[i];
        if (c.Get("secondary")) continue;   // a bundle's, inlined in the list that executes it: its Close is not the list's
        const std::string m = Str(c.Get("method"));
...
bool DxReplayer::IssueCommand(uint32_t index, const std::string& m, const JValue& command, const JValue* argsOrNull, ID3D12GraphicsCommandList* list, uint64_t listId) {
    ...
    if (m == "DrawInstanced") {
        ...
        if (d.Get("pCountBuffer")) d.Object("pCountBuffer", count, "ID3D12Resource");
        if (!resolved()) return leftOut("it names objects the replay does not have");
        ...
        if (hasBox) d.Struct("pSrcBox", box);
        if (!resolved() || !dst.pResource || !src.pResource) return leftOut("it names objects the replay does not have");
        ...
        auto hit = _heaps.find(heapId);
        if (hit == _heaps.end()) return leftOut("its view is in a descriptor heap the replay does not have");
        Resource* r = ResourceOf(IdOf(d.Get("resource")));
        if (!r) return leftOut("it clears a resource the replay does not have");
        ...
        if (hit->second.written.find(slot) == hit->second.written.end()) {
            ...
            if (depth) _device->CreateDepthStencilView(r->resource, nullptr, handle); else _device->CreateRenderTargetView(r->resource, nullptr, handle);
            _report->descriptorsWritten++;
            if (_x) _x->Block(DxExporter::Frame, "", [&](Source& s) {
        ...
        for (uint32_t k = 0; items && items->IsArray() && k < items->count; ++k) {
            ...
            if (viewports) { vps.emplace_back(); Reflect(id, vps.back()); } else { rects.emplace_back(); Reflect(id, rects.back()); }
        }
        if (viewports) list->RSSetViewports((UINT)vps.size(), vps.data()); else list->RSSetScissorRects((UINT)rects.size(), rects.data());
        ...
        if (!as(list1)) return leftOut("this runtime has no ID3D12GraphicsCommandList1");
        ...
        if (!resolved()) return leftOut("it names objects the replay does not have");
        if (pipeline) list->SetPipelineState(static_cast<ID3D12PipelineState*>(object));
        else if (m == "SetGraphicsRootSignature") { list->SetGraphicsRootSignature(static_cast<ID3D12RootSignature*>(object)); _graphicsRoot = IdOf(d.Get("pRootSignature")); }
        else { list->SetComputeRootSignature(static_cast<ID3D12RootSignature*>(object)); _computeRoot = IdOf(d.Get("pRootSignature")); }
        ...
        for (uint32_t k = 0; items && items->IsArray() && k < items->count; ++k) {
            ...
            if (!heap) return leftOut("it names a descriptor heap the replay does not have");
        ...
        emit(method("SetDescriptorHeaps"), [&](Source& s) {
            std::string names;
            for (ID3D12DescriptorHeap* h : heaps) names += (names.empty() ? "" : ", ") + s.Object(h);
        ...
        if (hit == _heaps.end() || !hit->second.gpu.ptr) return leftOut("its table is in no shader-visible heap the replay has");
        ...
        if (m == "SetGraphicsRootDescriptorTable") list->SetGraphicsRootDescriptorTable(parameter, handle); else list->SetComputeRootDescriptorTable(parameter, handle);
        ...
        if (bytes && bytes->Get("base64")) vkreplay::DecodeBase64(bytes->Get("base64")->Str(), data);
        data.resize(std::max<size_t>(data.size(), (size_t)count * 4), 0);
        if (compute) list->SetComputeRoot32BitConstants(parameter, count, data.data(), offset); else list->SetGraphicsRoot32BitConstants(parameter, count, data.data(), offset);
        ...
        if (!resolved()) return leftOut("its buffer is not one the replay has");
        const UINT parameter = U("RootParameterIndex");
        if (m == "SetGraphicsRootConstantBufferView") list->SetGraphicsRootConstantBufferView(parameter, address);
        else if (m == "SetComputeRootConstantBufferView") list->SetComputeRootConstantBufferView(parameter, address);
        else if (m == "SetGraphicsRootShaderResourceView") list->SetGraphicsRootShaderResourceView(parameter, address);
        else if (m == "SetComputeRootShaderResourceView") list->SetComputeRootShaderResourceView(parameter, address);
        else if (m == "SetGraphicsRootUnorderedAccessView") list->SetGraphicsRootUnorderedAccessView(parameter, address);
        else list->SetComputeRootUnorderedAccessView(parameter, address);
        emit(method(m.c_str()), [&](Source& s) { return Source::Uint(parameter) + ", " + s.Address(address); });
    } else if (m == "IASetIndexBuffer") {
        D3D12_INDEX_BUFFER_VIEW view{};
        const bool has = d.Get("pView") != nullptr;
        if (has) d.Struct("pView", view);
        if (!resolved()) return leftOut("its buffer is not one the replay has");
        ...
        if (!resolved()) return leftOut("one of its buffers is not one the replay has");
        ...
        if (hasDepth) depth = d.HandleOf(d.Get("pDepthStencilDescriptor"));
        if (!resolved()) return leftOut("a target's descriptor heap is not one the replay has");
        ...
        emit(method("OMSetRenderTargets"), [&](Source& s) {
            std::string names;
            for (const D3D12_CPU_DESCRIPTOR_HANDLE& h : handles) names += (names.empty() ? "" : ", ") + s.CpuHandle(h);
        ...
        if (!as(list4)) return leftOut("this runtime has no ID3D12GraphicsCommandList4");
        ...
        if (hasDepth) d.Struct("pDepthStencil", depth);
        ...
        if (!resolved()) return leftOut("it names resources the replay does not have");
        if (barriers.empty()) return true;
        list->ResourceBarrier((UINT)barriers.size(), barriers.data());
        for (const D3D12_RESOURCE_BARRIER& b : barriers) NoteBarrier(b);
        ...
        if (!resolved()) return leftOut("it names a query heap the replay does not have");
        D3D12_QUERY_TYPE type{};
        d.Enum("Type", type, DX_TABLE(D3D12_QUERY_TYPE));
        const UINT slot = U("Index");
        if (m == "BeginQuery") list->BeginQuery(heap, type, slot); else list->EndQuery(heap, type, slot);
        ...
        if (!as(list6)) return leftOut("this runtime has no ID3D12GraphicsCommandList6");
        const UINT x = U("ThreadGroupCountX"), y = U("ThreadGroupCountY"), z = U("ThreadGroupCountZ");
        ...
        if (!resolved() || !resource) return leftOut("it names a resource the replay does not have");
        ...
        ID3D12PipelineState* pipeline = nullptr;
        if (d.Get("pPipelineState") && !d.Get("pPipelineState")->IsNull()) d.Object("pPipelineState", pipeline, "ID3D12PipelineState");
        if (!resolved()) return leftOut("it names a pipeline the replay does not have");
        ...
        if (!as(list2)) return leftOut("this runtime has no ID3D12GraphicsCommandList2");
        ...
        if (d.Get("pBuffer") && !d.Get("pBuffer")->IsNull()) d.Object("pBuffer", buffer, "ID3D12Resource");
        if (!resolved()) return leftOut("it names a buffer the replay does not have");
        ...
        if (!as(list5)) return leftOut("this runtime has no ID3D12GraphicsCommandList5");
        ID3D12Resource* image = nullptr;
        if (d.Get("shadingRateImage")) d.Object("shadingRateImage", image, "ID3D12Resource");
        if (!resolved()) { list5->Release(); return leftOut("it names a resource the replay does not have"); }
        ...
        if (_x) _x->Comment(DxExporter::Frame, "[" + std::to_string(index) + "] " + m + (d.Get("label") ? ": " + Str(d.Get("label")) : std::string()));
        return true;
        ...
        if (!bundle) return leftOut("the bundle was not replayed");
...
bool DxReplayer::RecordBundle(uint32_t executeIndex, uint64_t bundleId, ID3D12GraphicsCommandList* bundle) {
    ...
    if (info == _bundles.end()) return false;
    if (info->second.recorded) return true;
    // The capture inlines what the bundle recorded after the command that executes it.
    uint32_t end = executeIndex + 1;
    while (end < commands->count && commands->items[end].Get("secondary") && commands->items[end].Get("secondary")->Uint() == bundleId) ++end;
    if (end == executeIndex + 1) return false;
    ...
    for (uint32_t i = executeIndex + 1; i < end; ++i) {
        ...
        if (IssueCommand(i, m, c, c.Get("args"), bundle, bundleId)) _report->commandsRecorded++;
    }
    _env.where = where;
    if (FAILED(bundle->Close())) Problem("bundle " + std::to_string(bundleId) + ": Close failed, so what it recorded is not valid");
    if (_x) _x->Block(DxExporter::Frame, "", [&](Source& s) { s.Line("DX_CHECK(" + name + "->Close());"); });
    return true;
}
...
void DxReplayer::RecordGroup(Group& group, ID3D12GraphicsCommandList* list, std::vector<Readback>& readbacks) {
    ...
    for (uint32_t i = group.first + 1; i < group.last; ++i) {
        const JValue& c = commands->items[i];
        if (IdOf(c.Get("object")) != group.list) continue;   // another list's recording interleaved
        if (c.Get("secondary")) continue;                    // a bundle's, recorded by the ExecuteBundle before it
        const std::string m = Str(c.Get("method"));
        const JValue* args = c.Get("args");
        _env.where = "command " + std::to_string(i) + " " + m;
        if (_options.trace) {
            ...
            auto writeTarget = [&](const JValue* entry, bool depth, uint32_t slot) {
                ...
                if (pass.realPass) {
                    ...
                    for (const char* key : {"heap", "index"})
                        if (const JValue* v = handle->Get(key)) { members[n].key = key; members[n].value = *v; ++n; }
                    for (const char* key : {"resource", "view"})
                        if (const JValue* v = entry->Get(key)) { members[n].key = key; members[n].value = *v; ++n; }
                    JValue merged;
...
void DxReplayer::InjectReadbacks(ID3D12GraphicsCommandList* list, const Pass& pass, std::vector<Readback>& readbacks) {
    ...
    for (uint32_t i = 0; i < textures->count; ++i) {
        ...
        const JValue* info = t.Get("info");
        if (!info || info->Get("kind")) continue;   // sampled textures are uploaded, not compared
        if (info->Get("commandBuffer")->Uint() != pass.list || info->Get("passIndex")->Uint() != pass.index || info->Get("frame")->Uint() != pass.frame) continue;
        ...
        if (info->Get("error")) { cmp.undefined = true; skip("the capture's read-back failed: " + Str(info->Get("error"))); continue; }
        Resource* r = ResourceOf(cmp.resource);
        if (!r) { skip("the target was not replayed"); continue; }
        const uint8_t* captured = nullptr;
        size_t capturedSize = 0;
        if (!_capture->Payload(t.Get("payload"), captured, capturedSize) || !capturedSize) { skip("the capture has no pixels for this target"); continue; }
        ...
        const uint32_t mip = info->Get("mip") ? (uint32_t)info->Get("mip")->Uint() : bound ? bound->mip : 0;
        ...
        if (multisampled && cmp.aspect != "color") { skip("a multisampled depth target is not read back"); continue; }
        ...
        if (cmp.aspect == "stencil") { format = dxinsp::FormatInfo{}; format.bytes = 1; }
        if (!format.bytes) { skip("its format cannot be read back"); continue; }
        ...
        for (uint32_t l = 0; l < layers; ++l) {
            ...
            if (multisampled) _device->GetCopyableFootprints(&resolved.desc, 0, 1, total, &fp, nullptr, nullptr, &bytes);
            rb.footprints.push_back(fp);
            total = (fp.Offset + bytes + 511) & ~511ull;
            if (multisampled) break;
        }
        ...
        if (_x) {
            ...
            _x->Block(DxExporter::Frame, "", [&](Source& s) {
                s.Line("ReadbackTexture(" + ListName(list) + ", " + s.Object(r->resource) + ", " + Source::String(name.c_str()) + ", " + std::to_string(firstSub) + ", " +
                       ...
                       ", " + FormatText(cmp.aspect == "color" ? dxinsp::TypedFormat(r->desc.Format, false) : r->desc.Format) + ", " + (cmp.aspect == "color" ? "0" : cmp.aspect == "depth" ? "1" : "2") + ", " +
...
void DxReplayer::CompareReadbacks(std::vector<Readback>& readbacks) {
    for (Readback& rb : readbacks) {
        ...
        if (capturedSize != replayed.size()) cmp.note = "sizes differ: captured " + std::to_string(capturedSize) + ", replayed " + std::to_string(replayed.size());
...
void DxReplayer::ReplayCommands() {
    ...
    for (uint32_t i = 0; i < commands->count; ++i) {
        ...
        for (uint32_t k = 0; ids && ids->IsArray() && k < ids->count; ++k) {
            ...
            if (group != _groups.end() && !list) list = MissingList(id, queue->GetDesc().Type);
            if (group == _groups.end() || !list) {
                ...
                if (_x) _x->Comment(DxExporter::Frame, "command list " + std::to_string(id) + " is left out: the capture holds no recording of it");
                continue;
            }
        ...
        if (_x) {
            _x->Block(DxExporter::Frame, "executed, and waited for", [&](Source& s) {
                std::string names;
                for (ID3D12CommandList* l : lists) names += (names.empty() ? "" : ", ") + s.Object(l);
...
bool DxReplayer::Run(const CaptureFile& capture, const DxReplayOptions& options, DxReplayReport& report) {
    ...
    CollectMessages();
    if (_x) {
        if (!_x->Finish(report, report.exported)) Problem("export to C++: " + report.exported.error);
        _x.reset();
    }
    return true;
}
