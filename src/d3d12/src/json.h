...
public:
    ...
    Args& ref(const char* key, const void* object, const char* type)
    {
        w_.Key(key);
    ...
    Args& cpuHandle(const char* key, D3D12_CPU_DESCRIPTOR_HANDLE h)
    {
        w_.Key(key);
        WriteCpuHandle(w_, h);
        return *this;
    }
    Args& gpuHandle(const char* key, D3D12_GPU_DESCRIPTOR_HANDLE h)
    {
        w_.Key(key);
        WriteGpuHandle(w_, h);
        return *this;
    }
    Args& address(const char* key, D3D12_GPU_VIRTUAL_ADDRESS a)
    {
        w_.Key(key);
        WriteGpuAddress(w_, a);
        return *this;
    }
