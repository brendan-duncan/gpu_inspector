...
uint64_t Tracker::Track(void* object, const char* type, const char* cmd, void* parent, std::string args, uint32_t index)
{
    ...
    uint64_t id;
    {
        ...
        if (parent)
        {
            ...
            if (pit != i.byHandle.end())
                o.parentId = pit->second;
        }
        if (StackTracesEnabled())
            o.stack = CaptureStack(1);
        if (i.live)
            message = AddObjectMessage(o);
...
void Tracker::Untrack(void* object)
{
    ...
    if (i.live)
        Transport::Get().SendJson(DeleteMessage(removed));
...
void Tracker::SetLabel(void* object, const std::string& label)
{
    ...
    uint64_t id = 0;
    {
        ...
        if (it == i.byHandle.end())
            return;
        auto oit = i.byId.find(it->second);
        if (oit == i.byId.end() || oit->second.label == label)
            return;
...
void Tracker::UpdateById(uint64_t id, const std::string& key, const std::string& messageJson)
{
    Impl& i = impl();
    {
        ...
        for (auto& u : oit->second.updates)
        {
            if (u.first == key)
            {
                u.second = messageJson;
                replaced = true;
                break;
            }
        }
        if (!replaced)
            oit->second.updates.emplace_back(key, messageJson);
    }
    if (i.live)
...
std::shared_ptr<std::vector<uint8_t>> Tracker::GetBlob(uint64_t id, uint32_t index)
{
    ...
    std::shared_lock lock(i.mutex);
    auto oit = i.byId.find(id);
    if (oit == i.byId.end() || index >= oit->second.blobs.size())
        return nullptr;
...
void Tracker::SendSnapshot()
{
    ...
    std::vector<std::string> messages;
    {
        ...
        for (auto& [id, o] : ordered)
        {
            ...
            for (auto& u : o->updates)
                messages.push_back(u.second);
        }
        i.live = true;
    }
    Transport& t = Transport::Get();
    for (auto& m : messages)
        t.SendJson(std::move(m));
...
void Tracker::SendLeakReport(void* owner)
{
    ...
    JsonWriter w;
    {
        ...
        for (auto& [id, o] : i.byId)
        {
            if (o.parentId != ownerId)
                continue;
            // What the application cannot release itself is not a leak.
            if (o.type == "ID3D12CommandQueue" && o.cmd == "D3D12CreateDevice")
                continue;
            ordered[id] = &o;
        }
        for (auto& [id, o] : ordered)
