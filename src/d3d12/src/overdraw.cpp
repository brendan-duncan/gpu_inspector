...
DeviceMeasurement* DeviceMeasurementFor(ID3D12Device* device)
{
    ...
    if (it != g_deviceMeasurements.end())
...
bool CompileDxbc(const char* source, const char* target, std::vector<uint8_t>& out, std::string& error)
{
    ...
    std::call_once(once, [] {
        ...
        if (module)
            compile = reinterpret_cast<PFN_D3DCompile>(GetProcAddress(module, "D3DCompile"));
    });
    if (!compile)
    {
    ...
    if (FAILED(hr) || !code)
    {
        ...
        if (messages && messages->GetBufferPointer())
            error += std::string(" ") + (const char*)messages->GetBufferPointer();
        return false;
    }
...
bool CompileDxil(const char* source, const wchar_t* target, std::vector<uint8_t>& out, std::string& error)
{
    ...
    if (FAILED(compiler->Compile(&buffer, args, (UINT32)(sizeof(args) / sizeof(args[0])), nullptr, IID_PPV_ARGS(result.put()))) || !result)
    {
    ...
    std::string& error)
{
    std::call_once(shader.once, [&] {
        ...
        if (ok)
            shader.bytecode = {shader.bytes.data(), shader.bytes.size()};
        else
            LogAlways("measurement: %s", shader.error.c_str());
    });
...
bool MeasurementDescriptor(ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE type, D3D12_CPU_DESCRIPTOR_HANDLE& out)
{
    ...
    if (used >= kMeasurementHeapSize)
    {
        ...
        ScopedInternal internal;
        if (FAILED(device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(heap.put()))) || !heap)
            return false;
...
ID3D12PipelineState* VariantOf(ID3D12PipelineState* pipeline, uint64_t key, const PipelineVariant& variant, std::string& error)
{
    ...
    if (!ShaderEditor::Get().VariantPipeline(pipeline, key, variant, &out, error))
        return nullptr;
    return out;
}
...
std::vector<const LoggedOp*> EffectiveOps(const std::vector<LoggedOp>& ops, size_t count)
{
    ...
    for (size_t i = 0; i < count && i < ops.size(); ++i)
    {
        ...
        if (k.policy == OpPolicy::Draw || k.policy == OpPolicy::Action)
            continue;
...
public:
    void SetPipeline(ID3D12GraphicsCommandList* list, ID3D12PipelineState* pipeline) override
    {
        ...
        if (pipeline)
            list->SetPipelineState(ShaderEditor::Get().Substitute(pipeline));
    }
...
std::string RecordedMethod(CommandRecorder* rec, uint32_t command)
{
    ...
    if (!commands || command >= commands->size())
        return std::string();
...
void MeasureOverdraw(MeasuredPass& pass, const ListOps& ops)
{
    ...
    for (PendingMeasurement& m : results)
...
{
    if (!pass.hasDepth || pass.samples > 1 || pass.layered || !pass.depth.resource)
        return;
    ...
    D3D12_RESOURCE_DESC desc{};
    ResourceInfo info;
    if (!ResourceTracker::Get().Get(pass.depth.resource, info))
        return;
    desc = info.desc;
    uint32_t planes = 1;
    ...
    for (uint32_t plane = 0; preserve && plane < planes; ++plane)
    {
        ...
        if (!known)
            state = pass.depth.readOnlyDepth ? D3D12_RESOURCE_STATE_DEPTH_READ : D3D12_RESOURCE_STATE_DEPTH_WRITE;
...
void PrepareMeasuredPass(CommandRecorder* rec, const std::vector<BoundTarget>& targets)
{
    ...
    if (rec->bundle())
        return;   // a bundle records no pass of its own
    ...
    for (const BoundTarget& t : targets)
    {
        ...
        D3D12_RESOURCE_DESC desc{};
        ResourceInfo info;
        if (!ResourceTracker::Get().Get(t.resource, info))
            continue;
        desc = info.desc;
        if (!pass->width)
        {
        ...
        if (desc.SampleDesc.Count > 1)
            pass->samples = desc.SampleDesc.Count;
        if (t.sliceCount > 1)
            pass->layered = true;
        if (t.depth)
        {
    ...
    else
    {
        ...
        if (attachment >= 0)
            PreparePixelHistory(*pass, attachment);
        // The overlay's depth-test run tests against the same copy the overdraw count does.
        if ((pass->measureOverdraw || pass->measureOverlay) && pass->note.empty())
            CopyDepthStart(*pass);
    }
    if (!pass->measureOverdraw && !pass->history && !pass->measureOverlay)
        return;   // nothing the capture measures renders here
    std::lock_guard<std::mutex> lock(s.mutex);
    if (s.active.load(std::memory_order_relaxed))
        s.open[rec->list()] = std::move(pass);
}
...
void EndMeasuredPass(CommandRecorder* rec, bool insideRenderPass)
{
    ...
    ListOps* ops = nullptr;
    {
        ...
        if (entry != s.lists.end())
            ops = entry->second.get();
    }
    ListOps none;
    ...
    {
        pass->note =
            "the pass executes a bundle whose calls were not kept, so it cannot be issued again"
    ...
    if (pass->measureOverdraw)
        MeasureOverdraw(*pass, kept);
    if (pass->measureOverlay && MatchDrawOverlayPass(*pass))
        MeasureDrawOverlay(*pass, rec, kept);
    if (pass->measureOverlay && MatchMeshOutputPass(*pass))
        MeasureMeshOutput(*pass, rec, kept);
    if (pass->history)
        FollowPixel(*pass, rec, kept);
    if (measurable)
    {
    ...
    if (ops)
    {
        ...
        {
            if (op.key.policy != OpPolicy::Draw && op.key.policy != OpPolicy::Action)
    ...
    const MeshOutputRequest& mesh, uint64_t maxDataSize)
{
    ...
    if (overdraw)
        Log("overdraw: measuring every render pass of the capture");
}
...
void SendOverdraw()
{
    ...
    struct Release
    {
        ...
        {
            std::lock_guard<std::mutex> lock(g_deviceMutex);
            for (auto& [device, dm] : g_deviceMeasurements)
    ...
    for (size_t i = 0; i < pending.size(); ++i)
    {
        ...
        const uint8_t* mapped = nullptr;
        {
            ScopedInternal internal;
            void* p = nullptr;
            if (SUCCEEDED(m.staging->Map(0, nullptr, &p)))
    ...
    for (size_t i = 0; i < pending.size(); ++i)
    {
        ...
        w.Key("histogram");
        w.BeginArray();
        for (uint64_t h : r.histogram)
            w.Uint(h);
        w.EndArray();
        w.Key("size");
        w.Uint(r.counts.size());
        if (!m.note.empty())
        {
            w.Key("note");
            w.String(m.note);
    ...
    for (size_t i = 0; i < pending.size(); ++i)
    {
        ...
        h.Uint(m.passIndex);
        h.Key("depthTested");
        h.Boolean(m.depthTested);
        h.Key("size");
        h.Uint(results[i].counts.size());
        h.EndObject();
