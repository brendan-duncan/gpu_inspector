...
ID3D12RootSignature* StreamOutputSignature(ID3D12Device* device, ID3D12RootSignature* original, std::string& error) {
    ...
    MeshState& s = State();
    {
        ...
        if (it != s.signatures.end()) return it->second.get();
    }
...
public:
    ...
    void IssueDraw(ID3D12GraphicsCommandList* list, const std::function<void(ID3D12GraphicsCommandList*)>& draw) override {
        ...
        if (!_signature) {
            if (_out.note.empty()) Note("the draw's root signature could not be copied with stream output");
            return;
        }
        std::string error;
        PipelineVariant v;
    ...
    void Skip() override {
        if (_drawIndex++ == _target) Note("an indirect execution's draws cannot be issued one at a time");
    }
...
void StartMeshOutput(const MeshOutputRequest& request) {
    ...
    if (request.enabled) Log("mesh output: following draw %u of pass %u", request.drawIndex, request.passIndex);
}
...
void MeasureMeshOutput(MeasuredPass& pass, CommandRecorder* rec, const ListOps& ops) {
    ...
    ID3D12PipelineState* pipeline = nullptr;
    {
        ...
        auto probeOp = [&](const LoggedOp& op) {
            if (op.key.policy == OpPolicy::Replace && op.key.name == std::string(ops::kPipeline)) op.op(nullptr, probe);
        };
        const std::vector<const LoggedOp*> state = EffectiveOps(ops.ops, ops.passFirst);
        for (const LoggedOp* op : state) probeOp(*op);
        uint32_t index = 0;
        for (size_t k = ops.passFirst; k < ops.ops.size(); ++k) {
            const LoggedOp& op = ops.ops[k];
            probeOp(op);
            if (op.key.policy != OpPolicy::Draw) continue;
            if (index++ != request.drawIndex) continue;
...
void SendMeshOutput() {
    ...
    for (PendingMesh& m : pending) {
        ...
        JsonWriter w;
        w.BeginObject();
        w.Key("action"); w.String("CaptureMeshOutput");
        ...
        w.Key("topology"); w.String(m.topology);
        w.Key("stride"); w.Uint(m.stride);
        w.Key("vertices"); w.Uint(vertices);
        w.Key("truncated"); w.Boolean(m.truncated);
        w.Key("outputs"); w.BeginArray();
        for (size_t k = 0; k < m.outputs.size(); ++k) {
            const ShaderOutputParam& o = m.outputs[k];
            w.BeginObject();
            w.Key("name"); w.String(o.semanticIndex ? o.semantic + std::to_string(o.semanticIndex) : o.semantic);
            w.Key("offset"); w.Uint(k < m.offsets.size() ? m.offsets[k] : 0);
            w.Key("components"); w.Uint(o.componentCount);
            w.Key("base"); w.String(o.base);
            ...
            auto isPosition = [](const std::string& text) {
                std::string upper;
                for (char c : text) upper += (char)toupper((unsigned char)c);
            ...
            if (position) { w.Key("builtin"); w.String("Position"); }
            else if (!o.systemValue.empty()) { w.Key("builtin"); w.String(o.systemValue); }
            w.EndObject();
        }
        w.EndArray();
        w.Key("size"); w.Uint(records.size());
        ...
        if (!records.empty()) {
            JsonWriter h;
            h.BeginObject();
            h.Key("action"); h.String("CaptureMeshOutputData");
            h.Key("command"); h.Uint(m.command);
            h.Key("commandBuffer"); h.Uint(m.listId);
            h.Key("size"); h.Uint(records.size());
            h.EndObject();
