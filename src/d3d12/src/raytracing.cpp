...
void EmitExports(ID3D12StateObject* stateObject, const StateObjectInfo& info)
{
    ...
    w.Key("id");
    w.Uint(id);
    w.Key("shaderIdentifiers");
    w.BeginObject();
    w.Key("size");
    w.Uint(kIdentifierSize);
    w.Key("pipelineStackSize");
    w.Uint(info.pipelineStackSize);
    w.Key("unlistedExports");
    w.Boolean(info.hasUnlistedExports);
    w.Key("exports");
    w.BeginArray();
    for (const Export& e : info.exports)
    {
        ...
        w.String(e.name);
        w.Key("identifier");
        w.String(HexBytes(e.identifier.data(), e.identifier.size()));
        ...
        w.Key("stackSize");
        if (e.stackSize == kUnknownStackSize)
            w.Null();
        else
            w.Uint(e.stackSize);
        w.EndObject();
...
{
    if (!stateObject || !identifier || name.empty())
        return;
    ...
    for (Export& e : info.exports)
    {
        if (e.name != name)
            continue;
        if (e.identifier == bytes && e.stackSize == stackSize)
...
void* STDMETHODCALLTYPE Hook_GetShaderIdentifier(ID3D12StateObjectProperties1* This, LPCWSTR pExportName)
{
    ...
    if (Internal() || !identifier || !pExportName)
        return identifier;
    ...
    if (!hitGroup)
    {
        ...
        if (stack)
            stackSize = stack(This, pExportName);
    }
    ...
    for (UINT i = 0; i < desc.NumSubobjects && desc.pSubobjects; ++i)
    {
        ...
        switch (s.Type)
        {
            case D3D12_STATE_SUBOBJECT_TYPE_DXIL_LIBRARY:
            {
                ...
                for (UINT e = 0; e < lib.NumExports; ++e)
                    add(lib.pExports[e].Name);
                break;
            }
            case D3D12_STATE_SUBOBJECT_TYPE_EXISTING_COLLECTION:
            {
                const auto& col = *static_cast<const D3D12_EXISTING_COLLECTION_DESC*>(s.pDesc);
                if (!col.NumExports || !col.pExports)
                {
                    unlisted = true;
                    break;
                }
                for (UINT e = 0; e < col.NumExports; ++e)
                    add(col.pExports[e].Name);
                break;
            }
            case D3D12_STATE_SUBOBJECT_TYPE_HIT_GROUP:
            {
                ...
                if (group.HitGroupExport && *group.HitGroupExport)
                    hitGroups.insert(Narrow(group.HitGroupExport));
                break;
            }
            default:
                break;
...
void AddLibraryBlobs(ID3D12StateObject* stateObject, const D3D12_STATE_OBJECT_DESC& desc, JsonWriter& shaders)
{
    ...
    {
        const D3D12_STATE_SUBOBJECT& s = desc.pSubobjects[i];
        if (s.Type != D3D12_STATE_SUBOBJECT_TYPE_DXIL_LIBRARY || !s.pDesc)
            continue;
        const auto& lib = *static_cast<const D3D12_DXIL_LIBRARY_DESC*>(s.pDesc);
...
std::vector<InputRange> InputRangesOf(const D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS& in)
{
    ...
    if (in.Type == D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL)
    {
        ...
        if (in.InstanceDescs && in.NumDescs)
            out.push_back({"InstanceDescs", -1, in.InstanceDescs, (UINT64)in.NumDescs * stride});
        return out;
    }
    ...
    for (UINT i = 0; i < in.NumDescs; ++i)
    {
        ...
        if (g->Type == D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES)
        {
            ...
            if (tri.Transform3x4)
                out.push_back({"Transform3x4", (int)i, tri.Transform3x4, 48});
        }
...
std::string StructureLabel(const std::string& bufferName, D3D12_GPU_VIRTUAL_ADDRESS address, ID3D12Resource* buffer)
{
    ...
    if (!AddressMap::Get().Resolve(address, owner, offset, remaining) || owner != buffer || offset == 0)
        return bufferName;
...
ResolvedInput CaptureInput(CommandRecorder* rec, D3D12_GPU_VIRTUAL_ADDRESS address, UINT64 size)
{
    ...
    if (rec && size)
        out.capture = Cap().QueueAddressCapture(rec, address, size, /*whole=*/true);
    return out;
}
    ...
    ID3D12StateObject* grownFrom)
{
    ...
    if (properties)
    {
        ...
        if (pipelineStack)
            info.pipelineStackSize = pipelineStack(properties);
    ...
    if (desc)
    {
        ...
        w.String("ObjectUpdate");
        w.Key("id");
        w.Uint(Tracker::Get().IdOf(stateObject));
        w.Key("shaders");
        w.Raw(shaders.str());
        w.EndObject();
...
std::string ExportWithIdentifier(ID3D12StateObject* stateObject, const void* identifier)
{
    ...
    {
        if (memcmp(e.identifier.data(), identifier, kIdentifierSize) == 0)
            return e.name;
    }
    return "";
}
    ...
    ID3D12Device* device)
{
    ...
    if (in.Type == D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL)
    {
        for (UINT i = 0; i < in.NumDescs; ++i)
        {
            ...
            primitives += g->Type == D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES
                ...
                : g->Type == D3D12_RAYTRACING_GEOMETRY_TYPE_PROCEDURAL_PRIMITIVE_AABBS ? g->AABBs.AABBCount
                                                                                       : 0;
        }
    }
    ...
    // the capture ids of the captured frame's build go on the command below.
    {
        ...
        w.BeginObject();
        w.Key("method");
        w.String("BuildRaytracingAccelerationStructure");
        w.Key("Type");
        ...
        w.Uint(primitives);
        w.Key("DestAccelerationStructureData");
        WriteGpuAddress(w, desc.DestAccelerationStructureData);
        w.Key("SourceAccelerationStructureData");
        WriteGpuAddress(w, desc.SourceAccelerationStructureData);
        w.Key("update");
        w.Boolean((in.Flags & D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PERFORM_UPDATE) != 0);
        w.Key("geometries");
        w.BeginArray();
        if (in.Type == D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL)
        {
        ...
        if (device && SUCCEEDED(device->QueryInterface(__uuidof(ID3D12Device5), (void**)&device5)) && device5)
        {
            ...
            {
                w.Key("resultSize");
                w.Uint(prebuild.ResultDataMaxSizeInBytes);
                w.Key("scratchSize");
                w.Uint(prebuild.ScratchDataSizeInBytes);
            }
        }
        w.EndObject();
        w.EndObject();
    ...
    D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE mode, ID3D12Device* device)
{
    ...
    w.BeginObject();
    w.Key("source");
    WriteStructureRef(w, source);
    w.Key("sourceAddress");
    w.String(Hex(source));
    w.Key("Mode");
    w.Enum(ToString_D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE(EnumValue(mode)), EnumValue(mode));
    w.EndObject();
    w.EndObject();
...
std::string NoteDispatchRays(CommandRecorder* rec, const D3D12_DISPATCH_RAYS_DESC& desc)
{
    ...
    if (ID3D12StateObject* stateObject = BoundStateObject(rec))
    {
        ...
        if (id)
            out += ",\"stateObject\":{\"__id\":" + std::to_string(id) + ",\"__class\":\"ID3D12StateObject\"}";
    }
    if (captured)
...
{
    if (rec)
        rec->state().stateObject = stateObject;
}
...
void OnResourceNamed(ID3D12Resource* resource, const std::string& name)
{
    if (!resource || name.empty())
        return;
    std::lock_guard<std::mutex> lock(g_structureMutex);
    if (!g_structureBuffers.count(Key(resource)))
        return;
    for (auto& [address, object] : g_structures)
    {
        if (object->buffer == resource)
            Tracker::Get().SetLabel(object->sentinel.get(), StructureLabel(name, address, resource));
    }
}
...
void ReadBackEarlierStructures(CommandRecorder* rec, uint64_t captureSerial)
{
    ...
    std::vector<Pending> pending;
    {
        ...
        {
            if (!object->inputs.empty())
                pending.push_back({object->id, object->inputs});
        }
    }
    Tracker& t = Tracker::Get();
    for (const Pending& p : pending)
    {
        ...
        for (const InputRange& r : p.inputs)
        {
            ID3D12Resource* buffer = nullptr;
            UINT64 offset = 0, remaining = 0;
            if (!AddressMap::Get().Resolve(r.address, buffer, offset, remaining))
                continue;
