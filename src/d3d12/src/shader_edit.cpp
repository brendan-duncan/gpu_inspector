...
void OwnInputLayout(PipelineRecord& rec, D3D12_INPUT_LAYOUT_DESC& il)
{
    ...
    for (D3D12_INPUT_ELEMENT_DESC& e : rec.inputElements)
        e.SemanticName = OwnString(rec, e.SemanticName);
...
void OwnStreamOutput(PipelineRecord& rec, D3D12_STREAM_OUTPUT_DESC& so)
{
    if (so.pSODeclaration && so.NumEntries)
    {
        ...
        for (D3D12_SO_DECLARATION_ENTRY& e : rec.soEntries)
            e.SemanticName = OwnString(rec, e.SemanticName);
...
{
    if (cached.pCachedBlob && cached.CachedBlobSizeInBytes)
        rec.note += "the cached PSO blob was dropped; ";
    cached = {nullptr, 0};
}
...
bool ContainerIsDxil(const void* data, size_t size)
{
    ...
    if (!bytes || size < 32 || memcmp(bytes, "DXBC", 4) != 0)
        return false;
    uint32_t parts = 0;
    memcpy(&parts, bytes + 28, 4);   // magic, digest, version, total size, part count
    if (parts > 64 || 32 + (size_t)parts * 4 > size)
        return false;
...
D3D12_RT_FORMAT_ARRAY CountFormats(DXGI_FORMAT format)
{
    ...
    for (UINT i = 1; i < 8; ++i)
        a.RTFormats[i] = DXGI_FORMAT_UNKNOWN;
    return a;
}
...
void PatchDepthStencil(bool desc2, uint8_t* payload, const PipelineVariant& v)
{
    ...
    if (v.disableDepth)
        memcpy(payload + offsetof(D3D12_DEPTH_STENCIL_DESC, DepthEnable), &off, sizeof(off));
    if (v.disableDepthWrite)
        memcpy(payload + offsetof(D3D12_DEPTH_STENCIL_DESC, DepthWriteMask), &zero, sizeof(zero));
    if (v.disableStencil)
        memcpy(payload + offsetof(D3D12_DEPTH_STENCIL_DESC, StencilEnable), &off, sizeof(off));
    ...
    while (pos < out.size())
    {
        ...
        switch (type)
        {
            ...
            case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL_FORMAT:
                hasDsFormat = true;
                if (v.setDepthFormat)
                    memcpy(payload, &v.depthFormat, sizeof(DXGI_FORMAT));
                break;
            ...
            case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_ROOT_SIGNATURE:
                ...
                if (v.rootSignature)
                    memcpy(payload, &v.rootSignature, sizeof(v.rootSignature));
                break;
...
void BuildVariant(PipelineRecord& rec, const PipelineVariant& v, PipelineRecord::Variant& made)
{
    ...
    if (rec.kind == PipelineRecord::Kind::Graphics)
    {
        ...
        ApplyEdits(rec, reinterpret_cast<uint8_t*>(&d));
        if (v.pixelShaderSize)
            d.PS = {v.pixelShader, v.pixelShaderSize};
        if (v.countFormat != DXGI_FORMAT_UNKNOWN)
        {
        ...
        if (v.singleSample)
            d.SampleDesc = {1, 0};
        if (v.rootSignature)
            d.pRootSignature = v.rootSignature;
        if (v.soEntryCount)
            d.StreamOutput = VariantStreamOutput(v);
        hr = rec.device->CreateGraphicsPipelineState(&d, IID_PPV_ARGS(&pipeline));
    }
    else
    {
        std::vector<uint8_t> stream;
        if (!BuildVariantStream(rec, v, stream, made.error))
            return;
        ComPtr<ID3D12Device2> device2;
        if (FAILED(rec.device->QueryInterface(IID_PPV_ARGS(device2.put()))) || !device2)
        {
...
struct ShaderEditor::Impl
{
    ...
    void Install(PipelineRecord& rec, ID3D12PipelineState* original, ID3D12PipelineState* pipeline)
    {
        ID3D12PipelineState* previous;
        {
            ...
            if (pipeline)
                substitutions[original] = pipeline;
            else
                substitutions.erase(original);
    ...
    {
        ID3D12PipelineState* pipeline = nullptr;
        if (!CreateWithEdits(rec, pipeline, error))
            return false;
        TrackedObject orig;
        ...
        const char* cmd = tracked && !orig.cmd.empty()  ? orig.cmd.c_str()
            ...
            : rec.kind == PipelineRecord::Kind::Compute ? "CreateComputePipelineState"
    ...
    void AddRecord(ID3D12PipelineState* pipeline, std::unique_ptr<PipelineRecord> rec)
    {
        ...
        auto it = records.find(pipeline);
        if (it != records.end())
            Install(*it->second, pipeline, nullptr);
    ...
    std::string& error, uint64_t& replacementId, std::string& note)
{
    ...
    if (hadPrevious)
        previousBytes = std::move(previous->second);
    ...
    if (!i.Rebuild(*rec, pipeline, error, replacementId))
    {
        ...
        if (hadPrevious)
            rec->edits[stage] = std::move(previousBytes);
        else
...
ID3D12PipelineState* ShaderEditor::Substitute(ID3D12PipelineState* pipeline)
{
    if (!pipeline || !_impl)
        return pipeline;
    Impl& i = *_impl;
    if (i.editCount.load(std::memory_order_acquire) == 0)
        return pipeline;
    ...
    ID3D12PipelineState** out, std::string& error)
{
    ...
    if (cached == rec.variants.end())
    {
        ...
        if (rec.kind == PipelineRecord::Kind::Compute)
            made.error = "a compute pipeline rasterizes nothing";
        else if (rec.unsupported)
            made.error = "the pipeline stream cannot be copied: " + rec.note;
        else
            BuildVariant(rec, variant, made);
        if (!made.error.empty())
            Log("measurement: %s", made.error.c_str());
...
bool ShaderEditor::StageBytecode(ID3D12PipelineState* pipeline, const char* stage, const void*& code, size_t& size)
{
    ...
    if (!bc || !bc->pShaderBytecode || !bc->BytecodeLength)
        return false;
...
bool ShaderEditor::PrimitiveTopologyTypeOf(ID3D12PipelineState* pipeline, D3D12_PRIMITIVE_TOPOLOGY_TYPE& out)
{
    ...
    if (rec.kind != PipelineRecord::Kind::Stream)
        return false;
    size_t pos = 0;
    while (pos < rec.stream.size())
    {
        D3D12_PIPELINE_STATE_SUBOBJECT_TYPE type;
        if (rec.stream.size() - pos < sizeof(type))
            return false;
        memcpy(&type, rec.stream.data() + pos, sizeof(type));
        SubobjectLayout layout;
        if (!SubobjectLayoutFor(type, layout) || layout.size > rec.stream.size() - pos)
...
bool ShaderEditor::PipelineIsDxil(ID3D12PipelineState* pipeline)
{
    ...
    if (rec.dxil < 0)
    {
        ...
        for (const char* stage : {"vertex", "mesh"})
        {
            ...
            {
                if (ContainerIsDxil(bc->pShaderBytecode, bc->BytecodeLength))
                    rec.dxil = 1;
                break;
            }
        }
