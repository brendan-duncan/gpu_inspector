...
T* QueryAs(IUnknown* object)
{
    ...
    T* p = nullptr;
    if (FAILED(object->QueryInterface(IID_PPV_ARGS(&p))) || !p)
        return nullptr;
...
void WriteCastableFormats(Args& a, UINT32 count, const DXGI_FORMAT* formats)
{
    ...
    if (formats && count)
        WriteFormats(a.key("pCastableFormats"), formats, count);
    else
        a.null("pCastableFormats");
}
    ...
    D3D12_HEAP_TYPE heapType, D3D12_RESOURCE_STATES initialState, ID3D12Heap* heap, UINT64 heapOffset)
{
    ...
    if (!heap)
        NoteCommittedAllocation(device, resource, desc, heapType);
    if (desc.Dimension == D3D12_RESOURCE_DIMENSION_BUFFER && address)
        AddressMap::Get().Add(resource, address, desc.Width);
...
{
    if (Internal())
        return DEV(CreateCommittedResource)(This, pHeapProperties, HeapFlags, pDesc, InitialResourceState, pOptimizedClearValue, riid, out);
...
{
    if (Internal())
        return DEV(CreateCommittedResource1)(This, pHeapProperties, HeapFlags, pDesc, InitialResourceState, pOptimizedClearValue, pProtectedSession, riid, out);
...
{
    if (Internal())
        return DEV(CreateCommittedResource2)(This, pHeapProperties, HeapFlags, pDesc, InitialResourceState, pOptimizedClearValue, pProtectedSession, riid, out);
...
{
    if (Internal())
        return DEV(CreateCommittedResource3)(This, pHeapProperties, HeapFlags, pDesc, InitialLayout, pOptimizedClearValue, pProtectedSession, NumCastableFormats, pCastableFormats, riid, out);
...
{
    if (Internal())
        return DEV(CreatePlacedResource)(This, pHeap, HeapOffset, pDesc, InitialState, pOptimizedClearValue, riid, out);
...
{
    if (Internal())
        return DEV(CreatePlacedResource1)(This, pHeap, HeapOffset, pDesc, InitialState, pOptimizedClearValue, riid, out);
...
{
    if (Internal())
        return DEV(CreatePlacedResource2)(This, pHeap, HeapOffset, pDesc, InitialLayout, pOptimizedClearValue, NumCastableFormats, pCastableFormats, riid, out);
...
{
    if (Internal())
        return DEV(CreateReservedResource)(This, pDesc, InitialState, pOptimizedClearValue, riid, out);
...
{
    if (Internal())
        return DEV(CreateReservedResource1)(This, pDesc, InitialState, pOptimizedClearValue, pProtectedSession, riid, out);
...
{
    if (Internal())
        return DEV(CreateReservedResource2)(This, pDesc, InitialLayout, pOptimizedClearValue, pProtectedSession, NumCastableFormats, pCastableFormats, riid, out);
...
void OnHeapCreated(ID3D12Device* device, ID3D12Heap* heap, const char* cmd, Args& a, const D3D12_HEAP_DESC* desc)
{
    ...
    Tracker::Get().Track(heap, "ID3D12Heap", cmd, device, a.str());
    {
        ...
        // once the tracker has it: a memory capture names the heap by the tracker's id.
        ScopedInternal internal;
...
{
    if (Internal())
        return DEV(MakeResident)(This, NumObjects, ppObjects);
    HRESULT hr = DEV(MakeResident)(This, NumObjects, ppObjects);
    if (SUCCEEDED(hr))
        NoteResidency(false, NumObjects, ppObjects);
    return hr;
}
...
{
    if (Internal())
        return DEV(Evict)(This, NumObjects, ppObjects);
    HRESULT hr = DEV(Evict)(This, NumObjects, ppObjects);
    if (SUCCEEDED(hr))
        NoteResidency(true, NumObjects, ppObjects);
    return hr;
}
...
{
    if (Internal())
        return DEV(EnqueueMakeResident)(This, Flags, NumObjects, ppObjects, pFenceToSignal, FenceValueToSignal);
...
{
    if (Internal())
        return DEV(CreateHeap)(This, pDesc, riid, out);
...
{
    if (Internal())
        return DEV(CreateHeap1)(This, pDesc, pProtectedSession, riid, out);
...
{
    if (Internal())
        return DEV(OpenExistingHeapFromAddress)(This, pAddress, riid, out);
...
{
    if (Internal())
        return DEV(OpenExistingHeapFromAddress1)(This, pAddress, size, riid, out);
...
{
    if (Internal())
        return DEV(OpenExistingHeapFromFileMapping)(This, hFileMapping, riid, out);
...
{
    if (Internal())
        return DEV(OpenSharedHandle)(This, NTHandle, riid, out);
...
std::vector<StageBytecode> StagesOf(const D3D12_PIPELINE_STATE_STREAM_DESC& d)
{
    ...
    while (offset + sizeof(UINT) <= d.SizeInBytes)
    {
        ...
        if (data + layout.size > d.SizeInBytes)
            break;
    ...
    std::string descJson, const std::vector<StageBytecode>& stages)
{
    ...
    for (auto& b : blobs)
        Tracker::Get().AddBlob(pipeline, b.first, b.second);
...
{
    if (Internal())
        return DEV(CreateGraphicsPipelineState)(This, pDesc, riid, out);
    ...
    if (pDesc)
        ShaderEditor::Get().OnGraphicsPipelineCreated(This, pipeline, *pDesc);
    return hr;
}
...
{
    if (Internal())
        return DEV(CreateComputePipelineState)(This, pDesc, riid, out);
    const uint64_t timed = CpuEventBegin();
    ...
    if (pDesc)
        ShaderEditor::Get().OnComputePipelineCreated(This, pipeline, *pDesc);
    return hr;
}
...
{
    if (Internal())
        return DEV(CreatePipelineState)(This, pDesc, riid, out);
    const uint64_t timed = CpuEventBegin();
    ...
    if (pDesc)
        ShaderEditor::Get().OnStreamPipelineCreated(This, pipeline, *pDesc);
    return hr;
}
...
{
    if (Internal())
        return LIB(LoadGraphicsPipeline)(This, pName, pDesc, riid, out);
    const uint64_t timed = CpuEventBegin();
    ...
    if (pDesc)
        ShaderEditor::Get().OnGraphicsPipelineCreated(DeviceOf(This), pipeline, *pDesc);
    return hr;
}
...
{
    if (Internal())
        return LIB(LoadComputePipeline)(This, pName, pDesc, riid, out);
    const uint64_t timed = CpuEventBegin();
    ...
    if (pDesc)
        ShaderEditor::Get().OnComputePipelineCreated(DeviceOf(This), pipeline, *pDesc);
    return hr;
}
...
{
    if (Internal())
        return LIB(LoadPipeline)(This, pName, pDesc, riid, out);
    const uint64_t timed = CpuEventBegin();
    ...
    if (pDesc)
        ShaderEditor::Get().OnStreamPipelineCreated(DeviceOf(This), pipeline, *pDesc);
    return hr;
}
...
{
    if (Internal())
        return DEV(CreatePipelineLibrary)(This, pLibraryBlob, BlobLength, riid, out);
...
{
    if (Internal())
        return DEV(CreateCommandQueue)(This, pDesc, riid, out);
...
{
    if (Internal())
        return DEV(CreateCommandQueue1)(This, pDesc, CreatorID, riid, out);
...
{
    if (Internal())
        return DEV(CreateCommandAllocator)(This, type, riid, out);
...
{
    if (Internal())
        return DEV(CreateCommandList)(This, nodeMask, type, pCommandAllocator, pInitialState, riid, out);
...
{
    if (Internal())
        return DEV(CreateCommandList1)(This, nodeMask, type, flags, riid, out);
...
{
    if (Internal())
        return DEV(CreateDescriptorHeap)(This, pDesc, riid, out);
...
void STDMETHODCALLTYPE Hook_CreateSamplerFeedbackUnorderedAccessView(ID3D12Device14* This, ID3D12Resource* pTargetedResource, ID3D12Resource* pFeedbackResource,
    D3D12_CPU_DESCRIPTOR_HANDLE DestDescriptor)
{
    DEV(CreateSamplerFeedbackUnorderedAccessView)
...
{
    DEV(CopyDescriptors)
    (This, NumDestDescriptorRanges, pDestDescriptorRangeStarts, pDestDescriptorRangeSizes, NumSrcDescriptorRanges,
...
{
    if (Internal())
        return DEV(CreateRootSignature)(This, nodeMask, pBlobWithRootSignature, blobLengthInBytes, riid, out);
...
{
    if (Internal())
        return DEV(CreateRootSignatureFromSubobjectInLibrary)(This, nodeMask, pLibraryBlob, blobLengthInBytes, subobjectName, riid, out);
    ...
    if (info)
        RootSignatures::Get().Register(signature, info);
    Args a;
    if (info)
        a.raw("pDesc", info->json);
...
{
    if (Internal())
        return DEV(CreateFence)(This, InitialValue, Flags, riid, out);
...
{
    if (Internal())
        return DEV(CreateQueryHeap)(This, pDesc, riid, out);
...
{
    if (Internal())
        return DEV(CreateCommandSignature)(This, pDesc, pRootSignature, riid, out);
...
{
    if (Internal())
        return DEV(CreateStateObject)(This, pDesc, riid, out);
    const uint64_t timed = CpuEventBegin();
...
{
    if (Internal())
        return DEV(AddToStateObject)(This, pAddition, pStateObjectToGrowFrom, riid, out);
...
{
    if (Internal())
        return DEV(CreateMetaCommand)(This, CommandId, NodeMask, pCreationParametersData, CreationParametersDataSizeInBytes, riid, out);
...
HRESULT WINAPI Hook_D3D12CreateDevice(IUnknown* pAdapter, D3D_FEATURE_LEVEL MinimumFeatureLevel, REFIID riid, void** ppDevice)
{
    ...
    if (VulkanLayerHasDevice())
    {
        ...
        if (!logged.exchange(true))
            LogAlways("a D3D12 device created while the Vulkan layer has a device in this process (the Vulkan driver presenting through DXGI): left to the driver, not tracked");
        return g_D3D12CreateDevice(pAdapter, MinimumFeatureLevel, riid, ppDevice);
    }
...
{
    if (Internal())
        return g_CreateDXGIFactory(riid, ppFactory);
...
{
    if (Internal())
        return g_CreateDXGIFactory1(riid, ppFactory);
...
bool InstallEntryPointHooks()
{
    ...
    if (!HookExport(d3d12, "D3D12CreateDevice", (void*)&Hook_D3D12CreateDevice, (void**)&g_D3D12CreateDevice))
        return false;
...
{
    if (!resource || VtableHooked(resource))
        return;
...
HRESULT STDMETHODCALLTYPE Hook_SetEventOnCompletion(ID3D12Fence1* This, UINT64 Value, HANDLE hEvent)
{
    ...
    if (SUCCEEDED(hr) && hEvent)
        NoteFenceEvent(hEvent);
    return hr;
}

void HookFence(ID3D12Fence* fence)
{
    if (!fence || VtableHooked(fence))
        return;
...
{
    if (!library || VtableHooked(library))
        return;
