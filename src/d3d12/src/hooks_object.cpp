...
HRESULT STDMETHODCALLTYPE Hook_SetName(ID3D12Object* This, LPCWSTR name)
{
    ...
    if (!Internal() && name)
        OnObjectNamed(This, Narrow(name));
    return hr;
}
...
void OnObjectNamed(void* object, const std::string& name)
{
    ...
    if (Tracker::Get().TypeOf(object) == "ID3D12Resource")
        OnResourceNamed(static_cast<ID3D12Resource*>(object), name);
}
...
ID3D12Device* DeviceOf(ID3D12DeviceChild* child)
{
    ...
    if (FAILED(child->GetDevice(IID_PPV_ARGS(&device))) || !device)
        return nullptr;
