...
void UpdatePresentStatistics(DeviceRecord& d, IDXGISwapChain* swapChain, LONGLONG calledAtQpc)
{
    ...
    if (stats.PresentCount == 0 || stats.PresentRefreshCount == 0)
        return;
    ...
    if (stats.PresentCount != d.lastPresentCount || !d.haveStats)
    {
        ...
        if (call.count == stats.PresentCount && call.qpc && stats.SyncQPCTime.QuadPart > call.qpc)
        {
            ...
            if (ms < 1000.0)
                d.latencySamplesMs.push_back(ms);
        }
    }
    d.haveStats = true;
...
DeviceRecord& RecordOf(ID3D12Device* device)
{
    ...
    if (!slot)
        slot = std::make_unique<DeviceRecord>();
    return *slot;
}
...
std::string AdapterArgs(IDXGIAdapter* adapter, std::string* name)
{
    ...
    if (SUCCEEDED(adapter->QueryInterface(IID_PPV_ARGS(adapter4.put()))) && SUCCEEDED(adapter4->GetDesc3(&desc3)))
    {
        ...
        if (name)
            *name = Narrow(desc3.Description, wcsnlen(desc3.Description, 128));
        return args.str();
    }
    ...
    if (SUCCEEDED(adapter->QueryInterface(IID_PPV_ARGS(adapter1.put()))) && SUCCEEDED(adapter1->GetDesc1(&desc1)))
    {
        ...
        if (name)
            *name = Narrow(desc1.Description, wcsnlen(desc1.Description, 128));
        return args.str();
    }
    args.null("Desc");
    return args.str();
...
#undef OPTIONS
    {
        ...
        w.Key("ROOT_SIGNATURE");
        w.BeginObject();
        w.Key("HighestVersion");
        w.Enum(ToString_D3D_ROOT_SIGNATURE_VERSION(EnumValue(version)), EnumValue(version));
        w.EndObject();
    }
    {
        ...
        w.Key("SHADER_MODEL");
        w.BeginObject();
        w.Key("HighestShaderModel");
        w.Enum(ToString_D3D_SHADER_MODEL(EnumValue(model)), EnumValue(model));
        w.EndObject();
    }
    w.EndObject();
    w.EndObject();
...
double MonitorRefreshMs(IDXGISwapChain* swapChain)
{
    ...
    if (swapChain)
    {
        ...
        {
            if (FAILED(swapChain1->GetHwnd(&hwnd)))
                hwnd = nullptr;
        }
        if (!hwnd)
        {
    ...
    if (hwnd)
    {
        ...
        if (monitor && GetMonitorInfoW(monitor, &info))
            ok = EnumDisplaySettingsW(info.szDevice, ENUM_CURRENT_SETTINGS, &mode);
    }
    if (!ok)
        ok = EnumDisplaySettingsW(nullptr, ENUM_CURRENT_SETTINGS, &mode);
    ...
    if (!ok || mode.dmDisplayFrequency < 2)
        return 0;
...
void RecordDeviceCreated(ID3D12Device* device, IUnknown* adapterArgument, D3D_FEATURE_LEVEL minimumFeatureLevel)
{
        ...
        return;
    {
        ...
        if (it != g_devices.end() && it->second->adapter)
            return;
    }
    ...
    UINT nodeCount = 1;
    {
        ...
        if (adapter)
            adapterArgs = AdapterArgs(adapter, &adapterName);
...
IDXGIAdapter* AdapterOf(ID3D12Device* device)
{
    ...
    if (adapter && !Tracker::Get().IdOf(adapter))
        return nullptr;
    return adapter;
}
...
bool EmitBoundary(DeviceRecord& d, Clock::time_point now, const char* boundary, double refreshMs, const std::string& presentMode)
{
    ...
    d.accumMs += ms;
    d.frames++;
    // Per-frame rather than per-report: a hitch is one frame, and the report below averages five
    // or six of them together (cpu_timeline.h).
    NoteFrameTiming((uint32_t)d.frame, ms);
    ...
    if (sinceReport >= 100.0)
    {
        ...
        if (Transport::Get().Connected())
        {
            ...
            w.Uint(d.frame);
            w.Key("frameTimeMs");
            w.Double(d.accumMs / (double)d.frames);
            w.Key("minMs");
            w.Double(d.minMs);
            w.Key("maxMs");
            w.Double(d.maxMs);
            ...
            w.Uint(d.frames);
            w.Key("submitMs");
            w.Double(d.submitMs / (double)d.frames);
            w.Key("refreshMs");
            w.Double(refreshMs);
            w.Key("refreshSource");
            w.String(refreshMs > 0 ? "monitor" : "");
            w.Key("displayRefreshMs");
            w.Double(refreshMs > 0 ? d.displayRefreshMs : 0);
            if (!presentMode.empty())
            {
                w.Key("presentMode");
                w.String(presentMode);
            }
            w.Key("frameBoundary");
            w.String(boundary);
            w.Key("dropped");
            w.Uint(d.droppedSinceReport);
            w.Key("droppedTotal");
            w.Uint(d.droppedTotal);
            ...
            if (!d.latencySamplesMs.empty())
            {
                ...
                w.Double(d.latencySamplesMs[d.latencySamplesMs.size() / 2]);
            }
            w.EndObject();
...
void OnFramePresented(ID3D12Device* device, IDXGISwapChain* swapChain, UINT syncInterval, UINT flags, HRESULT, LONGLONG calledAtQpc)
{
    ...
    if (syncInterval == 1)
        ...
        presentMode = "vsync/" + std::to_string(syncInterval);
    bool reported = false;
    {
    ...
    lock.unlock();
    // The frames a timing capture recorded since the last report ride on the same interval, so it
    // adds no traffic of its own.
    if (reported)
    {
        SendMemoryBudget(device);
